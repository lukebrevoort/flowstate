        ...
        for test_name in dct:
            ...
            for base in bases:
                if hasattr(base, test_name):
                    raise RuntimeError(
                        "duplicate test {}.{} (also defined in {} " "parent class)".format(name, test_name, base.__name__)
                    )
...
class BaseTestCase(unittest.TestCase, metaclass=BaseTestCaseMeta):
    ...
    def tearDown(self):
        ...
        if getattr(self.loop, "_debug_cc", False):
            ...
            self.assertEqual(
                self.loop._debug_uv_handles_total, self.loop._debug_uv_handles_freed, "not all uv_handle_t handles were freed"
            )
