...
def install() -> None:
    ...
    if _sys.version_info[:2] >= (3, 12):
        _warnings.warn(
            "uvloop.install() is deprecated in favor of uvloop.run() " "starting with Python 3.12.",
            DeprecationWarning,
            stacklevel=1,
        )
...
if _typing.TYPE_CHECKING:
    ...
    def run(main, *, loop_factory=new_event_loop, debug=None, **run_kwargs):
        ...
        if vi <= (3, 10):
            ...
            with __asyncio.Runner(loop_factory=loop_factory, debug=debug, **run_kwargs) as runner:
                return runner.run(wrapper())

        else:
            assert vi >= (3, 12)
            return __asyncio.run(wrapper(), loop_factory=loop_factory, debug=debug, **run_kwargs)
