...
def init_chat_model(
    ...
    **Note:** Must have the integration package corresponding to the model provider
    ...
    if not configurable_fields:
        return _init_chat_model_helper(cast(str, model), model_provider=model_provider, **kwargs)
    else:
        if model:
            kwargs["model"] = model
        if model_provider:
...
def _parse_model(model: str, model_provider: Optional[str]) -> tuple[str, str]:
    ...
    if not model_provider:
        raise ValueError(f"Unable to infer model provider for {model=}, please specify " f"model_provider directly.")
...
def _check_pkg(pkg: str, *, pkg_kebab: Optional[str] = None) -> None:
    if not util.find_spec(pkg):
        ...
        raise ImportError(f"Unable to import {pkg}. Please install with `pip install -U {pkg_kebab}`")
...
class _ConfigurableModel(Runnable[LanguageModelInput, Any]):
    def __init__(
        ...
        *,
        ...
        self._config_prefix = config_prefix + "_" if config_prefix and not config_prefix.endswith("_") else config_prefix
        self._queued_declarative_operations: list[tuple[str, tuple, dict]] = list(queued_declarative_operations)
    ...
    def with_config(
        ...
        **kwargs: Any,
        ...
        return _ConfigurableModel(
            ...
            configurable_fields=(
                list(self._configurable_fields) if isinstance(self._configurable_fields, list) else self._configurable_fields
            ),
            config_prefix=self._config_prefix,
            queued_declarative_operations=queued_declarative_operations,
        )
        ...
        **kwargs: Optional[Any],
        ...
        if config is None or isinstance(config, dict) or len(config) <= 1:
            if isinstance(config, list):
                config = config[0]
            return self._model(config).batch(inputs, config=config, return_exceptions=return_exceptions, **kwargs)
    ...
    async def abatch(
        ...
        **kwargs: Optional[Any],
        ...
        if config is None or isinstance(config, dict) or len(config) <= 1:
            if isinstance(config, list):
                config = config[0]
            return await self._model(config).abatch(inputs, config=config, return_exceptions=return_exceptions, **kwargs)
        # If multiple configs default to Runnable.batch which uses executor to invoke
        # in parallel.
        else:
    ...
    async def abatch_as_completed(
        ...
        **kwargs: Any,
        ...
        if config is None or isinstance(config, dict) or len(config) <= 1:
            if isinstance(config, list):
                config = config[0]
            async for x in self._model(cast(RunnableConfig, config)).abatch_as_completed(  # type: ignore[call-overload]
                inputs, config=config, return_exceptions=return_exceptions, **kwargs
            ):
                yield x
        # If multiple configs default to Runnable.batch which uses executor to invoke
