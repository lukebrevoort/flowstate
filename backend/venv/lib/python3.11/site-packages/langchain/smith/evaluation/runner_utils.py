...
class TestResult(dict):
    ...
    def to_dataframe(self) -> pd.DataFrame:
        ...
        try:
            ...
            raise ImportError(
                "Pandas is required to convert the results to a dataframe." " to install pandas, run `pip install pandas`."
            ) from e
...
def _get_prompt(inputs: dict[str, Any]) -> str:
    ...
    if "prompt" in inputs:
        ...
        if not isinstance(inputs["prompts"], list) or not all(isinstance(i, str) for i in inputs["prompts"]):
            raise InputFormatError("Expected list of strings for 'prompts'," f" got {type(inputs['prompts']).__name__}")
...
def _get_messages(inputs: dict[str, Any]) -> dict:
    ...
    if "input" in input_copy:
        ...
        raise InputFormatError(f"Chat Run expects single List[dict] or List[List[dict]] 'messages'" f" input. Got {inputs}")
...
def _validate_example_inputs_for_language_model(
    ...
    if input_mapper:
        ...
        if not isinstance(prompt_input, str) and not (
            isinstance(prompt_input, list) and all(isinstance(msg, BaseMessage) for msg in prompt_input)
        ):
...
def _determine_reference_key(
    ...
    if config.reference_key:
        ...
        if example_outputs and reference_key not in example_outputs:
            raise ValueError(f"Reference key {reference_key} not in Dataset" f" example outputs: {example_outputs}")
...
def _load_run_evaluators(
    ...
    if config.evaluators or (
        config.custom_evaluators and any([isinstance(e, StringEvaluator) for e in config.custom_evaluators])
    ):
    ...
    for custom_evaluator in custom_evaluators:
        if isinstance(custom_evaluator, RunEvaluator):
            ...
            raise ValueError(
                f"Unsupported custom evaluator: {custom_evaluator}." f" Expected RunEvaluator or StringEvaluator."
            )
...
async def _arun_chain(
    ...
    *,
    ...
    if isinstance(chain, Chain) and isinstance(inputs_, dict) and len(inputs_) == 1 and chain.input_keys:
        ...
        runnable_config = RunnableConfig(tags=tags or [], callbacks=callbacks, metadata=metadata or {})
...
"""
        raise ValueError(f"Test project {project_name} already exists. Please use a different name:" f"\n\n{example_msg}")
...
def _display_aggregate_results(aggregate_results: pd.DataFrame) -> None:
    if _is_jupyter_environment():
        ...
        formatted_string = aggregate_results.to_string(float_format=lambda x: f"{x:.2f}", justify="right")
...
async def arun_on_dataset(
    ...
    **kwargs: Any,
    ...
    if kwargs:
        warn_deprecated(
            "0.0.305",
            message="The following arguments are deprecated and " "will be removed in a future release: " f"{kwargs.keys()}.",
            removal="0.0.305",
        )
...
arun_on_dataset.__doc__ = _RUN_ON_DATASET_DOCSTRING.replace("run_on_dataset(", "await arun_on_dataset(")
