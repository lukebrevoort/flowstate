...
def _infer_model_and_provider(model: str, *, provider: Optional[str] = None) -> tuple[str, str]:
    if not model.strip():
        raise ValueError("Model name cannot be empty")
...
def _check_pkg(pkg: str) -> None:
    ...
    if not util.find_spec(pkg):
        raise ImportError(f"Could not import {pkg} python package. " f"Please install it with `pip install {pkg}`")
...
def init_embeddings(
    ...
    **Note:** Must have the integration package corresponding to the model provider
    ...
    if not model:
        ...
        raise ValueError(f"Must specify model name. Supported providers are: {', '.join(providers)}")
