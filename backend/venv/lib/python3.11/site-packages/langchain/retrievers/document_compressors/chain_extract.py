...
class LLMChainExtractor(BaseDocumentCompressor):
    ...
    def compress_documents(
        ...
        for doc in documents:
            ...
            if len(output) == 0:
                continue
            compressed_docs.append(Document(page_content=cast(str, output), metadata=doc.metadata))
    ...
    async def acompress_documents(
        ...
        for i, doc in enumerate(documents):
            if len(outputs[i]) == 0:
                continue
            compressed_docs.append(Document(page_content=outputs[i], metadata=doc.metadata))
        return compressed_docs
