...
class DocumentCompressorPipeline(BaseDocumentCompressor):
    ...
    def compress_documents(
        ...
        for _transformer in self.transformers:
            if isinstance(_transformer, BaseDocumentCompressor):
                accepts_callbacks = signature(_transformer.compress_documents).parameters.get("callbacks") is not None
                if accepts_callbacks:
                    documents = _transformer.compress_documents(documents, query, callbacks=callbacks)
                else:
    ...
    async def acompress_documents(
        ...
        for _transformer in self.transformers:
            if isinstance(_transformer, BaseDocumentCompressor):
                accepts_callbacks = signature(_transformer.acompress_documents).parameters.get("callbacks") is not None
                if accepts_callbacks:
                    documents = await _transformer.acompress_documents(documents, query, callbacks=callbacks)
                else:
