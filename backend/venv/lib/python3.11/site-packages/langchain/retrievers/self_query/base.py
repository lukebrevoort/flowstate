...
def _get_builtin_translator(vectorstore: VectorStore) -> Visitor:
    ...
    if isinstance(vectorstore, DatabricksVectorSearch):
        ...
        raise ValueError(f"Self query retriever with Vector Store type {vectorstore.__class__}" f" not supported.")
...
class SelfQueryRetriever(BaseRetriever):
    ...
    def _prepare_query(self, query: str, structured_query: StructuredQuery) -> tuple[str, dict[str, Any]]:
    ...
    async def _aget_docs_with_query(self, query: str, search_kwargs: dict[str, Any]) -> list[Document]:
    ...
    def _get_relevant_documents(self, query: str, *, run_manager: CallbackManagerForRetrieverRun) -> list[Document]:
        ...
        """
        structured_query = self.query_constructor.invoke({"query": query}, config={"callbacks": run_manager.get_child()})
        if self.verbose:
    ...
    def from_llm(
        ...
        **kwargs: Any,
        ...
        if "allowed_comparators" not in chain_kwargs and structured_query_translator.allowed_comparators is not None:
            chain_kwargs["allowed_comparators"] = structured_query_translator.allowed_comparators
        if "allowed_operators" not in chain_kwargs and structured_query_translator.allowed_operators is not None:
        ...
        query_constructor = query_constructor.with_config(run_name=QUERY_CONSTRUCTOR_RUN_NAME)
