...
class MapRerankDocumentsChain(BaseCombineDocumentsChain):
    ...
    def validate_llm_output(self) -> Self:
        ...
        if not isinstance(output_parser, RegexParser):
            raise ValueError("Output parser of llm_chain should be a RegexParser," f" got {output_parser}")
        output_keys = output_parser.output_keys
        if self.rank_key not in output_keys:
            raise ValueError(
                f"Got {self.rank_key} as key to rank on, but did not find " f"it in the llm_chain output keys ({output_keys})"
            )
        if self.answer_key not in output_keys:
            raise ValueError(
                f"Got {self.answer_key} as key to return, but did not find " f"it in the llm_chain output keys ({output_keys})"
            )
        return self
    ...
    async def acombine_docs(self, docs: list[Document], callbacks: Callbacks = None, **kwargs: Any) -> tuple[str, dict]:
        """Combine documents in a map rerank manner.
    ...
    def _process_results(
        ...
        sorted_res = sorted(zip(typed_results, docs), key=lambda x: -int(x[0][self.rank_key]))
