    ...
    def func_call(self, func_name: Any, args: list) -> FilterDirective:
        ...
        if isinstance(func, Comparator):
            if self.allowed_attributes and args[0] not in self.allowed_attributes:
                raise ValueError(
                    f"Received invalid attributes {args[0]}. Allowed attributes are " f"{self.allowed_attributes}"
                )
        ...
        if func_name in set(Comparator):
            if self.allowed_comparators is not None:
                if func_name not in self.allowed_comparators:
                    raise ValueError(
                        f"Received disallowed comparator {func_name}. Allowed " f"comparators are {self.allowed_comparators}"
                    )
            ...
            if self.allowed_operators is not None:
                if func_name not in self.allowed_operators:
                    raise ValueError(
                        f"Received disallowed operator {func_name}. Allowed operators" f" are {self.allowed_operators}"
                    )
    ...
    def datetime(self, item: Any) -> ISO8601DateTime:
        ...
        try:
            ...
            try:
                ...
                raise ValueError("Datetime values are expected to be in ISO 8601 format.")
...
def get_parser(
    ...
    if QueryTransformer is None:
        raise ImportError("Cannot import lark, please install it with 'pip install lark'.")
