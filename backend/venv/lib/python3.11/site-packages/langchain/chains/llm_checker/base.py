...
class LLMCheckerChain(Chain):
        ...
        if "llm" in values:
            ...
            if "question_to_checked_assertions_chain" not in values and values["llm"] is not None:
                question_to_checked_assertions_chain = _load_question_to_checked_assertions_chain(
                    values["llm"],
                    values.get("create_draft_answer_prompt", CREATE_DRAFT_ANSWER_PROMPT),
    ...
    def _call(
        ...
        output = self.question_to_checked_assertions_chain({"question": question}, callbacks=_run_manager.get_child())
