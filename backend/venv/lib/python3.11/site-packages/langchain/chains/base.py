...
class Chain(RunnableSerializable[dict[str, Any], dict[str, Any]], ABC):
    ...
    def invoke(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            final_outputs: dict[str, Any] = self.prep_outputs(inputs, outputs, return_only_outputs)
    ...
    async def ainvoke(
        ...
        **kwargs: Any,
        ...
        try:
            self._validate_inputs(inputs)
            outputs = await self._acall(inputs, run_manager=run_manager) if new_arg_supported else await self._acall(inputs)
            final_outputs: dict[str, Any] = await self.aprep_outputs(inputs, outputs, return_only_outputs)
        except BaseException as e:
    ...
    async def _acall(
        ...
        Returns:
            A dict of named outputs. Should contain all outputs specified in
                `Chain.output_keys`.
        """
        return await run_in_executor(None, self._call, inputs, run_manager.get_sync() if run_manager else None)
    ...
    def _run_output_key(self) -> str:
        if len(self.output_keys) != 1:
            raise ValueError(f"`run` not supported when there is not exactly " f"one output key. Got {self.output_keys}.")
    ...
    async def arun(
        ...
        **kwargs: Any,
        ...
        if len(self.output_keys) != 1:
            ...
            if len(args) != 1:
                raise ValueError("`run` supports only one positional argument.")
            return (await self.acall(args[0], callbacks=callbacks, tags=tags, metadata=metadata))[self.output_keys[0]]

        if kwargs and not args:
            return (await self.acall(kwargs, callbacks=callbacks, tags=tags, metadata=metadata))[self.output_keys[0]]
    ...
    def apply(self, input_list: list[builtins.dict[str, Any]], callbacks: Callbacks = None) -> list[builtins.dict[str, str]]:
