...
def _low_confidence_spans(
    ...
    try:
        ...
        _low_idx = [idx for idx, log_prob in enumerate(log_probs) if math.exp(log_prob) < min_prob]  # type: ignore[assignment]
...
class FlareChain(Chain):
    ...
    def _do_retrieval(
        ...
        if isinstance(self.question_generator_chain, LLMChain):
            ...
            questions = self.question_generator_chain.batch(question_gen_inputs, config={"callbacks": callbacks})
    ...
    def _call(
        ...
        for i in range(self.max_iter):
            ...
            tokens, log_probs = _extract_tokens_and_log_probs(
                self.response_chain.invoke(_input, {"callbacks": _run_manager.get_child()})
            )
    ...
    @classmethod
    def from_llm(cls, llm: BaseLanguageModel, max_generation_len: int = 32, **kwargs: Any) -> FlareChain:
        ...
        try:
            ...
            raise ImportError(
                "OpenAI is required for FlareChain. " "Please install langchain-openai." "pip install langchain-openai"
            )
        llm = ChatOpenAI(max_completion_tokens=max_generation_len, logprobs=True, temperature=0)
