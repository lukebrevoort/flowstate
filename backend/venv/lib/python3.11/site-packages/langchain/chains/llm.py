...
class LLMChain(Chain):
    ...
    llm: Union[Runnable[LanguageModelInput, str], Runnable[LanguageModelInput, BaseMessage]]
    ...
    def predict_and_parse(self, callbacks: Callbacks = None, **kwargs: Any) -> Union[str, list[str], dict[str, Any]]:
        ...
        warnings.warn("The predict_and_parse method is deprecated, " "instead pass an output parser directly to LLMChain.")
    ...
    async def apredict_and_parse(self, callbacks: Callbacks = None, **kwargs: Any) -> Union[str, list[str], dict[str, str]]:
        """Call apredict and then parse the results."""
        warnings.warn("The apredict_and_parse method is deprecated, " "instead pass an output parser directly to LLMChain.")
    ...
    def _parse_generation(self, generation: list[dict[str, str]]) -> Sequence[Union[str, list[str], dict[str, str]]]:
        if self.prompt.output_parser is not None:
    ...
    async def aapply_and_parse(
        ...
        """Call apply and then parse the results."""
        warnings.warn("The aapply_and_parse method is deprecated, " "instead pass an output parser directly to LLMChain.")
...
def _get_language_model(llm_like: Runnable) -> BaseLanguageModel:
    if isinstance(llm_like, BaseLanguageModel):
        ...
        raise ValueError(f"Unable to extract BaseLanguageModel from llm_like object of type " f"{type(llm_like)}")
