...
try:
    ...
    class APIChain(Chain):
        ...
        * Use a None if you want to allow all domains by default -- this is not
        ...
        async def _acall(
            ...
            await _run_manager.on_text(api_url, color="green", end="\n", verbose=self.verbose)
            api_url = api_url.strip()
            if self.limit_to_domains and not _check_in_allowed_domain(api_url, self.limit_to_domains):
                raise ValueError(f"{api_url} is not in the allowed domains: {self.limit_to_domains}")
    ...
    class APIChain:  # type: ignore[no-redef]
        def __init__(self, *args: Any, **kwargs: Any) -> None:
            raise ImportError(
                "To use the APIChain, you must install the langchain_community package." "pip install langchain_community"
            )
