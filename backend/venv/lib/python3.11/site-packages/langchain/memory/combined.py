        ...
        for val in value:
            if isinstance(val, BaseChatMemory):
                if val.input_key is None:
                    warnings.warn(
                        "When using CombinedMemory, " "input keys should be so the input is known. " f" Was not set on {val}"
                    )
        return value
        ...
        for memory in self.memories:
            ...
            for key, value in data.items():
                if key in memory_data:
                    raise ValueError(f"The variable {key} is repeated in the CombinedMemory.")
