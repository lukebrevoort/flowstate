...
class TrajectoryEvalChain(AgentTrajectoryEvaluator, LLMEvalChain):
    ...
    output_parser: TrajectoryOutputParser = Field(default_factory=TrajectoryOutputParser)
...
"""
    ...
    def from_llm(
        ...
        **kwargs: Any,
        ...
        if not isinstance(llm, BaseChatModel):
            raise NotImplementedError("Only chat models supported by the current trajectory eval")
        if agent_tools:
    ...
    def _call(
        ...
        raw_output = self.eval_chain.run(chain_input, callbacks=_run_manager.get_child())
    ...
    async def _acall(
        ...
        raw_output = await self.eval_chain.arun(chain_input, callbacks=_run_manager.get_child())
        return cast(dict, self.output_parser.parse(raw_output))
