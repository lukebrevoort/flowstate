    ...
    def import_by_name(name: str) -> Any:
        ...
        if all_module_lookup and name in all_module_lookup:
            ...
            if new_module.split(".")[0] not in ALLOWED_TOP_LEVEL_PKGS:
                raise AssertionError(
                    f"Importing from {new_module} is not allowed. " f"Allowed top-level packages are: {ALLOWED_TOP_LEVEL_PKGS}"
                )
            ...
            try:
                ...
                if not is_interactive_env() and deprecated_lookups and name in deprecated_lookups:
                    # Depth 3:
                    # internal.py
                    # module_import.py
                ...
                raise AttributeError(f"module {new_module} has no attribute {name}") from e
        ...
        if fallback_module:
            try:
                ...
                raise AttributeError(f"module {fallback_module} has no attribute {name}") from e
