...
class BooleanOutputParser(BaseOutputParser[bool]):
    ...
    def parse(self, text: str) -> bool:
        ...
        truthy = {val.upper() for val in re.findall(regexp, text, flags=re.IGNORECASE | re.MULTILINE)}
        if self.true_val.upper() in truthy:
            if self.false_val.upper() in truthy:
                raise ValueError(f"Ambiguous response. Both {self.true_val} and {self.false_val} " f"in received: {text}.")
            return True
