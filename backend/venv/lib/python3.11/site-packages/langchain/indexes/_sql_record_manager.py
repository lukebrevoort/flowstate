    ...
    def update(
        ...
        *,
        ...
        if len(keys) != len(group_ids):
            raise ValueError(f"Number of keys ({len(keys)}) does not match number of " f"group_ids ({len(group_ids)})")
        ...
        with self._make_session() as session:
            if self.dialect == "sqlite":
                ...
                sqlite_insert_stmt: SqliteInsertType = sqlite_insert(UpsertionRecord).values(records_to_upsert)
                ...
                # This code needs to be generalized a bit to work with more dialects.
                pg_insert_stmt: PgInsertType = pg_insert(UpsertionRecord).values(records_to_upsert)
