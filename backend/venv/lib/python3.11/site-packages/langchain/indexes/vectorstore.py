...
def _get_in_memory_vectorstore() -> type[VectorStore]:
    ...
    try:
        ...
        raise ImportError("Please install langchain-community to use the InMemoryVectorStore.")
    warnings.warn(
...
class VectorstoreIndexCreator(BaseModel):
    ...
    vectorstore_cls: type[VectorStore] = Field(default_factory=_get_in_memory_vectorstore)
    ...
    async def afrom_documents(self, documents: list[Document]) -> VectorStoreIndexWrapper:
        ...
        """
        sub_docs = self.text_splitter.split_documents(documents)
        vectorstore = await self.vectorstore_cls.afrom_documents(sub_docs, self.embedding, **self.vectorstore_kwargs)
        return VectorStoreIndexWrapper(vectorstore=vectorstore)
