...
class AgentExecutorIterator:
    ...
    def __iter__(self: AgentExecutorIterator) -> Iterator[AddableDict]:
        ...
        try:
            while self.agent_executor._should_continue(self.iterations, self.time_elapsed):
    ...
    async def _aprocess_next_step_output(
        ...
        if isinstance(next_step_output, AgentFinish):
            logger.debug("Hit AgentFinish: _areturn -> on_chain_end -> run final output logic")
    ...
    def _return(self, output: AgentFinish, run_manager: CallbackManagerForChainRun) -> AddableDict:
        """
        Return the final output of the iterator.
        """
        returned_output = self.agent_executor._return(output, self.intermediate_steps, run_manager=run_manager)
    ...
    async def _areturn(self, output: AgentFinish, run_manager: AsyncCallbackManagerForChainRun) -> AddableDict:
        """
        Return the final output of the async iterator.
        """
        returned_output = await self.agent_executor._areturn(output, self.intermediate_steps, run_manager=run_manager)
        returned_output["messages"] = output.messages
