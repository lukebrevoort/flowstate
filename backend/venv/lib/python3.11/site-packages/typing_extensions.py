        ...
        def __new__(cls, name, bases, ns, *, total=True, closed=None, extra_items=NoExtraItems):
            ...
            if ns_annotations is not None:
                ...
                if own_annotate is not None:
                    own_annotations = annotationlib.call_annotate_function(own_annotate, Format.FORWARDREF, owner=tp_dict)
                else:
                    own_annotations = {}
            else:
                own_annotations = {}
...
if _PEP_696_IMPLEMENTED:
    ...
    class TypeVar(metaclass=_TypeVarLikeMeta):
        ...
        def __new__(
            cls, name, *constraints, bound=None, covariant=False, contravariant=False, default=NoDefault, infer_variance=False
        ):
...
def _create_concatenate_alias(origin, parameters):
    ...
    if sys.version_info >= (3, 10, 3):
        concatenate = _ConcatenateGenericAlias(origin, parameters, _typevar_types=(TypeVar, ParamSpec), _paramspec_tvars=True)
    else:
...
# <=3.13
    ...
    class TypeAliasType:
        ...
        def _check_parameters(self, parameters):
            ...
            return tuple(typing._type_check(item, f"Subscripting {self.__name__} requires a type.") for item in parameters)
        ...
        * STRING: return annotations as strings, in a format close to the original
        ...
        if not eval_str:
            if format is Format.STRING:
                return {key: value if isinstance(value, str) else typing._type_repr(value) for key, value in ann.items()}
            return dict(ann)
