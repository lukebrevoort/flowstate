...
def _check_newer_version(pkg: str, timeout: float = 0.2) -> None:
    ...
    try:
        ...
        thread_logger.info(
            f"Failed to check for newer version of {pkg}." " To disable version checks, set LANGGRAPH_NO_VERSION_CHECK=true"
        )
    ...
    **kwargs: typing.Any,
    ...
    if isinstance(env, str | pathlib.Path):
        try:
            ...
            logger.warning("python_dotenv is not installed. Environment variables will not be available.")
    ...
    if debug_port is not None:
        ...
        logger.info("2. Use the 'Remote Attach' configuration with the following settings:")
                        ...
                        with urllib.request.urlopen(f"{local_url}/ok") as response:
                            if response.status == 200:
                                try:
                                    ...
                                    if org_id:
                                        full_studio_url = (
                                            f"{studio_origin}/studio/?baseUrl={local_url}&organizationId={org_id}"
                                        )
                                except TimeoutError as e:
                                    thread_logger.debug(f"Failed to get organization ID: {str(e)}")
                                    pass
...
"""
        ...
        if nvc is None or nvc.lower() not in ("true", "1"):
            threading.Thread(target=_check_newer_version, args=("langgraph-api",), daemon=True).start()
        supported_kwargs = {k: v for k, v in kwargs.items() if k in inspect.signature(uvicorn.run).parameters}
