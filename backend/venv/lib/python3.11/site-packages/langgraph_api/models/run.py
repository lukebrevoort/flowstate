...
def ensure_ids(
    ...
    try:
        results = [assistant_id if isinstance(assistant_id, UUID) else UUID(assistant_id)]
    except ValueError:
    ...
    if checkpoint_id := payload.get("checkpoint_id"):
        try:
            ...
            raise HTTPException(status_code=422, detail="Invalid checkpoint ID") from None
    else:
        results.append(None)
...
def assign_defaults(
    ...
    if payload.get("stream_mode"):
        stream_mode = payload["stream_mode"] if isinstance(payload["stream_mode"], list) else [payload["stream_mode"]]
    else:
        stream_mode = ["values"]
...
def get_header_patterns() -> tuple[list[re.Pattern[str] | None], list[re.Pattern[str] | None]]:
...
async def create_valid_run(
    ...
    if thread_id is None and (command := payload.get("command")) and command.get("resume"):
        raise HTTPException(
            status_code=400,
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail="Thread or assistant not found.") from None
    ...
    if first["run_id"] == run_id:
        logger.info(
            ...
            after_seconds=after_seconds,
            if_not_exists=if_not_exists,
            run_create_ms=(int(time.time() * 1_000) - request_start_time if request_start_time else None),
...
def _get_ids(
    ...
    assistant_id, thread_id, checkpoint_id = ensure_ids(assistant_id, thread_id, payload)
