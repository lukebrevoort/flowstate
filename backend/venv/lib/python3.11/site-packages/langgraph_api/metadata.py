...
if TENANT_ID:
    try:
        ...
        raise ValueError(f"Invalid tenant ID: {TENANT_ID}. Must be a valid UUID") from None
if VARIANT == "cloud":
    HOST = "saas"
elif PROJECT_ID:
    HOST = "byoc"
...
if LANGSMITH_AUTH_ENDPOINT:
    LANGCHAIN_METADATA_ENDPOINT = LANGSMITH_AUTH_ENDPOINT.rstrip("/") + "/v1/metadata/submit"
...
async def metadata_loop() -> None:
    ...
    if LANGGRAPH_CLOUD_LICENSE_KEY and not LANGGRAPH_CLOUD_LICENSE_KEY.startswith("lcl_") and not LANGSMITH_API_KEY:
    ...
    while True:
        ...
        if LANGGRAPH_CLOUD_LICENSE_KEY and LANGGRAPH_CLOUD_LICENSE_KEY.startswith("lcl_"):
            beacon_payload = {
                **base_payload,
            ...
            try:
                ...
                await logger.awarning("Beacon metadata submission failed.", error=str(e))
        ...
        if LANGSMITH_API_KEY and LANGCHAIN_METADATA_ENDPOINT:
            ...
            try:
                ...
                await logger.awarning("LangChain metadata submission failed.", error=str(e))

        if submissions_attempted and len(submissions_failed) == len(submissions_attempted):
