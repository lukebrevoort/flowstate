...
class HTTPMetricsCollector:
    def __init__(self):
        ...
        self._histogram_bucket_labels = ["+Inf" if value == float("inf") else str(value) for value in self._histogram_buckets]
    ...
    def record_request(self, method: str, route: Any, status: int, latency_ms: float) -> None:
        ...
        if request_count_key not in self._request_counts and len(self._request_counts) >= MAX_REQUEST_COUNT_ENTRIES:
            return

        if histogram_key not in self._histogram_data and len(self._histogram_data) >= MAX_HISTOGRAM_ENTRIES:
            return
    ...
    def get_metrics(self, project_id: str, revision_id: str, format: str = "prometheus") -> dict | list[str]:
        if format == "json":
            return {
                "api": {
                    "http_requests_total": [
