...
MAX_STREAM_CHUNK_SIZE_BYTES = env("MAX_STREAM_CHUNK_SIZE_BYTES", cast=int, default=1024 * 1024 * 128)
...
CHECKPOINTER_CONFIG: CheckpointerConfig | None = env("LANGGRAPH_CHECKPOINTER", cast=_parse_json, default=None)
THREAD_TTL: ThreadTTLConfig | None = env("LANGGRAPH_THREAD_TTL", cast=_parse_thread_ttl, default=None)
...
LANGGRAPH_POSTGRES_EXTENSIONS: Literal["standard", "lite"] = env("LANGGRAPH_POSTGRES_EXTENSIONS", cast=str, default="standard")
if LANGGRAPH_POSTGRES_EXTENSIONS not in ("standard", "lite"):
    raise ValueError(f"Unknown LANGGRAPH_POSTGRES_EXTENSIONS value: {LANGGRAPH_POSTGRES_EXTENSIONS}")
...
if LANGGRAPH_AUTH_TYPE == "langsmith":
    ...
    LANGSMITH_AUTH_VERIFY_TENANT_ID = env("LANGSMITH_AUTH_VERIFY_TENANT_ID", cast=bool, default=True)
...
LANGSMITH_API_KEY = env("LANGSMITH_API_KEY", cast=str, default=getenv("LANGCHAIN_API_KEY", ""))
...
# Metrics.
USES_INDEXING = STORE_CONFIG and STORE_CONFIG.get("index") and STORE_CONFIG.get("index").get("embed")
...
if not os.getenv("LANGCHAIN_REVISION_ID") and (ref_sha := os.getenv("LANGSMITH_LANGGRAPH_GIT_REF_SHA")):
