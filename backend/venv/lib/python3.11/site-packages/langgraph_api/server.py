...
if not ((disable_truststore := os.getenv("DISABLE_TRUSTSTORE")) and disable_truststore.lower() == "true"):
...
if config.HTTP_CONFIG and (app := config.HTTP_CONFIG.get("app")) and is_js_path(app.split(":")[0]):
...
if user_router:
    ...
    custom_route_paths = [route.path for route in user_router.router.routes if route.path not in meta_route_paths]
...
if config.MOUNT_PREFIX:
    ...
    class ASGIBypassMiddleware:
        ...
        async def __call__(self, scope: Scope, receive: Receive, send: Send) -> typing.Any:
