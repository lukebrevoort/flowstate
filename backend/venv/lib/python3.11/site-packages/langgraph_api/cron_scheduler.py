        ...
        try:
            async with connect() as conn:
                async for cron in Crons.next(conn):
                    ...
                    async with set_auth_ctx_for_run(run_payload, user_id=cron["user_id"]):
                        logger.debug(f"Scheduling cron run {cron}")
                        try:
                            run = await create_valid_run(
                                conn,
                                thread_id=(str(cron.get("thread_id")) if cron.get("thread_id") else None),
                                payload=run_payload,
                                headers={},
                            )
                            if not run:
                            ...
                            logger.error(
                                "Error scheduling cron run cron_id={}".format(cron["cron_id"]),
                                exc_info=e,
                            )
                        next_run_date = await run_in_executor(None, next_cron_date, cron["schedule"], cron["now"])
