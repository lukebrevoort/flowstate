...
def _graph_schemas(graph: Pregel) -> dict:
    try:
        ...
        logger.warning(f"Failed to get input schema for graph {graph.name} with error: `{str(e)}`")
        input_schema = None
    try:
        output_schema = graph.get_output_jsonschema()
    except Exception as e:
        logger.warning(f"Failed to get output schema for graph {graph.name} with error: `{str(e)}`")
        output_schema = None
    try:
        state_schema = _state_jsonschema(graph)
    except Exception as e:
        logger.warning(f"Failed to get state schema for graph {graph.name} with error: `{str(e)}`")
        state_schema = None
    try:
        config_schema = _get_configurable_jsonschema(graph)
    except Exception as e:
        logger.warning(f"Failed to get config schema for graph {graph.name} with error: `{str(e)}`")
        config_schema = None
    return {
...
async def get_assistant_graph(
    ...
    async with get_graph(
        ...
        try:
            ...
            raise HTTPException(422, detail="The graph does not support visualization") from None
...
async def set_latest_assistant_version(request: ApiRequest) -> ApiResponse:
    ...
    validate_uuid(assistant_id, "Invalid assistant ID: must be a UUID")
    async with connect() as conn:
        assistant = await Assistants.set_latest(conn, assistant_id, payload.get("version"))
...
assistants_routes: list[BaseRoute] = [
    ...
    ApiRoute("/assistants/{assistant_id}/versions", get_assistant_versions, methods=["POST"]),
    ...
    ApiRoute("/assistants/{assistant_id}/schemas", get_assistant_schemas, methods=["GET"]),
    ApiRoute("/assistants/{assistant_id}/subgraphs", get_assistant_subgraphs, methods=["GET"]),
    ApiRoute(
