...
async def cancel_runs(
    ...
    action_str = request.query_params.get("action", "interrupt")
    action: Literal["interrupt", "rollback"] = action_str if action_str in ("interrupt", "rollback") else "interrupt"
...
runs_routes = [
    ...
    (ApiRoute("/runs/crons", create_cron, methods=["POST"]) if config.FF_CRONS_ENABLED and plus_features_enabled() else None),
    (
