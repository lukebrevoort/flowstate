...
async def handle_post_request(request: ApiRequest) -> Response:
    ...
    if not is_valid_accept_header(request):
        return create_error_response("Accept header must include application/json or text/event-stream", 400)
    ...
    if message.get("jsonrpc") != "2.0":
        return create_error_response("Invalid JSON-RPC message. Missing or invalid jsonrpc version.", 400)
...
async def handle_jsonrpc_request(
    ...
    if len(exists) != 1:
        raise AssertionError("Internal server error. Invalid response in MCP protocol implementation.")
...
async def handle_tools_list(request: ApiRequest, params: dict[str, Any]) -> dict[str, Any]:
    ...
    assistants = await client.assistants.search(offset=cursor, limit=DEFAULT_PAGE_SIZE, headers=request.headers)
...
async def handle_tools_call(request: ApiRequest, params: dict[str, Any]) -> dict[str, Any]:
    ...
    assistants = await client.assistants.search(limit=MAX_ASSISTANTS, headers=request.headers)
    matching_assistant = [assistant for assistant in assistants if assistant["name"] == tool_name]
