...
async def put_item(request: ApiRequest):
    ...
    await (await get_store()).aput(namespace, handler_payload["key"], handler_payload["value"])
...
async def delete_item(request: ApiRequest):
    ...
    await (await get_store()).adelete(handler_payload["namespace"], handler_payload["key"])
    return Response(status_code=204)
