...
async def search_threads(
    ...
    return ApiResponse([thread async for thread in iter], headers={"X-Pagination-Total": str(total)})
...
async def get_thread_state(
    ...
    async with connect() as conn:
        state = state_snapshot_to_thread_state(
            await Threads.State.get(conn, {"configurable": {"thread_id": thread_id}}, subgraphs=subgraphs)
        )
...
async def get_thread_history(
    ...
    async with connect() as conn:
        states = [
            ...
            for c in await Threads.State.list(conn, config=config, limit=limit, before=before)
        ]
...
threads_routes: list[BaseRoute] = [
    ...
    ApiRoute("/threads/{thread_id}/state", endpoint=update_thread_state, methods=["POST"]),
    ApiRoute("/threads/{thread_id}/history", endpoint=get_thread_history, methods=["GET"]),
