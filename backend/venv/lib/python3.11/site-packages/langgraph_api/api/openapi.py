...
def get_openapi_spec() -> str:
    ...
    if LANGGRAPH_AUTH_TYPE == "langsmith":
        ...
        openapi["components"]["securitySchemes"] = {"x-api-key": {"type": "apiKey", "in": "header", "name": "x-api-key"}}
    if LANGGRAPH_AUTH:
        ...
        for schema_name in webhook_schemas:
            if schema_name in openapi["components"]["schemas"]:
                ...
                if "properties" in schema and "webhook" in schema["properties"]:
                    ...
                    logger.info(f"Removed webhook parameter from {schema_name} schema due to disable_webhooks setting")
...
def merge_openapi_specs(spec_a: dict, spec_b: dict) -> dict:
    ...
    merged["components"] = _merge_components(merged.get("components", {}), spec_b.get("components", {}))
    ...
    merged["tags"] = _merge_lists(merged.get("tags", []), spec_b.get("tags", []), key_func=lambda x: x.get("name"))
...
def _ensure_hashable(obj, depth=0, max_depth=3):
    ...
    if isinstance(obj, typing.Mapping):
        return tuple(sorted((k, _ensure_hashable(v, depth + 1, max_depth)) for k, v in obj.items()))
    return obj
