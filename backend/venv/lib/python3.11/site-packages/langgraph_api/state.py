...
def state_interrupt_to_thread_interrupt(interrupt: Interrupt | dict) -> dict:
    ...
    return {
        "interrupt_id": interrupt.interrupt_id if hasattr(interrupt, "interrupt_id") else None,
...
def state_snapshot_to_thread_state(state: StateSnapshot) -> ThreadState:
    return {
        ...
        "tasks": [
            {
                ...
                "path": t.path,
                "error": t.error,
                "interrupts": [state_interrupt_to_thread_interrupt(i) for i in t.interrupts],
                "checkpoint": (
                    t.state["configurable"] if t.state is not None and not isinstance(t.state, StateSnapshot) else None
                ),
                "state": state_snapshot_to_thread_state(t.state) if isinstance(t.state, StateSnapshot) else None,
        ...
        "parent_checkpoint_id": state.parent_config["configurable"]["checkpoint_id"] if state.parent_config else None,
    }
