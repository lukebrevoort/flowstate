...
async def wait_if_not_done(coro: Coroutine[Any, Any, T], done: ValueEvent) -> T:
    ...
    try:
        async with asyncio.TaskGroup() as tg:
            ...
            coro_task.add_done_callback(lambda _: done_task.cancel("Coro task completed"))
...
def create_task(coro: Coroutine[Any, Any, T], ignore_exceptions: tuple[Exception, ...] = ()) -> asyncio.Task[T]:
...
def call_soon_in_main_loop(coro: Coroutine[Any, Any, T]) -> asyncio.Future[T]:
    ...
    _MAIN_LOOP.call_soon_threadsafe(lg_future.chain_future, main_loop_fut, this_loop_fut)
...
class SimpleTaskGroup(AbstractAsyncContextManager["SimpleTaskGroup"]):
    ...
    def _create_task_done_callback(self, ignore_exceptions: tuple[Exception, ...], task: asyncio.Task) -> None:
        try:
