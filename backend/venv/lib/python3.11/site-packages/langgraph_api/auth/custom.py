...
def get_custom_auth_middleware() -> AuthenticationBackend:
    ...
    if not LANGGRAPH_AUTH:
        raise ValueError("LANGGRAPH_AUTH must be set to a Python file path or a config dict" " to use custom authentication.")
...
def get_auth_instance() -> Auth | Literal["js"] | None:
    logger.info(f"Getting auth instance: {LANGGRAPH_AUTH}", langgraph_auth=str(LANGGRAPH_AUTH))
    if not LANGGRAPH_AUTH:
        return None
...
class CustomAuthBackend(AuthenticationBackend):
    def __init__(
        ...
        if not inspect.iscoroutinefunction(fn):
            ...
            self.fn = fn
        self._param_names = _get_named_arguments(fn, supported_params=SUPPORTED_PARAMETERS) if fn else None
        self.ls_auth = None
    ...
    def __str__(self):
        return f"CustomAuthBackend(fn={self.fn}, " f"ls_auth={self.ls_auth}, " f"param_names={self._param_names}" ")"

    async def authenticate(self, conn: HTTPConnection) -> tuple[AuthCredentials, BaseUser] | None:
        if self.ls_auth is not None and ((auth_scheme := conn.headers.get("x-auth-scheme")) and auth_scheme == "langsmith"):
            return await self.ls_auth.authenticate(conn)
        if self.fn is None:
            return None
        try:
            args = _extract_arguments_from_scope(conn.scope, self._param_names, request=Request(conn.scope))
...
def _get_auth_instance(path: str | None = None) -> Auth | Literal["js"] | None:
    ...
    if auth_instance is not None and (deps := _get_dependencies(auth_instance._authenticate_handler)):
        auth_instance._authenticate_handler = _solve_fastapi_dependencies(auth_instance._authenticate_handler, deps)
...
def _get_dependencies(fn: Callable | None) -> dict[str, Any] | None:
    ...
    annotations = inspect.get_annotations(fn) if hasattr(inspect, "get_annotations") else getattr(fn, "__annotations__", {})
    deps = {}
...
def _solve_fastapi_dependencies(fn: Callable[..., Any], deps: Mapping[str, Any]) -> Callable:
    ...
    dependents = {name: get_parameterless_sub_dependant(depends=dep, path="") for name, dep in deps.items()}
    ...
    _param_names = {
        k for k in _get_named_arguments(fn, supported_params=SUPPORTED_PARAMETERS | dict(deps)) if k not in dependents
    }
        ...
        async with AsyncExitStack() as stack:
            ...
            all_injected = await asyncio.gather(
                *(
                    ...
                    for dependent, solved in zip(dependents.values(), all_solved, strict=False)
                )
            )
            kwargs = {name: value for name, value in zip(dependents.keys(), all_injected, strict=False)}
            other_params = _extract_arguments_from_scope(scope, _param_names, request=request)
...
def _normalize_auth_response(
    ...
    if isinstance(response, tuple):
        if len(response) != 2:
            raise ValueError(f"Expected a tuple with two elements (permissions, user), got {len(response)}")
...
def _load_auth_obj(path: str) -> Auth | Literal["js"]:
    ...
    if ":" not in path:
        raise ValueError(f"Invalid auth path format: {path}. " "Must be in format: './path/to/file.py:name' or 'module:name'")
    ...
    try:
        ...
        if loaded_auth is None:
            raise ValueError(f"Could not find auth '{callable_name}' in module: {module_name}")
...
async def _run_async(dep_call: Callable[..., Any], values: dict[str, Any], is_coroutine: bool) -> Any:
...
def _get_named_arguments(fn: Callable, supported_params: dict) -> set[str]:
    ...
    if unsupported:
        supported_str = "\n".join(
            f"  - {name} ({getattr(typ, '__name__', str(typ))})" for name, typ in supported_params.items()
        )
        raise ValueError(
