...
class LangsmithAuthBackend(AuthenticationBackend):
    ...
    async def authenticate(self, conn: HTTPConnection) -> tuple[AuthCredentials, BaseUser] | None:
        ...
        async with auth_client() as auth:
            if not LANGSMITH_AUTH_VERIFY_TENANT_ID and not conn.headers.get("x-api-key"):
                ...
                res = await auth.get("/auth/verify", headers=[h for h in headers if h[1] is not None])
            else:
                res = await auth.get("/auth/public", headers=[h for h in headers if h[1] is not None])
