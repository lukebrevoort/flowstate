    ...
    *,
    ...
    async def register_graph_db():
        async with connect() as conn:
            ...
            assistant_name = graph_name if graph_name is not None and graph_name != "LangGraph" else graph_id
...
def register_graph_sync(graph_id: str, graph: GraphValue, config: dict | None = None) -> None:
    ...
    if graph_id in FACTORY_ACCEPTS_CONFIG:
        ...
        if store is not None:
            if USE_RUNTIME_API:
                ...
                if ((runtime := config["configurable"].get(CONFIG_KEY_RUNTIME)) is not None) and runtime.store is None:
...
def _load_graph_config_from_env() -> dict | None:
    ...
    try:
        ...
        raise ValueError(
            "Provided environment variable LANGGRAPH_CONFIG must be a valid JSON object" f"\nFound: {config_str}"
        ) from e

    if not isinstance(config_per_id, dict):
        raise ValueError("Provided environment variable LANGGRAPH_CONFIG must be a JSON object" f"\nFound: {config_str}")
...
async def collect_graphs_from_env(register: bool = False) -> None:
    ...
    if paths_str:
        ...
        specs = [
            GraphSpec(
                ...
                path=graph_path,
                config=config_per_graph.get(graph_path.split("/")[-1].replace(".py", "")),
            )
    ...
    if js_specs:
        ...
        if config.HTTP_CONFIG and config.HTTP_CONFIG.get("app") and is_js_path(config.HTTP_CONFIG.get("app").split(":")[0]):
            js_bg_tasks.add(
                asyncio.create_task(
                    run_js_http_process(
                        paths_str,
        ...
        for spec in js_specs:
            ...
            if register:
                await register_graph(spec.id, graph, spec.config, description=spec.description)
...
def _graph_from_spec(spec: GraphSpec) -> GraphValue:
    ...
    if spec.module:
        ...
        try:
            modname = spec.path.replace("/", "__").replace(".py", "").replace(" ", "_").lstrip(".")
    ...
    if spec.variable:
        try:
            ...
            if available:
                ...
                if likely:
                    ...
                    likely_ = "\n".join([f"\t- {prefix}:{k}" if prefix else k for k in likely])
        ...
        if callable(graph):
            ...
            if config.API_VARIANT == "local_dev":
                ...
                if has_checkpointer or has_store:
                    components = []
                    if has_checkpointer:
                        components.append(f"checkpointer (type {type(graph.checkpointer)})")
                    if has_store:
            ...
            raise ValueError(f"Variable '{spec.variable}' in module '{spec.path}' is not a Graph or Graph factory function")
    else:
        ...
        for _, member in inspect.getmembers(module):
            ...
            for _, member in inspect.getmembers(module):
                ...
                raise ValueError(f"Could not find a Graph in module at path: {spec.path}")
    ...
    if ".py:" in embed:
        ...
        try:
            ...
            if embedding_fn is None:
                raise ValueError(f"Could not find embeddings function '{function}' in module: {module_name}")

            if isinstance(embedding_fn, Embeddings):
                return embedding_fn
            elif not callable(embedding_fn):
                raise ValueError(f"Embeddings function '{function}' in module: {module_name} is not callable")
