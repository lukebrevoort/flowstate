...
@asynccontextmanager
async def set_auth_ctx_for_run(run_kwargs: dict, user_id: str | None = None) -> AsyncGenerator[None, None]:
...
async def worker(
    ...
    request_created_at: int | None = run["kwargs"]["config"]["configurable"].get("__request_start_time_ms__")
    ...
    run_creation_ms = (
        int(((run["created_at"].timestamp() - after_seconds) * 1_000) - request_created_at)
    ...
    async with Runs.enter(run_id, main_loop) as done:
        ...
        try:
            if attempt > BG_JOB_MAX_RETRIES:
                await logger.aerror(
                    ...
                    run_id=run["run_id"],
                    run_completed_in_ms=(
                        int((time.time() * 1_000) - request_created_at) if request_created_at is not None else None
                    ),
                )
        ...
        async with connect() as conn:
            log_info = {
                ...
                "run_completed_in_ms": (
                    int((run_ended_at_dt.timestamp() * 1_000) - request_created_at) if request_created_at is not None else None
                ),
            }
            ...
            if exception is None:
                ...
                if checkpoint is None and not temporary:
                    ...
                    try:
                        state_snapshot = await Threads.State.get(conn, run["kwargs"]["config"])
                ...
                if not temporary:
                    await Threads.set_joint_status(conn, run["thread_id"], run_id, status, checkpoint=checkpoint)
                ...
                if not temporary:
                    await Threads.set_joint_status(conn, run["thread_id"], run_id, status, checkpoint, exception)
