    ...
    async def fetch_graph(
        ...
        *,
        ...
        return DrawableGraph(
            {data["id"]: Node(data["id"], data["id"], NoopModel(), data.get("metadata")) for data in nodes},
            {
                Edge(
                    data["source"],
                    data["target"],
...
async def run_js_process(paths_str: str, watch: bool = False):
    ...
    if tsx_path is None:
        raise FileNotFoundError("tsx not found in PATH. Please upgrade to latest LangGraph CLI to support running JS graphs.")
    attempt = 0
    while not asyncio.current_task().cancelled():
        client_file = os.path.join(os.path.dirname(__file__), "client.mts")
        client_preload_file = os.path.join(os.path.dirname(__file__), "src", "preload.mjs")
...
async def run_remote_checkpointer():
    ...
    async def store_batch(payload: dict):
        ...
        for op in operations:
            if "value" in op:
                ...
                processed_operations.append(GetOp(namespace=tuple(op["namespace"]), key=op["key"]))
    ...
    async def store_search(payload: dict):
        ...
        store = await _get_passthrough_store()
        result = await store.asearch(namespace_prefix, filter=filter, limit=limit, offset=offset, query=query)
    ...
    remote = Starlette(
        routes=[
            ...
            Route("/checkpointer_list", wrap_handler(checkpointer_list), methods=["POST"]),
            Route("/checkpointer_put", wrap_handler(checkpointer_put), methods=["POST"]),
            Route(
...
async def handle_js_auth_event(
    ...
    if isinstance(value, dict) and (updated_value := response.get("value")):
        if isinstance(value.get("metadata"), dict) and (metadata := updated_value.get("metadata")):
...
class JSCustomHTTPProxyMiddleware:
    ...
    async def __call__(self, scope: types.Scope, receive: types.Receive, send: types.Send) -> None:
        ...
        with DisableHttpxLoggingContextManager(filter=lambda record: "__langgraph_check" not in record.getMessage()):
        ...
        async with self.proxy_client.stream(
            ...
            if response.status_code == 404 and response.headers.get("x-langgraph-status") == "not-found":
