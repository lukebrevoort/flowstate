...
def _preproces_debug_checkpoint_task(task: dict[str, Any]) -> dict[str, Any]:
    if "state" not in task or not task["state"] or "configurable" not in task["state"] or not task["state"]["configurable"]:
        return task
...
async def astream_state(
    ...
    *,
                ...
                if event["event"] == "on_chain_stream" and event["run_id"] == run_id:
                    ...
                    if mode == "messages":
                        if "messages-tuple" in stream_mode:
                            ...
                            yield (
                                ("messages/partial" if isinstance(msg, BaseMessageChunk) else "messages/complete"),
    ...
    # Get feedback URLs
    if feedback_keys:
        feedback_urls = await run_in_executor(None, get_feedback_urls, run_id, feedback_keys)
