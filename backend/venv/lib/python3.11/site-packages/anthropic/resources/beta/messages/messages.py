    ...
    def create(
        ...
        *,
        ...
        if not stream and not is_given(timeout) and self._client.timeout == DEFAULT_TIMEOUT:
            timeout = self._client._calculate_nonstreaming_timeout(max_tokens, MODEL_NONSTREAMING_TOKENS.get(model, None))
        ...
        *,
        ...
        extra_headers = {
            **strip_not_given(
                {
                    "anthropic-beta": (
                        ",".join(chain((str(e) for e in betas), ["token-counting-2024-11-01"]))
                        if is_given(betas)
                        else NOT_GIVEN
                    )
                }
