    ...
    for curr in messages:
        if isinstance(curr, ToolMessage):
            if (
                ...
                and curr.content
                and all(isinstance(block, dict) and block.get("type") == "tool_result" for block in curr.content)
            ):
...
def _format_data_content_block(block: dict) -> dict:
    ...
    if block["type"] == "image":
        if block["source_type"] == "url":
            ...
            raise ValueError("Anthropic only supports 'url' and 'base64' source_type for image " "content blocks.")
    ...
    for i, message in enumerate(merged_messages):
        if message.type == "system":
            if system is not None:
                ...
                system = [(block if isinstance(block, dict) else {"type": "text", "text": block}) for block in message.content]
            else:
        ...
        if not isinstance(message.content, str):
            ...
            if not isinstance(message.content, list):
                raise ValueError("Anthropic message content must be str or list of dicts")
            ...
            for block in message.content:
                if isinstance(block, str):
                    ...
                    if "type" not in block:
                        ...
                        if isinstance(message, AIMessage) and block["id"] in [tc["id"] for tc in message.tool_calls]:
                            overlapping = [tc for tc in message.tool_calls if tc["id"] == block["id"]]
                        ...
                        if text.strip():
                            content.append({k: v for k, v in block.items() if k in ("type", "text", "cache_control")})
                    elif block["type"] == "thinking":
                        content.append(
                            {k: v for k, v in block.items() if k in ("type", "thinking", "cache_control", "signature")}
                        )
                    elif block["type"] == "redacted_thinking":
                        content.append({k: v for k, v in block.items() if k in ("type", "cache_control", "data")})
                    elif block["type"] == "tool_result":
                        tool_content = _format_messages([HumanMessage(block["content"])])[1][0]["content"]
                    ...
                    raise ValueError(f"Content blocks must be str or dict, instead was: " f"{type(block)}")
        else:
        ...
        if isinstance(message, AIMessage) and message.tool_calls:
            content = content or []
            content = [{"type": "text", "text": message.content}] if isinstance(content, str) and content else content
            tool_use_ids = [cast(dict, block)["id"] for block in content if cast(dict, block)["type"] == "tool_use"]
            missing_tool_calls = [tc for tc in message.tool_calls if tc["id"] not in tool_use_ids]
            cast(list, content).extend(_lc_tool_calls_to_anthropic_tool_use_blocks(missing_tool_calls))
    ...
    **NOTE**: Any param which is not explicitly supported will be passed directly to the
        ...
        **kwargs: Any,
        ...
        try:
            ...
            coerce_content_to_string = (
                not _tools_in_params(payload) and not _documents_in_params(payload) and not _thinking_in_params(payload)
            )
    ...
    def _format_output(self, data: Any, **kwargs: Any) -> ChatResult:
        ...
        for block in content:
            if isinstance(block, dict) and "citations" in block and block["citations"] is None:
                block.pop("citations")
            if isinstance(block, dict) and block.get("type") == "thinking" and "text" in block and block["text"] is None:
                block.pop("text")
        ...
        if len(content) == 1 and content[0]["type"] == "text" and not content[0].get("citations"):
    ...
    def bind_tools(
        ...
        **kwargs: Any,
        ...
        """  # noqa: E501
        formatted_tools = [tool if _is_builtin_tool(tool) else convert_to_anthropic_tool(tool) for tool in tools]
        if not tool_choice:
            pass
            ...
            raise ValueError(f"Unrecognized 'tool_choice' type {tool_choice=}. Expected dict, " f"str, or None.")
    ...
    def with_structured_output(
        ...
        **kwargs: Any,
        ...
        if self.thinking is not None and self.thinking.get("type") == "enabled":
            llm = self._get_llm_for_structured_output_when_thinking_is_enabled(schema, formatted_tool)
        else:
            llm = self.bind_tools(
                [schema],
...
def _tools_in_params(params: dict) -> bool:
    return "tools" in params or ("extra_body" in params and params["extra_body"].get("tools")) or "mcp_servers" in params
...
def _documents_in_params(params: dict) -> bool:
    for message in params.get("messages", []):
        if isinstance(message.get("content"), list):
            for block in message["content"]:
                if isinstance(block, dict) and block.get("type") == "document" and block.get("citations", {}).get("enabled"):
                    return True
    return False
...
def _make_message_chunk_from_anthropic_event(
    ...
    *,
    ...
    if event.type == "message_start" and stream_usage:
        ...
        usage_metadata["total_tokens"] = usage_metadata["total_tokens"] - usage_metadata["output_tokens"]
