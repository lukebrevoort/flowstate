...
def _set_config_context(config: RunnableConfig, run: Any = None) -> Token[RunnableConfig | None]:
...
@contextmanager
def set_config_context(config: RunnableConfig, run: Any = None) -> Generator[Context, None, None]:
    """Set the child Runnable config + tracing context.
...
class RunnableCallable(Runnable):
    ...
    def __repr__(self) -> str:
        repr_args = {k: v for k, v in self.__dict__.items() if k not in {"name", "func", "afunc", "config", "kwargs", "trace"}}
    ...
    def invoke(self, input: Any, config: RunnableConfig | None = None, **kwargs: Any) -> Any:
        ...
        for kw, (config_key, default_value) in self.func_accepts.items():
            ...
            if (
                ...
                config_key not in _conf
                and default_value is inspect.Parameter.empty
            ):
                raise ValueError(f"Missing required config key '{config_key}' for '{self.name}'.")
...
def is_async_callable(
    ...
    return asyncio.iscoroutinefunction(func) or hasattr(func, "__call__") and asyncio.iscoroutinefunction(func.__call__)
...
def is_async_generator(
    ...
    return inspect.isasyncgenfunction(func) or hasattr(func, "__call__") and inspect.isasyncgenfunction(func.__call__)
...
def coerce_to_runnable(thing: RunnableLike, *, name: str | None, trace: bool) -> Runnable:
    ...
    if isinstance(thing, Runnable):
        ...
        raise TypeError(f"Expected a Runnable, callable or dict." f"Instead got an unsupported type: {type(thing)}")
...
class RunnableSeq(Runnable):
    ...
    def __init__(
        ...
        *steps: RunnableLike,
        ...
        if len(steps_flat) < 2:
            raise ValueError(f"RunnableSeq must have at least 2 steps, got {len(steps_flat)}")
            ...
            for i, step in enumerate(self.steps):
                ...
                if i == 0:
                    if ASYNCIO_ACCEPTS_CONTEXT:
                        ...
                        # run in context
                        with set_config_context(config, run) as context:
                            input = await asyncio.create_task(step.ainvoke(input, config, **kwargs), context=context)
                    else:
                ...
                try:
                    async with AsyncExitStack() as stack:
                        ...
                        if _StreamingCallbackHandler is not None:
                            for h in run_manager.handlers:
                                if isinstance(h, _StreamingCallbackHandler):
                                    aiterator = h.tap_output_aiter(run_manager.run_id, aiterator)
                        # consume into final output
                        output = await asyncio.create_task(_consume_aiter(aiterator), context=context)
                        # sequence doesn't emit output, yield to mark as generator
                        yield
                except BaseException as e:
                    await run_manager.on_chain_error(e)
