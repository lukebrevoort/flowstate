...
def _is_optional_type(type_: Any) -> bool:
    ...
    if hasattr(type_, "__origin__") and hasattr(type_, "__args__"):
        ...
        if origin is Optional:
            return True
        if origin is Union:
            return any(arg is type(None) or _is_optional_type(arg) for arg in type_.__args__)
...
def get_field_default(name: str, type_: Any, schema: type[Any]) -> Any:
    ...
    if dataclasses.is_dataclass(schema):
        field_info = next((f for f in dataclasses.fields(schema) if f.name == name), None)
        if field_info:
            if field_info.default is not dataclasses.MISSING and field_info.default is not ...:
        ...
        try:
            if hasattr(type, "model_fields") and name in type.model_fields:
                ...
                if hasattr(field, "default") and field.default is not None:
                    ...
                    if hasattr(default, "__class__") and getattr(default.__class__, "__name__", "") == "PydanticUndefinedType":
                        default = None
...
def get_update_as_tuples(input: Any, keys: Sequence[str]) -> list[tuple[str, Any]]:
    ...
    return [
        ...
        and (value is not None or defaults.get(k, MISSING) is not None or (keep is not None and k in keep))
    ]


ANNOTATED_KEYS_CACHE: weakref.WeakKeyDictionary[type[Any], tuple[str, ...]] = weakref.WeakKeyDictionary()
