...
class _NodeWithWriterStore(Protocol[StateT_contra]):
    def __call__(self, state: StateT_contra, *, writer: StreamWriter, store: BaseStore) -> Any: ...


class _NodeWithConfigWriter(Protocol[StateT_contra]):
    def __call__(self, state: StateT_contra, *, config: RunnableConfig, writer: StreamWriter) -> Any: ...


class _NodeWithConfigStore(Protocol[StateT_contra]):
    def __call__(self, state: StateT_contra, *, config: RunnableConfig, store: BaseStore) -> Any: ...
...
class StateGraph(Generic[StateT, InputT, OutputT]):
    ...
    def _all_edges(self) -> set[tuple[str, str]]:
        return self.edges | {(start, end) for starts, end in self.waiting_edges for start in starts}
        ...
        **kwargs: Unpack[DeprecatedKwargs],
        ...
        for character in (NS_SEP, NS_END):
            if character in node:
                raise ValueError(f"'{character}' is a reserved character and is not allowed in the node names.")

        ends: tuple[str, ...] | dict[str, str] = EMPTY_SEQ
        try:
            if (isfunction(action) or ismethod(action) or ismethod(getattr(action, "__call__", None))) and (
                hints := get_type_hints(getattr(action, "__call__")) or get_type_hints(action)
            ):
                if input_schema is None:
                    first_parameter_name = next(iter(inspect.signature(cast(FunctionType, action)).parameters.keys()))
    ...
    def add_edge(self, start_key: str | list[str], end_key: str) -> Self:
        ...
        if isinstance(start_key, str):
            ...
            if not hasattr(self, "channels") and start_key in set(start for start, _ in self.edges):
                raise ValueError(
                ...
                if branch.ends is not None:
                    for end in branch.ends.values():
                        if end not in self.nodes and end != END:
                            raise ValueError(f"At '{start}' node, '{cond}' branch found unknown target '{end}'")
                        all_targets.add(end)
                else:
                    all_targets.add(END)
    ...
    def compile(
        ...
        *,
        ...
        self.validate(
            interrupt=(
                (interrupt_before if interrupt_before != "*" else []) + interrupt_after if interrupt_after != "*" else []
            )
        )
        ...
        output_channels = (
            "__root__"
            if len(self.schemas[self.output_schema]) == 1 and "__root__" in self.schemas[self.output_schema]
...
class CompiledStateGraph(Pregel[StateT, InputT, OutputT], Generic[StateT, InputT, OutputT]):
    ...
    def attach_node(self, key: str, node: StateNodeSpec | None) -> None:
        if key == START:
            output_keys = [k for k, v in self.builder.schemas[self.builder.input_schema].items() if not is_managed_value(v)]
        else:
                ...
                for i in input:
                    if isinstance(i, Command):
                        if i.graph == Command.PARENT:
                            continue
                        updates.extend((k, v) for k, v in i._update_as_tuples() if k in output_keys)
                    else:
        ...
        write_entries: tuple[ChannelWriteEntry | ChannelWriteTupleEntry, ...] = (
            ChannelWriteTupleEntry(mapper=_get_root if output_keys == ["__root__"] else _get_updates),
            ChannelWriteTupleEntry(
                mapper=_control_branch,
                static=_control_static(node.ends) if node is not None and node.ends is not None else None,
            ),
        )
    ...
    def attach_edge(self, starts: str | Sequence[str], end: str) -> None:
        if isinstance(starts, str):
            ...
            if end != END:
                self.nodes[starts].writers.append(ChannelWrite((ChannelWriteEntry(CHANNEL_BRANCH_TO.format(end), None),)))
        elif end != END:
    ...
    def attach_branch(self, start: str, name: str, branch: Branch, *, with_reader: bool = True) -> None:
        def get_writes(packets: Sequence[str | Send], static: bool = False) -> Sequence[ChannelWriteEntry | Send]:
            writes = [
                (ChannelWriteEntry(p if p == END else CHANNEL_BRANCH_TO.format(p), None) if not isinstance(p, Send) else p)
                for p in packets
        ...
        if with_reader:
            ...
            schema = branch.input_schema or (
                self.builder.nodes[start].input if start in self.builder.nodes else self.builder.state_schema
            )
    ...
    def _migrate_checkpoint(self, checkpoint: Checkpoint) -> None:
        ...
        for k in list(versions):
            if k.startswith("start:"):
                ...
                new_v = max(versions[new_k], versions.pop(k)) if new_k in versions else versions.pop(k)
                # update seen
...
def _pick_mapper(state_keys: Sequence[str], schema: type[Any]) -> Callable[[Any], Any] | None:
...
def _control_branch(value: Any) -> Sequence[tuple[str, Any]]:
    ...
    for command in commands:
        ...
        if isinstance(command.goto, Send):
            ...
            rtn.extend((TASKS, go) if isinstance(go, Send) else (CHANNEL_BRANCH_TO.format(go), None) for go in command.goto)
    return rtn
...
def _control_static(
    ...
    if isinstance(ends, dict):
        return [(k if k == END else CHANNEL_BRANCH_TO.format(k), None, label) for k, label in ends.items()]
    else:
        return [(e if e == END else CHANNEL_BRANCH_TO.format(e), None, None) for e in ends]
...
@overload
def _get_channel(name: str, annotation: Any, *, allow_managed: Literal[False]) -> BaseChannel: ...


@overload
def _get_channel(name: str, annotation: Any, *, allow_managed: Literal[True] = True) -> BaseChannel | ManagedValueSpec: ...


def _get_channel(name: str, annotation: Any, *, allow_managed: bool = True) -> BaseChannel | ManagedValueSpec:
