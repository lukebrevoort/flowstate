...
def msg_content_output(output: Any) -> Union[str, list[dict]]:
    if isinstance(output, str):
        return output
    elif isinstance(output, list) and all([isinstance(x, dict) and x.get("type") in TOOL_MESSAGE_BLOCK_TYPES for x in output]):
        return output
...
def _handle_tool_error(
    ...
    *,
    ...
    if isinstance(flag, (bool, tuple)):
        ...
        raise ValueError(f"Got unexpected type of `handle_tool_error`. Expected bool, str " f"or callable. Received: {flag}")
    return content
...
class ToolNode(RunnableCallable):
    ...
    async def _afunc(
        ...
        *,
        ...
        tool_calls, input_type = self._parse_input(input, store)
        outputs = await asyncio.gather(*(self._arun_one(call, input_type, config) for call in tool_calls))
    ...
    def _combine_tool_outputs(
        ...
        for output in outputs:
            if isinstance(output, Command):
                ...
                combined_outputs.append([output] if input_type == "list" else {self.messages_key: [output]})
    ...
    def _run_one(
        ...
        if isinstance(response, Command):
            ...
            raise TypeError(f"Tool {call['name']} returned unexpected type: {type(response)}")
    ...
    def _parse_input(
        ...
        tool_calls = [self.inject_tool_args(call, input, store) for call in latest_ai_message.tool_calls]
        return tool_calls, input_type
    ...
    def _inject_state(
        ...
        if state_args and isinstance(input, list):
            ...
            if len(required_fields) == 1 and required_fields[0] == self.messages_key or required_fields[0] is None:
                input = {self.messages_key: input}
            else:
                err_msg = f"Invalid input to ToolNode. Tool {tool_call['name']} requires " f"graph state dict as input."
        ...
        if isinstance(input, dict):
            tool_state_args = {
                tool_arg: input[state_field] if state_field else input for tool_arg, state_field in state_args.items()
            }

        else:
            tool_state_args = {
                tool_arg: getattr(input, state_field) if state_field else input for tool_arg, state_field in state_args.items()
            }
...
def _is_injection(type_arg: Any, injection_type: Union[Type[InjectedState], Type[InjectedStore]]) -> bool:
...
def _get_state_args(tool: BaseTool) -> dict[str, Optional[str]]:
    ...
    for name, type_ in get_all_basemodel_annotations(full_schema).items():
        injections = [type_arg for type_arg in get_args(type_) if _is_injection(type_arg, InjectedState)]
        if len(injections) > 1:
            raise ValueError(
