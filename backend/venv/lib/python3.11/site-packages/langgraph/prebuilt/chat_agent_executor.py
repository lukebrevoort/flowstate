    ...
    if post_model_hook is not None:
        ...
        def post_model_hook_router(state: StateSchema) -> Union[str, list[Send]]:
            ...
            * END: if no pending tool calls exist and no response_format is specified.
            ...
            pending_tool_calls = [c for c in last_ai_message.tool_calls if c["id"] not in tool_messages]

            if pending_tool_calls:
                pending_tool_calls = [
                    tool_node.inject_tool_args(call, state, store) for call in pending_tool_calls  # type: ignore[arg-type]
                ]
