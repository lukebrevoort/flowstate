...
class StreamMessagesHandler(BaseCallbackHandler, _StreamingCallbackHandler):
    ...
    def on_chat_model_start(
        ...
        **kwargs: Any,
        ...
        if metadata and (not tags or (TAG_NOSTREAM not in tags and TAG_NOSTREAM_ALT not in tags)):
    ...
    def on_chain_start(
        ...
        **kwargs: Any,
        ...
        if metadata and kwargs.get("name") == metadata.get("langgraph_node") and (not tags or TAG_HIDDEN not in tags):
            ns = tuple(cast(str, metadata["langgraph_checkpoint_ns"]).split(NS_SEP))[:-1]
            if not self.subgraphs and len(ns) > 0:
                return
            self.metadata[run_id] = (ns, metadata)
    ...
    def on_chain_end(
        ...
        **kwargs: Any,
        ...
        if meta := self.metadata.pop(run_id, None):
            ...
            elif isinstance(response, Sequence) and any(isinstance(value, Command) for value in response):
                for value in response:
