...
def apply_writes(
    ...
    for task in tasks:
        checkpoint["versions_seen"].setdefault(task.name, {}).update(
            {chan: checkpoint["channel_versions"][chan] for chan in task.triggers if chan in checkpoint["channel_versions"]}
        )
    ...
    for task in tasks:
        for chan, val in task.writes:
            if chan in (NO_WRITES, PUSH, RESUME, INTERRUPT, RETURN, ERROR):
                ...
                logger.warning(f"Task {task.name} with path {task.path} wrote to unknown channel {chan}, ignoring it.")
        ...
        if for_execution:
            ...
            return PregelExecutableTask(
                ...
                patch_config(
                    ...
                    run_name=name,
                    callbacks=call.callbacks or (manager.get_child(f"graph:step:{step}") if manager else None),
                    configurable={
        ...
        if _triggers(
            ...
            try:
                ...
                if SUPPORTS_EXC_NOTES:
                    exc.add_note(f"Before task with name '{name}' and path '{task_path[:3]}'")
                raise
...
def _triggers(
    ...
    if seen is None:
        ...
        for chan in proc.triggers:
            if channels[chan].is_available() and versions.get(chan, null_version) > seen.get(  # type: ignore[operator]
                chan, null_version
            ):
                return True
    return False
...
def _proc_input(
    ...
    *,
    ...
    if isinstance(proc.channels, list):
        ...
        raise RuntimeError(f"Invalid channels type, expected list or dict, got {proc.channels}")
...
def _xxhash_str(namespace: bytes, *parts: str | bytes) -> str:
    ...
    hex = xxh3_128_hexdigest(namespace + b"".join(p.encode() if isinstance(p, str) else p for p in parts))
    return f"{hex[:8]}-{hex[8:12]}-{hex[12:16]}-{hex[16:20]}-{hex[20:32]}"
