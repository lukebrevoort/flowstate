...
def draw_graph(
    ...
    *,
    ...
    get_next_version = checkpointer.get_next_version if isinstance(checkpointer, BaseCheckpointSaver) else increment
    ...
    # remove node mappers
    nodes = {k: v.copy(update={"mapper": None}) if v.mapper is not None else v for k, v in nodes.items()}
    # apply input writes
    ...
    for step in range(step, limit):
        ...
        for task in tasks.values():
            for w in task.writers:
                ...
                if isinstance(w, ChannelWrite):
                    empty_input = cast(BaseChannel, specs["__root__"]).ValueType() if "__root__" in specs else None
                ...
                if w not in static_seen:
                    ...
                    if writes := ChannelWrite.get_static_writes(w):
                        ...
                        conditionals.update({(task.name, t[0], t[1] or None): t[2] for t in writes})
        ...
        # invert triggers
        trigger_to_sources: dict[str, set[tuple[str, bool, str | None]]] = defaultdict(set)
        for src, triggers in sources.items():
            for trigger, cond, label in triggers:
                trigger_to_sources[trigger].add((src, cond, label))
        # apply writes
        updated_channels = apply_writes(checkpoint, channels, tasks.values(), get_next_version, trigger_to_nodes)
        # prepare next tasks
        tasks = prepare_next_tasks(
            checkpoint,
            [],
    ...
    # add end edges
    termini = {d for _, d, _, _ in edges if d != END}.difference(s for s, _, _, _ in edges)
    if termini:
    ...
    for name, subgraph in subgraphs.items():
        if len(subgraph.nodes) > 1 and name in graph.nodes and subgraph.first_node() and subgraph.last_node():
