...
SKIP_RERAISE_SET: weakref.WeakSet[concurrent.futures.Future | asyncio.Future] = weakref.WeakSet()
...
class PregelRunner:
    ...
    def tick(
        ...
        *,
                ...
                if reraise and futures:
                    ...
                    if tb := exc.__traceback__:
                        while tb.tb_next is not None and any(
                            tb.tb_frame.f_code.co_filename.endswith(name) for name in EXCLUDED_FRAME_FNAMES
                        ):
                            tb = tb.tb_next
        ...
        futures.event.wait(timeout=(max(0, end_time - time.monotonic()) if end_time else None))
        # give control back to the caller
        yield
    ...
    def commit(
        ...
        if isinstance(exception, asyncio.CancelledError):
            ...
            if self.node_finished and (task.config is None or TAG_HIDDEN not in task.config.get("tags", [])):
...
def _call(
    ...
    *,
    ...
    if next_task := schedule_task(
        ...
        if fut := next(
            (f for f, t in futures().items() if t is not None and t == next_task.id),  # type: ignore[union-attr]
            None,
        ):
...
def _acall(
    ...
    *,
    ...
    if in_async:
        fut: asyncio.Future[Any] | concurrent.futures.Future[Any] = asyncio.Future(loop=loop)
    else:
        fut = concurrent.futures.Future()
    # schedule the next task
