...
def map_debug_task_results(
    ...
    stream_channels_list = [stream_keys] if isinstance(stream_keys, str) else stream_keys
...
def rm_pregel_keys(config: RunnableConfig | None) -> RunnableConfig | None:
    ...
    if config is None:
        return config
    return {"configurable": {k: v for k, v in config.get("configurable", {}).items() if not k.startswith("__pregel_")}}
...
def tasks_w_writes(
    ...
    for task in tasks:
        rtn = next(
            (val for tid, chan, val in pending_writes if tid == task.id and chan == RETURN),
            MISSING,
        )
        out.append(
            PregelTask(
                ...
                (
                    (
                        ...
                        if rtn is not MISSING
                            ...
                            next(
                                (val for tid, chan, val in pending_writes if tid == task.id and chan == output_keys),
                                None,
                            )
                            if isinstance(output_keys, str)
                                ...
                                if tid == task.id
                                and (chan == output_keys if isinstance(output_keys, str) else chan in output_keys)
                            }
                        )
                    )
                    if any(w[0] == task.id and w[1] not in (ERROR, INTERRUPT) for w in pending_writes)
                    else None
                ),
            )
        )
