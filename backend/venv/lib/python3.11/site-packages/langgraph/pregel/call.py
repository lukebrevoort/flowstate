...
def _lookup_module_and_qualname(obj: Any, name: str | None = None) -> tuple[types.ModuleType, str] | None:
    if name is None:
        name = getattr(obj, "__qualname__", None)
    if name is None:  # pragma: no cover
        # This used to be needed for Python 2.7 support but is probably not
...
def _explode_args_trace_inputs(sig: inspect.Signature, input: tuple[tuple[Any, ...], dict[str, Any]]) -> dict[str, Any]:
    args, kwargs = input
...
def get_runnable_for_entrypoint(func: Callable[..., Any]) -> Runnable:
    ...
    if key in CACHE:
        ...
        if is_async_callable(func):
            ...
            afunc = functools.update_wrapper(functools.partial(run_in_executor, None, func), func)
