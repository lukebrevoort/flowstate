...
class RemoteGraph(PregelProtocol):
    ...
    def _get_drawable_nodes(self, graph: dict[str, list[dict[str, Any]]]) -> dict[str, DrawableNode]:
        nodes = {}
            ...
            tasks.append(
                PregelTask(
                    ...
                    state=(
                        ...
                        if task["state"]
                        else (cast(RunnableConfig, {"configurable": task["checkpoint"]}) if task["checkpoint"] else None)
                    ),
            ...
            parent_config=(
                {
                    "configurable": {
                        ...
                        "checkpoint_map": state["parent_checkpoint"].get("checkpoint_map", {}),
                    }
                }
    ...
    def _sanitize_config(self, config: RunnableConfig) -> RunnableConfig:
        ...
        if "metadata" in config:
            ...
            for k, v in config["metadata"].items():
                if isinstance(k, str) and (sanitized_value := sanitize_config_value(v)) is not None:
                    sanitized["metadata"][k] = sanitized_value

        if "configurable" in config:
            sanitized["configurable"] = {}
            for k, v in config["configurable"].items():
                if isinstance(k, str) and k not in CONF_DROPLIST and (sanitized_value := sanitize_config_value(v)) is not None:
    ...
    def _get_stream_modes(
        ...
        stream: StreamProtocol | None = (config or {}).get(CONF, {}).get(CONFIG_KEY_STREAM)
        if stream:
    ...
    def stream(
        ...
        **kwargs: Any,
        ...
        stream_modes, requested, req_single, stream = self._get_stream_modes(stream_mode, config)
