...
class NodeBuilder:
    ...
    def read_from(
        ...
        *channels: str,
        ...
        assert isinstance(self._channels, list), "Cannot read additional channels when subscribed to single channels"
        self._channels.extend(channels)
        return self
    ...
    def write_to(
        ...
        **kwargs: WriteValue,
        ...
        self._writes.extend(
            ChannelWriteEntry(k, mapper=v) if callable(v) else ChannelWriteEntry(k, value=v) for k, v in kwargs.items()
        )
...
class Pregel(PregelProtocol[StateT, InputT, OutputT], Generic[StateT, InputT, OutputT]):
    ...
    **Actors** read data from channels and write data to channels.
    ...
    def __init__(
        ...
        *,
        ...
        if TASKS in self.channels and not isinstance(self.channels[TASKS], Topic):
            raise ValueError(f"Channel '{TASKS}' is reserved and cannot be used in the graph.")
        else:
    ...
    async def aget_graph(self, config: RunnableConfig | None = None, *, xray: int | bool = False) -> Graph:
        ...
        # gather subgraphs
        if xray:
            subpregels: dict[str, PregelProtocol] = {k: v async for k, v in self.aget_subgraphs()}
            subgraphs = {
                k: v
                for k, v in zip(
                    subpregels,
    ...
    def get_config_jsonschema(self, *, include: Sequence[str] | None = None) -> dict[str, Any]:
    ...
    def stream_channels_asis(self) -> str | Sequence[str]:
        return self.stream_channels or [k for k in self.channels if isinstance(self.channels[k], BaseChannel)]
        ...
        for name, node in self.nodes.items():
            ...
            if graph:
                ...
                if recurse and isinstance(graph, Pregel):
                    ...
                    yield from (
                        (f"{name}{NS_SEP}{n}", s) for n, s in graph.get_subgraphs(namespace=namespace, recurse=recurse)
                    )
    ...
    def _migrate_checkpoint(self, checkpoint: Checkpoint) -> None:
        ...
        if checkpoint["v"] < 4 and checkpoint.get("pending_sends"):
            ...
            checkpoint["channel_versions"][TASKS] = max(checkpoint["channel_versions"].values())
    ...
    def _prepare_state_snapshot(
        ...
        next_tasks = prepare_next_tasks(
            ...
            for_execution=True,
            store=self.store,
            checkpointer=(self.checkpointer if isinstance(self.checkpointer, BaseCheckpointSaver) else None),
            manager=None,
        )
        # get the subgraphs
        ...
        for task in next_tasks.values():
            ...
            if not recurse:
                ...
                task_states[task.id] = subgraphs[task.name].get_state(config, subgraphs=True)
        # apply pending writes
        if null_writes := [w[1:] for w in saved.pending_writes or [] if w[0] == NULL_TASK_ID]:
            apply_writes(
                saved.checkpoint,
                channels,
        ...
        if apply_pending_writes and saved.pending_writes:
            ...
            if tasks := [t for t in next_tasks.values() if t.writes]:
                apply_writes(saved.checkpoint, channels, tasks, None, self.trigger_to_nodes)
    ...
    async def _aprepare_state_snapshot(
        ...
        for task in next_tasks.values():
            ...
            if not recurse:
                ...
                task_states[task.id] = await subgraphs[task.name].aget_state(config, subgraphs=True)
        # apply pending writes
        if null_writes := [w[1:] for w in saved.pending_writes or [] if w[0] == NULL_TASK_ID]:
            apply_writes(
                saved.checkpoint,
    ...
    def get_state(self, config: RunnableConfig, *, subgraphs: bool = False) -> StateSnapshot:
        ...
        checkpointer: BaseCheckpointSaver | None = ensure_config(config)[CONF].get(CONFIG_KEY_CHECKPOINTER, self.checkpointer)
        if not checkpointer:
            raise ValueError("No checkpointer set")

        if (checkpoint_ns := config[CONF].get(CONFIG_KEY_CHECKPOINT_NS, "")) and CONFIG_KEY_CHECKPOINTER not in config[CONF]:
        ...
        if self.checkpointer is True:
            ...
            config = merge_configs(config, {CONF: {CONFIG_KEY_CHECKPOINT_NS: recast_checkpoint_ns(ns)}})
    ...
    def get_state_history(
        ...
        *,
        ...
        for checkpoint_tuple in list(checkpointer.list(config, before=before, limit=limit, filter=filter)):
            yield self._prepare_state_snapshot(checkpoint_tuple.config, checkpoint_tuple)
    ...
    async def aget_state_history(
        ...
        *,
        ...
        # eagerly consume list() to avoid holding up the db cursor
        for checkpoint_tuple in [c async for c in checkpointer.alist(config, before=before, limit=limit, filter=filter)]:
            yield await self._aprepare_state_snapshot(checkpoint_tuple.config, checkpoint_tuple)
    ...
    def bulk_update_state(
        ...
        def perform_superstep(input_config: RunnableConfig, updates: Sequence[StateUpdate]) -> RunnableConfig:
            # get last checkpoint
            ...
            checkpoint_previous_versions = saved.checkpoint["channel_versions"].copy() if saved else {}
            ...
            if values is None and as_node == END:
                if len(updates) > 1:
                    raise InvalidUpdateError("Cannot apply multiple updates when clearing state")
            ...
            if as_node == INPUT:
                if len(updates) > 1:
                    raise InvalidUpdateError("Cannot apply multiple updates when updating as input")
                ...
                if input_writes := deque(map_input(self.input_channels, values)):
                    ...
                    next_step = step + 1 if saved and saved.metadata.get("step") is not None else -1
                    next_config = checkpointer.put(
                        checkpoint_config,
                    ...
                    raise InvalidUpdateError(f"Received no input writes for {self.input_channels}")
            ...
            if as_node == "__copy__":
                ...
                # copy checkpoint
                next_config = checkpointer.put(
                    saved.parent_config or patch_configurable(saved.config, {CONFIG_KEY_CHECKPOINT_ID: None}),
                    next_checkpoint,
                    {
                        "source": "fork",
                        "step": step + 1,
                ...
                if isinstance(values, list) and len(values) > 0:
                    ...
                    for item in values:
                        if not isinstance(item, Sequence):
                            raise InvalidUpdateError(f"Invalid update item: {item} when copying checkpoint")
                        ...
                        user_group_by[as_node].append(StateUpdate(values=values, as_node=as_node, task_id=task_id))
            ...
            if CONFIG_KEY_CHECKPOINT_ID not in config[CONF] and saved is not None and saved.pending_writes:
                # tasks for this checkpoint
                next_tasks = prepare_next_tasks(
                    checkpoint,
                    saved.pending_writes,
            ...
            if len(updates) == 1:
                ...
                elif as_node is None and not any(v for vv in checkpoint["versions_seen"].values() for v in vv.values()):
                    if isinstance(self.input_channels, str) and self.input_channels in self.nodes:
                        as_node = self.input_channels
                elif as_node is None:
                    last_seen_by_node = sorted(
                        (v, n) for n, seen in checkpoint["versions_seen"].items() if n in self.nodes for v in seen.values()
                    )
                ...
                for values, as_node, task_id in updates:
                    if as_node is None:
                        raise InvalidUpdateError("as_node is required when applying multiple updates")
                    if as_node not in self.nodes:
                        raise InvalidUpdateError(f"Node {as_node} does not exist")
            ...
            for as_node, values, provided_task_id in valid_updates:
                ...
                task_id = provided_task_id or str(uuid5(UUID(checkpoint["id"]), INTERRUPT))
            ...
            next_config = checkpointer.put(
                ...
                get_new_channel_versions(checkpoint_previous_versions, checkpoint["channel_versions"]),
            )
            for task_id, task in zip(run_task_ids, run_tasks):
                # save push writes
        ...
        current_config = patch_configurable(config, {CONFIG_KEY_THREAD_ID: str(config[CONF][CONFIG_KEY_THREAD_ID])})
    ...
    async def abulk_update_state(
        ...
        async def aperform_superstep(input_config: RunnableConfig, updates: Sequence[StateUpdate]) -> RunnableConfig:
            # get last checkpoint
            config = ensure_config(self.config, input_config)
            saved = await checkpointer.aget_tuple(config)
            if saved is not None:
    ...
    async def aupdate_state(
        ...
        that updated the state, if not ambiguous.
        """
        return await self.abulk_update_state(config, [[StateUpdate(values, as_node, task_id)]])
    ...
    def stream(
        ...
        *,
        ...
        if stream_mode is None:
            ...
            stream_mode = "values" if config is not None and CONFIG_KEY_TASK_ID in config.get(CONF, {}) else self.stream_mode
        ...
        try:
            ...
            if "messages" in stream_modes:
                run_manager.inheritable_handlers.append(StreamMessagesHandler(stream.put, subgraphs))
            ...
            elif CONFIG_KEY_STREAM not in config[CONF] and CONFIG_KEY_STREAM_WRITER in config[CONF]:
            ...
            with SyncPregelLoop(
                ...
                # create runner
                runner = PregelRunner(
                    submit=config[CONF].get(CONFIG_KEY_RUNNER_SUBMIT, weakref.WeakMethod(loop.submit)),
                ...
                if self.stream_eager or subgraphs or "messages" in stream_modes or "custom" in stream_modes:
                ...
                while loop.tick():
                    ...
                    for _ in runner.tick(
                        ...
                        # emit output
                        yield from _output(stream_mode, print_mode, subgraphs, stream.get, queue.Empty)
                    loop.after_tick()
            # emit output
            yield from _output(stream_mode, print_mode, subgraphs, stream.get, queue.Empty)
            # handle exit
    ...
    async def astream(
        ...
        *,
        ...
        do_stream = (
            next(
                (
                    ...
                    if isinstance(h, _StreamingCallbackHandler) and not isinstance(h, StreamMessagesHandler)
                ),
                False,
            )
        ...
        try:
            ...
            # set up messages stream mode
            if "messages" in stream_modes:
                run_manager.inheritable_handlers.append(StreamMessagesHandler(stream_put, subgraphs))
            # set up custom stream mode
            if "custom" in stream_modes:
                config[CONF][CONFIG_KEY_STREAM_WRITER] = lambda c: aioloop.call_soon_threadsafe(
                    stream.put_nowait,
                    (
                        tuple(get_config()[CONF][CONFIG_KEY_CHECKPOINT_NS].split(NS_SEP)[:-1]),
                        "custom",
            ...
            async with AsyncPregelLoop(
                ...
                # enable subgraph streaming
                if subgraphs:
                    loop.config[CONF][CONFIG_KEY_STREAM] = StreamProtocol(stream_put, stream_modes)
                # enable concurrent streaming
                if self.stream_eager or subgraphs or "messages" in stream_modes or "custom" in stream_modes:
