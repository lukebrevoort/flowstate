...
class TaskFunction(Generic[P, T]):
    ...
    async def aclear_cache(self, cache: BaseCache) -> None:
        """Clear the cache for this task."""
        if self.cache_policy is not None:
            await cache.aclear(((CACHE_NS_WRITES, identifier(self.func) or "__dynamic__"),))
...
def task(
    ...
    **kwargs: Unpack[DeprecatedKwargs],
    ...
    retry_policies: Sequence[RetryPolicy] = (
        () if retry_policy is None else (retry_policy,) if isinstance(retry_policy, RetryPolicy) else retry_policy
    )

    def decorator(
        func: Callable[P, Awaitable[T]] | Callable[P, T],
    ) -> Callable[P, concurrent.futures.Future[T]] | Callable[P, asyncio.Future[T]]:
        return TaskFunction(func, retry_policy=retry_policies, cache_policy=cache_policy, name=name)
...
class entrypoint:
    ...
    def __call__(self, func: Callable[..., Any]) -> Pregel:
        ...
        if sig.return_annotation is not inspect.Signature.empty:
            ...
            if sig.return_annotation is entrypoint.final:  # Un-parameterized entrypoint.final
