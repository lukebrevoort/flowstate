...
class Item:
    ...
    def __init__(
        ...
        *,
        ...
        self.created_at = datetime.fromisoformat(cast(str, created_at)) if isinstance(created_at, str) else created_at
        self.updated_at = datetime.fromisoformat(cast(str, updated_at)) if isinstance(updated_at, str) else updated_at
...
class BaseStore(ABC):
    ...
    def get(
        ...
        *,
        ...
        """
        return self.batch([GetOp(namespace, str(key), _ensure_refresh(self.ttl_config, refresh_ttl))])[0]
...
def _validate_namespace(namespace: tuple[str, ...]) -> None:
    ...
    for label in namespace:
        ...
        if "." in label:
            ...
            raise InvalidNamespaceError(f"Namespace labels cannot be empty strings. Got {label} in {namespace}")
    if namespace[0] == "langgraph":
        raise InvalidNamespaceError(f'Root label for namespace cannot be "langgraph". Got: {namespace}')


def _ensure_refresh(ttl_config: TTLConfig | None, refresh_ttl: bool | None = None) -> bool:
