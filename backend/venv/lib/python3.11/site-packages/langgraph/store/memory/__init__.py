...
class InMemoryStore(BaseStore):
    ...
    def __init__(self, *, index: IndexConfig | None = None) -> None:
        ...
        # [ns][key][path]
        self._vectors: dict[tuple[str, ...], dict[str, dict[str, list[float]]]] = defaultdict(lambda: defaultdict(dict))
    ...
    def _filter_items(self, op: SearchOp) -> list[tuple[Item, list[list[float]]]]:
        ...
        def filter_func(item: Item) -> bool:
            ...
            return all(_compare_values(item.value.get(key), filter_value) for key, filter_value in op.filter.items())

        filtered = []
        for namespace in self._data:
            if not (
                namespace[: len(namespace_prefix)] == namespace_prefix if len(namespace) >= len(namespace_prefix) else False
            ):
                continue
        ...
        if self.index_config and self.embeddings and search_ops:
            ...
            if queries:
                with cf.ThreadPoolExecutor() as executor:
                    futures = {q: executor.submit(self.embeddings.embed_query, q) for q in list(queries)}
                        ...
                        if texts:
                            if len(texts) > 1:
                                for i, text in enumerate(texts):
                                    to_embed[text].append((op.namespace, op.key, f"{path}.{i}"))
    ...
    def _insertinmem_store(
        ...
        if len(indices) != len(embeddings):
            raise ValueError(f"Number of embeddings ({len(embeddings)}) does not" f" match number of indices ({len(indices)})")
        for embedding, (ns, key, path) in zip(embeddings, indices):
            self._vectors[ns][key][path] = embedding

    def _handle_list_namespaces(self, op: ListNamespacesOp) -> list[tuple[str, ...]]:
        all_namespaces = list(self._data.keys())  # Avoid collection size changing while iterating
        namespaces = all_namespaces
        if op.match_conditions:
            namespaces = [ns for ns in namespaces if all(_does_match(condition, ns) for condition in op.match_conditions)]
