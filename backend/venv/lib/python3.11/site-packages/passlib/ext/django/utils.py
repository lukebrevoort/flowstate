    ...
    def django_to_passlib(self, django_name, cached=True):
        ...
        if context is None:
            ...
            candidates = (
                ...
                if passlib_name.startswith(DJANGO_COMPAT_PREFIX) or passlib_name in _other_django_hashes
            )
        else:
            # check context
...
class _PasslibHasherWrapper(object):
    ...
    def __init__(self, passlib_handler):
        ...
        if getattr(passlib_handler, "django_name", None):
            raise ValueError(
                "handlers that reflect an official django " "hasher shouldn't be wrapped: %r" % (passlib_handler.name,)
            )
