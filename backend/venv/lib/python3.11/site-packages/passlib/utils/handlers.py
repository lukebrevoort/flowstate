...
class HasSalt(GenericHandler):
    ...
    @classmethod
    def using(
        cls, default_salt_size=None, salt_size=None, salt=None, **kwds  # keyword only...  # aliases used by CryptContext
    ):
        ...
        **kwds,
        ...
        if default_rounds is not None:
            ...
            if min_desired_rounds and default_rounds < min_desired_rounds:
                raise ValueError(
                    "%s: default_rounds (%r) below min_desired_rounds (%r)" % (subcls.name, default_rounds, min_desired_rounds)
                )
            elif max_desired_rounds and default_rounds > max_desired_rounds:
                raise ValueError(
                    "%s: default_rounds (%r) above max_desired_rounds (%r)" % (subcls.name, default_rounds, max_desired_rounds)
                )
