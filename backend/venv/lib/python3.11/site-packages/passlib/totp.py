...
from passlib.utils import to_unicode, to_bytes, consteq, getrandbytes, rng, SequenceMixin, xor_bytes, getrandstr
...
class AppWallet(object):
    ...
    def _cipher_aes_key(value, secret, salt, cost, decrypt=False):
        ...
        cipher = _cg_ciphers.Cipher(
            _cg_ciphers.algorithms.AES(keyiv[:32]), _cg_ciphers.modes.CTR(keyiv[32:]), _cg_default_backend()
        )
...
class TOTP(object):
    ...
    def using(cls, digits=None, alg=None, period=None, issuer=None, wallet=None, now=None, **kwds):
        ...
        if now is not None:
            assert isinstance(now(), num_types) and now() >= 0, "now() function must return non-negative int/float"
    ...
    def _adapt_uri_params(cls, label=None, secret=None, issuer=None, digits=None, algorithm=None, period=None, **extra):
        ...
        if extra:
            ...
            warn("%s: unexpected parameters encountered in otp uri: %r" % (cls, extra), exc.PasslibRuntimeWarning)
        return kwds
