...
class HandlerCase(TestCase):
        ...
        for secret in self.stock_passwords:
            ...
            if self.handler.is_disabled and not self.disabled_contains_salt:
                self.assertEqual(
                    ...
                    "disabled-hash: secret=%r hash=%r other_secret=%r: result=%r" % (secret, result, wrong_secret, other),
                )
            else:
                self.assertNotEqual(
                    other,
        ...
        for secret, hash in self.iter_known_hashes():
            ...
            try:
                ...
                self.assertEqual(
                    result,
                    hash,
                    "genhash() failed to reproduce " "known hash: secret=%r, hash=%r: result=%r" % (secret, hash, result),
                )
    ...
    def test_72_configs(self):
        ...
        for config, secret, hash in self.known_correct_configs:
            ...
            self.assertRaises(
                ValueError, self.do_verify, secret, config, __msg__="verify() failed to reject config string: %r" % (config,)
            )
    ...
    def test_74_malformed(self):
        ...
        for hash in self.known_malformed_hashes:
            ...
            self.assertTrue(self.do_identify(hash), "identify() failed to identify known malformed " "hash: %r" % (hash,))
    ...
    def test_75_foreign(self):
        ...
        for name, hash in self.known_other_hashes:
            ...
            if name == self.handler.name:
                ...
                # identify should reject these
                self.assertFalse(
                    self.do_identify(hash), "identify() incorrectly identified hash belonging to " "%s: %r" % (name, hash)
                )
            ...
            for verify in verifiers:
                ...
                if not result:
                    raise self.failureException(
                        "failed to verify against %r verifier: " "secret=%r config=%r hash=%r" % (name, secret, settings, hash)
                    )
    ...
    def test_78_fuzz_threading(self):
        ...
        if failed[0]:
            raise self.fail(
                "%d/%d threads failed concurrent fuzz testing " "(see error log for details)" % (failed[0], thread_count)
            )
        if stalled:
            raise self.fail(
                "%d/%d threads stalled during concurrent fuzz testing " "(see error log for details)" % (stalled, thread_count)
            )
    ...
    def test_disable_and_enable(self):
        ...
        self.assertTrue(
            handler.identify(disabled_default), msg="identify() didn't recognize disable() result: %r" % (disabled_default)
        )
