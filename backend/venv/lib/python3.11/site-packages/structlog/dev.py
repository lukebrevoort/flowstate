...
class LogLevelColumnFormatter:
    ...
    def __init__(
        ...
        if level_styles:
            self.width = 0 if width == 0 else len(max(self.level_styles.keys(), key=lambda e: len(e)))
    ...
    *after* the log line. If Rich_ or better-exceptions_ are present, in colors
    ...
    def __init__(  # noqa: PLR0912, PLR0915
        ...
        if columns is not None:
            ...
            if not defaults:
                raise ValueError("Must pass a default column formatter (a column with `key=''`).")
            if len(defaults) > 1:
        ...
        level_to_color = (self.get_default_level_styles(colors) if level_styles is None else level_styles).copy()
        ...
        self._columns = [
            ...
            Column(
                "level",
                LogLevelColumnFormatter(level_to_color, reset_style=styles.reset, width=level_width),
            ),
            Column(
                event_key,
                KeyValueColumnFormatter(
    ...
    def __call__(self, logger: WrappedLogger, name: str, event_dict: EventDict) -> str:
        ...
        kvs = [
            col.formatter(col.key, val) for col in self._columns if (val := event_dict.pop(col.key, _NOTHING)) is not _NOTHING
        ] + [
