...
def to_repr(
    ...
    if use_rich and rich is not None:
        ...
        obj_repr = rich.pretty.traverse(obj, max_length=max_length, max_string=max_string).render()
    else:
        ...
        for suppress_entity in suppress:
            if not isinstance(suppress_entity, str):
                if suppress_entity.__file__ is None:
                    msg = f'"suppress" item {suppress_entity!r} must be a ' f"module with '__file__' attribute"
                    raise ValueError(msg)
    ...
    def _as_dict(self, trace: Trace) -> list[dict[str, Any]]:
        ...
        for stack in trace.stacks:
            ...
            for frame_dict in stack_dict["frames"]:
                if frame_dict["locals"] is None or any(frame_dict["filename"].startswith(path) for path in self.suppress):
