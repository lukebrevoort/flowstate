...
def render_to_log_args_and_kwargs(_: logging.Logger, __: str, event_dict: EventDict) -> tuple[tuple[Any, ...], dict[str, Any]]:
    ...
    kwargs = {kwarg_name: event_dict.pop(kwarg_name) for kwarg_name in LOG_KWARG_NAMES if kwarg_name in event_dict}
    if event_dict:
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        *record* has been patched by `wrap_for_formatter` first though, so the
        ...
        if logger is not _SENTINEL and meth_name != "__structlog_sentinel__":
            ...
            ed = {
                "event": (record.getMessage() if self.use_get_message else str(record.msg)),
                "_record": record,
