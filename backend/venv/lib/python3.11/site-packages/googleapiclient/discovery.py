...
def _discovery_service_uri_options(discoveryServiceUrl, version):
    ...
    if version is None:
        ...
        logger.warning("Discovery V1 does not support empty versions. Defaulting to V2...")
...
def build_from_document(
    ...
    if http is None:
        ...
        if client_cert_to_use:
            ...
            http_channel = (
                http.http if google_auth_httplib2 and isinstance(http, google_auth_httplib2.AuthorizedHttp) else http
            )
        ...
        # api endpoint to use.
        if "mtlsRootUrl" in service and (not client_options or not client_options.api_endpoint):
            ...
            if use_mtls_endpoint == "always" or (use_mtls_endpoint == "auto" and client_cert_to_use):
                base = mtls_endpoint
...
def _fix_up_media_path_base_url(media_path_url, base_url):
    ...
    return urllib.parse.urlunparse(parsed_media_url._replace(netloc=parsed_base_url.netloc))
            ...
            if name in kwargs:
                ...
                for value in values:
                    if value not in enums:
                        raise TypeError(
                            'Parameter "%s" value "%s" is not an allowed value in "%s"' % (name, value, str(enums))
                        )
...
def _findPageTokenName(fields):
    ...
    Returns:
      ...
      otherwise None.
    """
    return next((tokenName for tokenName in _PAGE_TOKEN_NAMES if tokenName in fields), None)
