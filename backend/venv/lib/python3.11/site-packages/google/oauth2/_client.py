...
def _token_endpoint_request_no_throw(
    request, token_uri, body, access_token=None, use_json=False, can_retry=True, headers=None, **kwargs
):
    ...
    for _ in retries:
        response = request(method="POST", url=token_uri, headers=headers_to_use, body=body, **kwargs)
...
def _token_endpoint_request(
    ...
    response_status_ok, response_data, retryable_error = _token_endpoint_request_no_throw(
        request, token_uri, body, access_token=access_token, use_json=use_json, can_retry=can_retry, headers=headers, **kwargs
    )
...
def jwt_grant(request, token_uri, assertion, can_retry=True):
    ...
    response_data = _token_endpoint_request(
        ...
        body,
        can_retry=can_retry,
        headers={metrics.API_CLIENT_HEADER: metrics.token_request_access_token_sa_assertion()},
    )
...
def call_iam_generate_id_token_endpoint(
    ...
    response_data = _token_endpoint_request(
        request,
        iam_id_token_endpoint.replace(credentials.DEFAULT_UNIVERSE_DOMAIN, universe_domain).format(signer_email),
        body,
...
def id_token_jwt_grant(request, token_uri, assertion, can_retry=True):
    ...
    response_data = _token_endpoint_request(
        ...
        body,
        can_retry=can_retry,
        headers={metrics.API_CLIENT_HEADER: metrics.token_request_id_token_sa_assertion()},
    )
