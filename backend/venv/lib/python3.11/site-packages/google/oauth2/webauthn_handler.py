...
class PluginHandler(WebAuthnHandler):
    ...
    def _call_plugin(self, cmd: str, input_json: str) -> str:
        ...
        # Call plugin
        process_result = subprocess.run([cmd], input=request, capture_output=True, check=True)
        ...
        if response_len != len(response):
            raise exceptions.MalformedError(
                "Plugin response length {} does not match data {}".format(response_len, len(response))
            )
    ...
    def _find_plugin(self) -> str:
        ...
        if plugin_cmd is None:
            raise exceptions.InvalidResource("{} env var is not set".format(PluginHandler._ENV_VAR))
        return plugin_cmd
