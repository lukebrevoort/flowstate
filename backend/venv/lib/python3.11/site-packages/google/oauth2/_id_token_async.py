...
async def verify_oauth2_token(id_token, request, audience=None, clock_skew_in_seconds=0):
    ...
    if idinfo["iss"] not in sync_id_token._GOOGLE_ISSUERS:
        raise exceptions.GoogleAuthError(
            "Wrong issuer. 'iss' should be one of the following: {}".format(sync_id_token._GOOGLE_ISSUERS)
        )

    return idinfo


async def verify_firebase_token(id_token, request, audience=None, clock_skew_in_seconds=0):
...
async def fetch_id_token(request, audience):
        ...
        try:
            with open(credentials_filename, "r") as f:
                ...
                if info.get("type") == "service_account":
                    credentials = service_account.IDTokenCredentials.from_service_account_info(info, target_audience=audience)
    ...
    try:
        ...
        if _metadata.ping(request_new):
            credentials = compute_engine.IDTokenCredentials(request_new, audience, use_metadata_identity_endpoint=True)
