...
async def _get_challenges(request, supported_challenge_types, access_token, requested_scopes=None):
...
async def _send_challenge_result(request, session_id, challenge_id, client_input, access_token):
...
async def _run_next_challenge(msg, request, access_token):
    ...
    for challenge in msg["challenges"]:
        ...
        if not c.is_locally_eligible:
            raise exceptions.ReauthFailError("Challenge {0} is not locally eligible".format(challenge["challengeType"]))
...
async def _obtain_rapt(request, access_token, requested_scopes):
    ...
    for _ in range(0, reauth.RUN_CHALLENGE_RETRY_LIMIT):
        if not (msg["status"] == reauth._CHALLENGE_REQUIRED or msg["status"] == reauth._CHALLENGE_PENDING):
            raise exceptions.ReauthFailError("Reauthentication challenge failed due to API error: {}".format(msg["status"]))
...
async def refresh_grant(
    ...
    if (
        ...
        and (
            response_data.get("error_subtype") == reauth._REAUTH_NEEDED_ERROR_INVALID_RAPT
            or response_data.get("error_subtype") == reauth._REAUTH_NEEDED_ERROR_RAPT_REQUIRED
        )
    ):
        ...
        rapt_token = await get_rapt_token(request, client_id, client_secret, refresh_token, token_uri, scopes=scopes)
        body["rapt"] = rapt_token
        (
            response_status_ok,
            response_data,
    ...
    refresh_response = _client._handle_refresh_grant_response(response_data, refresh_token)
