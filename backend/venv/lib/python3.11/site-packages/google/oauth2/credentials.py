...
class Credentials(credentials.ReadOnlyScoped, credentials.CredentialsWithQuotaProject):
    ...
    def __setstate__(self, d):
        ...
        self._universe_domain = d.get("_universe_domain") or credentials.DEFAULT_UNIVERSE_DOMAIN
    ...
    def refresh(self, request):
        if self._universe_domain != credentials.DEFAULT_UNIVERSE_DOMAIN:
            raise exceptions.RefreshError(
                ...
                "please update your code with a valid token.".format(self._universe_domain)
            )
        ...
        if self._refresh_token is None and self.refresh_handler:
            ...
            if not isinstance(token, str):
                raise exceptions.RefreshError("The refresh_handler returned token is not a string.")
            if not isinstance(expiry, datetime):
                raise exceptions.RefreshError("The refresh_handler returned expiry is not a datetime object.")
            if _helpers.utcnow() >= expiry - _helpers.REFRESH_THRESHOLD:
                raise exceptions.RefreshError("The credentials returned by the refresh_handler are " "already expired.")
            self.token = token
            self.expiry = expiry
            return

        if self._refresh_token is None or self._token_uri is None or self._client_id is None or self._client_secret is None:
            raise exceptions.RefreshError(
        ...
        if scopes and "scope" in grant_response:
            ...
            if scopes_requested_but_not_granted:
                ...
                _LOGGER.warning(
                    ...
                    "authorization server, missing scopes {}.".format(", ".join(scopes_requested_but_not_granted))
                )
    ...
    def from_authorized_user_info(cls, info, scopes=None):
        ...
        if missing:
            raise ValueError(
                "Authorized user info was not in the expected format, missing " "fields {}.".format(", ".join(missing))
            )
