...
async def _token_endpoint_request_no_throw(request, token_uri, body, access_token=None, use_json=False, can_retry=True):
    ...
    for _ in retries:
        ...
        response_body = response_body1.decode("utf-8") if hasattr(response_body1, "decode") else response_body1
        ...
        retryable_error = client._can_retry(status_code=response.status, response_data=response_data)
...
async def _token_endpoint_request(request, token_uri, body, access_token=None, use_json=False, can_retry=True):
    """Makes a request to the OAuth 2.0 authorization server's token endpoint.
...
async def jwt_grant(request, token_uri, assertion, can_retry=True):
    ...
    response_data = await _token_endpoint_request(request, token_uri, body, can_retry=can_retry)
