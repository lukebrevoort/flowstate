...
class _CombinedResponse(transport.Response):
    ...
    def _is_compressed(self):
        ...
        return "Content-Encoding" in headers and (
            headers["Content-Encoding"] == "gzip" or headers["Content-Encoding"] == "deflate"
        )
...
class Request(transport.Request):
    ...
    def __init__(self, session=None):
        ...
        if session is not None and session._auto_decompress:
            raise exceptions.InvalidOperation("Client sessions with auto_decompress=True are not supported.")
        ...
        **kwargs,
        ...
        async with aiohttp.ClientSession(
            ...
            if response.status in self._refresh_status_codes and _credential_refresh_attempt < self._max_refresh_attempts:
                ...
                # _auth_request's default timeout.
                auth_request = (
                    self._auth_request if timeout is None else functools.partial(self._auth_request, timeout=timeout)
                )
