...
def _generate_authentication_header_map(
    ...
    if aws_security_credentials.session_token is not None:
        full_headers[_AWS_SECURITY_TOKEN_HEADER] = aws_security_credentials.session_token
    ...
    # https://docs.aws.amazon.com/general/latest/gr/sigv4-calculate-signature.html
    signing_key = _get_signing_key(aws_security_credentials.secret_access_key, date_stamp, region, service_name)
        ...
        if request is not None and self._imdsv2_session_token_url is not None:
            ...
            if imdsv2_session_token_response.status != http_client.OK:
                raise exceptions.RefreshError(
                    "Unable to retrieve AWS Session Token: {}".format(imdsv2_session_token_response.data)
                )
...
class Credentials(external_account.Credentials):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if credential_source is None and aws_security_credentials_supplier is None:
            raise exceptions.InvalidValue("A valid credential source or AWS security credentials supplier must be provided.")
        if credential_source is not None and aws_security_credentials_supplier is not None:
        ...
        if aws_security_credentials_supplier:
            ...
            self._cred_verification_url = _DEFAULT_AWS_REGIONAL_CREDENTIAL_VERIFICATION_URL
        else:
            environment_id = credential_source.get("environment_id") or ""
            self._aws_security_credentials_supplier = _DefaultAwsSecurityCredentialsSupplier(credential_source)
            self._cred_verification_url = credential_source.get("regional_cred_verification_url")
            ...
            if env_id != "aws" or self._cred_verification_url is None:
                raise exceptions.InvalidResource("No valid AWS 'credential_source' provided")
            elif env_version is None or int(env_version) != 1:
                raise exceptions.InvalidValue("aws version '{}' is not supported in the current build.".format(env_version))
    ...
    def retrieve_subject_token(self, request):
        ...
        for key in request_headers.keys():
            aws_signed_req["headers"].append({"key": key, "value": request_headers[key]})

        return urllib.parse.quote(json.dumps(aws_signed_req, separators=(",", ":"), sort_keys=True))
    ...
    def _constructor_args(self):
        ...
        if self._has_custom_supplier():
            args.update({"aws_security_credentials_supplier": self._aws_security_credentials_supplier})
        return args
    ...
    def from_info(cls, info, **kwargs):
        ...
        Raises:
            ValueError: For invalid parameters.
        """
        aws_security_credentials_supplier = info.get("aws_security_credentials_supplier")
        kwargs.update({"aws_security_credentials_supplier": aws_security_credentials_supplier})
