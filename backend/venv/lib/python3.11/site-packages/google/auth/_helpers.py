...
def to_bytes(value, encoding="utf-8"):
    ...
    if isinstance(result, bytes):
        ...
        raise exceptions.InvalidValue("{0!r} could not be converted to bytes".format(value))
...
def from_bytes(value):
    ...
    if isinstance(result, str):
        ...
        raise exceptions.InvalidValue("{0!r} could not be converted to unicode".format(value))
...
def update_query(url, params, remove=None):
    ...
    query_params = {key: value for key, value in query_params.items() if key not in remove}
...
def request_log(
    ...
    if is_logging_enabled(logger):
        content_type = headers["Content-Type"] if headers and "Content-Type" in headers else ""
