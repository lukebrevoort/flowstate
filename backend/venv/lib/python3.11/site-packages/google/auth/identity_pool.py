...
class _X509Supplier(SubjectTokenSupplier):
    ...
    def get_subject_token(self, context, request):
        ...
        leaf_cert = crypto.load_certificate(crypto.FILETYPE_PEM, self._leaf_cert_callback())
        ...
        for i in range(1, len(trust_chain)):
            ...
            if encoded == cert_chain[0]:
                raise exceptions.RefreshError("The leaf certificate must be at the top of the trust chain file")
            else:
                    ...
                    if cert_block.strip():
                        ...
                        try:
                            ...
                            raise exceptions.RefreshError(
                                "Error loading PEM certificates from the trust chain file '{}'".format(self._trust_chain_path)
                            ) from e
                return certificate_trust_chain
        except FileNotFoundError:
            raise exceptions.RefreshError("Trust chain file '{}' was not found.".format(self._trust_chain_path))
...
class Credentials(external_account.Credentials):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if credential_source is None and subject_token_supplier is None:
            raise exceptions.InvalidValue("A valid credential source or a subject token supplier must be provided.")
        ...
        if subject_token_supplier is not None:
            ...
            if not isinstance(credential_source, Mapping):
                ...
                raise exceptions.MalformedError("Invalid credential_source. The credential_source is not a dict.")
            ...
            if "environment_id" in credential_source:
                raise exceptions.MalformedError("Invalid Identity Pool credential_source field 'environment_id'")
            ...
            if self._credential_source_file:
                ...
                self._subject_token_supplier = _X509Supplier(self._trust_chain_path, self._get_cert_bytes)
    ...
    def _get_mtls_cert_and_key_paths(self):
        if self._credential_source_certificate is None:
            ...
            return _mtls_helper._get_workload_cert_and_key_paths(self._certificate_config_location)
    ...
    def _validate_certificate_config(self):
        self._certificate_config_location = self._credential_source_certificate.get("certificate_config_location")
        use_default = self._credential_source_certificate.get("use_default_certificate_config")
        self._trust_chain_path = self._credential_source_certificate.get("trust_chain_path")
    ...
    def _validate_file_or_url_config(self, credential_source):
        ...
        # defaults to text.
        self._credential_source_format_type = credential_source_format.get("type") or "text"
        if self._credential_source_format_type not in ["text", "json"]:
            raise exceptions.MalformedError(
                "Invalid credential_source format '{}'".format(self._credential_source_format_type)
            )
        # For JSON types, get the required subject_token field name.
        if self._credential_source_format_type == "json":
            self._credential_source_field_name = credential_source_format.get("subject_token_field_name")
            if self._credential_source_field_name is None:
                raise exceptions.MalformedError("Missing subject_token_field_name for JSON credential_source format")
        else:
    ...
    def _validate_single_source(self):
        ...
        if len(valid_credential_sources) != 1:
            raise exceptions.MalformedError("Missing credential_source. A 'file', 'url', or 'certificate' must be provided.")
