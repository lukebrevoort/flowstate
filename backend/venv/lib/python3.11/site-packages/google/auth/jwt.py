...
def _verify_iat_and_exp(payload, clock_skew_in_seconds=0):
    ...
    if now < earliest:
        raise exceptions.InvalidValue(
            "Token used too early, {} < {}. Check that your computer's clock is set correctly.".format(now, iat)
        )
...
def decode(token, certs=None, verify=True, audience=None, clock_skew_in_seconds=0):
    ...
    try:
        ...
        if key_alg in _CRYPTOGRAPHY_BASED_ALGORITHMS:
            raise exceptions.InvalidValue(
                "The key algorithm {} requires the cryptography package to be installed.".format(key_alg)
            ) from exc
        else:
            raise exceptions.InvalidValue("Unsupported signature algorithm {}".format(key_alg)) from exc
    # If certs is specified as a dictionary of key IDs to certificates, then
    # use the certificate identified by the key ID in the token header.
    if isinstance(certs, Mapping):
        if key_id:
            if key_id not in certs:
                raise exceptions.MalformedError("Certificate for key id {} not found.".format(key_id))
    ...
    if not crypt.verify_signature(signed_section, signature, certs_to_check, verifier_cls):
    ...
    if audience is not None:
        ...
        if claim_audience not in audience:
            raise exceptions.InvalidValue("Token has wrong audience {}, expected one of {}".format(claim_audience, audience))
