...
class Credentials(external_account.Credentials):
    ...
    def __init__(self, audience, subject_token_type, token_url, credential_source, *args, **kwargs):
        ...
        if not isinstance(credential_source, Mapping):
            ...
            raise exceptions.MalformedError("Missing credential_source. The credential_source is not a dict.")
        self._credential_source_executable = credential_source.get("executable")
        if not self._credential_source_executable:
            raise exceptions.MalformedError("Missing credential_source. An 'executable' must be provided.")
        ...
        if not self._credential_source_executable_command:
            raise exceptions.MalformedError("Missing command field. Executable command must be provided.")
        if not self._credential_source_executable_timeout_millis:
            self._credential_source_executable_timeout_millis = EXECUTABLE_TIMEOUT_MILLIS_DEFAULT
        elif (
            self._credential_source_executable_timeout_millis < EXECUTABLE_TIMEOUT_MILLIS_LOWER_BOUND
            or self._credential_source_executable_timeout_millis > EXECUTABLE_TIMEOUT_MILLIS_UPPER_BOUND
        ):
        ...
        if self._credential_source_executable_interactive_timeout_millis:
            if (
                ...
                raise exceptions.InvalidValue("Interactive timeout must be between 30 seconds and 30 minutes.")
    ...
    def retrieve_subject_token(self, request):
        ...
        if self._credential_source_executable_output_file is not None:
            try:
                with open(self._credential_source_executable_output_file, encoding="utf-8") as output_file:
        ...
        if not _helpers.is_python_3():
            raise exceptions.RefreshError("Pluggable auth is only supported for python 3.7+")
        ...
        if not response and self._credential_source_executable_output_file is not None:
            response = json.load(open(self._credential_source_executable_output_file, encoding="utf-8"))
    ...
    def revoke(self, request):
        ...
        if not self.interactive:
            raise exceptions.InvalidValue("Revoke is only enabled under interactive mode.")
        self._validate_running_mode()
        ...
        result = subprocess.run(
            self._credential_source_executable_command.split(),
            timeout=self._credential_source_executable_interactive_timeout_millis / 1000,
