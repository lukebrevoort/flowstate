...
def get(
    ...
    for attempt in backoff:
        try:
            response = request(url=url, method="GET", headers=headers_to_use, timeout=timeout)
            if response.status in transport.DEFAULT_RETRYABLE_STATUS_CODES:
                _LOGGER.warning(
                    "Compute Engine Metadata server unavailable on " "attempt %s of %s. Response status: %s",
                    attempt,
                    retry_count,
                    response.status,
                )
                failure_reason = response.data.decode("utf-8") if hasattr(response.data, "decode") else response.data
                continue
            else:
                break
    ...
    if response.status == http_client.OK:
        if _helpers.parse_content_type(response.headers["content-type"]) == "application/json":
            try:
                return json.loads(content)
            except ValueError as caught_exc:
                new_exc = exceptions.TransportError(
                    "Received invalid JSON from the Google Compute Engine " "metadata service: {:.20}".format(content)
                )
    ...
    raise exceptions.TransportError(
        "Failed to retrieve {} from the Google Compute Engine "
        "metadata service. Status: {} Response:\n{}".format(url, response.status, response.data),
        response,
    )
...
def get_service_account_token(request, service_account="default", scopes=None):
    ...
    metrics_header = {metrics.API_CLIENT_HEADER: metrics.token_request_access_token_mds()}
