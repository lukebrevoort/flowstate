...
def _make_iam_token_request(
    ...
    try:
        ...
        new_exc = exceptions.RefreshError(
            "{}: No access token or invalid expiration in response.".format(_REFRESH_ERROR),
            response_body,
        )
...
class Credentials(credentials.Scoped, credentials.CredentialsWithQuotaProject, credentials.Signing):
    ...
    def __init__(
        ...
        if isinstance(self._source_credentials, credentials.Scoped):
            self._source_credentials = self._source_credentials.with_scopes(iam._IAM_SCOPE)
    ...
    def _update_token(self, request):
        ...
        if self._subject:
            ...
            self.token, self.expiry, _ = _client.jwt_grant(request, _GOOGLE_OAUTH2_TOKEN_ENDPOINT, assertion)
    ...
    def sign_bytes(self, message):
        ...
        try:
            ...
            for _ in retries:
                response = authed_session.post(url=iam_sign_endpoint, headers=headers, json=body)
                if response.status_code in iam.IAM_RETRY_CODES:
                    continue
                if response.status_code != http_client.OK:
                    raise exceptions.TransportError("Error calling sign_bytes: {}".format(response.json()))
    ...
    def from_impersonated_service_account_info(cls, info, scopes=None):
        ...
        if source_credentials_type == _SOURCE_CREDENTIAL_AUTHORIZED_USER_TYPE:
            ...
            source_credentials = credentials.Credentials.from_authorized_user_info(source_credentials_info)
        elif source_credentials_type == _SOURCE_CREDENTIAL_SERVICE_ACCOUNT_TYPE:
            from google.oauth2 import service_account

            source_credentials = service_account.Credentials.from_service_account_info(source_credentials_info)
        elif source_credentials_type == _SOURCE_CREDENTIAL_EXTERNAL_ACCOUNT_AUTHORIZED_USER_TYPE:
            from google.auth import external_account_authorized_user

            source_credentials = external_account_authorized_user.Credentials.from_info(source_credentials_info)
        else:
...
class IDTokenCredentials(credentials.CredentialsWithQuotaProject):
    ...
    def __init__(
        ...
        if not isinstance(target_credentials, Credentials):
            raise exceptions.GoogleAuthError("Provided Credential must be " "impersonated_credentials")
    ...
    def refresh(self, request):
        ...
        authed_session = AuthorizedSession(self._target_credentials._source_credentials, auth_request=request)
        ...
        if response.status_code != http_client.OK:
            raise exceptions.RefreshError("Error getting ID token: {}".format(response.json()))

        id_token = response.json()["token"]
        self.token = id_token
        self.expiry = datetime.utcfromtimestamp(jwt.decode(id_token, verify=False)["exp"])
