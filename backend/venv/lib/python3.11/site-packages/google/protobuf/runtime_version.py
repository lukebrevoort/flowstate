...
def ValidateProtobufRuntimeVersion(gen_domain, gen_major, gen_minor, gen_patch, gen_suffix, location):
    ...
    error_prompt = "See Protobuf version guarantees at" " https://protobuf.dev/support/cross-version-runtime-guarantee."
        ...
        if gen_major == MAJOR - 1:
            if _warning_count < _MAX_WARNING_COUNT:
                warnings.warn(
                    ...
                    " avoid compatibility violations in the next runtime release." % (gen_version, version, location)
                )
                _warning_count += 1
        else:
            _ReportVersionError(
