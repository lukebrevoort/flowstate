    ...
    def FromDatetime(self, dt):
        ...
        try:
            ...
            raise AttributeError(
                "Fail to convert to Timestamp. Expected a datetime like " "object got {0} : {1}".format(type(dt).__name__, e)
            ) from e
        _CheckTimestampValid(seconds, nanos)
        self.seconds = seconds
        self.nanos = nanos
...
def _CheckTimestampValid(seconds, nanos):
    if seconds < _TIMESTAMP_SECONDS_MIN or seconds > _TIMESTAMP_SECONDS_MAX:
        raise ValueError(
            "Timestamp is not valid: Seconds {0} must be in range " "[-62135596800, 253402300799].".format(seconds)
        )
...
class Duration(object):
    ...
    def FromTimedelta(self, td):
        ...
        try:
            ...
            raise AttributeError(
                "Fail to convert to Duration. Expected a timedelta like " "object got {0}: {1}".format(type(td).__name__, e)
            ) from e
