...
class DescriptorPool(object):
    ...
    def __init__(self, descriptor_db=None, use_deprecated_legacy_json_field_conflicts=False):
        ...
        self._serialized_edition_defaults = python_edition_defaults._PROTOBUF_INTERNAL_PYTHON_EDITION_DEFAULTS
        ...
        try:
            ...
            if extension is not existing_desc:
                raise AssertionError(
                    ...
                    "with field number %d."
                    % (extension.full_name, existing_desc.full_name, extension.containing_type.full_name, extension.number)
                )
        ...
        if self._descriptor_db and hasattr(self._descriptor_db, "FindAllExtensionNumbers"):
            ...
            try:
                ...
                if isinstance(all_numbers, list):
                    ...
                    warnings.warn(
                        "FindAllExtensionNumbers() on fall back DB must return a list," " not {0}".format(type(all_numbers))
                    )
    ...
    def _ConvertMessageDescriptor(self, desc_proto, package=None, file_desc=None, scope=None, syntax=None):
        ...
        nested = [
            self._ConvertMessageDescriptor(nested, desc_name, file_desc, scope, syntax) for nested in desc_proto.nested_type
        ]
        enums = [self._ConvertEnumDescriptor(enum, desc_name, file_desc, None, scope, False) for enum in desc_proto.enum_type]
        fields = [
            self._MakeFieldDescriptor(field, desc_name, index, file_desc) for index, field in enumerate(desc_proto.field)
        ]
        extensions = [
