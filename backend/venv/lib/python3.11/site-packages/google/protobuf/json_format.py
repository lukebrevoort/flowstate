...
class _Parser(object):
            ...
            try:
                ...
                if not field and _VALID_EXTENSION_NAME.match(name):
                    if not message_descriptor.is_extendable:
                        raise ParseError(
                            "Message type {0} does not have extensions at {1}".format(message_descriptor.full_name, path)
                        )
                ...
                if field.containing_oneof is not None and value is not None:
                    ...
                    if oneof_name in names:
                        raise ParseError(
                            'Message type "{0}" should not have multiple '
                            '"{1}" oneof fields at "{2}".'.format(message.DESCRIPTOR.full_name, oneof_name, path)
                        )
                ...
                if _IsMapEntry(field):
                    ...
                    if field.cpp_type == descriptor.FieldDescriptor.CPPTYPE_MESSAGE:
                        ...
                        for index, item in enumerate(value):
                            ...
                            self._ConvertAndAppendScalar(message, field, item, "{0}.{1}[{2}]".format(path, name, index))
    ...
    def _ConvertAndSetScalarExtension(self, message, extension_field, js_value, path):
        ...
        try:
            message.Extensions[extension_field] = _ConvertScalarFieldValue(js_value, extension_field, path)
    ...
    def _ConvertAndAppendScalar(self, message, repeated_field, js_value, path):
        ...
        try:
            getattr(message, repeated_field.name).append(_ConvertScalarFieldValue(js_value, repeated_field, path))
        except EnumStringValueParseError:
            if not self.ignore_unknown_fields:
                raise
            ...
            if enum_value is None:
                try:
                    ...
                    raise EnumStringValueParseError(
                        "Invalid enum value {0} for enum type {1}".format(value, field.enum_type.full_name)
                    ) from e
                if enum_value is None:
                    if field.enum_type.is_closed:
