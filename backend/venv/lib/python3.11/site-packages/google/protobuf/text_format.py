...
def ParseLines(
    lines, message, allow_unknown_extension=False, allow_field_number=False, descriptor_pool=None, allow_unknown_field=False
):
    """Parses a text representation of a protocol message into a message.
    ...
    """
    parser = _Parser(
        allow_unknown_extension, allow_field_number, descriptor_pool=descriptor_pool, allow_unknown_field=allow_unknown_field
    )
        ...
        if field.label == descriptor.FieldDescriptor.LABEL_REPEATED:
            ...
            if field.is_extension:
                ...
                if not self._allow_multiple_scalars and message.HasField(field.name):
                    raise tokenizer.ParseErrorPreviousToken(
                        'Message type "%s" should not have multiple "%s" fields.' % (message.DESCRIPTOR.full_name, field.name)
                    )
            ...
            if field.is_extension:
                ...
                if not self._allow_multiple_scalars:
                    if field.has_presence:
                        ...
                        duplicate_error = not decoder.IsDefaultScalarValue(getattr(message, field.name))
