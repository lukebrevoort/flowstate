...
def is_new_type(tp):
    ...
    if not WITH_NEWTYPE:
        ...
        return tp is NewType or (
            getattr(tp, "__supertype__", None) is not None and tp.__module__ in ("typing", "typing_extensions")
        )
            ...
            for bp_ in base_params:
                for bp in get_args(bp_) if is_tuple_type(bp_) else (bp_,):
                    if _has_type_var(bp) and not isinstance(bp, TypeVar):
                        raise TypeError(
                            "Cannot inherit from a generic class " "parameterized with " "non-type-variable %s" % bp
                        )
                    if params is None:
                        params = []
                    if bp not in params:
...
def _remove_dups_flatten(parameters):
    ...
    for t1 in params:
        ...
        if any(
            ...
            if (not (isinstance(t2, GenericMeta) and get_origin(t2) is not None) and not isinstance(t2, TypeVar))
        ):
