...
def _convert_to_v03_ai_message(message: AIMessage, has_reasoning: bool = False) -> AIMessage:
        ...
        for block in message.content:
            if isinstance(block, dict):
                if block.get("type") == "reasoning":
                    ...
                    if (call_id := block.get("call_id")) and (function_call_id := block.get("id")):
                        message.additional_kwargs[_FUNCTION_CALL_IDS_MAP_KEY][call_id] = function_call_id
                elif (block.get("type") == "refusal") and (refusal := block.get("refusal")):
                ...
                elif set(block.keys()) == {"id", "index"} and isinstance(block["id"], str) and block["id"].startswith("msg_"):
...
def _convert_from_v03_ai_message(message: AIMessage) -> AIMessage:
    ...
    is_chatopenai_v03 = (isinstance(message.content, list) and all(isinstance(b, dict) for b in message.content)) and (
        any(
    ...
    for tool_call in message.tool_calls:
        ...
        if function_call_ids is not None and (_id := function_call_ids.get(tool_call["id"])):
