        ...
        for block in content:
            ...
            # Anthropic image blocks
                ...
                if source.get("type") == "base64" and (
                    (media_type := source.get("media_type")) and (data := source.get("data"))
                ):
...
def _update_token_usage(overall_token_usage: Union[int, dict], new_usage: Union[int, dict]) -> Union[int, dict]:
    ...
    if isinstance(new_usage, int):
        if not isinstance(overall_token_usage, int):
            raise ValueError(f"Got different types for token usage: " f"{type(new_usage)} and {type(overall_token_usage)}")
        return new_usage + overall_token_usage
    elif isinstance(new_usage, dict):
        if not isinstance(overall_token_usage, dict):
            raise ValueError(f"Got different types for token usage: " f"{type(new_usage)} and {type(overall_token_usage)}")
        return {k: _update_token_usage(overall_token_usage.get(k, 0), v) for k, v in new_usage.items()}
    else:
...
class BaseChatOpenAI(BaseChatModel):
    ...
    def validate_environment(self) -> Self:
        ...
        if not self.async_client:
            if self.openai_proxy and not self.http_async_client:
                ...
                self.http_async_client = httpx.AsyncClient(proxy=self.openai_proxy, verify=global_ssl_context)
            async_specific = {
    ...
    def _convert_chunk_to_generation_chunk(
        ...
        usage_metadata: Optional[UsageMetadata] = _create_usage_metadata(token_usage) if token_usage else None
        if len(choices) == 0:
    ...
    def _create_chat_result(
        ...
        try:
            ...
            raise KeyError(f"Response missing `choices` key: {response_dict.keys()}") from e
        ...
        for res in choices:
            ...
            generation_info["finish_reason"] = (
                res.get("finish_reason") if res.get("finish_reason") is not None else generation_info.get("finish_reason")
            )
            if "logprobs" in res:
        ...
        **kwargs: Any,
        ...
        if "response_format" in payload:
            ...
            if original_schema_obj and _is_pydantic_class(original_schema_obj):
                ...
                if self.include_response_headers:
                    raw_response = await self.root_async_client.with_raw_response.responses.create(**payload)
                    response = raw_response.parse()
                    generation_info = {"headers": dict(raw_response.headers)}
                else:
                ...
                if isinstance(value, list):
                    ...
                    for val in value:
                        ...
                        # This is an approximation.
                            ...
                            warnings.warn("Token counts for file inputs are not supported. " "Ignoring file inputs.")
                            pass
                        else:
        ...
        **kwargs: Any,
        ...
        if method == "json_schema":
            ...
            if self.model_name and (
                self.model_name.startswith("gpt-3") or self.model_name.startswith("gpt-4-") or self.model_name == "gpt-4"
            ):
                warnings.warn(
        ...
        if method == "function_calling":
            ...
            if is_pydantic_schema:
                output_parser = RunnableLambda(partial(_oai_structured_outputs_parser, schema=cast(type, schema))).with_types(
            ...
            raise ValueError(
                f"Unrecognized method argument. Expected one of 'function_calling' or " f"'json_mode'. Received: '{method}'"
            )
    ...
    def _get_generation_chunk_from_completion(self, completion: openai.BaseModel) -> ChatGenerationChunk:
...
def _url_to_size(image_source: str) -> Optional[tuple[int, int]]:
    try:
        ...
        logger.info("Unable to count image tokens. To count image tokens please install " "`pip install -U pillow httpx`.")
        return None
    if _is_url(image_source):
        try:
            import httpx
        except ImportError:
            logger.info("Unable to count image tokens. To count image tokens please install " "`pip install -U httpx`.")
            return None
...
def _oai_structured_outputs_parser(ai_msg: AIMessage, schema: type[_BM]) -> Optional[PydanticBaseModel]:
...
def _create_usage_metadata_responses(oai_token_usage: dict) -> UsageMetadata:
    ...
    output_token_details: dict = {"reasoning": (oai_token_usage.get("output_tokens_details") or {}).get("reasoning_tokens")}
    input_token_details: dict = {"cache_read": (oai_token_usage.get("input_tokens_details") or {}).get("cached_tokens")}
    return UsageMetadata(
        input_tokens=input_tokens,
        output_tokens=output_tokens,
        total_tokens=total_tokens,
...
def _use_responses_api(payload: dict) -> bool:
    uses_builtin_tools = "tools" in payload and any(_is_builtin_tool(tool) for tool in payload["tools"])
    responses_only_args = {
        "include",
...
def _construct_responses_api_payload(messages: Sequence[BaseMessage], payload: dict) -> dict:
    ...
    if tool_choice := payload.pop("tool_choice", None):
        ...
        if isinstance(tool_choice, dict) and tool_choice["type"] == "function" and "function" in tool_choice:
    ...
    if schema := payload.pop("response_format", None):
        if payload.get("text"):
            text = payload["text"]
            raise ValueError("Can specify at most one of 'response_format' or 'text', received both:" f"\n{schema=}\n{text=}")
        ...
        if not payload.get("stream") and _is_pydantic_class(schema):
            ...
            if schema_dict == {"type": "json_object"}:  # JSON mode
                ...
                (response_format := _convert_to_openai_response_format(schema_dict, strict=strict))
...
def _make_computer_call_output_from_message(message: ToolMessage) -> dict:
    ...
    if isinstance(message.content, list):
        ...
        output = next(block for block in message.content if cast(dict, block)["type"] == "input_image")
    else:
    ...
    if "acknowledged_safety_checks" in message.additional_kwargs:
        computer_call_output["acknowledged_safety_checks"] = message.additional_kwargs["acknowledged_safety_checks"]
    ...
    for lc_msg in messages:
        ...
        if msg["role"] == "tool":
            ...
            if lc_msg.additional_kwargs.get("type") == "computer_call_output":
                computer_call_output = _make_computer_call_output_from_message(cast(ToolMessage, lc_msg))
                ...
                for block in msg["content"]:
                    if isinstance(block, dict) and (block_type := block.get("type")):
                        ...
                        if block_type in ("text", "output_text", "refusal"):
                            ...
                            input_.append({"type": "image_generation_call", "id": block["id"]})
                        else:
                            pass
            ...
            if tool_calls := msg.pop("tool_calls", None):
                content_call_ids = {
                    block["call_id"] for block in input_ if block.get("type") == "function_call" and "call_id" in block
                }
            ...
            for content in output.content:
                if content.type == "output_text":
                    block = {
                        "type": "text",
                        "text": content.text,
                        "annotations": [annotation.model_dump() for annotation in content.annotations],
                        "id": output.id,
                    }
                ...
                if content.type == "refusal":
                    content_blocks.append({"type": "refusal", "refusal": content.refusal, "id": output.id})
...
def _convert_responses_chunk_to_generation_chunk(
    ...
    if chunk.type == "response.output_text.delta":
        ...
        msg = cast(
            AIMessage,
            (
                _construct_lc_result_from_responses_api(chunk.response, schema=schema, output_version=output_version)
                .generations[0]
                .message
            ),
        )
        ...
        response_metadata = {k: v for k, v in msg.response_metadata.items() if k != "id"}
        ...
        _advance(chunk.output_index)
        tool_call_chunks.append({"type": "tool_call_chunk", "args": chunk.delta, "index": current_index})
