...
def _process_batched_chunked_embeddings(
    ...
    for i in range(num_texts):
        ...
        if len(_result) == 0:
            ...
            average = [
                sum(val * weight for val, weight in zip(embedding, num_tokens_in_batch[i])) / total_weight
...
class OpenAIEmbeddings(BaseModel, Embeddings):
    ...
    openai_api_base: Optional[str] = Field(alias="base_url", default_factory=from_env("OPENAI_API_BASE", default=None))
    """Base URL path for API requests, leave blank if not using a proxy or service
        emulator."""
    # to support Azure OpenAI Service custom endpoints
    openai_api_type: Optional[str] = Field(default_factory=from_env("OPENAI_API_TYPE", default=None))
    ...
    def validate_environment(self) -> Self:
        ...
        if self.openai_api_type in ("azure", "azure_ad", "azuread"):
            raise ValueError("If you are using Azure, please use the `AzureOpenAIEmbeddings` class.")
        client_params: dict = {
    ...
    def _tokenize(self, texts: list[str], chunk_size: int) -> tuple[Iterable[int], list[Union[list[int], str]], list[int]]:
        """
    ...
    def _get_len_safe_embeddings(
        ...
        **kwargs: Any,
        ...
        batched_embeddings: list[list[float]] = []
        for i in _iter:
            response = self.client.create(input=tokens[i : i + _chunk_size], **client_kwargs)
            if not isinstance(response, dict):
                response = response.model_dump()
            batched_embeddings.extend(r["embedding"] for r in response["data"])

        embeddings = _process_batched_chunked_embeddings(len(texts), tokens, batched_embeddings, indices, self.skip_empty)
    ...
    async def _aget_len_safe_embeddings(
        ...
        **kwargs: Any,
        ...
        _chunk_size = chunk_size or self.chunk_size
        client_kwargs = {**self._invocation_params, **kwargs}
        _iter, tokens, indices = await run_in_executor(None, self._tokenize, texts, _chunk_size)
        batched_embeddings: list[list[float]] = []
        for i in range(0, len(tokens), _chunk_size):
            response = await self.async_client.create(input=tokens[i : i + _chunk_size], **client_kwargs)
        ...
        async def empty_embedding() -> list[float]:
            nonlocal _cached_empty_embedding
            if _cached_empty_embedding is None:
                average_embedded = await self.async_client.create(input="", **client_kwargs)
                if not isinstance(average_embedded, dict):
                    average_embedded = average_embedded.model_dump()
                _cached_empty_embedding = average_embedded["data"][0]["embedding"]
            return _cached_empty_embedding
    ...
    def embed_documents(self, texts: list[str], chunk_size: Optional[int] = None, **kwargs: Any) -> list[list[float]]:
        ...
        if not self.check_embedding_ctx_length:
            ...
            for i in range(0, len(texts), chunk_size_):
                response = self.client.create(input=texts[i : i + chunk_size_], **client_kwargs)
                if not isinstance(response, dict):
                    response = response.model_dump()
        ...
        return self._get_len_safe_embeddings(texts, engine=engine, chunk_size=chunk_size, **kwargs)

    async def aembed_documents(self, texts: list[str], chunk_size: Optional[int] = None, **kwargs: Any) -> list[list[float]]:
        ...
        if not self.check_embedding_ctx_length:
            embeddings: list[list[float]] = []
            for i in range(0, len(texts), chunk_size_):
                response = await self.async_client.create(input=texts[i : i + chunk_size_], **client_kwargs)
                if not isinstance(response, dict):
                    response = response.model_dump()
                embeddings.extend(r["embedding"] for r in response["data"])
            return embeddings
        ...
        #       than the maximum context and use length-safe embedding function.
        engine = cast(str, self.deployment)
        return await self._aget_len_safe_embeddings(texts, engine=engine, chunk_size=chunk_size, **kwargs)
