...
ExistingType = Union[Dict[str, Any], List[SchemaInstance], List[tuple[str, str, dict[str, Any]]]]
...
def create_extractor(
    ...
    *,
        ...
        for m in reversed(state.messages):
            ...
            if isinstance(m, ToolMessage):
                if m.status == "error":
                    ...
                    messages_for_fixing = _get_history_for_tool_call(state.messages, m.tool_call_id)
                    to_send.append(
                        Send(
                            "patch",
    ...
    builder.add_conditional_edges("validate", handle_retries, path_map=["__end__", "patch", "del_tool_call"])
    ...
    builder.add_conditional_edges("sync", validate_or_repatch, path_map=["validate", "patch", "__end__"])
    ...
    def filter_state(state: dict) -> ExtractionOutputs:
        ...
        msg: Optional[AIMessage] = next(
            (m for m in state["messages"] if m.id == msg_id and isinstance(m, AIMessage)),  # type: ignore
            None,
        )
        if not msg:
        ...
        for tc in msg.tool_calls:
            ...
            try:
                responses.append(
                    sch.model_validate(tc["args"]) if hasattr(sch, "model_validate") else sch.parse_obj(tc["args"])
                )
                meta = {"id": tc["id"]}
    ...
    for t in tools:
        if isinstance(t, dict):
            if all(k in t for k in ("name", "description", "parameters")):
                schema = create_model_from_schema({"title": t["name"], **t["parameters"]})
                schema.__doc__ = (getattr(schema, __doc__, "") or "") + (t.get("description") or "")
...
def _exclude_none(d: Dict[str, Any]) -> Dict[str, Any]:
    return {k: v if not isinstance(v, dict) else _exclude_none(v) for k, v in d.items() if v is not None}
...
class _ExtractUpdates:
    ...
    def __init__(
        ...
        if not any((enable_inserts, enable_updates, enable_deletes)):
            raise ValueError("At least one of enable_inserts, enable_updates," " or enable_deletes must be True.")
...
"""
                schema_strings.append(f"<schema id={k}>\n<instance>\n{v}\n" f"</instance>{schema_str}</schema>")
        else:
            for schema_id, tname, d in existing:
                schema_strings.append(f'<instance id={schema_id} schema_type="{tname}">\n{d}\n</instance>')

        existing_schemas = "\n".join(schema_strings)
        cmd = "Generate JSONPatches to update the existing schema instances."
        if self.enable_inserts:
            cmd += " If you need to extract or insert *new* instances of the schemas" ", call the relevant function(s)."
...
"""
            ...
            if tc["name"] == PatchDoc.__name__:
                ...
                if isinstance(existing, dict):
                    ...
                    try:
                        ...
                        if not tool_name:
                            raise ValueError("Could not find tool name " f"for json_doc_id {json_doc_id}")
                    except StopIteration:
                        logger.error(f"Could not find existing schema in dict for {json_doc_id}")
                        if rt:
                            rt.error = f"Could not find existing schema for {json_doc_id}"
                        continue
                    except (ValueError, IndexError, TypeError):
                        logger.error(f"Could not find existing schema in list for {json_doc_id}")
                        if rt:
                            rt.error = f"Could not find existing schema for {json_doc_id}"
                        continue
    ...
    def _validate_existing(self, existing: ExistingType) -> Union[Dict[str, Any], List[SchemaInstance]]:
        ...
        if isinstance(existing, dict):
            ...
            for key, record in existing.items():
                if key in self.tools or key == "__any__":
                    ...
                    if self.existing_schema_policy is True:
                        raise ValueError(f"Key '{key}' doesn't match any schema. " f"Known schemas: {list(self.tools.keys())}")
            ...
            for i, item in enumerate(existing):
                if isinstance(item, SchemaInstance):
                    if item.schema_name not in self.tools and item.schema_name != "__any__":
                        if self.existing_schema_policy is True:
                            raise ValueError(f"Unknown schema '{item.schema_name}' at index {i}")
                        elif self.existing_schema_policy is False:
                            coerced.append(SchemaInstance(item.record_id, item.schema_name, item.record))
                        else:  # "ignore"
                    ...
                    if schema_name not in self.tools and schema_name != "__any__":
                        if self.existing_schema_policy is True:
                            raise ValueError(f"Unknown schema '{schema_name}' at index {i}")
                        elif self.existing_schema_policy is False:
                            coerced.append(SchemaInstance(record_id, schema_name, record_dict))
                        else:  # "ignore"
                    ...
                    if schema_name not in self.tools and schema_name != "__any__":
                        if self.existing_schema_policy is True:
                            ...
                            val = model.model_dump(mode="json") if isinstance(model, BaseModel) else model
    ...
    async def ainvoke(self, state: ExtractionState, config: RunnableConfig) -> dict:
        ...
        try:
            ...
            return {
                "messages": [HumanMessage(content="Fix the validation error while" f" also avoiding: {repr(str(e))}")],
                "attempts": 1,
            }
    ...
    def as_runnable(self):
        return RunnableCallable(self.invoke, self.ainvoke, name="extract_updates", trace=False)
...
class _Patch:
    ...
    def __init__(self, llm: BaseChatModel, valid_tool_names: Optional[List[str]] = None):
    ...
    async def ainvoke(self, state: ExtendedExtractState, config: RunnableConfig) -> Command[Literal["sync", "__end__"]]:
    ...
    def invoke(self, state: ExtendedExtractState, config: RunnableConfig) -> Command[Literal["sync", "__end__"]]:
        try:
...
def _apply_message_ops(messages: Sequence[AnyMessage], message_ops: Sequence[MessageOp]) -> List[AnyMessage]:
        ...
        if message_op["op"] == "delete":
            ...
            for m in messages:
                if isinstance(m, AIMessage):
                    ...
                    new = [targ if tc["id"] == targ["id"] else tc for tc in m.tool_calls]
                    if old != new:
                        m = m.model_copy()
                        m.tool_calls = new
                    ...
                    for tc in m.tool_calls:
                        if tc["id"] == update_targ["id"]:
                            new.append(
                                {
                                    "id": update_targ["id"],
                                    "name": update_targ["name"],  # Just updating the name
                                    "args": tc["args"],
                                }
                            )
                        else:
...
def _get_message_op(messages: Sequence[AnyMessage], tool_call: dict, tool_call_name: str, target_id: str) -> List[MessageOp]:
...
@ls.traceable(tags=["langsmith:hidden"])
def _infer_patch_message_ops(messages: Sequence[AnyMessage], tool_calls: List[ToolCall], target_id: str):
    return [
        op
        for tool_call in tool_calls
        for op in _get_message_op(messages, tool_call["args"], tool_call["name"], target_id=target_id)
    ]
...
class ExtractionState:
    messages: Annotated[List[AnyMessage], _reduce_messages] = field(default_factory=list)
...
def _is_injected_arg_type(type_: Type) -> bool:
    return any(
        isinstance(arg, InjectedToolArg) or (isinstance(arg, type) and issubclass(arg, InjectedToolArg))
...
def _strip_injected(fn: Callable) -> Callable:
    ...
    injected = [p.name for p in inspect.signature(fn).parameters.values() if _is_injected_arg_type(p.annotation)]
...
def _fix_string_concat(doc: dict, patch: list[jsonpatch.JsonPatch]) -> Optional[list[jsonpatch.JsonPatch]] | None:
    fixed = False
    result = []
    for p in patch:
