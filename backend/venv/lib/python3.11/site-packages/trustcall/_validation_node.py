        ...
        *,
        ...
        for schema in schemas:
            if isinstance(schema, BaseTool):
                ...
                raise ValueError(
                    "Unsupported input to ValidationNode. Expected BaseModel, tool or" f" function. Got: {type(schema)}."
                )

    def _get_message(self, input: Union[list[AnyMessage], dict[str, Any]]) -> Tuple[str, AIMessage]:
        ...
        def run_one(call: ToolCall) -> ToolMessage:
            ...
            try:
                if issubclass(schema, BaseModel):
                    ...
                    raise ValueError(f"Unsupported schema type: {type(schema)}. Expected BaseModel" " or BaseModelV1.")
                return ToolMessage(
                    content=content,
                    name=call["name"],
