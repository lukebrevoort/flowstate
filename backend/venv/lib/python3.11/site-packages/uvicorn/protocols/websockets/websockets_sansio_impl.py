...
class WebSocketsSansIOProtocol(asyncio.Protocol):
    ...
    def handle_connect(self, event: Request) -> None:
        ...
        headers = [
            (key.encode("ascii"), value.encode("ascii", errors="surrogateescape")) for key, value in event.headers.raw_items()
        ]
    ...
    async def send(self, message: ASGISendEvent) -> None:
        ...
        if not self.handshake_complete and self.initial_response is None:
            if message_type == "websocket.accept":
                ...
                headers = [
                    (name.decode("latin-1"), value.decode("latin-1")) for name, value in list(message.get("headers", []))
                ]
