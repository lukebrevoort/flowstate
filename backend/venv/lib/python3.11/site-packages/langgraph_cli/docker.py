...
DEFAULT_POSTGRES_URI = "postgres://postgres:postgres@langgraph-postgres:5432/postgres?sslmode=disable"
...
def check_capabilities(runner) -> DockerCapabilities:
    ...
    try:
        compose = next(p for p in info["ClientInfo"]["Plugins"] if p["Name"] == "compose")
        ...
        compose_version_str, _ = runner.run(subp_exec("docker-compose", "--version", "--short", collect=True))
...
def debugger_compose(*, port: Optional[int] = None, base_url: Optional[str] = None) -> dict:
    if port is None:
        return ""
    ...
    if base_url:
        config["langgraph-debugger"]["environment"] = {"VITE_STUDIO_LOCAL_GRAPH_URL": base_url}
...
def compose_as_dict(
    ...
    *,
    ...
    if debugger_port:
        services["langgraph-debugger"] = debugger_compose(port=debugger_port, base_url=debugger_base_url)["langgraph-debugger"]
    ...
    if include_db:
        services["langgraph-api"]["depends_on"]["langgraph-postgres"] = {"condition": "service_healthy"}
