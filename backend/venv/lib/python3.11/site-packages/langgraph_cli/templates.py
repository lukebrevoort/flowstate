...
TEMPLATE_HELP_STRING = "The name of the template to use. Available options:\n" + "\n".join(
    f"{id_}" for id_ in TEMPLATE_ID_TO_CONFIG
)
...
def _choose_template() -> str:
    ...
    version_choice: int = click.prompt("Choose language (1 for Python 🐍, 2 for JS/TS 🌐)", type=int)
...
def _get_template_url(template_name: str) -> Optional[str]:
    ...
    if template_name in TEMPLATES:
        ...
        version_choice: int = click.prompt("Choose version (1 for Python 🐍, 2 for JS/TS 🌐)", type=int)
...
def create_new(path: Optional[str], template: Optional[str]) -> None:
    ...
    if not path:
        path = click.prompt("📂 Please specify the path to create the application", default=".")
    ...
    if template:
        if template not in TEMPLATE_ID_TO_CONFIG:
            ...
            click.secho("Please select from the available options:\n", fg="yellow", bold=True)
