    ...
    with Runner() as runner, Progress(message="Pulling...") as set:
        ...
        def on_stdout(line: str):
            if "unpacking to docker.io" in line:
                ...
                debugger_origin = f"http://localhost:{debugger_port}" if debugger_port else "https://smith.langchain.com"
                debugger_base_url_query = debugger_base_url or f"http://127.0.0.1:{port}"
                set("")
                sys.stdout.write(
                    f"""Ready!
...
def _build(
    ...
    # apply config
    stdin, additional_contexts = langgraph_cli.config.config_to_docker(config, config_json, base_image)
...
@cli.command(help="🐳 Generate a Dockerfile for the LangGraph API server, with Docker Compose options.")
@click.option(
...
def dockerfile(
    ...
    if additional_contexts:
        additional_contexts_str = ",".join(f"{k}={v}" for k, v in additional_contexts.items())
        secho(
    ...
    if add_docker_compose:
        ...
        with open(path, "w", encoding="utf-8") as f:
            ...
            if base_image:
                compose_dict["services"]["langgraph-api"]["build"]["args"] = {"BASE_IMAGE": base_image}
            ...
            with open(str(save_path.parent / ".env"), "w", encoding="utf-8") as f:
                f.writelines(
                    [
                        ...
                        "\n",
                        "# Or if you have a LangGraph Platform license key, " "then uncomment the following line: ",
                        "\n",
