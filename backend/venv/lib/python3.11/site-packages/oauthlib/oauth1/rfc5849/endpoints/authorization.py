...
class AuthorizationEndpoint(BaseEndpoint):
    ...
    def create_authorization_response(self, uri, http_method="GET", body=None, headers=None, realms=None, credentials=None):
        ...
        An example of a request with an "oob" callback::
            ...
            200
        """
        request = self._create_request(uri, http_method=http_method, body=body, headers=headers)

        if not request.resource_owner_key:
            raise errors.InvalidRequestError("Missing mandatory parameter oauth_token.")
        if not self.request_validator.verify_request_token(request.resource_owner_key, request):
            raise errors.InvalidClientError()

        request.realms = realms
        if request.realms and not self.request_validator.verify_realms(request.resource_owner_key, request.realms, request):
            raise errors.InvalidRequestError(
                description=("User granted access to realms outside of " "what the client may request.")
            )

        verifier = self.create_verifier(request, credentials or {})
        redirect_uri = self.request_validator.get_redirect_uri(request.resource_owner_key, request)
        if redirect_uri == "oob":
