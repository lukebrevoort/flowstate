...
CertificateOrCRL.componentType = namedtype.NamedTypes(
    namedtype.NamedType(
        "cert", rfc5280.Certificate().subtype(explicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 0))
    ),
    namedtype.NamedType(
        "crl", rfc5280.CertificateList().subtype(explicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 1))
    ),
)
