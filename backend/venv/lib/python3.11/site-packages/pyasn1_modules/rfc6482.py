...
class ROAIPAddress(univ.Sequence):
    componentType = namedtype.NamedTypes(
        namedtype.NamedType("address", IPAddress()), namedtype.OptionalNamedType("maxLength", univ.Integer())
    )
...
class ROAIPAddressFamily(univ.Sequence):
    componentType = namedtype.NamedTypes(
        ...
        namedtype.NamedType(
            "addresses",
            univ.SequenceOf(componentType=ROAIPAddress()).subtype(subtypeSpec=constraint.ValueSizeConstraint(1, MAX)),
        ),
    )
...
class RouteOriginAttestation(univ.Sequence):
    componentType = namedtype.NamedTypes(
        namedtype.DefaultedNamedType(
            "version",
            univ.Integer().subtype(explicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 0)).subtype(value=0),
        ),
        namedtype.NamedType("asID", ASID()),
        namedtype.NamedType(
            "ipAddrBlocks",
            univ.SequenceOf(componentType=ROAIPAddressFamily()).subtype(subtypeSpec=constraint.ValueSizeConstraint(1, MAX)),
        ),
    )
