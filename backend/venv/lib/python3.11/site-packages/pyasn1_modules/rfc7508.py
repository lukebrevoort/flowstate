...
class Algorithm(univ.Enumerated):
    namedValues = namedval.NamedValues(("canonAlgorithmSimple", 0), ("canonAlgorithmRelaxed", 1))
...
class HeaderField(univ.Sequence):
    componentType = namedtype.NamedTypes(
        ...
        namedtype.DefaultedNamedType("field-Status", HeaderFieldStatus().subtype(value="duplicated")),
    )
...
class SecureHeaderFields(univ.Set):
    componentType = namedtype.NamedTypes(
        namedtype.NamedType("canonAlgorithm", Algorithm()), namedtype.NamedType("secHeaderFields", HeaderFields())
    )
