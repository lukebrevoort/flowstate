...
SMIMECapability.componentType = namedtype.NamedTypes(
    ...
    namedtype.OptionalNamedType("parameters", univ.Any(), openType=opentype.OpenType("capabilityID", smimeCapabilityMap)),
)
...
SMIMEEncryptionKeyPreference.componentType = namedtype.NamedTypes(
    namedtype.NamedType(
        ...
        IssuerAndSerialNumber().subtype(implicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 0)),
    ),
    namedtype.NamedType(
        "receipentKeyId",
        # Yes, 'receipentKeyId' is spelled incorrectly, but kept
        # this way for alignment with the ASN.1 module in the RFC.
        RecipientKeyIdentifier().subtype(implicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 1)),
    ),
    namedtype.NamedType(
        "subjectAltKeyIdentifier",
        SubjectKeyIdentifier().subtype(implicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 2)),
    ),
)
