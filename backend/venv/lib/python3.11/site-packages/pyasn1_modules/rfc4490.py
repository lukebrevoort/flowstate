...
class Gost28147_89_KeyWrapParameters(univ.Sequence):
    componentType = namedtype.NamedTypes(
        ...
        namedtype.OptionalNamedType("ukm", univ.OctetString().subtype(subtypeSpec=constraint.ValueSizeConstraint(8, 8))),
    )
...
class GostR3410_TransportParameters(univ.Sequence):
    componentType = namedtype.NamedTypes(
        ...
        namedtype.OptionalNamedType(
            "ephemeralPublicKey",
            SubjectPublicKeyInfo().subtype(implicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 0)),
        ),
        namedtype.NamedType("ukm", univ.OctetString().subtype(subtypeSpec=constraint.ValueSizeConstraint(8, 8))),
    )
...
class GostR3410_KeyTransport(univ.Sequence):
    componentType = namedtype.NamedTypes(
        ...
        namedtype.OptionalNamedType(
            "transportParameters",
            GostR3410_TransportParameters().subtype(implicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatConstructed, 0)),
        ),
    )
