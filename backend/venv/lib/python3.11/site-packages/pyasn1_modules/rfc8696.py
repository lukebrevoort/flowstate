...
class KeyTransPSKRecipientInfo(univ.Sequence):
    componentType = namedtype.NamedTypes(
        ...
        namedtype.NamedType("keyEncryptionAlgorithm", rfc5652.KeyEncryptionAlgorithmIdentifier()),
...
class KeyAgreePSKRecipientInfo(univ.Sequence):
    componentType = namedtype.NamedTypes(
        ...
        namedtype.NamedType(
            "originator",
            rfc5652.OriginatorIdentifierOrKey().subtype(explicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatConstructed, 0)),
        ),
        namedtype.OptionalNamedType(
            "ukm", rfc5652.UserKeyingMaterial().subtype(explicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 1))
        ),
        namedtype.NamedType("kdfAlgorithm", rfc5652.KeyDerivationAlgorithmIdentifier()),
        namedtype.NamedType("keyEncryptionAlgorithm", rfc5652.KeyEncryptionAlgorithmIdentifier()),
...
class CMSORIforPSKOtherInfo(univ.Sequence):
    componentType = namedtype.NamedTypes(
        ...
        namedtype.NamedType(
            "keyMgmtAlgType", univ.Enumerated(namedValues=namedval.NamedValues(("keyTrans", 5), ("keyAgree", 10)))
        ),
        namedtype.NamedType("keyEncryptionAlgorithm", rfc5652.KeyEncryptionAlgorithmIdentifier()),
        namedtype.NamedType("pskLength", univ.Integer().subtype(subtypeSpec=constraint.ValueRangeConstraint(1, MAX))),
        namedtype.NamedType("kdkLength", univ.Integer().subtype(subtypeSpec=constraint.ValueRangeConstraint(1, MAX))),
    )
