...
JWTClaimPermittedValues.componentType = namedtype.NamedTypes(
    ...
    namedtype.NamedType(
        "permitted", univ.SequenceOf(componentType=char.UTF8String()).subtype(sizeSpec=constraint.ValueSizeConstraint(1, MAX))
    ),
)
...
JWTClaimConstraints.componentType = namedtype.NamedTypes(
    namedtype.OptionalNamedType(
        "mustInclude", JWTClaimNames().subtype(explicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 0))
    ),
    namedtype.OptionalNamedType(
        "permittedValues",
        JWTClaimPermittedValuesList().subtype(explicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 1)),
    ),
)

JWTClaimConstraints.subtypeSpec = constraint.ConstraintsUnion(
    constraint.WithComponentsConstraint(("mustInclude", constraint.ComponentPresentConstraint())),
    constraint.WithComponentsConstraint(("permittedValues", constraint.ComponentPresentConstraint())),
)
...
TNEntry.componentType = namedtype.NamedTypes(
    namedtype.NamedType(
        "spc", ServiceProviderCode().subtype(explicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 0))
    ),
    namedtype.NamedType(
        "range", TelephoneNumberRange().subtype(explicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatConstructed, 1))
    ),
