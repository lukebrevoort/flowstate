...
class FileAndHash(univ.Sequence):
    componentType = namedtype.NamedTypes(
        namedtype.NamedType("file", char.IA5String()), namedtype.NamedType("hash", univ.BitString())
    )
...
class Manifest(univ.Sequence):
    componentType = namedtype.NamedTypes(
        ...
        namedtype.NamedType("manifestNumber", univ.Integer().subtype(subtypeSpec=constraint.ValueRangeConstraint(0, MAX))),
        ...
        namedtype.NamedType(
            "fileList",
            univ.SequenceOf(componentType=FileAndHash()).subtype(subtypeSpec=constraint.ValueSizeConstraint(0, MAX)),
        ),
    )
