...
class _BindValue(univ.Choice):
    componentType = namedtype.NamedTypes(
        ...
        namedtype.NamedType(
            "noSuchObject", univ.Null().subtype(implicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 0))
        ),
        namedtype.NamedType(
            "noSuchInstance", univ.Null().subtype(implicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 1))
        ),
        namedtype.NamedType(
            "endOfMibView", univ.Null().subtype(implicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 2))
        ),
    )
...
class PDU(univ.Sequence):
    componentType = namedtype.NamedTypes(
        ...
        namedtype.NamedType(
            "error-index", univ.Integer().subtype(subtypeSpec=constraint.ValueRangeConstraint(0, max_bindings))
        ),
...
class BulkPDU(univ.Sequence):
    componentType = namedtype.NamedTypes(
        namedtype.NamedType("request-id", rfc1902.Integer32()),
        namedtype.NamedType(
            "non-repeaters", univ.Integer().subtype(subtypeSpec=constraint.ValueRangeConstraint(0, max_bindings))
        ),
        namedtype.NamedType(
            "max-repetitions", univ.Integer().subtype(subtypeSpec=constraint.ValueRangeConstraint(0, max_bindings))
        ),
        namedtype.NamedType("variable-bindings", VarBindList()),
    )
