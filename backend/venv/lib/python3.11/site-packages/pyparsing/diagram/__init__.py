...
def railroad_to_html(diagrams: list[NamedDiagram], embed=False, **kwargs) -> str:
    ...
    for diagram in diagrams:
        ...
        if diagram.index == 0:
            title += " (root)"
        data.append({"title": title, "text": "", "svg": io.getvalue(), "bookmark": diagram.bookmark})
...
def _to_diagram_element(
    ...
    if _worth_extracting(element):
        ...
        if looked_up and looked_up.name is not None:
            ...
            ret = EditablePartial.from_call(railroad.NonTerminal, text=text, href=f"#{_make_bookmark(text)}")
            return ret
    ...
    if ret and (
        ("items" in ret.kwargs and len(ret.kwargs["items"]) == 0) or ("item" in ret.kwargs and ret.kwargs["item"] is None)
    ):
