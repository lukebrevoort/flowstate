    ...
    def with_line_numbers(
        ...
        *,
        ...
        if max_line_len >= 99:
            ...
            header0 = ""
        header1 = (
            ("" if base_1 else " ") + lead + "".join(f"         {(i + 1) % 10}" for i in range(-(-max_line_len // 10))) + "\n"
        )
        digits = "1234567890"
        ...
        return (
            ...
            + header2
            + "\n".join(
                f"{indent}{i:{lineno_width}d}:{line}{eol_mark}" for i, line in enumerate(s_lines, start=start_line + base_1)
            )
            + "\n"
        )
