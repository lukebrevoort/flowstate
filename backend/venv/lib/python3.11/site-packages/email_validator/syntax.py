...
def split_email(email: str) -> Tuple[Optional[str], str, str, bool]:
    ...
    if right_part.startswith("<"):
        ...
        if not display_name_quoted:
            bad_chars = {safe_character_display(c) for c in display_name if (not ATEXT_RE.match(c) and c != " ") or c == "."}
            if bad_chars:
                raise EmailSyntaxError(
                    "The display name contains invalid characters when not quoted: " + ", ".join(sorted(bad_chars)) + "."
                )
...
def validate_email_local_part(
    local: str, allow_smtputf8: bool = True, allow_empty_local: bool = False, quoted_local_part: bool = False
    ...
    if DOT_ATOM_TEXT_INTL.match(local):
        ...
        if not allow_smtputf8:
            ...
            if bad_chars:
                raise EmailSyntaxError(
                    "Internationalized characters before the @-sign are not supported: " + ", ".join(sorted(bad_chars)) + "."
                )
