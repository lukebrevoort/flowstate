...
class Headers:
    ...
    def get(self, key, default=None, type=None, as_bytes=None):
        ...
        if as_bytes is not None:
            warnings.warn(
                "The 'as_bytes' parameter is deprecated and will be" " removed in Werkzeug 3.0.",
                DeprecationWarning,
                stacklevel=2,
            )
...
def _str_header_key(key: t.Any) -> str:
    if not isinstance(key, str):
        warnings.warn(
            "Header keys must be strings. Passing other types is deprecated and will" " not be supported in Werkzeug 3.0.",
            DeprecationWarning,
            stacklevel=2,
        )
...
def _str_header_value(value: t.Any) -> str:
    if isinstance(value, bytes):
        warnings.warn(
            "Passing bytes as a header value is deprecated and will not be supported in" " Werkzeug 3.0.",
            DeprecationWarning,
            stacklevel=2,
        )
