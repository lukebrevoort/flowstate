    ...
    for key, value in _iter_data(data):
        ...
        if reader is not None:
            ...
            if not isinstance(value, str):
                value = str(value)
            write_binary(encoder.send_event(Field(name=key, headers=Headers())))
            write_binary(encoder.send_event(Data(data=value.encode(charset), more_data=False)))
...
class EnvironBuilder:
    ...
    def __init__(
        ...
        if base_url is not None:
            base_url = iri_to_uri(base_url, charset=charset if charset != "utf-8" else None)
    ...
    def delete_cookie(
        ...
        **kwargs: t.Any,
        ...
        if kwargs:
            ...
            warnings.warn(
                f"The {kwargs_keys} {plural} deprecated and will be" f" removed in Werkzeug 2.4.",
                DeprecationWarning,
                stacklevel=2,
            )
...
class Cookie:
    ...
    def _matches_request(self, server_name: str, path: str) -> bool:
        return (
            ...
            or (not self.origin_only and server_name.endswith(self.domain) and server_name[: -len(self.domain)].endswith("."))
        ) and (
            path == self.path
