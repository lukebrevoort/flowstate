...
class LocalProxy(t.Generic[T]):
    ...
    __doc__ = _ProxyLookup(class_value=__doc__, fallback=lambda self: type(self).__doc__, is_attr=True)  # type: ignore
    __wrapped__ = _ProxyLookup(fallback=lambda self: self._LocalProxy__wrapped, is_attr=True)
    # __del__ should only delete the proxy
    __repr__ = _ProxyLookup(repr, fallback=lambda self: f"<{type(self).__name__} unbound>")  # type: ignore
