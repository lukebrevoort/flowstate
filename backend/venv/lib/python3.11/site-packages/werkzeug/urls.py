...
_hextobyte = {f"{a}{b}".encode("ascii"): int(f"{a}{b}", 16) for a in _hexdigits for b in _hexdigits}
...
class BaseURL(_URLTuple):
    ...
    def get_file_location(self, pathformat: str | None = None) -> tuple[str | None, str | None]:
...
def _unquote_to_bytes(string: str | bytes, unsafe: str | bytes = "") -> bytes:
    ...
    try:
        ...
        hex_to_byte = _unquote_maps[unsafe] = {h: b for h, b in _hextobyte.items() if b not in unsafe}
...
def url_parse(url: str, scheme: str | None = None, allow_fragments: bool = True) -> BaseURL:
...
def url_quote(
    ...
    """
    warnings.warn(
        "'werkzeug.urls.url_quote' is deprecated and will be removed in Werkzeug 3.0." " Use 'urllib.parse.quote' instead.",
        DeprecationWarning,
        stacklevel=2,
    )
...
def url_quote_plus(string: str, charset: str = "utf-8", errors: str = "strict", safe: str = "") -> str:
...
def url_unquote_plus(s: str | bytes, charset: str = "utf-8", errors: str = "replace") -> str:
...
def iri_to_uri(
    ...
    if safe_conversion is not None:
        warnings.warn(
            "The 'safe_conversion' parameter is deprecated and will be removed in" " Werkzeug 3.0.",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    fragment = quote(parts.fragment, safe="%!#$&'()*+,/:;=?@", encoding=charset, errors=errors)
...
def url_join(
    ...
    """
    warnings.warn(
        "'werkzeug.urls.url_join' is deprecated and will be removed in Werkzeug 2.4." " Use 'urllib.parse.urljoin' instead.",
        DeprecationWarning,
        stacklevel=2,
    )
...
def _urlencode(query: t.Mapping[str, str] | t.Iterable[tuple[str, str]], encoding: str = "utf-8") -> str:
