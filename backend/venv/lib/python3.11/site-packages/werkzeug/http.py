    ...
    if isinstance(iterable, dict):
        ...
        for key, value in iterable.items():
            if value is None:
                ...
                items.append(f"{key}={quote_header_value(value, allow_token=allow_token)}")
    else:
...
def parse_cookie(
    ...
    return _sansio_http.parse_cookie(cookie=cookie, charset=charset, errors=errors, cls=cls)
...
def dump_cookie(
    ...
    if isinstance(key, bytes):
        warnings.warn(
            "The 'key' parameter must be a string. Bytes are deprecated" " and will not be supported in Werkzeug 3.0.",
            DeprecationWarning,
            stacklevel=2,
        )
        key = key.decode()
    ...
    if isinstance(value, bytes):
        warnings.warn(
            "The 'value' parameter must be a string. Bytes are" " deprecated and will not be supported in Werkzeug 3.0.",
            DeprecationWarning,
            stacklevel=2,
        )
        value = value.decode()
    ...
    if not _cookie_no_quote_re.fullmatch(value):
        ...
        value = _cookie_slash_re.sub(lambda m: _cookie_slash_map[m.group()], value.encode(charset)).decode("ascii")
        value = f'"{value}"'
