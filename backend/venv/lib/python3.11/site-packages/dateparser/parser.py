...
class _time_parser:
    time_directives = ["%H:%M:%S", "%I:%M:%S %p", "%H:%M", "%I:%M %p", "%I %p", "%H:%M:%S.%f", "%I:%M:%S.%f %p", "%H:%M %p"]
...
class _no_spaces_parser:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._all = self._dateformats + [x + y for x in self._dateformats for y in self._timeformats] + self._timeformats

        self.date_formats = {
            "%m%d%y": (
                self._preferred_formats + sorted(self._all, key=lambda x: x.lower().startswith("%m%d%y"), reverse=True)
            ),
...
class _parser:
    ...
    def _results(self):
        ...
        if time:
            params.update(dict(hour=time.hour, minute=time.minute, second=time.second, microsecond=time.microsecond))
    ...
    def _correct_for_time_frame(self, dateobj, tz):
        ...
        if self.month and not self.year:
            try:
                ...
                if dateobj.day == 29 and dateobj.month == 2:
                    valid_year = self._get_correct_leap_year(self.settings.PREFER_DATES_FROM, dateobj.year)
        ...
        if self._token_time and not any(
            [self._token_year, self._token_month, self._token_day, hasattr(self, "_token_weekday")]
        ):
    ...
    def _correct_for_day(self, dateobj):
        if getattr(self, "_token_day", None) or getattr(self, "_token_weekday", None) or getattr(self, "_token_time", None):
            return dateobj
