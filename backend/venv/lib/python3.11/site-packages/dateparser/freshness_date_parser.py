    ...
    def parse(self, date_string, settings):
        ...
        def apply_time(dateobj, timeobj):
            ...
            return dateobj.replace(
                hour=timeobj.hour, minute=timeobj.minute, second=timeobj.second, microsecond=timeobj.microsecond
            )
        ...
        if date:
            ...
            if not settings.RETURN_AS_TIMEZONE_AWARE or (
                settings.RETURN_AS_TIMEZONE_AWARE and "default" == settings.RETURN_AS_TIMEZONE_AWARE and not ptz
            ):
