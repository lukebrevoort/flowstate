...
def _check_repeated_values(setting_name, setting_value):
    if len(setting_value) != len(set(setting_value)):
        raise SettingValidationError('There are repeated values in the "{}" setting'.format(setting_name))
    return
...
def _check_require_part(setting_name, setting_value):
    ...
    if invalid_values:
        raise SettingValidationError(
            '"{}" setting contains invalid values: {}'.format(setting_name, ", ".join(invalid_values))
        )
...
def _check_parsers(setting_name, setting_value):
    ...
    if unknown_parsers:
        raise SettingValidationError(
            'Found unknown parsers in the "{}" setting: {}'.format(setting_name, ", ".join(unknown_parsers))
        )
    _check_repeated_values(setting_name, setting_value)
...
def _check_default_languages(setting_name, setting_value):
    ...
    if unsupported_languages:
        raise SettingValidationError(
            "Found invalid languages in the '{}' setting: {}".format(setting_name, ", ".join(map(repr, unsupported_languages)))
        )
    _check_repeated_values(setting_name, setting_value)
...
def check_settings(settings):
    ...
    settings_values = {
        ...
        "LANGUAGE_DETECTION_CONFIDENCE_THRESHOLD": {"type": float, "extra_check": _check_between_0_and_1},
        "CACHE_SIZE_LIMIT": {
            "type": int,
        },
    }
    ...
    for setting_name, setting_value in modified_settings.items():
        ...
        if not setting_type == setting_props["type"]:
            raise SettingValidationError(
                '"{}" must be "{}", not "{}".'.format(setting_name, setting_props["type"].__name__, setting_type.__name__)
            )
