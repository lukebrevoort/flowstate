...
from dateparser.utils import apply_timezone_from_settings, set_correct_day_from_settings, get_timezone_from_tz_string
...
def date_range(begin, end, **kwargs):
    dateutil_error_prone_args = ["year", "month", "week", "day", "hour", "minute", "second"]
...
def get_date_from_timestamp(date_string, settings, negative=False):
    ...
    if match:
        ...
        date_obj = datetime.fromtimestamp(seconds, timezone).replace(microsecond=millis * 1000 + micros, tzinfo=None)
...
class _DateLocaleParser:
    ...
    def _try_given_formats(self):
        ...
        return parse_with_formats(self._get_translated_date_with_formatting(), self.date_formats, settings=self._settings)

    def _get_translated_date(self):
        if self._translated_date is None:
            self._translated_date = self.locale.translate(self.date_string, keep_formatting=False, settings=self._settings)
...
class DateDataParser:
    ...
    def __init__(
        ...
        if not isinstance(try_previous_locales, bool):
            raise TypeError("try_previous_locales argument must be a boolean (%r given)" % type(try_previous_locales))
    ...
    def get_date_data(self, date_string, date_formats=None):
        ...
        *Period* represents the granularity of date parsed from the given string.
        ...
        for locale in self._get_applicable_locales(date_string):
            parsed_date = _DateLocaleParser.parse(locale, date_string, date_formats, settings=self._settings)
            if parsed_date:
    ...
    def _get_applicable_locales(self, date_string):
        ...
        for locale in self._get_locale_loader().get_locales(
            languages=self.languages, locales=self.locales, region=self.region, use_given_order=self.use_given_order
        ):
            for s in date_strings():
                if self._is_applicable_locale(locale, s):
                    yield locale
    ...
    def _is_applicable_locale(self, locale, date_string):
        return locale.is_applicable(date_string, strip_timezone=False, settings=self._settings)  # it is stripped outside
