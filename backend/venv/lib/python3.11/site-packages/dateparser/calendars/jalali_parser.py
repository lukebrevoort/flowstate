...
class jalali_parser(non_gregorian_parser):
    ...
    def _replace_months(cls, source):
        ...
        for pers, latin in reduce(
            lambda a, b: a + b, [[(value, month) for value in repl[-1]] for month, repl in cls._months.items()]
        ):
    ...
    def _replace_weekdays(cls, source):
        result = source
        for pers, latin in reduce(
            lambda a, b: a + b, [[(value, weekday) for value in repl] for weekday, repl in cls._weekdays.items()]
        ):
            result = result.replace(pers, latin)
        return result
    ...
    def _replace_days(cls, source):
        ...
        for persian_number, number in reduce(
            lambda a, b: a + b, [[(val, repl) for val in persian_word] for repl, persian_word in day_pairs]
        ):
