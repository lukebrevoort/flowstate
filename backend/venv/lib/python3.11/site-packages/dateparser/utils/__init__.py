...
def normalize_unicode(string, form="NFKD"):
    return "".join(c for c in unicodedata.normalize(form, string) if unicodedata.category(c) != "Mn")
...
def _get_missing_parts(fmt):
    ...
    """
    directive_mapping = {"day": ["%d", "%-d", "%j", "%-j"], "month": ["%b", "%B", "%m", "%-m"], "year": ["%y", "%-y", "%Y"]}

    missing = [
        field for field in ("day", "month", "year") if not any(directive in fmt for directive in directive_mapping[field])
    ]
    return missing
...
def registry(cls):
    ...
    if not (hasattr(cls, "get_key") and isinstance(cls.get_key, types.MethodType) and cls.get_key.__self__ is cls):
