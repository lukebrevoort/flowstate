...
class LanguageValidator:
    ...
    def _validate_sentence_splitter_group(cls, language_id, info):
        ...
        if isinstance(group, int) or not group:
            ...
            cls.get_logger().error(
                "Invalid 'sentence_splitter_group' for '%(id)s' language: " "expected int type but have got %(type)s",
        ...
        if isinstance(skip_tokens_list, list):
            for token in skip_tokens_list:
                if not isinstance(token, str) or not token:
                    cls.get_logger().error(
                        "Invalid 'skip' token %(token)r for '%(id)s' language: " "expected not empty string",
                        {"token": token, "id": language_id},
                    )
                    result = False
        else:
            cls.get_logger().error(
                "Invalid 'skip' list for '%(id)s' language: " "expected list type but have got %(type)s",
    ...
    def _validate_pertain_list(cls, language_id, info):
        ...
        if isinstance(pertain_tokens_list, list):
            for token in pertain_tokens_list:
                if not isinstance(token, str) or not token:
                    cls.get_logger().error(
                        "Invalid 'pertain' token %(token)r for '%(id)s' language: " "expected not empty string",
                        {"token": token, "id": language_id},
                    )
                    result = False
        else:
            cls.get_logger().error(
                "Invalid 'pertain' list for '%(id)s' language: " "expected list type but have got %(type)s",
    ...
    def _validate_weekdays(cls, language_id, info):
        ...
        for weekday in "monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday":
            if weekday not in info or not info[weekday]:
                cls.get_logger().error(
                    "No translations for '%(weekday)s' provided for '%(id)s' language", {"weekday": weekday, "id": language_id}
                )
                result = False
                continue
            ...
            if isinstance(translations_list, list):
                for token in translations_list:
                    if not isinstance(token, str) or not token:
                        cls.get_logger().error(
                            "Invalid '%(weekday)s' translation %(token)r for '%(id)s' language: " "expected not empty string",
    ...
    def _validate_months(cls, language_id, info):
        ...
        for month in (
            ...
            if month not in info or not info[month]:
                cls.get_logger().error(
                    "No translations for '%(month)s' provided for '%(id)s' language", {"month": month, "id": language_id}
                )
                result = False
                continue
            ...
            if isinstance(translations_list, list):
                for token in translations_list:
                    if not isinstance(token, str) or not token:
                        cls.get_logger().error(
                            "Invalid '%(month)s' translation %(token)r for '%(id)s' language: " "expected not empty string",
                            {"month": month, "token": token, "id": language_id},
                        )
                        result = False
            else:
                cls.get_logger().error(
                    "Invalid '%(month)s' translations list for '%(id)s' language: " "expected list type but have got %(type)s",
    ...
    def _validate_units(cls, language_id, info):
        ...
        for unit in "year", "month", "week", "day", "hour", "minute", "second":
            if unit not in info or not info[unit]:
                cls.get_logger().error(
                    "No translations for '%(unit)s' provided for '%(id)s' language", {"unit": unit, "id": language_id}
                )
                result = False
                continue
            ...
            if isinstance(translations_list, list):
                for token in translations_list:
                    if not isinstance(token, str) or not token:
                        cls.get_logger().error(
                            "Invalid '%(unit)s' translation %(token)r for '%(id)s' language: " "expected not empty string",
                            {"unit": unit, "token": token, "id": language_id},
                        )
                        result = False
            else:
                cls.get_logger().error(
                    "Invalid '%(unit)s' translations list for '%(id)s' language: " "expected list type but have got %(type)s",
    ...
    def _validate_other_words(cls, language_id, info):
        ...
        for word in ("ago",):
            if word not in info or not info[word]:
                cls.get_logger().error(
                    "No translations for '%(word)s' provided for '%(id)s' language", {"word": word, "id": language_id}
                )
                result = False
                continue
            ...
            if isinstance(translations_list, list):
                for token in translations_list:
                    if not isinstance(token, str) or not token:
                        cls.get_logger().error(
                            "Invalid '%(word)s' translation %(token)r for '%(id)s' language: " "expected not empty string",
                            {"word": word, "token": token, "id": language_id},
                        )
                        result = False
            else:
                cls.get_logger().error(
                    "Invalid '%(word)s' translations list for '%(id)s' language: " "expected list type but have got %(type)s",
    ...
    def _validate_simplifications(cls, language_id, info):
        ...
        if isinstance(simplifications_list, list):
            for simplification in simplifications_list:
                ...
                for group in replacements:
                    if group.isdigit():
                        ...
                        cls.get_logger().error(
                            "Invalid simplification %(simplification)r for '%(id)s' language: " "unknown group %(group)s",
                ...
                if extra_groups:
                    cls.get_logger().error(
                        "Invalid simplification %(simplification)r for '%(id)s' language: " "unknown groups %(groups)s",
                        {
                ...
                if not_used_groups:
                    cls.get_logger().error(
                        "Invalid simplification %(simplification)r for '%(id)s' language: " "groups %(groups)s were not used",
                        {
                            "simplification": simplification,
                            "id": language_id,
            ...
            cls.get_logger().error(
                "Invalid 'simplifications' list for '%(id)s' language: " "expected list type but have got %(type)s",
