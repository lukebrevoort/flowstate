...
class LocaleDataLoader:
    ...
    def get_locale_map(
        self, languages=None, locales=None, region=None, use_given_order=False, allow_conflicting_locales=False
    ):
        """
    ...
    def get_locales(self, languages=None, locales=None, region=None, use_given_order=False, allow_conflicting_locales=False):
        """
    ...
    def _load_data(self, languages=None, locales=None, region=None, use_given_order=False, allow_conflicting_locales=False):
        ...
        if locales:
            ...
            locale_dict.update(zip_longest(locales, tuple(zip_longest(languages, [], fillvalue=region))))

        if not use_given_order:
            locale_dict = OrderedDict(sorted(locale_dict.items(), key=lambda x: language_order.index(x[1][0])))
        ...
        for shortname, lang_reg in locale_dict.items():
            if shortname not in self._loaded_locales:
                ...
                if lang in self._loaded_languages:
                    ...
                    language_info = getattr(import_module("dateparser.data.date_translation_data." + lang), "info")
