...
class Dictionary:
    ...
    def __init__(self, locale_info, settings=None):
        ...
        dictionary.update(zip_longest(map(methodcaller("lower"), PARSER_KNOWN_TOKENS), PARSER_KNOWN_TOKENS))
    ...
    def _split_by_known_words(self, string, keep_formatting):
        ...
        if not match:
            return self._split_by_numerals(string, keep_formatting) if self._should_capture(string, keep_formatting) else []
    ...
    def _split_by_numerals(self, string, keep_formatting):
        return [token for token in NUMERAL_PATTERN.split(string) if self._should_capture(token, keep_formatting)]
    ...
    def _get_sorted_relative_strings_from_cache(self):
        if (
            ...
            self._add_to_cache(
                ...
                value=sorted([PARENTHESES_PATTERN.sub("", key) for key in self._relative_strings], key=len, reverse=True),
            )
