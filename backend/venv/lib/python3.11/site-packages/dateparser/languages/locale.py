...
class Locale:
    ...
    def _get_relative_translations(self, settings=None):
        if settings.NORMALIZE:
            if self._normalized_relative_translations is None:
                self._normalized_relative_translations = self._generate_relative_translations(normalize=True)
    ...
    def _simplify_split_align(self, original, settings):
        ...
        simplified_tokens = self._word_split(self._simplify(normalize_unicode(original), settings=settings), settings=settings)
