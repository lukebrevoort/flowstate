...
def analyze_param(
    *,
    ...
    if isinstance(value, params.Depends):
        assert depends is None, "Cannot specify `Depends` in `Annotated` and default value" f" together for {param_name!r}"
...
def _get_multidict_value(field: ModelField, values: Mapping[str, Any], alias: Union[str, None] = None) -> Any:
    ...
    if (
        value is None
        or (isinstance(field.field_info, params.Form) and isinstance(value, str) and value == "")  # For type checks
