...
_PARENT_RUN_TREE = contextvars.ContextVar[Optional[run_trees.RunTree]]("_PARENT_RUN_TREE", default=None)
...
_TRACING_ENABLED = contextvars.ContextVar[Optional[Union[bool, Literal["local"]]]]("_TRACING_ENABLED", default=None)
...
def tracing_context(
    ...
    **kwargs: Any,
    ...
    parent_run = _get_parent_run({"parent": parent or kwargs.get("parent_run")}) if parent is not False else None
...
def traceable(
    ...
    **kwargs: Any,
    ...
    run_type = cast(
        ...
        (args[0] if args and isinstance(args[0], str) else (kwargs.pop("run_type", None) or "chain")),
    )
    ...
    if kwargs:
        warnings.warn(
            f"The following keyword arguments are not recognized and will be ignored: " f"{sorted(kwargs.keys())}.",
            DeprecationWarning,
        )
    ...
    def decorator(func: Callable):
            ...
            *args: Any, langsmith_extra: Optional[LangSmithExtra] = None, **kwargs: Any
            ...
            try:
                ...
                async for item in _process_async_iterator(
                    ...
                    is_llm_run=(run_container["new_run"].run_type == "llm" if run_container["new_run"] else False),
        ...
        @functools.wraps(func)
        def stream_wrapper(*args: Any, langsmith_extra: Optional[LangSmithExtra] = None, **kwargs: Any) -> Any:
            if not func_accepts_config:
                kwargs.pop("config", None)
        ...
        @functools.wraps(func)
        async def async_stream_wrapper(*args: Any, langsmith_extra: Optional[LangSmithExtra] = None, **kwargs: Any) -> Any:
            if not func_accepts_config:
                kwargs.pop("config", None)
...
class trace:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if kwargs:
            warnings.warn(
                "The `trace` context manager no longer supports the following kwargs: " f"{sorted(kwargs.keys())}.",
                DeprecationWarning,
            )
        self.name = name
    ...
    def _setup(self) -> run_trees.RunTree:
        ...
        if parent_run_ is not None and enabled:
            ...
            self.new_run = run_trees.RunTree(
                ...
                reference_example_id=ls_client._ensure_uuid(self.reference_example_id, accept_null=True),
                run_type=self.run_type,
                extra=extra_outer,
    ...
    async def __aexit__(
        ...
        ctx = copy_context()
        if exc_type is not None:
            await asyncio.shield(aitertools.aio_to_thread(self._teardown, exc_type, exc_value, traceback, __ctx=ctx))
        else:
            await aitertools.aio_to_thread(self._teardown, exc_type, exc_value, traceback, __ctx=ctx)
        _set_tracing_context(get_tracing_context(ctx))
...
def as_runnable(traceable_fn: Callable) -> Runnable:
    ...
    try:
        ...
        raise ImportError(
            "as_runnable requires langchain-core to be installed. " "You can install it with `pip install langchain-core`."
        ) from e
    ...
    class RunnableTraceable(RunnableLambda):
        ...
        def _wrap_async(
            ...
            if not is_traceable_function(afunc):
                raise ValueError(
                    "RunnableTraceable expects a function wrapped by the LangSmith" f" @traceable decorator. Got {afunc}"
                )
...
def _get_parent_run(
    ...
    if _runtime_env.get_langchain_core_version() is not None:
        if rt := run_trees.RunTree.from_runnable_config(config, client=langsmith_extra.get("client")):
...
def _setup_run(
    ...
    selected_project = (
        ...
        or (parent_run_.session_name if parent_run_ else None)  # from parent run attempt 2 (not managed by traceable)
    ...
    if invocation_params_fn:
        try:
            invocation_params = {k: v for k, v in invocation_params_fn(inputs).items() if v is not None}
...
def _is_attachment(param: inspect.Parameter) -> bool:
    return param.annotation == schemas.Attachment or (
        get_origin(param.annotation) == Annotated and any(arg == schemas.Attachment for arg in get_args(param.annotation))
    )


def _attachment_args_helper(signature: inspect.Signature) -> set[str]:
    return {name for name, param in signature.parameters.items() if _is_attachment(param)}
    ...
    try:
        while True:
            ...
            if process_chunk:
                ...
                traced_item = item
            if is_llm_run and run_container["new_run"] and not run_container.get("_token_event_logged"):
...
class _TracedStreamBase(Generic[T]):
    ...
    def __init__(
        ...
        self.__is_llm_run__ = trace_container["new_run"].run_type == "llm" if trace_container["new_run"] else False
...
def _cleanup_traceback(e: BaseException):
    tb_ = e.__traceback__
    if tb_:
        while tb_.tb_next is not None and tb_.tb_frame.f_code.co_filename.endswith(_EXCLUDED_FRAME_FNAME):
            tb_ = tb_.tb_next
        e.__traceback__ = tb_
