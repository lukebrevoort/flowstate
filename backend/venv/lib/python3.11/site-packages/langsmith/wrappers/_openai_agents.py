...
try:
    ...
    class OpenAIAgentsTracingProcessor:
        ...
        def __init__(self, *args, **kwargs):
            raise ImportError(
                "The `agents` package is not installed. " "Please install it with `pip install langsmith[openai-agents]`."
            )
...
if HAVE_AGENTS:
    ...
    class OpenAIAgentsTracingProcessor(tracing.TracingProcessor):  # type: ignore[no-redef]
        ...
        def on_span_start(self, span: tracing.Span) -> None:
            parent_run = self._runs.get(span.parent_id) if span.parent_id else self._runs.get(span.trace_id)
            ...
            span_start_time = datetime.fromisoformat(span.started_at) if span.started_at else datetime.now(timezone.utc)
            dotted_order = agent_utils.ensure_dotted_order(
