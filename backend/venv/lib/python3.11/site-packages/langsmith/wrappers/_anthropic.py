...
def _accumulate_event(*, event: MessageStreamEvent, current_snapshot: Message | None) -> Message | None:
    try:
...
def _process_chat_completion(outputs: Any):
    try:
        ...
        rdict["usage_metadata"] = _create_usage_metadata(anthropic_token_usage) if anthropic_token_usage else None
        rdict.pop("type", None)
...
def wrap_anthropic(client: C, *, tracing_extra: Optional[TracingExtra] = None) -> C:
    ...
    if hasattr(client, "beta") and hasattr(client.beta, "messages") and hasattr(client.beta.messages, "create"):
