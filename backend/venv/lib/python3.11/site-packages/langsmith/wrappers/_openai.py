...
def _strip_not_given(d: dict) -> dict:
    try:
        ...
        if not_given is None:
            return d
        return {k: v for k, v in d.items() if not (isinstance(v, not_given) or (k.startswith("extra_") and v is None))}
...
def _create_usage_metadata(oai_token_usage: dict) -> UsageMetadata:
    ...
    output_tokens = oai_token_usage.get("completion_tokens") or oai_token_usage.get("output_tokens") or 0
    total_tokens = oai_token_usage.get("total_tokens") or input_tokens + output_tokens
    input_token_details: dict = {
        "audio": (oai_token_usage.get("prompt_tokens_details") or oai_token_usage.get("input_tokens_details") or {}).get(
            "audio_tokens"
        ),
        "cache_read": (oai_token_usage.get("prompt_tokens_details") or oai_token_usage.get("input_tokens_details") or {}).get(
            "cached_tokens"
        ),
    }
    output_token_details: dict = {
        "audio": (oai_token_usage.get("completion_tokens_details") or oai_token_usage.get("output_tokens_details") or {}).get(
            "audio_tokens"
        ),
        "reasoning": (
            oai_token_usage.get("completion_tokens_details") or oai_token_usage.get("output_tokens_details") or {}
...
def _process_chat_completion(outputs: Any):
    try:
        ...
        rdict["usage_metadata"] = _create_usage_metadata(oai_token_usage) if oai_token_usage else None
        return rdict
