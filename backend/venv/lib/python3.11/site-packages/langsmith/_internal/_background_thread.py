    ...
    try:
        ...
        if use_multipart:
            ...
            if any(isinstance(op, SerializedFeedbackOperation) for op in ops):
                logger.warn("Feedback operations are not supported in non-multipart mode")
                ops = [op for op in ops if not isinstance(op, SerializedFeedbackOperation)]
...
def _otel_tracing_thread_handle_batch(
    ...
    try:
        ...
        otel_context_map = {item.item.id: item.otel_context for item in batch if isinstance(item.item, SerializedRunOperation)}
        if run_ops:
...
def _hybrid_tracing_thread_handle_batch(
    ...
    try:
        ...
        if "cannot schedule new futures after interpreter shutdown" in str(e):
            ...
            _tracing_thread_handle_batch(client, tracing_queue, batch, use_multipart, False, langsmith_ops)
...
def _is_using_internal_otlp_provider(client: Client) -> bool:
    ...
    try:
        ...
        if hasattr(tracer_provider, "resource") and hasattr(tracer_provider.resource, "attributes"):
            is_internal = tracer_provider.resource.attributes.get("langsmith.internal_provider", False)
            logger.debug(f"TracerProvider resource check: " f"langsmith.internal_provider={is_internal}")
            return is_internal
...
def get_size_limit_from_env() -> Optional[int]:
    ...
    if size_limit_str is not None:
        try:
            ...
            logger.warning(f"Invalid value for BATCH_INGEST_SIZE_LIMIT: {size_limit_str}, " "continuing with default")
    return None
...
def tracing_control_thread_func(client_ref: weakref.ref[Client]) -> None:
    ...
    disable_compression = (
        ls_utils.is_truish(ls_utils.get_env_var("DISABLE_RUN_COMPRESSION")) or client.otel_exporter is not None
    )
...
def tracing_control_thread_func_compress_parallel(client_ref: weakref.ref[Client], flush_interval: float = 0.5) -> None:
    client = client_ref()
    if client is None:
        return
    logger.debug("Tracing control thread func compress parallel called")
    if client.compressed_traces is None or client._data_available_event is None or client._futures is None:
        logger.error(
