...
class EditDistance:
    ...
    def _get_metric(distance: str, normalize_score: bool = True) -> Callable:
        try:
            ...
            raise ImportError(
                "This operation requires the rapidfuzz library to use." "Please install it with `pip install -U rapidfuzz`."
            )
        ...
        if distance not in module_map:
            raise ValueError(f"Invalid distance metric: {distance}" f"\nMust be one of: {list(module_map)}")
