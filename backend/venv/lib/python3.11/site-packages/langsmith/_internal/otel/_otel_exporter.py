...
class OTELExporter:
        ...
        if op.inputs:
            try:
                ...
                if isinstance(inputs, dict):
                    if "model" in inputs and isinstance(inputs.get("messages"), list) and inputs["model"] is not None:
        ...
        if op.outputs:
            try:
                ...
                if isinstance(outputs, dict):
                    ...
                    if "system_fingerprint" in outputs and outputs["system_fingerprint"] is not None:
                        span.set_attribute(
                    ...
                    if "usage_metadata" in outputs and isinstance(outputs["usage_metadata"], dict):
                        ...
                        if "input_token_details" in usage_metadata and usage_metadata["input_token_details"] is not None:
                            input_token_details = str(usage_metadata["input_token_details"])
                            span.set_attribute(GEN_AI_USAGE_INPUT_TOKEN_DETAILS, input_token_details)
                        if "output_token_details" in usage_metadata and usage_metadata["output_token_details"] is not None:
                            output_token_details = str(usage_metadata["output_token_details"])
                            span.set_attribute(GEN_AI_USAGE_OUTPUT_TOKEN_DETAILS, output_token_details)
    ...
    def get_unified_run_tokens(self, outputs: Optional[dict]) -> Optional[tuple[int, int]]:
        ...
        for key in keys:
            ...
            if (
                ...
                and (output := self._extract_unified_run_tokens(haystack["kwargs"].get("usage_metadata")))
            ):
                return output
        ...
        for generation in [x for xs in generations for x in xs]:
            if (
                ...
                and (output := self._extract_unified_run_tokens(generation["message"]["kwargs"].get("usage_metadata")))
            ):
                return output
        return None
    ...
    def _extract_unified_run_tokens(self, outputs: Optional[Any]) -> Optional[tuple[int, int]]:
        ...
        if not isinstance(outputs["input_tokens"], int) or not isinstance(outputs["output_tokens"], int):
            return None
