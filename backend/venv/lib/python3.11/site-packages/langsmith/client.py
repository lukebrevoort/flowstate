...
try:
    ...
    raise ImportError("To use OTEL tracing, you must install it with `pip install langsmith[otel]`")
...
def _validate_api_key_if_hosted(api_url: str, api_key: Optional[str]) -> None:
    ...
    if not api_key:
        if _is_langchain_hosted(api_url) and not ls_utils.is_truish(ls_utils.get_env_var("OTEL_ENABLED")):
            warnings.warn(
...
def _get_tracing_sampling_rate(
    ...
    if sampling_rate < 0 or sampling_rate > 1:
        raise ls_utils.LangSmithUserError(
            "LANGSMITH_TRACING_SAMPLING_RATE must be between 0 and 1 if set." f" Got: {sampling_rate}"
        )
...
@typing.overload
def _ensure_uuid(value: Optional[Union[str, uuid.UUID]], *, accept_null: bool = True) -> Optional[uuid.UUID]: ...
...
class Client:
    ...
    def __init__(
        ...
        *,
        ...
        if (os.getenv("LANGSMITH_ENDPOINT") or os.getenv("LANGCHAIN_ENDPOINT")) and os.getenv("LANGSMITH_RUNS_ENDPOINTS"):
            raise ls_utils.LangSmithUserError(
        ...
        self.timeout_ms = (timeout_ms, timeout_ms) if isinstance(timeout_ms, int) else (timeout_ms or (10_000, 90_001))
        ...
        self._info = info if info is None or isinstance(info, ls_schemas.LangSmithInfo) else ls_schemas.LangSmithInfo(**info)
        ...
        self._hide_inputs = hide_inputs if hide_inputs is not None else ls_utils.get_env_var("HIDE_INPUTS") == "true"
        self._hide_outputs = hide_outputs if hide_outputs is not None else ls_utils.get_env_var("HIDE_OUTPUTS") == "true"
        self._hide_metadata = hide_metadata if hide_metadata is not None else ls_utils.get_env_var("HIDE_METADATA") == "true"
        ...
        if ls_utils.get_env_var("USE_PYO3_CLIENT") is not None:
            ...
            try:
                ...
                logger.warning(
                    "Failed to import `langsmith_pyo3` when PyO3 client was requested, " "falling back to Python impl: %s",
                    repr(e),
                )
    ...
    def _content_above_size(self, content_length: Optional[int]) -> Optional[str]:
        ...
        if content_length > size_limit:
            return f"The content length of {content_length} bytes exceeds the " f"maximum size limit of {size_limit} bytes."
        return None
        ...
        for idx in range(stop_after_attempt):
            try:
                try:
                    ...
                    if response is not None:
                        ...
                        if response.status_code == 500:
                            ...
                            raise ls_utils.LangSmithRequestTimeout(
                                f"Client took too long to send request to {method}" f"{pathname} {_context}"
                            )
                            ...
                            raise ls_utils.LangSmithAuthError(f"Authentication failed for {pathname}. {repr(e)}" f"{_context}")
                        elif response.status_code == 404:
                            raise ls_utils.LangSmithNotFoundError(
                                f"Resource not found for {pathname}. {repr(e)}" f"{_context}"
                            )
                        elif response.status_code == 409:
                            raise ls_utils.LangSmithConflictError(f"Conflict for {pathname}. {repr(e)}{_context}")
                        else:
                            raise ls_utils.LangSmithError(f"Failed to {method} {pathname} in LangSmith" f" API. {repr(e)}")
                    ...
                    try:
                        content_length = int(str(e.request.headers.get("Content-Length")) if e.request else "")
                    ...
                    if args:
                        emsg = "\n".join([str(args[0])] + [msg] + [str(arg) for arg in (args[2:] if len(args) > 2 else [])])
                    else:
                        emsg = msg
        ...
        **kwargs: Any,
        ...
        if run_create.get("attachments") is not None:
            for attachment in run_create["attachments"].values():
                if isinstance(attachment, tuple) and isinstance(attachment[1], Path) and not dangerously_allow_filesystem:
                    raise ValueError("Must set dangerously_allow_filesystem=True to allow passing in Paths for attachments.")
        ...
        if (
            ...
            if self._pyo3_client is not None:
                ...
                if self._data_available_event is None:
                    raise ValueError("Run compression is enabled but threading event is not configured")
                serialized_op = serialize_run_dict("post", run_create)
                multipart_form, opened_files = serialized_run_operation_to_multipart_parts_and_context(serialized_op)
                logger.log(
                    5,
                ...
                if HAS_OTEL:
                    self.tracing_queue.put(
                        TracingQueueItem(
                            ...
                            serialized_op,
                            otel_context=set_span_in_context(otel_trace.get_current_span()),
                        )
                    )
                else:
                    self.tracing_queue.put(TracingQueueItem(run_create["dotted_order"], serialized_op))
            else:
    ...
    def _batch_ingest_run_ops(
        ...
        for op in ops:
            if isinstance(op, SerializedRunOperation):
                ...
                if op.attachments:
                    logger.warning("Attachments are not supported when use_multipart_endpoint " "is False")
                ids_and_partial_body[op.operation].append((f"trace={op.trace_id},id={op.id}", _orjson.dumps(curr_dict)))
        ...
        info = self.info
        size_limit_bytes = (info.batch_ingest_config or {}).get("size_limit_bytes") or _SIZE_LIMIT_BYTES
        ...
        if body_size:
            ...
            self._post_batch_ingest_runs(_orjson.dumps(body_chunks), _context="\n" + context)
    ...
    def batch_ingest_runs(
        ...
        *,
        ...
        update_dicts = [self._run_transform(run, update=True, copy=False) for run in update or EMPTY_SEQ]
    ...
    def _multipart_ingest_ops(self, ops: list[Union[SerializedRunOperation, SerializedFeedbackOperation]]) -> None:
        parts: list[MultipartPartsAndContext] = []
        opened_files_dict: dict[str, io.BufferedReader] = {}
        for op in ops:
            if isinstance(op, SerializedRunOperation):
                part, opened_files = serialized_run_operation_to_multipart_parts_and_context(op)
                parts.append(part)
        ...
        *,
        ...
        if create_dicts:
            for run in create_dicts:
                if not run.get("trace_id") or not run.get("dotted_order"):
                    raise ls_utils.LangSmithUserError(
                        "Multipart ingest requires trace_id and dotted_order" " to be set in create dicts."
                    )
            else:
                del run
        if update_dicts:
            for run in update_dicts:
                if not run.get("trace_id") or not run.get("dotted_order"):
                    raise ls_utils.LangSmithUserError(
                        "Multipart ingest requires trace_id and dotted_order" " to be set in update dicts."
                    )
            else:
                del run
                ...
                try:
                    headers = {
                        **self._headers,
                        ...
                        "X-Pre-Compressed-Size": (str(compressed_traces_info[0]) if compressed_traces_info else ""),
                        "X-Post-Compressed-Size": (str(compressed_traces_info[1]) if compressed_traces_info else ""),
                    }
        ...
        if self._pyo3_client is not None:
            ...
            if self.compressed_traces is not None:
                ...
                if HAS_OTEL:
                    ...
                    self.tracing_queue.put(TracingQueueItem(data["dotted_order"], serialized_op))
        else:
    ...
    def flush_compressed_traces(self, attempts: int = 3) -> None:
        ...
        final_data_stream, compressed_traces_info = _tracing_thread_drain_compressed_buffer(
        ...
        if final_data_stream is not None:
            ...
            try:
                ...
                self._send_compressed_multipart_req(final_data_stream, compressed_traces_info, attempts=attempts)
    ...
    def _load_child_runs(self, run: ls_schemas.Run) -> ls_schemas.Run:
        ...
        treemap: collections.defaultdict[uuid.UUID, list[ls_schemas.Run]] = collections.defaultdict(list)
    ...
    def read_run(self, run_id: ID_TYPE, load_child_runs: bool = False) -> ls_schemas.Run:
        ...
        attachments = _convert_stored_attachments_to_attachments_dict(response.json(), attachments_key="s3_urls")
    ...
    def list_runs(
        ...
        **kwargs: Any,
        ...
        for i, run in enumerate(self._get_cursor_paginated_list("/runs/query", body=body_query)):
            ...
            attachments = _convert_stored_attachments_to_attachments_dict(run, attachments_key="s3_urls")
        ...
        *,
        ...
        if project_names:
            with ThreadPoolExecutor() as executor:
                futures = [executor.submit(self.read_project, project_name=name) for name in project_names]
    ...
    def get_run_url(
        ...
        *,
        ...
        return f"{self._host_url}/o/{self._get_tenant_id()}/projects/p/{session_id_}/" f"r/{run.id}?poll=true"
    ...
    def read_shared_run(self, share_token: Union[ID_TYPE, str], run_id: Optional[ID_TYPE] = None) -> ls_schemas.Run:
        """Get shared runs.
    ...
    def read_dataset_shared_schema(
        ...
        *,
        ...
        return cast(
            ...
            {
                **d,
                "url": f"{self._host_url}/public/" f"{_as_uuid(d['share_token'], 'response.share_token')}/d",
            },
        )
    ...
    def _get_optional_tenant_id(self) -> Optional[uuid.UUID]:
        ...
        try:
            ...
            if isinstance(result, list) and len(result) > 0:
                tracer_session = ls_schemas.TracerSessionResult(**result[0], _host_url=self._host_url)
            ...
            for r in runs:
                row = {
                    ...
                    **{f"outputs.{k}": v for k, v in (r.outputs or {}).items()},
                    "execution_time": ((r.end_time - r.start_time).total_seconds() if r.end_time else None),
                    "error": r.error,
                    "id": r.id,
                }
                if r.feedback_stats:
                    row.update({f"feedback.{k}": v.get("avg") for k, v in r.feedback_stats.items()})
    ...
    def list_projects(
        ...
        if reference_dataset_id is not None:
            if reference_dataset_name is not None:
                raise ValueError("Only one of reference_dataset_id or" " reference_dataset_name may be given")
    ...
    def delete_project(self, *, project_name: Optional[str] = None, project_id: Optional[str] = None) -> None:
    ...
    def diff_dataset_versions(
        ...
        *,
        ...
        response = self.request_with_retries(
            ...
            headers=self._headers,
            params={
                "from_version": (from_version.isoformat() if isinstance(from_version, datetime.datetime) else from_version),
                "to_version": (to_version.isoformat() if isinstance(to_version, datetime.datetime) else to_version),
            },
        )
        ls_utils.raise_for_status_with_text(response)
    ...
    def create_example_from_run(
        ...
        if dataset_type == ls_schemas.DataType.llm:
            ...
            try:
                ...
                raise ValueError("Error converting LLM run inputs to prompt for run" f" {run.id} with inputs {run.inputs}")
            ...
            if not run.outputs:
                ...
                try:
                    ...
                    raise ValueError(
                        "Error converting LLM run outputs to generation for run" f" {run.id} with outputs {run.outputs}"
                    )
                outputs = {"output": generation}
            ...
            try:
                ...
                raise ValueError(
                    "Error converting LLM run inputs to chat messages for run" f" {run.id} with inputs {run.inputs}"
                )
            if not run.outputs:
                outputs = None
            else:
                try:
                    ...
                    raise ValueError(
                        "Error converting LLM run outputs to chat generations" f" for run {run.id} with outputs {run.outputs}"
                    )
    ...
    def _prepare_multipart_data(
        self,
        examples: Union[
            list[ls_schemas.ExampleCreate] | list[ls_schemas.ExampleUpsertWithAttachments] | list[ls_schemas.ExampleUpdate],
        ],
        ...
        if include_dataset_id:
            if not isinstance(examples[0], ls_schemas.ExampleUpsertWithAttachments):
                raise ValueError("The examples must be of type ExampleUpsertWithAttachments" " if include_dataset_id is True")
        ...
        for example in examples:
            if (
                ...
                raise ValueError(
                    "The examples must be of type ExampleCreate" " or ExampleUpsertWithAttachments" " or ExampleUpdate"
                )
            ...
            example_body = {
                ...
                **({"use_source_run_attachments": use_source_run_attachments} if use_source_run_attachments else {}),
                ...
                for name, attachment in example.attachments.items():
                    ...
                    if isinstance(attachment_data, Path):
                        if dangerously_allow_filesystem:
                            ...
                            raise ValueError("dangerously_allow_filesystem must be True to upload files from the filesystem")
                    else:
                        parts.append(
                            (
                                f"{example_id}.attachment.{name}",
    ...
    def create_examples(
        ...
        **kwargs: Any,
        ...
        if examples:
            uploads = [ls_schemas.ExampleCreate(**x) if isinstance(x, dict) else x for x in examples]
        ...
        # For backwards compatibility
            ...
            for arg_name, arg_value in kwargs.items():
                if arg_value is not None and len(arg_value) != input_len:
                    raise ValueError(
                        f"Length of {arg_name} ({len(arg_value)}) does not match" f" length of inputs ({input_len})"
                    )
            uploads = [
        ...
        if (self.info.instance_flags or {}).get("dataset_examples_multipart_enabled", False):
            ...
            response = self.request_with_retries(
                ...
                "/examples/bulk",
                headers={**self._headers, "Content-Type": "application/json"},
                data=_dumps_json([{**dump_model(upload), "dataset_id": str(dataset_id)} for upload in uploads]),
            )
            ls_utils.raise_for_status_with_text(response)
            return response.json()
    ...
    def create_example(
        ...
        data.id = (uuid.UUID(example_id) if isinstance(example_id, str) else example_id) if example_id else uuid.uuid4()
    ...
    def update_example(
        ...
        *,
        ...
        example = ls_schemas.ExampleUpdate(**{k: v for k, v in example_dict.items() if v is not None})
    ...
    def update_examples(
        ...
        **kwargs: Any,
        ...
        if updates:
            updates_obj = [ls_schemas.ExampleUpdate(**x) if isinstance(x, dict) else x for x in updates]
        ...
        # For backwards compatibility
            ...
            multipart_enabled = (self.info.instance_flags or {}).get("dataset_examples_multipart_enabled")
            if not multipart_enabled and (kwargs.get("attachments_operations") or kwargs.get("attachments")) is not None:
                raise ValueError(
            ...
            for arg_name, arg_value in kwargs.items():
                if arg_value is not None and len(arg_value) != examples_len:
                    raise ValueError(
                        f"Length of {arg_name} ({len(arg_value)}) does not match" f" length of examples ({examples_len})"
                    )
            updates_obj = [
        ...
        if (self.info.instance_flags or {}).get("dataset_examples_multipart_enabled", False):
            ...
            response = self.request_with_retries(
                ...
                headers={**self._headers, "Content-Type": "application/json"},
                data=(
                    _dumps_json([{k: v for k, v in dump_model(example).items() if v is not None} for example in updates_obj])
                ),
            )
            ls_utils.raise_for_status_with_text(response)
            return response.json()
    ...
    def delete_examples(self, example_ids: Sequence[ID_TYPE]) -> None:
        ...
        response = self.request_with_retries(
            ...
            "/examples",
            headers={**self._headers, "Content-Type": "application/json"},
            params={"example_ids": [str(_as_uuid(id_, f"example_ids[{i}]")) for i, id_ in enumerate(example_ids)]},
        )
        ls_utils.raise_for_status_with_text(response)
    ...
    def update_dataset_splits(
        ...
        *,
        ...
        data = {
            ...
            "examples": [str(_as_uuid(id_, f"example_ids[{i}]")) for i, id_ in enumerate(example_ids)],
            "remove": remove,
        }

        response = self.request_with_retries("PUT", f"/datasets/{_as_uuid(dataset_id, 'dataset_id')}/splits", json=data)
        ls_utils.raise_for_status_with_text(response)
    ...
    def _select_eval_results(
        ...
        *,
        ...
        if isinstance(results, ls_evaluator.EvaluationResult):
            ...
            raise ValueError(
                f"Invalid evaluation results type: {type(results)}." " Must be EvaluationResult, EvaluationResults."
            )
        return results_
    ...
    def _log_evaluation_feedback(
        self,
        evaluator_response: Union[ls_evaluator.EvaluationResult, ls_evaluator.EvaluationResults, dict],
    ...
    def create_feedback(
        ...
        **NOTE**: To enable feedback to be batch uploaded in the background you must
        ...
        if kwargs:
            warnings.warn(
                "The following arguments are no longer used in the create_feedback" f" endpoint: {sorted(kwargs)}",
                DeprecationWarning,
            )
        try:
            ...
            use_multipart = (self.info.batch_ingest_config or {}).get("use_multipart_endpoint", False)
            ...
            if (
                ...
                if self.compressed_traces is not None:
                    multipart_form = serialized_feedback_operation_to_multipart_parts_and_context(serialized_op)
                    with self.compressed_traces.lock:
                        compress_multipart_parts_and_context(
                            multipart_form,
                            self.compressed_traces,
    ...
    def create_presigned_feedback_tokens(
        ...
        *,
        ...
        with cf.ThreadPoolExecutor(max_workers=len(self._write_api_urls)) as executor:
            futs = [executor.submit(req, api_url, api_key) for api_url, api_key in self._write_api_urls.items()]
    ...
    def list_annotation_queues(
        ...
        *,
        ...
        """
        params: dict = {
            "ids": ([_as_uuid(id_, f"queue_ids[{i}]") for i, id_ in enumerate(queue_ids)] if queue_ids is not None else None),
            "name": name,
    ...
    def get_run_from_annotation_queue(self, queue_id: ID_TYPE, *, index: int) -> ls_schemas.RunWithAnnotationQueueInfo:
    ...
    def _like_or_unlike_prompt(self, prompt_identifier: str, like: bool) -> dict[str, int]:
        ...
        response = self.request_with_retries("POST", f"/likes/{owner}/{prompt_name}", json={"like": like})
    ...
    def _get_prompt_url(self, prompt_identifier: str) -> str:
        ...
        settings = self._get_settings()
        return f"{self._host_url}/prompts/{prompt_name}/{commit_hash[:8]}" f"?organizationId={settings.id}"
    ...
    def create_commit(
        ...
        *,
        ...
        response = self.request_with_retries("POST", f"/commits/{prompt_owner_and_name}", json=request_dict)
    ...
    def pull_prompt_commit(
        ...
        *,
        ...
        response = self.request_with_retries(
            "GET",
            (f"/commits/{owner}/{prompt_name}/{commit_hash}" f"{'?include_model=true' if include_model else ''}"),
        )
