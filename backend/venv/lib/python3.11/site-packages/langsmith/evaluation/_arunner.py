...
async def _aevaluate(
    ...
    num_include_attachments = int(_target_include_attachments(target)) + _evaluators_include_attachments(evaluators)
    ...
    with ls_utils.with_optional_cache(cache_path, ignore_hosts=[client.api_url]):
        if is_async_target:
            if evaluators:
                ...
                manager = await manager.awith_predictions(cast(ATARGET_T, target), max_concurrency=max_concurrency)
            if summary_evaluators:
                manager = await manager.awith_summary_evaluators(summary_evaluators)
        else:
            if evaluators:
                manager = await manager.awith_evaluators(evaluators, max_concurrency=max_concurrency)
            if summary_evaluators:
                manager = await manager.awith_summary_evaluators(summary_evaluators)
...
class _AsyncExperimentManager(_ExperimentManagerMixin):
        ...
        if self._examples is None:
            ...
            if self._num_repetitions > 1:
                ...
                self._examples = async_chain_from_iterable(
                    [
                        async_iter_from_list([self._reset_example_attachments(example) for example in examples_list])
    ...
    async def astart(self) -> _AsyncExperimentManager:
        try:
            ...
            raise ValueError("No examples found in the dataset. " "Please ensure the data provided to aevaluate is not empty.")
    ...
    async def aget_summary_scores(self) -> dict[str, list[dict]]:
        if self._summary_results is None:
            return {"results": []}
        return {
            "results": [res async for results in self._summary_results for res in results["results"]]  # type: ignore[misc]
        }
        ...
        with cf.ThreadPoolExecutor(max_workers=4) as feedback_executor:
            ...
            async def score_all():
                async for current_results in self.aget_results():
                    ...
                    yield self._arun_evaluators(evaluators, current_results, feedback_executor=feedback_executor)
            ...
            async def _run_single_evaluator(evaluator: RunEvaluator):
                ...
                try:
                    ...
                    if self._upload_results:
                        self.client._log_evaluation_feedback(evaluator_response, run=run, _executor=feedback_executor)
                    ...
                    try:
                        ...
                        if self._upload_results:
                            self.client._log_evaluation_feedback(error_response, run=run, _executor=feedback_executor)
                        return selected_results
...
class AsyncExperimentResults:
    ...
    async def __anext__(self) -> ExperimentResultRow:
        ...
        while True:
            ...
            await asyncio.shield(asyncio.wait_for(_wait_until_index(len(self._results)), timeout=None))
...
def _aresolve_data(
    ...
    *,
    ...
    return aitertools.ensure_async_iterator(_resolve_data(data, client=client, include_attachments=include_attachments))
