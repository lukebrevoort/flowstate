    ...
    def check_value_non_numeric(cls, v, values):
        ...
        if "score" not in values or values["score"] is None:
            if isinstance(v, (int, float)):
                logger.warning("Numeric values should be provided in" " the 'score' field, not 'value'." f" Got: {v}")
        return v
...
class DynamicRunEvaluator(RunEvaluator):
    ...
    def _coerce_evaluation_result(
        ...
        try:
            ...
            raise ValueError(
                "Expected an EvaluationResult object, or dict with a metric" f" 'key' and optional 'score'; got {result}"
            ) from e
    ...
    def _coerce_evaluation_results(
        ...
        return self._coerce_evaluation_result(cast(dict, results), source_run_id=source_run_id, allow_no_key=True)
...
class DynamicComparisonRunEvaluator:
    ...
    def compare_runs(self, runs: Sequence[Run], example: Optional[Example] = None) -> ComparisonEvaluationResult:
    ...
    async def acompare_runs(self, runs: Sequence[Run], example: Optional[Example] = None) -> ComparisonEvaluationResult:
        """Evaluate a run asynchronously using the wrapped async function.
    ...
    def __call__(self, runs: Sequence[Run], example: Optional[Example] = None) -> ComparisonEvaluationResult:
        """Make the evaluator callable, allowing it to be used like a function.
    ...
    def _format_results(
        ...
        if isinstance(result, ComparisonEvaluationResult):
            ...
            msg = "Expected 'dict', 'list' or 'ComparisonEvaluationResult' result " f"object. Received: {result=}"
...
def _normalize_evaluator_func(
    ...
    args_with_defaults = [pname for pname, p in sig.parameters.items() if p.default is not inspect.Parameter.empty]
    if not all_args or (
    ...
    # types, respectively.
    elif not all(pname in supported_args or pname in args_with_defaults for pname in all_args) or all_args == [
        "run",
        "example",
    ]:
        return func, None
        ...
        if inspect.iscoroutinefunction(func):
                ...
                for param_name, param in sig.parameters.items():
                    ...
                    if param_name in arg_map:
                        ...
                        traced_inputs[param_name] = (
                            _maxsize_repr(arg_map[param_name]) if param_name in ("run", "example") else arg_map[param_name]
                        )
            ...
            awrapper.__name__ = getattr(func, "__name__") if hasattr(func, "__name__") else awrapper.__name__
            ...
            wrapper.__name__ = getattr(func, "__name__") if hasattr(func, "__name__") else wrapper.__name__
...
def _normalize_comparison_evaluator_func(
    ...
    # List[Example] types, respectively.
        ...
        if inspect.iscoroutinefunction(func):
            ...
            def _prepare_inputs(runs: Sequence[Run], example: Optional[Example]) -> tuple[list, dict, dict]:
                arg_map = {
                    "runs": runs,
                    "example": example,
                    "inputs": example.inputs if example else {},
                ...
                for param_name, param in sig.parameters.items():
                    ...
                    if param_name in arg_map:
                        if param.kind in (
                            ...
                            kwargs[param_name] = arg_map[param_name]
                        traced_inputs[param_name] = (
                            _maxsize_repr(arg_map[param_name]) if param_name in ("runs", "example") else arg_map[param_name]
                        )
                return args, kwargs, traced_inputs

            async def awrapper(runs: Sequence[Run], example: Optional[Example]) -> _COMPARISON_OUTPUT:
...
def _format_evaluator_result(
    ...
    if isinstance(result, (bool, float, int)):
        ...
        raise ValueError(
            f"Expected a non-empty dict, str, bool, int, float, list, " f"EvaluationResult, or EvaluationResults. Got {result}"
        )
    elif isinstance(result, list):
        ...
        raise ValueError(
            f"Expected a dict, str, bool, int, float, list, EvaluationResult, or " f"EvaluationResults. Got {result}"
        )
    return result
...
def _normalize_summary_evaluator(func: Callable) -> SUMMARY_EVALUATOR_T:
    ...
    if not all_args or (
        ...
        msg = f"Invalid evaluator function. Must have at least one " f"argument. Supported arguments are {supported_args}."
        if all_args:
