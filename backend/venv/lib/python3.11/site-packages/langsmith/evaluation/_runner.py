...
def evaluate(
    ...
    **kwargs: Any,
    ...
    if isinstance(target, (str, uuid.UUID, schemas.TracerSession)):
        ...
        if len(target) != 2 or not all(isinstance(t, (str, uuid.UUID, schemas.TracerSession)) for t in target):
            msg = (
        ...
        msg = f"Received unsupported arguments {kwargs}. These arguments are not " f"supported when creating a new experiment."
        raise ValueError(msg)
    elif not data:
...
class ExperimentResults:
    ...
    def __iter__(self) -> Iterator[ExperimentResultRow]:
        ix = 0
        while not self._processing_complete.is_set() or not self._queue.empty() or ix < len(self._results):
            try:
...
def evaluate_comparative(
    ...
    def evaluate_and_submit_feedback(
        ...
        comments = (
            {str(rid): result.comment for rid in result.scores} if isinstance(result.comment, str) else (result.comment or {})
        )
        ...
        for example_id, runs_list in tqdm(runs_dict.items()):
            ...
            for comparator in comparators:
                if max_concurrency > 1:
                    ...
                    result = evaluate_and_submit_feedback(runs_list, data[example_id], comparator, executor)
...
def _evaluate(
    ...
    with ls_utils.with_optional_cache(cache_path, ignore_hosts=[client.api_url]):
        if _is_callable(target):
            ...
            manager = manager.with_predictions(cast(TARGET_T, target), max_concurrency=max_concurrency)
        if evaluators:
...
def _load_examples_map(client: langsmith.Client, project: schemas.TracerSession) -> dict[uuid.UUID, schemas.Example]:
    return {
        e.id: e
        for e in client.list_examples(
...
class _ExperimentManagerMixin:
    ...
    def _print_experiment_start(self, project: Optional[schemas.TracerSession], first_example: schemas.Example) -> None:
        if project and project.url:
            # TODO: Make this a public API
            project_url = project.url.split("?")[0]
            dataset_id = first_example.dataset_id
            base_url = project_url.split("/projects/p/")[0]
            comparison_url = f"{base_url}/datasets/{dataset_id}/compare?" f"selectedSessions={project.id}"
            print(  # noqa: T201
                f"View the evaluation results for experiment: '{self.experiment_name}'" f" at:\n{comparison_url}\n\n"
            )
        else:
            # HACKHACK
...
class _ExperimentManager(_ExperimentManagerMixin):
    ...
    def _reset_example_attachment_readers(self, example: schemas.Example) -> schemas.Example:
        ...
        for name, attachment in example.attachments.items():
            if self._attachment_raw_data_dict is not None and str(example.id) + name in self._attachment_raw_data_dict:
    ...
    def get_summary_scores(self) -> dict[str, list[dict]]:
        ...
        return {"results": [res for results in self._summary_results for res in results["results"]]}  # type: ignore[misc]
            ...
            for evaluator in evaluators:
                ...
                try:
                    ...
                    if self._upload_results:
                        # TODO: This is a hack
                        self.client._log_evaluation_feedback(evaluator_response, run=run, _executor=executor)
                    ...
                    try:
                        ...
                        eval_results["results"].extend(self.client._select_eval_results(error_response))
                        if self._upload_results:
                            # TODO: This is a hack
                            self.client._log_evaluation_feedback(error_response, run=run, _executor=executor)
                    except Exception as e2:
                        logger.debug(f"Error parsing feedback keys: {e2}")
                        pass
                    logger.error(
                        f"Error running evaluator {repr(evaluator)} on" f" run {run.id if run else ''}: {repr(e)}",
                        exc_info=True,
                    )
...
def _resolve_data(
    ...
    *,
    ...
    if isinstance(data, uuid.UUID):
        ...
        return client.list_examples(dataset_id=uuid.UUID(data), include_attachments=include_attachments)
...
def _get_target_args(target: Any) -> list[str]:
    ...
    if len(positional_params) == 0:
        ...
        raise ValueError(
            "Target function must accept at most three " "arguments without default values: (inputs, attachments, metadata)."
        )
...
def _resolve_experiment(
    ...
    if experiment is not None:
        ...
        if not experiment_.reference_dataset_id:
            raise ValueError("Experiment must have an associated reference_dataset_id, " "but none was provided.")
                        ...
                        for elt in value.elts:
                            if isinstance(elt, ast.Dict):
                                for elt_key, elt_value in zip(elt.keys, elt.values):
                                    if isinstance(elt_key, (ast.Str, ast.Constant)) and elt_key.s == "key":
                                ...
                                for keyword in elt.keywords:
                                    if keyword.arg == "key" and isinstance(keyword.value, (ast.Str, ast.Constant)):
                                        keys.append(
                                            keyword.value.s if isinstance(keyword.value, ast.Str) else keyword.value.value
                                        )
    ...
    return [
        {
            ...
            **{f"outputs.{k}": v for k, v in (x["run"].outputs or {}).items()},
            "error": x["run"].error,
            **({f"reference.{k}": v for k, v in x["example"].outputs.items()} if x["example"].outputs is not None else {}),
            **{f"feedback.{r.key}": r.score if r.score is not None else r.value for r in x["evaluation_results"]["results"]},
            "execution_time": ((x["run"].end_time - x["run"].start_time).total_seconds() if x["run"].end_time else None),
