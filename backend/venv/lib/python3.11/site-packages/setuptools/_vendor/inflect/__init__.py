...
plverb_special_s = enclose(
    "|".join([pl_sb_singular_s] + pl_sb_uninflected_s + list(pl_sb_irregular_s) + ["(.*[csx])is", "(.*)ceps", "[A-Z].*s"])
)
...
class engine:
    ...
    def _string_to_substitute(self, mo: Match, methods_dict: Dict[str, Callable]) -> str:
        ...
        kwargs_list = {
            kw.arg: self._get_value_from_ast(kw.value) for kw in a_tree.body[0].value.keywords  # type: ignore[attr-defined]
        }
    ...
    def get_count(self, count: Optional[Union[str, int]] = None) -> Union[str, int]:
        ...
        if count is not None:
            count = (
                1
                if ((str(count) in pl_count_one) or (self.classical_dict["zero"] and str(count).lower() in pl_count_zero))
                else 2
            )
        else:
            count = ""
