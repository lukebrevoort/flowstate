...
def check_tuple(
    ...
    if use_ellipsis:
        ...
        if len(value) != len(tuple_params):
            raise TypeCheckError(f"has wrong number of elements (expected {len(tuple_params)}, got " f"{len(value)} instead)")
...
def check_protocol(
    ...
    for attrname in dir(origin_type):
        ...
        if callable(member):
            ...
            return_annotation = signature.return_annotation if signature.return_annotation is not Parameter.empty else Any
    ...
    try:
        for attrname, callable_args in expected_methods.items():
            ...
            try:
                ...
                raise TypeCheckError(
                    f"is not compatible with the {origin_type.__qualname__} protocol " f"because its {attrname!r} method {exc}"
                ) from None
