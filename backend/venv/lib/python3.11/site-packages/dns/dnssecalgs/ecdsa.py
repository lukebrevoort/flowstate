...
class PrivateECDSA(CryptographyPrivateKey):
    ...
    def sign(
        ...
        algorithm = ec.ECDSA(self.public_cls.chosen_hash, deterministic_signing=deterministic)
        der_signature = self.key.sign(data, algorithm)
        dsa_r, dsa_s = utils.decode_dss_signature(der_signature)
        signature = int.to_bytes(dsa_r, length=self.public_cls.octets, byteorder="big") + int.to_bytes(
            dsa_s, length=self.public_cls.octets, byteorder="big"
        )
        if verify:
    ...
    def generate(cls) -> "PrivateECDSA":
        return cls(
            key=ec.generate_private_key(curve=cls.public_cls.curve, backend=default_backend()),
        )
