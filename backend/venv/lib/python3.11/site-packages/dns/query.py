...
if _have_httpx:
    ...
    class _NetworkBackend(_CoreNetworkBackend):
        ...
        def connect_tcp(self, host, port, timeout, local_address, socket_options=None):  # pylint: disable=signature-differs
            addresses = []
        ...
        def connect_unix_socket(self, path, timeout, socket_options=None):  # pylint: disable=signature-differs
...
def https(
    ...
    *http_version*, a ``dns.query.HTTPVersion``, indicating which HTTP version to use.
    ...
    if response.status_code < 200 or response.status_code > 299:
        raise ValueError(f"{where} responded with status code {response.status_code}" f"\nResponse body: {response.content}")
...
def _http3(
    ...
    q.id = 0
    wire = q.to_wire()
    manager = dns.quic.SyncQuicManager(verify_mode=verify, server_name=hostname, h3=True)
