...
class CliSettingsSource(EnvSettingsSource, Generic[T]):
    ...
    def __init__(
        ...
        self.cli_exit_on_error = (
            cli_exit_on_error if cli_exit_on_error is not None else settings_cls.model_config.get("cli_exit_on_error", True)
        )
        ...
        if self.cli_parse_args not in (None, False):
            if self.cli_parse_args is True:
                ...
                raise SettingsError(f"cli_parse_args must be a list or tuple of strings, received {type(self.cli_parse_args)}")
    ...
    def _load_env_vars(
        ...
        parsed_args = {
            key: val for key, val in parsed_args.items() if not key.endswith(":subcommand") and val is not PydanticUndefined
        }
    ...
    def _get_merge_parsed_list_types(self, parsed_list: list[str], field_name: str) -> tuple[Optional[type], Optional[type]]:
        merge_type = self._cli_dict_args.get(field_name, list)
        if (
            merge_type is list
            or not is_union_origin(get_origin(merge_type))
            or not any(
                type_ for type_ in get_args(merge_type) if type_ is not type(None) and get_origin(type_) not in (dict, Mapping)
            )
        ):
        ...
        for field_name, field_info in self._sort_arg_fields(model):
            ...
            if _CliSubCommand in field_info.metadata:
                for model in sub_models:
                    subcommand_alias = self._check_kebab_name(model.__name__ if len(sub_models) > 1 else preferred_alias)
                ...
                if _CliPositionalArg in field_info.metadata:
                    arg_names, flag_prefix = self._convert_positional_arg(kwargs, field_info, preferred_alias, model_default)
                ...
                if is_parser_submodel:
                    ...
                    if group is not None:
                        ...
                        added_args += list(arg_names)
                        self._add_argument(parser, *(f"{flag_prefix[: len(name)]}{name}" for name in arg_names), **kwargs)
    ...
    def _add_parser_submodels(
        ...
        if not self.cli_avoid_json:
            ...
            kwargs["nargs"] = "?"
            kwargs["const"] = "{}"
            kwargs["help"] = CLI_SUPPRESS if is_model_suppressed else f"set {arg_names[0]} from JSON string (default: {{}})"
    ...
    def _help_format(self, field_name: str, field_info: FieldInfo, model_default: Any, is_model_suppressed: bool) -> str:
