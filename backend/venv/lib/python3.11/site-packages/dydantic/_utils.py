...
if TYPE_CHECKING:
    ...
    try:
        ...
        logger.warning("EmailStr requires email_validator package to be installed. Will use str instead.")
        EmailStr = str
...
def create_model_from_schema(
    ...
    **Examples:**
    ...
    field_definitions = {
        name: _json_schema_to_pydantic_field(name, prop, json_schema.get("required", []), root_schema or json_schema)
...
def _json_schema_to_pydantic_type(
    ...
    *,
    ...
    if any_of_schemas:
        any_of_types = [_json_schema_to_pydantic_type(schema, root_schema) for schema in any_of_schemas]
        return Union[tuple(any_of_types)]

    all_of_schemas = json_schema.get("allOf")
    if all_of_schemas:
        all_of_types = [_json_schema_to_pydantic_type(schema, root_schema) for schema in all_of_schemas]
    ...
    if type_ == "string":
        ...
        if items_schema:
            item_type = _json_schema_to_pydantic_type(items_schema, root_schema, name_=name_)
        ...
        if properties:
            ...
            nested_model = create_model_from_schema(json_schema_, root_schema=root_schema)
            return nested_model
        else:
            return Dict
    elif type_ == "null":
