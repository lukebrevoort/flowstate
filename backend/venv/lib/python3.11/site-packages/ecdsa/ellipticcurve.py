...
class CurveFp(object):
    ...
    def __eq__(self, other):
        ...
        if isinstance(other, CurveFp):
            p = self.__p
            return self.__p == other.__p and self.__a % p == other.__a % p and self.__b % p == other.__b % p
...
class CurveEdTw(object):
    ...
    def __eq__(self, other):
        ...
        if isinstance(other, CurveEdTw):
            p = self.__p
            return self.__p == other.__p and self.__a % p == other.__a % p and self.__d % p == other.__d % p
        return NotImplemented
...
class AbstractPoint(object):
    ...
    def _from_hybrid(cls, data, raw_encoding_length, validate_encoding):
        ...
        if validate_encoding and (y & 1 and data[:1] != b"\x07" or (not y & 1) and data[:1] != b"\x06"):
    ...
    def from_bytes(cls, curve, data, validate_encoding=True, valid_encodings=None):
        ...
        if not all(i in set(("uncompressed", "compressed", "hybrid", "raw")) for i in valid_encodings):
        ...
        elif key_len == raw_encoding_length + 1 and ("hybrid" in valid_encodings or "uncompressed" in valid_encodings):
...
class PointJacobi(AbstractPoint):
    ...
    def from_bytes(
        ...
        :rtype: PointJacobi
        """
        coord_x, coord_y = super(PointJacobi, cls).from_bytes(curve, data, validate_encoding, valid_encodings)
...
class Point(AbstractPoint):
    ...
    def from_bytes(
        ...
        :rtype: Point
        """
        coord_x, coord_y = super(Point, cls).from_bytes(curve, data, validate_encoding, valid_encodings)
...
class PointEdwards(AbstractPoint):
    ...
    def from_bytes(
        ...
        :rtype: PointEdwards
        """
        coord_x, coord_y = super(PointEdwards, cls).from_bytes(curve, data, validate_encoding, valid_encodings)
        return PointEdwards(curve, coord_x, coord_y, 1, coord_x * coord_y, order, generator)
