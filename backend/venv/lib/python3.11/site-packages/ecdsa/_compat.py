...
if sys.version_info < (3, 0):  # pragma: no branch
    ...
    if sys.version_info < (2, 7) or sys.version_info < (2, 7, 4) or platform.system() == "Java":  # pragma: no branch
    ...
    def int_to_bytes(val, length=None, byteorder="big"):
        ...
        if byteorder == "big":
            return bytearray((val >> i) & 0xFF for i in reversed(range(0, length * 8, 8)))
        if byteorder == "little":
