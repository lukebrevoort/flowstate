...
_p = 2**255 - 19
_a = -1
_d = int(remove_whitespace("370957059346694393431380835087545651895421138798432190163887855330" "85940283555"))
_h = 8

_Gx = int(remove_whitespace("151122213495354007725011514095885315114540126930418572060461132" "83949847762202"))
_Gy = int(remove_whitespace("463168356949264781694283940034751631413079938662562256157830336" "03165251855960"))
...
generator_ed25519 = ellipticcurve.PointEdwards(curve_ed25519, _Gx, _Gy, 1, _Gx * _Gy % _p, _r, generator=True)
...
generator_ed448 = ellipticcurve.PointEdwards(curve_ed448, _Gx, _Gy, 1, _Gx * _Gy % _p, _r, generator=True)
...
class PublicKey(object):
    ...
    def __init__(self, generator, public_key, public_point=None):
        ...
        if len(public_key) != self.baselen:
            raise ValueError("Incorrect size of the public key, expected: {0} bytes".format(self.baselen))
        if public_point:
    ...
    def verify(self, data, signature):
        ...
        if len(signature) != 2 * self.baselen:
            raise ValueError("Invalid signature length, expected: {0} bytes".format(2 * self.baselen))
...
class PrivateKey(object):
    ...
    def __init__(self, generator, private_key):
        ...
        if len(private_key) != self.baselen:
            raise ValueError("Incorrect size of private key, expected: {0} bytes".format(self.baselen))
    ...
    def __eq__(self, other):
        if isinstance(other, PrivateKey):
            return self.curve == other.curve and self.__private_key == other.__private_key
        return NotImplemented
    ...
    def public_key(self):
        ...
        self.__public_key = PublicKey(self.generator, public_point.to_bytes(), public_point)
