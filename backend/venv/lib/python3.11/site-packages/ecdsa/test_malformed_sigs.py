...
algorithms_available = [i for i in algorithms_available if i not in ("mdc2", "md2", "md4", "whirlpool", "ripemd160")]
...
for curve in sorted(curves, key=lambda x: x.baselen):
    for hash_alg in [name for name, size in sorted(hash_and_size, key=lambda x: x[1]) if 0 < size <= curve.baselen]:
...
def st_fuzzed_sig(draw, keys_and_sigs):  # pragma: no cover
    ...
    to_remove = draw(st.lists(st.integers(min_value=0, max_value=len(sig) - 1), unique=True))
    to_remove.sort()
...
def st_random_der_ecdsa_sig_value(draw):  # pragma: no cover
    ...
    r = draw(st.integers(min_value=0, max_value=order << 4) | st.integers(min_value=order >> 2, max_value=order + 1))
    s = draw(st.integers(min_value=0, max_value=order << 4) | st.integers(min_value=order >> 2, max_value=order + 1))
...
keys_and_string_sigs = [
    (
        name,
        verifying_key,
        sigencode_string(*sigdecode_der(sig, verifying_key.curve.order), order=verifying_key.curve.order),
    )
