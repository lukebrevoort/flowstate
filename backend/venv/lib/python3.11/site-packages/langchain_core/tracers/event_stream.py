...
class _AstreamEventsCallbackHandler(AsyncCallbackHandler, _StreamingCallbackHandler):
    ...
    def _get_parent_ids(self, run_id: UUID) -> list[str]:
        ...
        while parent_id := self.parent_map.get(run_id):
            ...
            if str_parent_id in parent_ids:
                msg = f"Parent ID {parent_id} is already in the parent_ids list. " f"This should never happen."
    ...
    async def on_tool_end(self, output: Any, *, run_id: UUID, **kwargs: Any) -> None:
        ...
        if "inputs" not in run_info:
            msg = f"Run ID {run_id} is a tool call and is expected to have " f"inputs associated with it."
            raise AssertionError(msg)
...
async def _astream_events_implementation_v2(
    ...
    **kwargs: Any,
    ...
    if callbacks is None:
        ...
        msg = f"Unexpected type for callbacks: {callbacks}." "Expected None, list or AsyncCallbackManager."
        raise ValueError(msg)
    ...
    try:
        async for event in event_streamer:
            ...
            if event["run_id"] == first_event_run_id and event["event"].endswith("_end") and "input" in event["data"]:
