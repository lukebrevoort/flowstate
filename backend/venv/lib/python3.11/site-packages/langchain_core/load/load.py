...
class Reviver:
    ...
    def __call__(self, value: dict[str, Any]) -> Any:
        ...
        if value.get("lc") == 1 and value.get("type") == "not_implemented" and value.get("id") is not None:
            if self.ignore_unserializable_fields:
                return None
            msg = "Trying to load an object that doesn't implement " f"serialization: {value}"
            raise NotImplementedError(msg)

        if value.get("lc") == 1 and value.get("type") == "constructor" and value.get("id") is not None:
