...
def _validate_deprecation_params(
    ...
    *,
    ...
    if alternative_import and "." not in alternative_import:
        msg = "alternative_import must be a fully qualified module path. Got " f" {alternative_import}"
        raise ValueError(msg)
        ...
        *,
        ...
        if isinstance(obj, type):
            ...
            def finalize(wrapper: Callable[..., Any], new_doc: str) -> T:  # noqa: ARG001
                ...
                obj.__init__ = functools.wraps(obj.__init__)(warn_if_direct_instance)  # type: ignore[misc]
                return cast("T", obj)
        ...
        if _alternative and _alternative.split(".")[-1].lower() == _alternative.split(".")[-1]:
            _alternative = f":meth:`~{_alternative}`"
        elif _alternative:
            _alternative = f":class:`~{_alternative}`"

        if _alternative_import and _alternative_import.split(".")[-1].lower() == _alternative_import.split(".")[-1]:
...
def warn_deprecated(
    ...
    *,
    ...
    if not message:
        message = ""
        package_ = package or name.split(".")[0].replace("_", "-") if "." in name else "LangChain"
