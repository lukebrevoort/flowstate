...
class Graph:
    ...
    def to_json(self, *, with_schemas: bool = False) -> dict[str, list[dict[str, Any]]]:
        ...
        """
        stable_node_ids = {node.id: i if is_uuid(node.id) else node.id for i, node in enumerate(self.nodes.values())}
    ...
    def extend(self, graph: Graph, *, prefix: str = "") -> tuple[Optional[Node], Optional[Node]]:
        ...
        self.edges.extend([edge.copy(source=prefixed(edge.source), target=prefixed(edge.target)) for edge in graph.edges])
    ...
    def reid(self) -> Graph:
        ...
        return Graph(
            nodes={_get_node_id(id_): node.copy(id=_get_node_id(id_)) for id_, node in self.nodes.items()},
            edges=[
                edge.copy(
...
def _first_node(graph: Graph, exclude: Sequence[str] = ()) -> Optional[Node]:
    ...
    found: list[Node] = [node for node in graph.nodes.values() if node.id not in exclude and node.id not in targets]
...
def _last_node(graph: Graph, exclude: Sequence[str] = ()) -> Optional[Node]:
    ...
    found: list[Node] = [node for node in graph.nodes.values() if node.id not in exclude and node.id not in sources]
    return found[0] if len(found) == 1 else None
