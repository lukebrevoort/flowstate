        ...
        if callable(attr):
            ...
            def wrapper(*args: Any, **kwargs: Any) -> Any:
                for key, arg in kwargs.items():
                    if key == "config" and (
                        isinstance(arg, dict) and "configurable" in arg and isinstance(arg["configurable"], dict)
                    ):
...
class RunnableConfigurableFields(DynamicRunnable[Input, Output]):
    ...
    def _prepare(self, config: Optional[RunnableConfig] = None) -> tuple[Runnable[Input, Output], RunnableConfig]:
        ...
        if configurable:
            init_params = {k: v for k, v in self.default.__dict__.items() if k in type(self.default).model_fields}
            return (
...
class RunnableConfigurableAlternatives(DynamicRunnable[Input, Output]):
    ...
    def config_specs(self) -> list[ConfigurableFieldSpec]:
        ...
        return get_unique_config_specs(
            ...
            + (
                [prefix_config_spec(s, f"{self.which.id}=={self.default_key}") for s in self.default.config_specs]
                if self.prefix_keys
        ...
        if self.prefix_keys:
            config = cast(
                "RunnableConfig",
                {
                    **config,
                    "configurable": {
                        _strremoveprefix(k, f"{self.which.id}=={which}/"): v for k, v in config.get("configurable", {}).items()
                    },
                },
            )
