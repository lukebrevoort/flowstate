...
def draw_mermaid(
    ...
    *,
    ...
    def render_node(key: str, node: Node, indent: str = "\t") -> str:
        ...
        label = (
            f"<p>{node_name}</p>"
            if node_name.startswith(tuple(MARKDOWN_SPECIAL_CHARS)) and node_name.endswith(tuple(MARKDOWN_SPECIAL_CHARS))
            else node_name
        )
        if node.metadata:
            label = (
        ...
        node_label = format_dict.get(key, format_dict[default_class_label]).format(_escape_node_label(key), label)
    ...
    def add_subgraph(edges: list[Edge], prefix: str) -> None:
        ...
        for edge in edges:
            ...
            mermaid_graph += f"\t{_escape_node_label(source)}{edge_label}" f"{_escape_node_label(target)};\n"
...
def draw_mermaid_png(
    ...
    if draw_method == MermaidDrawMethod.PYPPETEER:
        ...
        img_bytes = asyncio.run(_render_mermaid_using_pyppeteer(mermaid_syntax, output_file_path, background_color, padding))
        ...
        msg = f"Invalid draw method: {draw_method}. " f"Supported draw methods are: {supported_methods}"
        raise ValueError(msg)
...
async def _render_mermaid_using_pyppeteer(
    ...
    if output_file_path is not None:
        await asyncio.get_event_loop().run_in_executor(None, Path(output_file_path).write_bytes, img_bytes)
...
def _render_mermaid_using_api(
    ...
    *,
    ...
    image_url = f"https://mermaid.ink/img/{mermaid_syntax_encoded}" f"?type={file_type}&bgColor={background_color}"
    ...
    msg = (
        "Failed to reach https://mermaid.ink/ API while trying to render " f"your graph after {max_retries} retries. "
    ) + error_msg_suffix
    raise ValueError(msg)
