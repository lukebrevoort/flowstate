...
class RunnableRetry(RunnableBindingBase[Input, Output]):
        ...
        try:
            for attempt in self._sync_retrying():
                with attempt:
                    ...
                    result = super().batch(
                        pending(inputs),
                        self._patch_config_list(pending(config), pending(run_manager), attempt.retry_state),
    ...
    async def abatch(
        ...
        **kwargs: Any,
        ...
        return await self._abatch_with_config(self._abatch, inputs, config, return_exceptions=return_exceptions, **kwargs)
