...
class RunnableBranch(RunnableSerializable[Input, Output]):
    ...
    def __init__(
        ...
        *branches: Union[
        ...
        default_ = cast("Runnable[Input, Output]", coerce_to_runnable(cast("RunnableLike", default)))
        ...
        for branch in branches[:-1]:
            ...
            if len(branch) != 2:
                msg = f"RunnableBranch branches must be " f"tuples or lists of length 2, not {len(branch)}"
                raise ValueError(msg)
    ...
    def config_specs(self) -> list[ConfigurableFieldSpec]:
        ...
        specs = get_unique_config_specs(
            spec
            for step in ([self.default] + [r for _, r in self.branches] + [r for r, _ in self.branches])
            for spec in step.config_specs
        )
        if any(s.id.startswith(CONTEXT_CONFIG_PREFIX) and s.id.endswith(CONTEXT_CONFIG_SUFFIX_SET) for s in specs):
        ...
        try:
            for idx, branch in enumerate(self.branches):
                ...
                output = self.default.invoke(
                    input,
                    config=patch_config(config, callbacks=run_manager.get_child(tag="branch:default")),
                    **kwargs,
                )
