...
def _set_config_context(
    ...
    if (
        ...
        and (
            tracer := next(
                (handler for handler in getattr(callbacks, "handlers", []) if isinstance(handler, LangChainTracer)),
                None,
            )
        )
...
def ensure_config(config: Optional[RunnableConfig] = None) -> RunnableConfig:
    ...
    for key, value in empty.get("configurable", {}).items():
        if not key.startswith("__") and isinstance(value, (str, int, float, bool)) and key not in empty["metadata"]:
            empty["metadata"][key] = value
    return empty


def get_config_list(config: Optional[Union[RunnableConfig, Sequence[RunnableConfig]]], length: int) -> list[RunnableConfig]:
    ...
    if isinstance(config, Sequence) and len(config) != length:
        msg = f"config must be a list of the same length as inputs, " f"but got {len(config)} configs for {length} inputs"
        raise ValueError(msg)
