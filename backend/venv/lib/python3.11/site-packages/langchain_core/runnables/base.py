...
class Runnable(ABC, Generic[Input, Output]):
    ...
    **RunnableParallel** invokes runnables concurrently, providing the same input
    ...
    def get_input_schema(
        ...
        if inspect.isclass(root_type) and not isinstance(root_type, GenericAlias) and issubclass(root_type, BaseModel):
            return root_type
    ...
    def get_prompts(self, config: Optional[RunnableConfig] = None) -> list[BasePromptTemplate]:
        ...
        return [
            node.data for node in self.get_graph(config=config).nodes.values() if isinstance(node.data, BasePromptTemplate)
        ]
    ...
    def batch_as_completed(
        ...
        **kwargs: Optional[Any],
        ...
        def invoke(i: int, input_: Input, config: RunnableConfig) -> tuple[int, Union[Output, Exception]]:
            if return_exceptions:
                try:
        ...
        with get_executor_for_config(configs[0]) as executor:
            futures = {executor.submit(invoke, i, input_, config) for i, (input_, config) in enumerate(zip(inputs, configs))}
    ...
    async def abatch_as_completed(
        ...
        **kwargs: Optional[Any],
        ...
        async def ainvoke_task(i: int, input_: Input, config: RunnableConfig) -> tuple[int, Union[Output, Exception]]:
            if return_exceptions:
                try:
        ...
        coros = [
            gated_coro(semaphore, ainvoke_task(i, input_, config)) if semaphore else ainvoke_task(i, input_, config)
    ...
    def configurable_fields(self, **kwargs: AnyConfigurableField) -> RunnableSerializable[Input, Output]:
        ...
        for key in kwargs:
            if key not in model_fields:
                msg = f"Configuration key {key} not found in {self}: " f"available keys are {model_fields.keys()}"
    ...
    if isinstance(last, RunnableAssign):
        ...
        if not issubclass(prev_output_schema, RootModel):
            ...
            return create_model_v2(
                ...
                field_definitions={
                    **{k: (v.annotation, v.default) for k, v in prev_output_schema.model_fields.items()},
        ...
        if not issubclass(prev_output_schema, RootModel):
            ...
            if isinstance(last.keys, list):
                return create_model_v2(
                    "RunnableSequenceOutput",
                    field_definitions={
                        k: (v.annotation, v.default) for k, v in prev_output_schema.model_fields.items() if k in last.keys
                    },
                )
...
class RunnableSequence(RunnableSerializable[Input, Output]):
    ...
    **RunnableSequence** is the most important composition operator in LangChain
    ...
    def config_specs(self) -> list[ConfigurableFieldSpec]:
        ...
        # get all specs
        all_specs = [(spec, idx) for idx, step in enumerate(self.steps) for spec in step.config_specs]
            ...
            if spec.id.startswith(CONTEXT_CONFIG_PREFIX):
                all_specs[pos] = (
                    ConfigurableFieldSpec(
                        ...
                        dependencies=[d for d in deps_by_pos[idx] if _key_from_id(d) != _key_from_id(spec.id)]
    ...
    def __repr__(self) -> str:
        return "\n| ".join(repr(s) if i == 0 else indent_lines_after_first(repr(s), "| ") for i, s in enumerate(self.steps))
        ...
        try:
            if return_exceptions:
                ...
                for stepidx, step in enumerate(self.steps):
                    ...
                    inputs = step.batch(
                        [inp for i, inp in zip(remaining_idxs, inputs) if i not in failed_inputs_map],
                        [
                    ...
                    failed_inputs_map.update({i: inp for i, inp in zip(remaining_idxs, inputs) if isinstance(inp, Exception)})
    ...
    async def abatch(
        ...
        **kwargs: Optional[Any],
        ...
        # setup callbacks and context
        configs = [aconfig_with_context(c, self.steps) for c in get_config_list(config, len(inputs))]
        callback_managers = [
...
class RunnableParallel(RunnableSerializable[Input, dict[str, Any]]):
    ...
    def __init__(
        ...
        **kwargs: Union[
        ...
        super().__init__(steps__={key: coerce_to_runnable(r) for key, r in merged.items()})  # type: ignore[call-arg]
    ...
    def get_input_schema(self, config: Optional[RunnableConfig] = None) -> type[BaseModel]:
        ...
        """
        if all(
            s.get_input_schema(config).model_json_schema().get("type", "object") == "object" for s in self.steps__.values()
        ):
    ...
    def __repr__(self) -> str:
        map_for_repr = ",\n  ".join(
            f"{k}: {indent_lines_after_first(repr(v), '  ' + k + ': ')}" for k, v in self.steps__.items()
        )
        return "{\n  " + map_for_repr + "\n}"

    @override
    def invoke(self, input: Input, config: Optional[RunnableConfig] = None, **kwargs: Any) -> dict[str, Any]:
        from langchain_core.callbacks.manager import CallbackManager
        ...
        def _invoke_step(step: Runnable[Input, Any], input_: Input, config: RunnableConfig, key: str) -> Any:
        ...
        try:
            ...
            with get_executor_for_config(config) as executor:
                futures = [executor.submit(_invoke_step, step, input, config, key) for key, step in steps.items()]
    ...
    async def ainvoke(
        ...
        **kwargs: Optional[Any],
        ...
        async def _ainvoke_step(step: Runnable[Input, Any], input_: Input, config: RunnableConfig, key: str) -> Any:
            child_config = patch_config(
                config,
                callbacks=run_manager.get_child(f"map:key:{key}"),
            )
            with set_config_context(child_config) as context:
                return await coro_with_context(step.ainvoke(input_, child_config), context, create_task=True)
    ...
    async def _atransform(
        ...
        # Start the first iteration of each generator
        tasks = {
            asyncio.create_task(get_next_chunk(generator)): (step_name, generator) for step_name, generator in named_generators
        }
        # Yield chunks from each as they become available,
    ...
    def __init__(
        ...
        *,
        ...
        if is_async_generator(transform):
            ...
            msg = "Expected a generator function type for `transform`." f"Instead got an unsupported type: {type(transform)}"
            raise TypeError(msg)
...
class RunnableLambda(Runnable[Input, Output]):
    ...
    def __init__(
        ...
        if is_async_callable(func) or is_async_generator(func):
            ...
            msg = "Expected a callable type for `func`." f"Instead got an unsupported type: {type(func)}"
            raise TypeError(msg)
    ...
    async def _ainvoke(
        ...
        **kwargs: Any,
            ...
            if inspect.isgeneratorfunction(self.func):
                ...
                def func(
                    ...
                    **kwargs: Any,
                    ...
                    return call_func_with_variable_args(self.func, value, config, run_manager.get_sync(), **kwargs)
        ...
        if is_async_generator(afunc):
            ...
            output = await acall_func_with_variable_args(cast("Callable", afunc), value, config, run_manager, **kwargs)
        # If the output is a Runnable, invoke it
        if isinstance(output, Runnable):
            recursion_limit = config["recursion_limit"]
            if recursion_limit <= 0:
    ...
    def _transform(
        ...
        **kwargs: Any,
        ...
        if inspect.isgeneratorfunction(self.func):
            output: Optional[Output] = None
            for chunk in call_func_with_variable_args(self.func, final, config, run_manager, **kwargs):
                yield chunk
                if output is None:
                    output = chunk
                else:
    ...
    async def _atransform(
        ...
        **kwargs: Any,
        ...
        if hasattr(self, "afunc"):
            ...
            if inspect.isgeneratorfunction(self.func):
                msg = "Cannot stream from a generator function asynchronously." "Use .stream() instead."
                raise TypeError(msg)
            ...
            def func(
                ...
                **kwargs: Any,
                ...
                return call_func_with_variable_args(self.func, input_, config, run_manager.get_sync(), **kwargs)
...
class RunnableEach(RunnableEachBase[Input, Output]):
    ...
    def with_alisteners(
        ...
        *,
        ...
        added to the run.
        """
        return RunnableEach(bound=self.bound.with_alisteners(on_start=on_start, on_end=on_end, on_error=on_error))
...
class RunnableBinding(RunnableBindingBase[Input, Output]):
    ...
    def with_types(
        ...
        return self.__class__(
            ...
            custom_output_type=(output_type if output_type is not None else self.custom_output_type),
        )
...
class _RunnableCallableAsyncIterator(Protocol[Input, Output]):
    def __call__(self, _in: AsyncIterator[Input], /, *, config: RunnableConfig) -> AsyncIterator[Output]: ...
...
def coerce_to_runnable(thing: RunnableLike) -> Runnable[Input, Output]:
    ...
    msg = f"Expected a Runnable, callable or dict." f"Instead got an unsupported type: {type(thing)}"
    raise TypeError(msg)
