...
class BaseLLM(BaseLanguageModel[str], ABC):
    ...
    def _convert_input(self, model_input: LanguageModelInput) -> PromptValue:
        ...
        msg = f"Invalid input type {type(model_input)}. " "Must be a PromptValue, str, or list of BaseMessages."
        raise ValueError(msg)
    ...
    def batch(
        ...
        **kwargs: Any,
        ...
        if max_concurrency is None:
            ...
            batches = [inputs[i : i + max_concurrency] for i in range(0, len(inputs), max_concurrency)]
    ...
    async def astream(
        ...
        **kwargs: Any,
        ...
        if type(self)._astream is BaseLLM._astream and type(self)._stream is BaseLLM._stream:  # noqa: SLF001  # noqa: SLF001
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not isinstance(prompts, list):
            msg = "Argument 'prompts' is expected to be of type list[str], received" f" argument of type {type(prompts)}."
        ...
        if (
            ...
            if tags is not None and not (isinstance(tags, list) and len(tags) == len(prompts)):
                ...
                raise ValueError(msg)
            if metadata is not None and not (isinstance(metadata, list) and len(metadata) == len(prompts)):
                msg = "metadata must be a list of the same length as prompts"
                raise ValueError(msg)
            if run_name is not None and not (isinstance(run_name, list) and len(run_name) == len(prompts)):
        ...
        if (self.cache is None and get_llm_cache() is None) or self.cache is False:
            run_managers = [
                ...
                for callback_manager, prompt, run_name, run_id_ in zip(callback_managers, prompts, run_name_list, run_ids_list)
            ]
        ...
        if len(missing_prompts) > 0:
            ...
            run_info = [RunInfo(run_id=run_manager.run_id) for run_manager in run_managers] if run_managers else None
        else:
            llm_output = {}
            run_info = None
    ...
    def _get_run_ids_list(run_id: Optional[Union[uuid.UUID, list[Optional[uuid.UUID]]]], prompts: list) -> list:
        ...
        if isinstance(run_id, list):
            if len(run_id) != len(prompts):
                msg = "Number of manually provided run_id's does not match batch length." f" {len(run_id)} != {len(prompts)}"
                raise ValueError(msg)
            return run_id
        ...
        **kwargs: Any,
        ...
        try:
            ...
            await asyncio.gather(
                *[run_manager.on_llm_error(e, response=LLMResult(generations=[])) for run_manager in run_managers]
            )
            raise
        flattened_outputs = output.flatten()
        await asyncio.gather(
    ...
    async def agenerate(
        ...
        **kwargs: Any,
        ...
        # Create callback managers
        if isinstance(callbacks, list) and (isinstance(callbacks[0], (list, BaseCallbackManager)) or callbacks[0] is None):
            # We've received a list of callbacks args to apply to each input
            if len(callbacks) != len(prompts):
                msg = "callbacks must be the same length as prompts"
                raise ValueError(msg)
