...
class MessagesPlaceholder(BaseMessagePromptTemplate):
    ...
    def format_messages(self, **kwargs: Any) -> list[BaseMessage]:
        ...
        if not isinstance(value, list):
            msg = f"variable {self.variable_name} should be a list of base messages, " f"got {value} of type {type(value)}"
        ...
        if isinstance(template, list):
            ...
            for tmpl in template:
                if isinstance(tmpl, str) or (
                    isinstance(tmpl, dict) and "text" in tmpl and set(tmpl.keys()) <= {"type", "text"}
                ):
                    ...
                    if isinstance(img_template, str):
                        ...
                        for key in ["url", "path", "detail"]:
                            if key in img_template:
                                input_variables.extend(get_template_variables(img_template[key], template_format))
                        img_template_obj = ImagePromptTemplate(
                            input_variables=input_variables,
                            template=img_template,
                            template_format=template_format,
...
class ChatPromptTemplate(BaseChatPromptTemplate):
    ...
    def validate_input_variables(cls, values: dict) -> Any:
        ...
        if "input_variables" in values and values.get("validate_template"):
            if input_vars != set(values["input_variables"]):
                msg = "Got mismatched input_variables. " f"Expected: {input_vars}. " f"Got: {values['input_variables']}"
                raise ValueError(msg)
        else:
    ...
    def from_role_strings(cls, string_messages: list[tuple[str, str]]) -> ChatPromptTemplate:
        ...
        """
        return cls(
            messages=[ChatMessagePromptTemplate.from_template(template, role=role) for role, template in string_messages]
        )
    ...
    if message_type in {"human", "user"}:
        ...
        if isinstance(template, str):
            if template[0] != "{" or template[-1] != "}":
                msg = f"Invalid placeholder template: {template}." " Expected a variable name surrounded by curly braces."
                raise ValueError(msg)
        ...
        msg = f"Unexpected message type: {message_type}. Use one of 'human'," f" 'user', 'ai', 'assistant', or 'system'."
        raise ValueError(msg)
    return message
...
def _convert_to_message_template(
    ...
    if isinstance(message, (BaseMessagePromptTemplate, BaseChatPromptTemplate)):
        ...
        message_ = _create_template_from_message_type("human", message, template_format=template_format)
        ...
        if isinstance(message_type_str, str):
            message_ = _create_template_from_message_type(message_type_str, template, template_format=template_format)
        else:
            message_ = message_type_str(
                prompt=PromptTemplate.from_template(cast("str", template), template_format=template_format)
            )
    else:
