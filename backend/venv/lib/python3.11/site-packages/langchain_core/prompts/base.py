...
class BasePromptTemplate(RunnableSerializable[dict, PromptValue], ABC, Generic[FormatOutputType]):
    ...
    def validate_variable_names(self) -> Self:
        ...
        if "stop" in self.input_variables:
            msg = "Cannot have an input variable named 'stop', as it is used internally," " please rename."
            raise ValueError(create_message(message=msg, error_code=ErrorCode.INVALID_PROMPT_INPUT))
        if "stop" in self.partial_variables:
            msg = "Cannot have an partial variable named 'stop', as it is used " "internally, please rename."
            raise ValueError(create_message(message=msg, error_code=ErrorCode.INVALID_PROMPT_INPUT))
    ...
    def get_input_schema(self, config: Optional[RunnableConfig] = None) -> type[BaseModel]:
        ...
        optional_input_variables = {k: (self.input_types.get(k, str), None) for k in self.optional_variables}
    ...
    def _validate_input(self, inner_input: Any) -> dict:
        if not isinstance(inner_input, dict):
            if len(self.input_variables) == 1:
                ...
                raise TypeError(create_message(message=msg, error_code=ErrorCode.INVALID_PROMPT_INPUT))
    ...
    @override
    def invoke(self, input: dict, config: Optional[RunnableConfig] = None, **kwargs: Any) -> PromptValue:
        """Invoke the prompt.
    ...
    @override
    async def ainvoke(self, input: dict, config: Optional[RunnableConfig] = None, **kwargs: Any) -> PromptValue:
    ...
    def _merge_partial_and_user_variables(self, **kwargs: Any) -> dict[str, Any]:
        # Get partial params:
        partial_kwargs = {k: v if not callable(v) else v() for k, v in self.partial_variables.items()}
