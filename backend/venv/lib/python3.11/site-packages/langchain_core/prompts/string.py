...
def mustache_template_vars(
    ...
    for type_, key in mustache.tokenize(template):
        if type_ == "end":
            section_depth -= 1
        elif type_ in {"variable", "section", "inverted section", "no escape"} and key != "." and section_depth == 0:
...
def _create_model_recursive(name: str, defs: Defs) -> type:
    return create_model(  # type: ignore[call-overload]
        name,
        **{k: (_create_model_recursive(k, v), None) if v else (type(v), None) for k, v in defs.items()},
    )
...
def check_valid_template(template: str, template_format: str, input_variables: list[str]) -> None:
    ...
    try:
        ...
        msg = f"Invalid template format {template_format!r}, should be one of" f" {list(DEFAULT_FORMATTER_MAPPING)}."
        raise ValueError(msg) from exc
    try:
        validator_func(template, input_variables)
    except (KeyError, IndexError) as exc:
        msg = "Invalid prompt schema; check for mismatched or missing input parameters" f" from {input_variables}."
        raise ValueError(msg) from exc
