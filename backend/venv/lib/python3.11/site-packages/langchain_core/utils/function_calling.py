...
def _convert_pydantic_to_openai_function(
    ...
    *,
    ...
    if hasattr(model, "model_json_schema"):
        ...
        raise TypeError(msg)
    return _convert_json_schema_to_openai_function(schema, name=name, description=description, rm_titles=rm_titles)
...
def _convert_any_typed_dicts_to_pydantic(
    ...
    *,
    ...
    if (origin := get_origin(type_)) and (type_args := get_args(type_)):
        ...
        type_args = tuple(_convert_any_typed_dicts_to_pydantic(arg, depth=depth + 1, visited=visited) for arg in type_args)
...
def _format_tool_to_openai_function(tool: BaseTool) -> FunctionDescription:
    ...
    if tool.tool_call_schema and not is_simple_oai_tool:
        if isinstance(tool.tool_call_schema, dict):
            return _convert_json_schema_to_openai_function(tool.tool_call_schema, name=tool.name, description=tool.description)
        if issubclass(tool.tool_call_schema, (BaseModel, BaseModelV1)):
            return _convert_pydantic_to_openai_function(tool.tool_call_schema, name=tool.name, description=tool.description)
        error_msg = (
    ...
    *,
    ...
    if strict is not None:
        ...
        if strict:
            ...
            oai_function["parameters"] = _recursive_set_additional_properties_false(oai_function["parameters"])
    return oai_function
...
def convert_to_json_schema(
    ...
    *,
    ...
    if not isinstance(openai_tool, dict) or "function" not in openai_tool or "name" not in openai_tool["function"]:
    ...
    if docstring:
        ...
        if error_on_invalid_docstring:
            ...
            if filtered_annotations and (
                len(docstring_blocks) < 2 or not any(block.startswith("Args:") for block in docstring_blocks[1:])
            ):
...
def _py_38_safe_origin(origin: type) -> type:
    origin_union_type_map: dict[type, Any] = {types.UnionType: Union} if hasattr(types, "UnionType") else {}
