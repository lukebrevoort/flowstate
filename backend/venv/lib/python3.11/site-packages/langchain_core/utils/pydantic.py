    ...
    with warnings.catch_warnings():
        ...
        def wrapper(cls: type[BaseModel], values: dict[str, Any]) -> dict[str, Any]:
            ...
            for name, field_info in fields.items():
                ...
                if hasattr(cls, "model_config") and cls.model_config.get("populate_by_name") and field_info.alias in values:
                    values[name] = values.pop(field_info.alias)
...
def _create_subset_model_v2(
    ...
    *,
    ...
    selected_annotations = [(name, annotation) for name, annotation in model.__annotations__.items() if name in field_names]
...
def _create_root_model(
    ...
    def schema(
        ...
        schema_ = super(cls, cls).schema(by_alias=by_alias, ref_template=ref_template)  # type: ignore[misc]
        schema_["title"] = name
        return schema_
    ...
    with warnings.catch_warnings():
        try:
            if isinstance(type_, type) and not isinstance(type_, GenericAlias) and issubclass(type_, BaseModelV1):
                warnings.filterwarnings(action="ignore", category=PydanticDeprecationWarning)
        except TypeError:
            pass
    ...
    *,
    ...
    if root:
        if field_definitions:
            msg = "When specifying __root__ no other " f"fields should be provided. Got {field_definitions}"
            raise NotImplementedError(msg)
        ...
        try:
            named_root_model = _create_root_model_cached(model_name, module_name=module_name, **kwargs)
        except TypeError:
