    ...
    for right in others:
        for right_k, right_v in right.items():
            if right_k not in merged or (right_v is not None and merged[right_k] is None):
                ...
                msg = f'additional_kwargs["{right_k}"] already exists in this message,' " but with a different type."
                raise TypeError(msg)
...
def merge_obj(left: Any, right: Any) -> Any:
    ...
    if type(left) is not type(right):
        msg = f"left and right are of different types. Left type:  {type(left)}. Right " f"type: {type(right)}."
        raise TypeError(msg)
    ...
    if left == right:
        return left
    msg = f"Unable to merge {left=} and {right=}. Both must be of type str, dict, or " f"list, or else be two equal objects."
