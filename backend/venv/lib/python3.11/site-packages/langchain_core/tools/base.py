...
def _function_annotations_are_pydantic_v1(signature: inspect.Signature, func: Callable) -> bool:
    ...
    any_v1_annotations = any(
        _is_pydantic_annotation(parameter.annotation, pydantic_version="v1") for parameter in signature.parameters.values()
    )
    any_v2_annotations = any(
        _is_pydantic_annotation(parameter.annotation, pydantic_version="v2") for parameter in signature.parameters.values()
    )
...
class ChildTool(BaseTool):
    ...
    args_schema: Annotated[Optional[ArgsSchema], SkipValidation()] = Field(default=None, description="The tool schema.")
    ...
    handle_validation_error: Optional[Union[bool, str, Callable[[Union[ValidationError, ValidationErrorV1]], str]]] = False
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        if (
            ...
            msg = (
                "args_schema must be a subclass of pydantic BaseModel or " f"a JSON schema dict. Got: {kwargs['args_schema']}."
            )
            raise TypeError(msg)
        ...
        if isinstance(tool_input, str):
            if input_args is not None:
                if isinstance(input_args, dict):
                    msg = "String tool inputs are not allowed when " "using tools with JSON schema args_schema."
                    raise ValueError(msg)
        ...
        if input_args is not None:
            ...
            if issubclass(input_args, BaseModel):
                for k, v in get_all_basemodel_annotations(input_args).items():
                    if _is_injected_arg_type(v, injected_type=InjectedToolCallId) and k not in tool_input:
    ...
    async def arun(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            child_config = patch_config(config, callbacks=run_manager.get_child())
            with set_config_context(child_config) as context:
                func_to_check = self._run if self.__class__._arun is BaseTool._arun else self._arun  # noqa: SLF001
...
def _handle_validation_error(
    ...
    *,
    ...
    if isinstance(flag, bool):
        ...
        msg = f"Got unexpected type of `handle_validation_error`. Expected bool, " f"str or callable. Received: {flag}"
...
def _handle_tool_error(
    ...
    *,
    ...
    if isinstance(flag, bool):
        ...
        msg = f"Got unexpected type of `handle_tool_error`. Expected bool, str " f"or callable. Received: {flag}"
        raise ValueError(msg)  # noqa: TRY004
    return content
...
def _is_message_content_type(obj: Any) -> bool:
    ...
    """
    return isinstance(obj, str) or (isinstance(obj, list) and all(_is_message_content_block(e) for e in obj))
...
def _is_injected_arg_type(type_: type, injected_type: Optional[type[InjectedToolArg]] = None) -> bool:
...
def get_all_basemodel_annotations(cls: Union[TypeBaseModel, Any], *, default_to_bound: bool = True) -> dict[str, type]:
    ...
    if not (isinstance(cls, type) and is_pydantic_v2_subclass(cls)):
        ...
        for parent in orig_bases:
            ...
            for field in getattr(parent_origin, "__annotations__", {}):
                annotations[field] = _replace_type_vars(annotations[field], generic_map, default_to_bound=default_to_bound)

    return {k: _replace_type_vars(v, default_to_bound=default_to_bound) for k, v in annotations.items()}
...
def _replace_type_vars(
    ...
    *,
    ...
    if (origin := get_origin(type_)) and (args := get_args(type_)):
        new_args = tuple(_replace_type_vars(arg, generic_map, default_to_bound=default_to_bound) for arg in args)
