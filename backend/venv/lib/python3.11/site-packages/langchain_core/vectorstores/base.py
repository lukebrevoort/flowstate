...
class VectorStore(ABC):
    ...
    async def asimilarity_search_with_score(self, *args: Any, **kwargs: Any) -> list[tuple[Document, float]]:
    ...
    async def asimilarity_search_with_relevance_scores(
        ...
        **kwargs: Any,
        ...
        docs_and_similarities = await self._asimilarity_search_with_relevance_scores(query, k=k, **kwargs)
        if any(similarity < 0.0 or similarity > 1.0 for _, similarity in docs_and_similarities):
            warnings.warn(
                "Relevance scores must be between" f" 0 and 1, got {docs_and_similarities}",
                stacklevel=2,
...
class VectorStoreRetriever(BaseRetriever):
    ...
    def validate_search_type(cls, values: dict) -> Any:
        ...
        if search_type not in cls.allowed_search_types:
            msg = f"search_type of {search_type} not allowed. Valid values are: " f"{cls.allowed_search_types}"
            raise ValueError(msg)
        if search_type == "similarity_score_threshold":
    ...
    def _get_ls_params(self, **kwargs: Any) -> LangSmithRetrieverParams:
        ...
        elif hasattr(self.vectorstore, "embedding") and isinstance(self.vectorstore.embedding, Embeddings):
    ...
    async def _aget_relevant_documents(
        ...
        **kwargs: Any,
        ...
        if self.search_type == "similarity":
            ...
            docs_and_similarities = await self.vectorstore.asimilarity_search_with_relevance_scores(query, **kwargs_)
            docs = [doc for doc, _ in docs_and_similarities]
        elif self.search_type == "mmr":
