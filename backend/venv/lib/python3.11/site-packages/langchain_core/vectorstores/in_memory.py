...
class InMemoryVectorStore(VectorStore):
    ...
    def add_documents(
        ...
        **kwargs: Any,
        ...
        if ids and len(ids) != len(texts):
            msg = f"ids must be the same length as texts. " f"Got {len(ids)} ids and {len(texts)} texts."
            raise ValueError(msg)

        id_iterator: Iterator[Optional[str]] = iter(ids) if ids else iter(doc.id for doc in documents)
    ...
    @override
    async def aadd_documents(self, documents: list[Document], ids: Optional[list[str]] = None, **kwargs: Any) -> list[str]:
        """Add documents to the store."""
        texts = [doc.page_content for doc in documents]
    ...
    def _similarity_search_with_score_by_vector(
        ...
        if filter is not None:
            docs = [doc for doc in docs if filter(Document(page_content=doc["text"], metadata=doc["metadata"]))]
    ...
    def similarity_search_with_score_by_vector(
        ...
        **_kwargs: Any,
        ...
        return [
            (doc, similarity)
            for doc, similarity, _ in self._similarity_search_with_score_by_vector(embedding=embedding, k=k, filter=filter)
        ]
    ...
    def max_marginal_relevance_search_by_vector(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            msg = "numpy must be installed to use max_marginal_relevance_search " "pip install numpy"
