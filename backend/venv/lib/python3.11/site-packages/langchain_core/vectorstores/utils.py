...
def _cosine_similarity(x: Matrix, y: Matrix) -> np.ndarray:
    ...
    if x.shape[1] != y.shape[1]:
        msg = f"Number of columns in X and Y must be the same. X has shape {x.shape} " f"and Y has shape {y.shape}."
...
def maximal_marginal_relevance(
    ...
    try:
        ...
        msg = "maximal_marginal_relevance requires numpy to be installed. " "Please install numpy with `pip install numpy`."
        raise ImportError(msg) from e
