        ...
        try:
            if partial:
                try:
                    ...
                    return {
                        **function_call,
                        "arguments": parse_partial_json(function_call["arguments"], strict=self.strict),
                    }
...
class PydanticOutputFunctionsParser(OutputFunctionsParser):
    ...
    def validate_schema(cls, values: dict) -> Any:
        ...
        if "args_only" not in values:
            values["args_only"] = (
                isinstance(schema, type) and not isinstance(schema, GenericAlias) and issubclass(schema, BaseModel)
            )
