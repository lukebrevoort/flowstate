...
def _hash_string(input_string: str, *, algorithm: Literal["sha1", "sha256", "sha512", "blake2b"]) -> uuid.UUID:
...
def _hash_nested_dict(data: dict[Any, Any], *, algorithm: Literal["sha1", "sha256", "sha512", "blake2b"]) -> uuid.UUID:
...
def _get_source_id_assigner(
    ...
    msg = (
        f"source_id_key should be either None, a string or a callable. " f"Got {source_id_key} of type {type(source_id_key)}."
    )
    ...
    *,
    ...
    if callable(key_encoder):
        ...
        try:
            ...
            msg = f"Failed to hash metadata: {e}. " f"Please use a dict that can be serialized using json."
...
def _delete(
    ...
    if isinstance(vector_store, VectorStore):
        ...
        msg = f"Vectorstore should be either a VectorStore or a DocumentIndex. " f"Got {type(vector_store)}."
        raise TypeError(msg)
...
def index(
    ...
    *,
    ...
    key_encoder: Union[Literal["sha1", "sha256", "sha512", "blake2b"], Callable[[Document], str]] = "sha1",
    ...
    if cleanup not in {"incremental", "full", "scoped_full", None}:
        msg = f"cleanup should be one of 'incremental', 'full', 'scoped_full' or None. " f"Got {cleanup}."
        raise ValueError(msg)
    ...
    if isinstance(destination, VectorStore):
        ...
        msg = f"Vectorstore should be either a VectorStore or a DocumentIndex. " f"Got {type(destination)}."
        raise TypeError(msg)
    ...
    for doc_batch in _batch(batch_size, doc_iterator):
        hashed_docs = list(_deduplicate_in_order([_get_document_with_hash(doc, key_encoder=key_encoder) for doc in doc_batch]))

        source_ids: Sequence[Optional[str]] = [source_id_assigner(hashed_doc) for hashed_doc in hashed_docs]
...
async def aindex(
    ...
    *,
    ...
    async for doc_batch in _abatch(batch_size, async_doc_iterator):
        ...
        exists_batch = await record_manager.aexists(cast("Sequence[str]", [doc.id for doc in hashed_docs]))
