...
class Visitor(ABC):
    ...
    def _validate_func(self, func: Union[Operator, Comparator]) -> None:
        if isinstance(func, Operator) and self.allowed_operators is not None and func not in self.allowed_operators:
            msg = f"Received disallowed operator {func}. Allowed " f"comparators are {self.allowed_operators}"
            raise ValueError(msg)
        if isinstance(func, Comparator) and self.allowed_comparators is not None and func not in self.allowed_comparators:
            msg = f"Received disallowed comparator {func}. Allowed " f"comparators are {self.allowed_comparators}"
            raise ValueError(msg)
...
class Comparison(FilterDirective):
    ...
    def __init__(self, comparator: Comparator, attribute: str, value: Any, **kwargs: Any) -> None:
        ...
        super().__init__(comparator=comparator, attribute=attribute, value=value, **kwargs)  # type: ignore[call-arg]
...
class Operation(FilterDirective):
    ...
    def __init__(self, operator: Operator, arguments: list[FilterDirective], **kwargs: Any) -> None:
        ...
        """
        # super exists from BaseModel
        super().__init__(operator=operator, arguments=arguments, **kwargs)  # type: ignore[call-arg]
...
class StructuredQuery(Expr):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        """
        # super exists from BaseModel
        super().__init__(query=query, filter=filter, limit=limit, **kwargs)  # type: ignore[call-arg]
