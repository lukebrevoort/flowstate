...
async def atrace_as_chain_group(
    ...
    *,
    ...
    cb = _get_trace_callbacks(project_name, example_id, callback_manager=callback_manager)
    cm = AsyncCallbackManager.configure(inheritable_callbacks=cb, inheritable_tags=tags, inheritable_metadata=metadata)
...
class CallbackManagerForChainGroup(CallbackManager):
    ...
    def merge(self: CallbackManagerForChainGroup, other: BaseCallbackManager) -> CallbackManagerForChainGroup:
...
class AsyncCallbackManagerForChainGroup(AsyncCallbackManager):
    ...
    def merge(self: AsyncCallbackManagerForChainGroup, other: BaseCallbackManager) -> AsyncCallbackManagerForChainGroup:
        """Merge the group callback manager with another callback manager.
...
def _configure(
    ...
    *,
    ...
    if inheritable_callbacks or local_callbacks:
        ...
        local_handlers_ = (
            local_callbacks if isinstance(local_callbacks, list) else (local_callbacks.handlers if local_callbacks else [])
        )
    ...
    if verbose or debug or tracing_v2_enabled_:
        ...
        if tracing_v2_enabled_ and not any(isinstance(handler, LangChainTracer) for handler in callback_manager.handlers):
            if tracer_v2:
                ...
                try:
                    handler = LangChainTracer(
                        ...
                        client=(run_tree.client if run_tree is not None else tracing_context["client"]),
                        tags=tracing_tags,
                    )
    ...
    for var, inheritable, handler_class, env_var in _configure_hooks:
        ...
        if var.get() is not None or create_one:
            ...
            if handler_class is None:
                if not any(handler is var_handler for handler in callback_manager.handlers):  # direct pointer comparison
                    callback_manager.add_handler(var_handler, inheritable)
            elif not any(isinstance(handler, handler_class) for handler in callback_manager.handlers):
                callback_manager.add_handler(var_handler, inheritable)
    return callback_manager


async def adispatch_custom_event(name: str, data: Any, *, config: Optional[RunnableConfig] = None) -> None:
