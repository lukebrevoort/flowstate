...
class _Config:
    ...
    conf_features = dict(
        ...
        AVX512_KNL=dict(interest=40, implies="AVX512CD", group="AVX512ER AVX512PF", detect="AVX512_KNL", implies_detect=False),
        AVX512_KNM=dict(
            interest=41,
            implies="AVX512_KNL",
        ...
        AVX512_SPR=dict(interest=46, implies="AVX512_ICL", group="AVX512FP16", detect="AVX512_SPR", implies_detect=False),
        # IBM/Power
        ## Power7/ISA 2.06
        ...
        ## Power9/ISA 3.00
        VSX3=dict(interest=3, implies="VSX2", implies_detect=False, extra_checks="VSX3_HALF_DOUBLE"),
        ## Power10/ISA 3.1
...
class _Parse:
    ...
    def _parse_arg_features(self, arg_name, req_features):
        ...
        for tok in tokens:
            if tok[0] in ("#", "$"):
                self.dist_fatal(
                    arg_name, "target groups and policies " "aren't allowed from arguments, " "only from dispatch-able sources"
                )
            if tok == "+":
                append = True
                continue
    ...
    def _parse_target_tokens(self, tokens):
        ...
        for tok in tokens:
            ...
            if ch in ("+", "-"):
                self.dist_fatal(
                    "+/- are 'not' allowed from target's groups or @targets, " "only from cpu_baseline and cpu_dispatch parms"
                )
            elif ch == "$":
...
class CCompilerOpt(_Config, _Distutils, _Cache, _CCompiler, _Feature, _Parse):
    ...
    def generate_dispatch_header(self, header_path):
        ...
        with open(header_path, "w") as f:
            baseline_calls = " \\\n".join(
                [("\t%sWITH_CPU_EXPAND_(MACRO_TO_CALL(%s, __VA_ARGS__))") % (self.conf_c_prefix, f) for f in baseline_names]
            )
            dispatch_calls = " \\\n".join(
                [("\t%sWITH_CPU_EXPAND_(MACRO_TO_CALL(%s, __VA_ARGS__))") % (self.conf_c_prefix, f) for f in dispatch_names]
            )
            f.write(
                textwrap.dedent(
                    """\
