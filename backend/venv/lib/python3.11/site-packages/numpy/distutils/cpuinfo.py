...
class LinuxCPUInfo(CPUInfoBase):
    ...
    def _is_Core2(self):
        return self.is_64bit() and self.is_Intel() and re.match(r".*?Core\(TM\)2\b", self.info[0]["model name"]) is not None
...
class Win32CPUInfo(CPUInfoBase):
    ...
    def __init__(self):
        ...
        try:
            ...
            prgx = re.compile(r"family\s+(?P<FML>\d+)\s+model\s+(?P<MDL>\d+)" r"\s+stepping\s+(?P<STP>\d+)", re.IGNORECASE)
