...
class _TestConfFeatures(FakeCCompilerOpt):
    ...
    def test_feature(self, log, search_in, feature_name, feature_dict):
        error_msg = ("during validate '{}' within feature '{}', " "march '{}' and compiler '{}'\n>> ").format(
            log, feature_name, self.cc_march, self.cc_name
        )
    ...
    def test_option_types(self, error_msg, option, val):
        for tp, available in (
            ((str, list), ("implies", "headers", "flags", "group", "detect", "extra_checks")),
            ((str,), ("disable",)),
            ((int,), ("interest",)),
            ((bool,), ("implies_detect",)),
            ...
            if not isinstance(val, tp):
                ...
                raise AssertionError(
                    error_msg + "expected '%s' type for option '%s' not '%s'" % (error_tp, option, type(val).__name__)
                )
            break
    ...
    def test_extra_checks(self, error_msg, search_in, feature_name, feature_dict):
        ...
        for f in extra_checks:
            ...
            if not impl_dict or "disable" in impl_dict:
                continue
            raise AssertionError(
                error_msg + "in option 'extra_checks', extra test case '%s' already exists as a feature name" % f
            )
