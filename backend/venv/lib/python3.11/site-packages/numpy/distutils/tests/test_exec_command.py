...
class TestExecCommand:
    ...
    def check_basic(self, *kws):
        ...
        s, o = exec_command.exec_command(
            '"%s" -c "import sys;sys.stderr.write(\'0\');' "sys.stderr.write('1');sys.stderr.write('2')\"" % self.pyexe, **kws
        )
        assert_(s == 0)
        assert_(o == "012")
    ...
    def check_execute_in(self, **kws):
        with tempdir() as tmpdir:
            ...
            assert_(o != "")
            s, o = exec_command.exec_command(
                "\"%s\" -c \"f = open('%s', 'r'); print(f.read()); " 'f.close()"' % (self.pyexe, fn), execute_in=tmpdir, **kws
            )
            assert_(s == 0)
            assert_(o == "Hello")
