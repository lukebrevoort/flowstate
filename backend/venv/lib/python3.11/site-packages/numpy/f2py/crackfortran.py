...
typespattern4implicit = re.compile(
    beforethisafter % ("", fortrantypes + "|static|automatic|undefined", fortrantypes + "|static|automatic|undefined", ".*"),
    re.I,
)
#
...
def analyzebody(block, args, tab=""):
    ...
    maybe_private = {
        key: value for key, value in block["vars"].items() if "attrspec" not in value or "public" not in value["attrspec"]
    }
...
def traverse(obj, visit, parents=[], result=None, *args, **kwargs):
    ...
    if isinstance(obj, list):
        ...
        for index, value in enumerate(obj):
            new_index, new_item = traverse((index, value), visit, parents=parents + [parent], result=result, *args, **kwargs)
            if new_index is not None:
                new_result.append(new_item)
    elif isinstance(obj, dict):
        new_result = dict()
        for key, value in obj.items():
            new_key, new_value = traverse((key, value), visit, parents=parents + [parent], result=result, *args, **kwargs)
