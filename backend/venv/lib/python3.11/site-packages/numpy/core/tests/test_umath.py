        ...
        for dt in ["f", "d", "g"]:
            try:
                ...
                if dt == "g" and IS_MUSL:
                    ...
                    pytest.skip("Overflow has occurred for" " np.log(np.finfo(np.longdouble).max)")
                else:
                    raise exc
...
class TestSpecialFloats:
    ...
    @pytest.mark.skipif(
        sys.platform == "win32" and sys.maxsize < 2**31 + 1, reason="failures on 32-bit Python, see FIXME below"
    )
...
def assert_arctan2_ispinf(x, y):
    assert_(
        (np.isinf(ncu.arctan2(x, y)) and ncu.arctan2(x, y) > 0), "arctan(%s, %s) is %s, not +inf" % (x, y, ncu.arctan2(x, y))
    )
...
def assert_arctan2_isnzero(x, y):
    assert_(
        (ncu.arctan2(x, y) == 0 and np.signbit(ncu.arctan2(x, y))), "arctan(%s, %s) is %s, not -0" % (x, y, ncu.arctan2(x, y))
    )
...
class TestSpecialMethods:
    ...
    def test_ufunc_override_methods(self):
        ...
        assert_equal(
            res[4],
            {"dtype": "dtype0", "out": ("out0",), "keepdims": "keep0", "axis": "axis0", "initial": "init0", "where": "where0"},
        )
    ...
    def test_ufunc_override_with_super(self):
        ...
        class A(np.ndarray):
            def __array_ufunc__(self, ufunc, method, *inputs, out=None, **kwargs):
                ...
                results = tuple(
                    (np.asarray(result).view(A) if output is None else output) for result, output in zip(results, outputs)
                )
...
def test_spacing_gfortran():
    ...
    #       END PROGRAM
    ref = {
        np.float64: [1.69406589450860068e-021, 2.22044604925031308e-016, 1.13686837721616030e-013, 1.81898940354585648e-012],
