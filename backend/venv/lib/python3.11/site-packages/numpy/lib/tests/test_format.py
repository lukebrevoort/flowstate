...
import numpy as np
from numpy.testing import assert_, assert_array_equal, assert_raises, assert_raises_regex, assert_warns, IS_PYPY, IS_WASM
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            np.dtype(
                {"names": ["a", "b"], "formats": [float, np.dtype({"names": ["c"], "formats": [np.dtype(int, metadata={})]})]}
            ),
            False,
        ),
    ],
