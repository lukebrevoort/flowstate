...
M   33  21.99
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            assert_(w[0].category is np.VisibleDeprecationWarning)
        ctrl = np.array(
            [("M", 21, 72.1), ("F", 35, 58.33), ("M", 33, 21.99)], dtype=[("gender", "|S1"), ("age", int), ("weight", float)]
        )
        assert_equal(test, ctrl)
...
M   33  21.99
    ...
    def test_utf8_byte_encoding(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            assert_(w[0].category is np.VisibleDeprecationWarning)
        ctl = np.array(
            [[b"norm1", b"norm2", b"norm3"], [b"test1", b"testNonethe" + utf8, b"test3"], [b"norm1", b"norm2", b"norm3"]]
        )
