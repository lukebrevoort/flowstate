...
class SQLCompiler(Compiled):
    ...
    def _imv_sentinel_value_resolvers(
        ...
        sentinel_value_resolvers = [
            _scol.type._cached_sentinel_value_processor(self.dialect) for _scol in imv.sentinel_columns
        ]
        ...
        if extracted_parameters:
            ...
            if self.cache_key is None:
                raise exc.CompileError(
                    "This compiled object has no original cache key; " "can't pass extracted_parameters to construct_params"
                )
            else:
    ...
    def visit_case(self, clause, **kwargs):
        ...
        for cond, result in clause.whens:
            x += "WHEN " + cond._compiler_dispatch(self, **kwargs) + " THEN " + result._compiler_dispatch(self, **kwargs) + " "
        ...
        if self.dialect.div_is_floordiv:
            return (
                ...
                + self.process(
                    elements.Cast(
                        binary.right,
                        binary.right.type if binary.right.type._type_affinity is sqltypes.Numeric else sqltypes.Numeric(),
                    ),
                    **kw,
                )
            )
    ...
    def _literal_execute_expanding_parameter_literal_binds(self, parameter, values, bind_expression_template=None):
        ...
        if not values:
            ...
            if typ_dialect_impl._is_tuple_type:
                replacement_expression = ("VALUES " if self.dialect.tuple_in_values else "") + self.visit_empty_set_op_expr(
    ...
    def _literal_execute_expanding_parameter(self, name, parameter, values):
        ...
        if not values:
            ...
            replacement_expression = ("VALUES " if dialect.tuple_in_values else "") + ", ".join(
                ...
                % (
                    ", ".join(
                        _render_bindtemplate(to_update[i * len(tuple_element) + j][0]) for j, value in enumerate(tuple_element)
                    )
                )
                for i, tuple_element in enumerate(values)
            )
    ...
    def visit_bindparam(
        ...
        **kwargs,
        ...
        if not literal_binds:
            literal_execute = literal_execute or bindparam.literal_execute or (within_columns_clause and self.ansi_bind_rules)
        ...
        **kwargs: Any,
        ...
        if existing_cte is not None:
            ...
            if cte is existing_cte._restates or cte is existing_cte:
                ...
                if (cte._is_clone_of is not None or existing_cte._is_clone_of is not None) and cte.compare(existing_cte):
                    is_new_cte = False
                else:
        ...
        if is_new_cte:
            ...
            if not cte_pre_alias_name and cte not in self_ctes:
                ...
                if cte.recursive:
                    ...
                    text += "(%s)" % (
                        ", ".join(self.preparer.format_label_name(ident, anon_map=self.anon_map) for ident in recur_cols)
                    )
    ...
    **kw)`` and returns a select object.   this is used for structural changes
    ...
    def visit_select(
        ...
        **kwargs,
        ...
        populate_result_map = need_column_expressions = (
            toplevel or entry.get("need_result_map_for_compound", False) or entry.get("need_result_map_for_nested", False)
        )
    ...
    def _render_cte_clause(
        ...
        if nesting_level and nesting_level > 1:
            ...
            for cte in list(self.ctes.keys()):
                ...
                nesting = cte.nesting or cte_opts.nesting
                is_rendered_level = cte_level == nesting_level or (include_following_stack and cte_level == nesting_level + 1)
    ...
    def _setup_crud_hints(self, stmt, table_text):
        dialect_hints = {
            table: hint_text for (table, dialect), hint_text in stmt._hints.items() if dialect in ("*", self.dialect.name)
        }
    ...
    def _deliver_insertmanyvalues_batches(
        ...
        if use_row_at_a_time:
            for batchnum, param in enumerate(cast("Sequence[_DBAPISingleExecuteParams]", parameters), 1):
        ...
        if not self.positional:
            ...
            if imv.embed_values_counter:
                executemany_values_w_comma = f"({imv.single_values_expr}, _IMV_VALUES_COUNTER), "
            else:
            ...
            if self.positional:
                ...
                if imv.embed_values_counter:
                    expanded_values_string = (
                        "".join(executemany_values_w_comma.replace("_IMV_VALUES_COUNTER", str(i)) for i, _ in enumerate(batch))
                    )[:-2]
                else:
                    expanded_values_string = ((executemany_values_w_comma * len(batch)))[:-2]
    ...
    def visit_insert(self, insert_stmt, visited_bindparam=None, **kw):
        ...
        if compile_state._has_multi_parameters:
            if not self.dialect.supports_multivalues_insert:
                ...
                raise exc.CompileError(
                    "RETURNING cannot be determinstically sorted when " "using an INSERT which includes multi-row values()."
                )
            crud_params_single = crud_params_struct.single_params
        else:
            crud_params_single = crud_params_struct.single_params
        ...
        if insert_stmt.select is not None:
            ...
            if use_insertmanyvalues:
                if (
                    implicit_sentinel
                    and (self.dialect.insertmanyvalues_implicit_sentinel & InsertmanyvaluesSentinelOpts.USE_INSERT_FROM_SELECT)
                    ...
                    render_bind_casts = (
                        self.dialect.insertmanyvalues_implicit_sentinel & InsertmanyvaluesSentinelOpts.RENDER_SELECT_COL_CASTS
                    )
...
class DDLCompiler(Compiled):
    ...
    def visit_create_index(self, create, include_schema=False, include_table_schema=True, **kw):
        ...
        text += "%s ON %s (%s)" % (
            ...
            ", ".join(self.sql_compiler.process(expr, include_table=False, literal_binds=True) for expr in index.expressions),
        )
        return text
