            ...
            if item is not None:
                try:
                    ...
                    raise exc.ArgumentError(
                        "'SchemaItem' object, such as a 'Column' or a " "'Constraint' expected, got %r" % item
                    ) from err
                else:
                    spwd(self, **kw)
...
class Table(DialectKWArgs, HasSchemaAttr, TableClause, inspection.Inspectable["Table"]):
    ...
    _traverse_internals: _TraverseInternalsType = TableClause._traverse_internals + [("schema", InternalTraversal.dp_string)]
    ...
    def append_column(self, column: ColumnClause[Any], replace_existing: bool = False) -> None:
        ...
        try:
            ...
            raise exc.DuplicateColumnError(
                f"{de.args[0]} Specify replace_existing=True to " "Table.append_column() to replace an " "existing column."
            ) from de
    ...
    def to_metadata(
        ...
        if key in metadata.tables:
            util.warn("Table '%s' already exists within the given " "MetaData - not copying." % self.description)
...
class Column(DialectKWArgs, SchemaItem, ColumnClause[_T]):
    ...
    def _set_parent(  # type: ignore[override]
        ...
        **kw: Any,
        ...
        if not self.name:
            raise exc.ArgumentError(
                "Column must be constructed with a non-blank name or " "assign a non-blank .name before adding to a Table."
            )
        ...
        if existing is not None and existing is not table:
            raise exc.ArgumentError("Column object '%s' already assigned to Table '%s'" % (self.key, existing.description))
    ...
    def _get_colspec(
        ...
        if schema not in (None, RETAIN_SCHEMA):
            ...
            if self._table_column.table is None:
                if _is_copy:
                    raise exc.InvalidRequestError(
                        f"Can't copy ForeignKey object which refers to " f"non-table bound Column {self._table_column!r}"
                    )
                else:
    ...
    def _link_to_col_by_colstring(self, parenttable: Table, table: Table, colname: Optional[str]) -> Column[Any]:
        ...
        if _column is None:
            raise exc.NoReferencedColumnError(
                ...
                "table '%s' has no column named '%s'" % (self._colspec, parenttable.name, table.name, key),
                table.name,
                key,
            )
        ...
        if isinstance(self._colspec, str):
            ...
            if self._unresolvable or tablekey not in parenttable.metadata:
                ...
                raise exc.NoReferencedTableError(
                    ...
                    "foreign key to target column '%s'" % (self.parent, tablekey, colname),
                    tablekey,
                )
            elif parenttable.key not in parenttable.metadata:
                if not raiseerr:
                    return None
                raise exc.InvalidRequestError("Table %s is no longer associated with its " "parent MetaData" % parenttable)
            else:
...
class _InsertSentinelColumnDefault(ColumnDefault):
    ...
    def _set_parent(self, parent: SchemaEventTarget, **kw: Any) -> None:
        ...
        if not col._insert_sentinel:
            ...
            raise exc.ArgumentError("The _InsertSentinelColumnDefault may only be applied to a " "Column that is nullable")
...
class ColumnCollectionConstraint(ColumnCollectionMixin, Constraint):
    ...
    @util.deprecated(
        "1.4",
        "The :meth:`_schema.ColumnCollectionConstraint.copy` method " "is deprecated and will be removed in a future release.",
    )
    def copy(
        self,
        *,
...
class ForeignKeyConstraint(ColumnCollectionConstraint):
        ...
        **dialect_kw: Any,
        ...
        if len(set(columns)) != len(refcolumns):
            if len(set(columns)) != len(columns):
                ...
                raise exc.ArgumentError(
                    "ForeignKeyConstraint number " "of constrained columns must match the number of " "referenced columns."
                )
    ...
    def _validate_dest_table(self, table: Table) -> None:
        ...
        if None not in table_keys and len(table_keys) > 1:
            ...
            raise exc.ArgumentError(
                ...
                "multiple remote tables: %s and %s" % (table.fullname, self._col_description, elem0, elem1)
            )
    ...
    @util.deprecated(
        "1.4",
        "The :meth:`_schema.ForeignKeyConstraint.copy` method is deprecated " "and will be removed in a future release.",
    )
    def copy(
        self,
        *,
    ...
    def _autoincrement_column(self) -> Optional[Column[int]]:
        def _validate_autoinc(col: Column[Any], autoinc_true: bool) -> bool:
            if col.type._type_affinity is None or not issubclass(
                ...
                if autoinc_true:
                    raise exc.ArgumentError(
                        "Column type %s on column '%s' is not " "compatible with autoincrement=True" % (col.type, col)
                    )
                else:
                    return False
                ...
                if col.autoincrement is True:
                    ...
                    if autoinc is not None:
                        raise exc.ArgumentError(
                            ...
                            "autoincrement=True, found both %s and %s." % (col.name, autoinc.name)
                        )
                    else:
                        autoinc = col
    ...
    def _set_parent(self, parent: SchemaEventTarget, **kw: Any) -> None:
        ...
        if self.table is not None and table is not self.table:
            raise exc.ArgumentError(
                ...
                "cannot be associated with table '%s'." % (self.name, self.table.description, table.description)
            )
        self.table = table
    ...
    def reflect(
        ...
        **dialect_kwargs: Any,
        ...
        with inspection.inspect(bind)._inspection_context() as insp:
            ...
            if only is None:
                load = [
                    name for name, schname in zip(available, available_w_schema) if extend_existing or schname not in current
                ]
            elif callable(only):
                load = [
                    name
...
class Identity(IdentityOptions, FetchedValue, SchemaItem):
    ...
    def _set_parent(self, parent: SchemaEventTarget, **kw: Any) -> None:
        assert isinstance(parent, Column)
        if not isinstance(parent.server_default, (type(None), Identity)) or not isinstance(parent.server_onupdate, type(None)):
            raise exc.ArgumentError(
