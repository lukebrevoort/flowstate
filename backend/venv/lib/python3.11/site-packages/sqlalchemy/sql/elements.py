...
class ColumnElement(
    ...
    @util.deprecated(
        "1.4",
        "The :attr:`_expression.ColumnElement.anon_label` attribute is now " "private, and the public accessor is deprecated.",
    )
    ...
    def __init__(
        ...
        *clauses: _ColumnExpressionArgument[Any],
        ...
        if self.group_contents:
            self.clauses = [
                coercions.expect(text_converter_role, clause, apply_propagate_attrs=self).self_group(against=self.operator)
...
class Over(ColumnElement[_T]):
    ...
    def _from_objects(self) -> List[FromClause]:
        return list(
            itertools.chain(*[c._from_objects for c in (self.element, self.partition_by, self.order_by) if c is not None])
        )
...
class NamedColumn(KeyedColumnElement[_T]):
    ...
    def _make_proxy(
        ...
        **kw: Any,
        ...
        c = ColumnClause(
            coercions.expect(roles.TruncatedLabelRole, name or self.name) if name_is_truncatable else (name or self.name),
            type_=self.type,
            _selectable=selectable,
            is_literal=False,
        )
...
class ColumnClause(
    ...
    def _compare_name_for_result(self, other):
        if (
            ...
            or (isinstance(other, ColumnClause) and (other.is_literal or other.table is None or other.table._is_textual))
        ):
            return (hasattr(other, "name") and self.name == other.name) or (
...
class CollationClause(ColumnElement[str]):
    ...
    @classmethod
    def _create_collation_expression(cls, expression: _ColumnExpressionArgument[str], collation: str) -> BinaryExpression[str]:
