        ...
        for k in kwargs:
            ...
            try:
                ...
                if "*" not in construct_arg_dictionary and arg_name not in construct_arg_dictionary:
                    raise exc.ArgumentError(
                        "Argument %r is not accepted by " "dialect %r on behalf of %r" % (k, dialect_name, self.__class__)
                    )
                else:
...
class Options(metaclass=_MetaOptions):
    ...
    def __init_subclass__(cls) -> None:
        dict_ = cls.__dict__
        cls._cache_attrs = tuple(sorted(d for d in dict_ if not d.startswith("__") and d not in ("_cache_key_traversal",)))
