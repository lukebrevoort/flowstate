...
class Join(roles.DMLTableRole, FromClause):
    ...
    def _populate_column_collection(self) -> None:
        ...
        self.primary_key.extend(sqlutil.reduce_columns((c for c in columns if c.primary_key), self.onclause))  # type: ignore
        self._columns._populate_separate_keys((col._tq_key_label, col) for col in columns)
        self.foreign_keys.update(itertools.chain(*[col.foreign_keys for col in columns]))  # type: ignore
        ...
        *,
        ...
        if len(constraints) == 0:
            ...
            raise exc.NoForeignKeysError(
                "Can't find any foreign key relationships " "between '%s' and '%s'.%s" % (a.description, b.description, hint)
            )
        ...
        *clauses: _ColumnExpressionOrStrLabelArgument[Any],
        ...
        if not clauses and __first is None:
            ...
            self._order_by_clauses += tuple(
                coercions.expect(roles.OrderByRole, clause, apply_propagate_attrs=self) for clause in (__first,) + clauses
            )
        return self
        ...
        *clauses: _ColumnExpressionOrStrLabelArgument[Any],
        ...
        if not clauses and __first is None:
            ...
            self._group_by_clauses += tuple(
                coercions.expect(roles.GroupByRole, clause, apply_propagate_attrs=self) for clause in (__first,) + clauses
            )
        return self
...
class Select(
    ...
    @util.deprecated(
        "1.4.23",
        "The :attr:`_expression.Select.froms` attribute is moved to " "the :meth:`_expression.Select.get_final_froms` method.",
    )
    ...
    def _copy_internals(self, clone: _CloneCallableType = _clone, **kw: Any) -> None:
        ...
        add_froms = (
            {f for f in new_froms.values() if isinstance(f, Join)}.difference(all_the_froms).difference(existing_from_obj)
        )
