...
class ORMCompileState(AbstractORMCompileState):
    ...
    def orm_pre_session_exec(
        ...
        if (
            ...
            execution_options = execution_options.union({"compiled_cache": None})
...
class DMLReturningColFilter:
    ...
    def __init__(self, target_mapper, immediate_dml_mapper):
        ...
        self.columns = self.columns = util.WeakPopulateDict(self.adapt_check_present)  # type: ignore
                ...
                for opt in memoized_entities._with_options:
                    if opt._is_compile_state:
                        opt.process_compile_state_replaced_entities(
                            self,
                            [ent for ent in self._memoized_entities[memoized_entities] if isinstance(ent, _MapperEntity)],
                        )
    ...
    def _setup_for_generate(self):
        ...
        if query._correlate:
            ...
            self.correlate = tuple(
                util.flatten_iterator(sql_util.surface_selectables(s) if s is not None else None for s in query._correlate)
            )
    ...
    d = [
        {
            ...
            "expr": ent.expr,
            "entity": (
                getattr(insp_ent, "entity", None) if ent.entity_zero is not None and not insp_ent.is_clause_element else None
            ),
        }
