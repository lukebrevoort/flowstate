...
class Query(
    ...
    def _no_statement_condition(self, meth: str) -> None:
        ...
        if self._statement is not None:
            raise sa_exc.InvalidRequestError(
                ("Query.%s() being called on a Query with an existing full " "statement - can't apply criterion.") % meth
            )
    ...
    def cte(
        ...
        """
        return self.enable_eagerloads(False)._get_select_statement_only().cte(name=name, recursive=recursive, nesting=nesting)
    ...
    def with_parent(
        ...
        if property is None:
            ...
            for prop in mapper.iterate_properties:
                if isinstance(prop, relationships.RelationshipProperty) and prop.mapper is entity_zero.mapper:  # type: ignore
    ...
    def _legacy_from_self(self, *entities: _ColumnsClauseArgument[Any]) -> Self:
        ...
        fromclause = self.set_label_style(LABEL_STYLE_TABLENAME_PLUS_COL).correlate(None).subquery()._anonymous_fromclause()
