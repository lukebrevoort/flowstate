...
_ORMColCollectionElement = Union[ColumnClause[Any], _HasClauseElement, roles.DMLColumnRole, "Mapped[Any]"]
...
class RelationshipProperty(_IntrospectsAnnotations, StrategizedProperty[_T], log.Identified):
    ...
    def _get_attr_w_warn_on_none(
        ...
        def _go() -> Any:
            ...
            current_value = mapper._get_state_attr_by_column(
                ...
                dict_,
                column,
                passive=PassiveFlag.PASSIVE_OFF if state.persistent else PassiveFlag.PASSIVE_NO_FETCH ^ PassiveFlag.INIT_OK,
            )
    ...
    def declarative_scan(
        ...
        is_write_only = mapped_container is not None and issubclass(mapped_container, WriteOnlyMapped)
        if is_write_only:
            self.lazy = "write_only"
            self.strategy_key = (("lazy", self.lazy),)

        is_dynamic = mapped_container is not None and issubclass(mapped_container, DynamicMapped)
        if is_dynamic:
            self.lazy = "dynamic"
            self.strategy_key = (("lazy", self.lazy),)
    ...
    def _check_cascade_settings(self, cascade: CascadeOptions) -> None:
        ...
        if self.passive_deletes == "all" and ("delete" in cascade or "delete-orphan" in cascade):
            raise sa_exc.ArgumentError(
                "On %s, can't set passive_deletes='all' in conjunction " "with 'delete' or 'delete-orphan' cascade" % self
            )
...
class JoinCondition:
    ...
    def _annotate_remote_with_overlap(self) -> None:
        ...
        def proc_left_right(
            ...
            if isinstance(left, expression.ColumnClause) and isinstance(right, expression.ColumnClause):
                if self.child_persist_selectable.c.contains_column(right) and self.parent_persist_selectable.c.contains_column(
                    left
                ):
    ...
    def _check_foreign_cols(self, join_condition: ColumnElement[bool], primary: bool) -> None:
        ...
        if self.support_sync and has_foreign and not can_sync:
            ...
            err = "Could not locate any relevant foreign key columns " "for %s join condition '%s' on relationship %s." % (
                primary and "primary" or "secondary",
                join_condition,
                self.prop,
            )
    ...
    def _gather_columns_with_annotation(
        ...
        return {
            cast(ColumnElement[Any], col) for col in visitors.iterate(clause, {}) if annotation_set.issubset(col._annotations)
        }
    ...
    def join_targets(
        ...
        if extra_criteria:
            ...
            def mark_unrelated_columns_as_ok_to_adapt(
                ...
                if parentmapper_for_element is not self.prop.parent and parentmapper_for_element is not self.prop.mapper:
        ...
        if aliased:
            if secondary is not None:
                ...
                primary_aliasizer = ClauseAdapter(secondary, exclude_fn=_ColInAnnotations("local"))
                secondary_aliasizer = ClauseAdapter(dest_selectable, equivalents=self.child_equivalents).chain(
                    primary_aliasizer
                )
                if source_selectable is not None:
                    primary_aliasizer = ClauseAdapter(secondary, exclude_fn=_ColInAnnotations("local")).chain(
                        ClauseAdapter(
                            source_selectable,
...
class _ColInAnnotations:
    ...
    def __call__(self, c: ClauseElement) -> bool:
        return self.name in c._annotations or "ok_to_adapt_in_join_condition" in c._annotations
