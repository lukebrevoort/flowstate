    ...
    def _listen(
        ...
        **kw: Any,
        ...
        if identifier in ("before_configured", "after_configured") and target is not mapperlib.Mapper:
            util.warn(
                "'before_configured' and 'after_configured' ORM events " "only invoke with the Mapper class " "as the target."
            )
...
class SessionEvents(event.Events[Session]):
    ...
    @classmethod
    def _accept_with(cls, target: Any, identifier: str) -> Union[Session, type]:  # type: ignore [return]
