...
class ColumnProperty(
    ...
    def _memoized_attr__renders_in_subqueries(self) -> bool:
        ...
        return ("deferred", True) not in self.strategy_key or (self not in self.parent._readonly_props)  # type: ignore
...
class MappedColumn(
    ...
    def __init__(self, *arg: Any, **kw: Any):
        ...
        self._has_dataclass_arguments = (
            ...
            and any(
                attr_opts[i] is not _NoArg.NO_ARG for i, attr in enumerate(attr_opts._fields) if attr != "dataclasses_default"
            )
        )
    ...
    def declarative_scan(
        ...
        if self._use_existing_column and decl_scan.inherits and decl_scan.single:
            ...
            column = self.column = supercls_mapper.local_table.c.get(colname, column)  # type: ignore # noqa: E501
        ...
        if use_args_from is not None:
            ...
            if use_args_from.column.key is not None or use_args_from.column.name is not None:
                util.warn_deprecated(
                    "Can't use the 'key' or 'name' arguments in " "Annotated with mapped_column(); this will be ignored",
                    "2.0.22",
                )
