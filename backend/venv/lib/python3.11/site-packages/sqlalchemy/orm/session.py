...
class SessionTransaction(_StateChange, TransactionalContext):
    ...
    def _raise_for_prerequisite_state(self, operation_name: str, state: _StateChangeState) -> NoReturn:
        if state is SessionTransactionState.DEACTIVE:
            ...
            raise sa_exc.InvalidRequestError(
                f"This session is in '{state.name.lower()}' state; no " "further SQL can be emitted within this transaction."
            )
...
class Session(_SessionClassMethods, EventTarget):
    ...
    def __init__(
        ...
        *,
        ...
        if join_transaction_mode and join_transaction_mode not in JoinTransactionMode.__args__:  # type: ignore
    ...
    def get_bind(
        ...
        **kw: Any,
        ...
        raise sa_exc.UnboundExecutionError(f"Could not locate a bind configured on " f'{", ".join(context)} or this Session.')
    ...
    def _get_impl(
        ...
        *,
        ...
        if is_composite_class(primary_key_identity) and type(primary_key_identity) in descriptor_props._composite_getters:
