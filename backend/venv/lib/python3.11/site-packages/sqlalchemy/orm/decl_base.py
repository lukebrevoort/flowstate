...
class _ClassScanMapperConfig(_MapperConfig):
    ...
    def _apply_dataclasses_to_any_class(
        ...
        try:
            dataclass_callable(
                klass,
                **{k: v for k, v in dataclass_setup_arguments.items() if v is not _NoArg.NO_ARG and k != "dataclass_callable"},
            )
            ...
            # to change that now (2.0 betas)
                ...
                if isinstance(value, _DCAttributeOptions):
                    if value._has_dataclass_arguments and not look_for_dataclass_things:
                        ...
                        args = {
                            a for a in argnames if getattr(value._attribute_options, f"dataclasses_{a}") is not _NoArg.NO_ARG
                        }
...
class _DeferredMapperConfig(_ClassScanMapperConfig):
    ...
    def classes_for_base(cls, base_cls: Type[Any], sort: bool = True) -> List[_DeferredMapperConfig]:
        classes_for_base = [
            m for m, cls_ in [(m, m.cls) for m in cls._configs.values()] if cls_ is not None and issubclass(cls_, base_cls)
        ]
        ...
        for m_cls in all_m_by_cls:
            tuples.extend(
                (all_m_by_cls[base_cls], all_m_by_cls[m_cls]) for base_cls in m_cls.__bases__ if base_cls in all_m_by_cls
            )
    ...
    if "__mapper__" in cls.__dict__:
        ...
        def _table_or_raise(mc: MappedClassProtocol[Any]) -> Table:
            if isinstance(mc.__table__, Table):
                return mc.__table__
            raise exc.InvalidRequestError(
                f"Cannot add a new attribute to mapped class {mc.__name__!r} " "because it's not mapped against a table."
            )
