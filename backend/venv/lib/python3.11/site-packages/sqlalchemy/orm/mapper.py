...
class Mapper(
                ...
                if hasattr(method, "__sa_reconstructor__"):
                    ...
                    for name in method.__sa_validators__:
                        if name in self.validators:
                            raise sa_exc.InvalidRequestError(
                                "A validation function for mapped " "attribute %r on mapper %s already exists." % (name, self)
                            )
    ...
    def _configure_pks(self) -> None:
        ...
        for fc in set(self.tables).union([self.persist_selectable]):
            if fc.primary_key and pk_cols.issuperset(fc.primary_key):
                ...
                self._pks_by_table[fc] = util.ordered_column_set(fc.primary_key).intersection(  # type: ignore  # noqa: E501
                    pk_cols
                )
        ...
        if setter:
            ...
            def _set_polymorphic_identity(state):
                ...
                if polymorphic_identity is None and state.manager.mapper.polymorphic_abstract:
                    raise sa_exc.InvalidRequestError(
                        f"Can't instantiate class for {state.manager.mapper}; " "mapper is marked polymorphic_abstract=True"
                    )
    ...
    def _insert_cols_as_none(self):
        return {
            table: frozenset(
                col.key
                for col in columns
                if not col.primary_key and not col.server_default and not col.default and not col.type.should_evaluate_none
            )
            for table, columns in self._cols_by_table.items()
        }
    ...
    def _propkey_to_col(self):
        return {
            table: {self._columntoproperty[col].key: col for col in columns} for table, columns in self._cols_by_table.items()
        }
    ...
    def _pk_attr_keys_by_table(self):
        return {
            table: frozenset([self._columntoproperty[col].key for col in pks]) for table, pks in self._pks_by_table.items()
        }
    ...
    def _optimized_get_statement(self, state, attribute_names):
        ...
        tables: Set[FromClause] = set(
            chain(*[sql_util.find_tables(c, check_columns=True) for key in col_attribute_names for c in props[key].columns])
        )
