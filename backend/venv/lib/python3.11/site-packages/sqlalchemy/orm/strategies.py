...
class LazyLoader(AbstractRelationshipLoader, util.MemoizedSlots, log.Identified):
    ...
    def __init__(self, parent: RelationshipProperty[Any], strategy_key: Tuple[Any, ...]):
        ...
        if self.parent_property.order_by:
            self._order_by = [
                sql_util._deep_annotate(elem, {"_orm_adapt": True}) for elem in util.to_list(self.parent_property.order_by)
            ]
        else:
            self._order_by = None
    ...
    def _load_for_state(
        ...
        if (not passive & PassiveFlag.SQL_OK and not use_get) or (not passive & attributes.NON_PERSISTENT_OK and pending):
            return LoaderCallableStatus.PASSIVE_NO_RESULT
    ...
    def _emit_lazyload(
        ...
        if self.uselist:
            ...
            if l:
                if l > 1:
                    util.warn(
                        "Multiple rows returned with " "uselist=False for lazily-loaded attribute '%s' " % self.parent_property
                    )
