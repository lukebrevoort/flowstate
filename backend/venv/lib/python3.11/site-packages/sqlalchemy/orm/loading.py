...
def load_on_pk_identity(
    ...
    *,
    ...
    if primary_key_identity is not None:
        ...
        params = {
            _get_params[primary_key].key: id_val for id_val, primary_key in zip(primary_key_identity, mapper.primary_key)
        }
    else:
        params = None
    ...
    if require_pk_cols and only_load_props:
        ...
        has_changes = {
            key for key in refresh_state_prokeys.difference(only_load_props) if refresh_state.attrs[key].history.has_changes()
        }
        if has_changes:
...
class PostLoad:
    ...
    def invoke(self, context, path):
        ...
        for (
            ...
            states = [
                (state, overwrite) for state, overwrite in self.states.items() if state.manager.mapper.isa(limit_to_mapper)
            ]
            if states:
