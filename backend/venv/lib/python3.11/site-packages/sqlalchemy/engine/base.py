...
class Connection(ConnectionEventsTarget, inspection.Inspectable["Inspector"]):
    ...
    def _schema_translate_map(self) -> Optional[SchemaTranslateMapType]:
        schema_translate_map: Optional[SchemaTranslateMapType] = self._execution_options.get("schema_translate_map", None)
    ...
    def _is_autocommit_isolation(self) -> bool:
        ...
        return bool(
            opt_iso == "AUTOCOMMIT" or (opt_iso is None and self.engine.dialect._on_connect_isolation_level == "AUTOCOMMIT")
        )
    ...
    def _rollback_impl(self) -> None:
        ...
        if self._still_open_and_dbapi_connection_is_valid:
            if self._echo:
                if self._is_autocommit_isolation():
                    self._log_info("ROLLBACK using DBAPI connection.rollback(), " "DBAPI should ignore due to autocommit mode")
                else:
    ...
    def _execute_ddl(
        ...
        execution_options = ddl._execution_options.merge_with(self._execution_options, execution_options)
        ...
        if self._has_events or self.engine._has_events:
            (
                ...
                event_multiparams,
                event_params,
            ) = self._invoke_before_exec_event(ddl, distilled_parameters, execution_options)
        else:
            event_multiparams = event_params = None
...
class Engine(ConnectionEventsTarget, log.Identified, inspection.Inspectable["Inspector"]):
    ...
    def _lru_size_alert(self, cache: util.LRUCache[Any, Any]) -> None:
        if self._should_log_info():
            self.logger.info(
                "Compiled cache size pruning from %d items to %d.  " "Increase cache size to reduce the frequency of pruning.",
                len(cache),
                cache.capacity,
            )
