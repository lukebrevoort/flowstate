    ...
    def _deliver_insertmanyvalues_batches(self, cursor, statement, parameters, generic_setinputsizes, context):
        ...
        if is_returning:
            ...
            if imv.num_sentinel_columns:
                sentinel_value_resolvers = compiled._imv_sentinel_value_resolvers
        else:
            ...
            if is_returning:
                ...
                if imv.num_sentinel_columns and not imv_batch.is_downgraded:
                    ...
                    if composite_sentinel:
                        ...
                        rows_by_sentinel = {tuple(row[-_nsc:]): row for row in rows}
                    else:
                    ...
                    try:
                        if composite_sentinel:
                            if sentinel_value_resolvers:
                                ...
                                ordered_rows = [
                                    rows_by_sentinel[
                                        tuple(parameters[_spk] for _spk in imv.sentinel_param_keys)  # type: ignore
                                    ]
                                    for parameters in imv_batch.batch
                                ]
                        else:
                            _sentinel_param_key = imv.sentinel_param_keys[0]
                            if sentinel_value_resolvers and sentinel_value_resolvers[0]:
                                # single-column sentinel with value resolver
                                _sentinel_value_resolver = sentinel_value_resolvers[0]
                                ordered_rows = [
                                    rows_by_sentinel[
                                        _sentinel_value_resolver(parameters[_sentinel_param_key])  # type: ignore  # noqa: E501
                                    ]
                                    for parameters in imv_batch.batch
                                ]
                            else:
                                ...
                                ordered_rows = [
                                    rows_by_sentinel[parameters[_sentinel_param_key]]  # type: ignore  # noqa: E501
                                    for parameters in imv_batch.batch
                                ]
...
class DefaultExecutionContext(ExecutionContext):
    ...
    def _init_compiled(
        ...
        if compiled.positional:
            ...
            for compiled_params in self.compiled_parameters:
                l_param: List[Any] = [
                    flattened_processors[key](compiled_params[key]) if key in flattened_processors else compiled_params[key]
            ...
            if key in self._expanded_parameters:
                if is_tuple_type(bindparam.type):
                    ...
                    generic_inputsizes.extend(
                        (
                            (escaped_bind_names.get(paramname, paramname) if escaped_bind_names is not None else paramname),
                            dbtypes[idx % num],
    ...
    def _exec_default_clause_element(self, column, default, type_):
        ...
        if compiled.positional:
            parameters = self.dialect.execute_sequence_format(
                [
                    processors[key](compiled_params[key]) if key in processors else compiled_params[key]  # type: ignore
                    for key in compiled.positiontup or ()
                ]
            )
        else:
            parameters = {
                key: processors[key](compiled_params[key]) if key in processors else compiled_params[key]  # type: ignore
                for key in compiled_params
            }
