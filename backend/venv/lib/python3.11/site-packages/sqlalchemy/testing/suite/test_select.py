...
class ExpandingBoundInTest(fixtures.TablesTest):
    ...
    def test_multiple_empty_sets_bindparam(self):
        ...
        table = self.tables.some_table
        stmt = (
            select(table.c.id).where(table.c.x.in_(bindparam("q"))).where(table.c.y.in_(bindparam("p"))).order_by(table.c.id)
        )
    ...
    def test_bound_in_heterogeneous_two_tuple_direct(self):
        table = self.tables.some_table
        stmt = (
            select(table.c.id).where(tuple_(table.c.x, table.c.z).in_([(2, "z2"), (3, "z3"), (4, "z4")])).order_by(table.c.id)
        )
        self._assert_result(
            stmt,
            [(2,), (3,), (4,)],
