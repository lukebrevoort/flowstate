...
class ComponentReflectionTest(ComparesTables, OneConnectionTablesTest):
    ...
    def exp_indexes(
        ...
        if not testing.requires.indexes_with_ascdesc.enabled or not testing.requires.reflect_indexes_with_ascdesc.enabled:
    ...
    def test_multi_get_table_options(self, use_schema):
        ...
        if testing.requires.reflect_table_options.enabled:
            ...
            exp = {
                (schema, table): insp.get_table_options(table, schema=schema) for table in insp.get_table_names(schema=schema)
            }
            eq_(res, exp)
        else:
            with expect_raises(NotImplementedError):
