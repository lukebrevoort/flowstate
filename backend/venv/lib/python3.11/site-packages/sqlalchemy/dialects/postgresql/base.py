...
class PGCompiler(compiler.SQLCompiler):
    ...
    def _on_conflict_target(self, clause, **kw):
        if clause.constraint_target is not None:
            ...
            target_text = "ON CONSTRAINT %s" % self.preparer.truncate_and_render_constraint_name(clause.constraint_target)
        elif clause.inferred_target_elements is not None:
            target_text = "(%s)" % ", ".join(
                (self.preparer.quote(c) if isinstance(c, str) else self.process(c, include_table=False, use_schema=False))
    ...
    def _domain_query(self, schema):
        con_sq = (
            select(
                ...
                sql.func.array_agg(pg_catalog.pg_get_constraintdef(pg_catalog.pg_constraint.c.oid, True)).label("condefs"),
                sql.func.array_agg(
                    # NOTE: cast since some postgresql derivatives may
                    # not support array_agg on the name type
