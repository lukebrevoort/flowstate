    ...
    def post_exec(self):
        ...
        if self._enable_identity_insert:
            ...
            conn._cursor_execute(
                self.cursor,
                self._opt_encode(
                    "SET IDENTITY_INSERT %s OFF" % self.identifier_preparer.format_table(self.compiled.compile_state.dml_table)
                ),
                (),
                self,
            )
...
class MSDialect(default.DefaultDialect):
    ...
    def get_isolation_level(self, dbapi_connection):
        ...
        try:
            cursor.execute(
                ("SELECT name FROM {} WHERE name IN " "('dm_exec_sessions', 'dm_pdw_nodes_exec_sessions')").format(view_name)
            )
            row = cursor.fetchone()
            if not row:
    ...
    def _get_internal_temp_table_name(self, connection, tablename):
        ...
        try:
            return connection.execute(
                sql.text(
                    "select table_schema, table_name " "from tempdb.information_schema.tables " "where table_name like :p1"
                ),
