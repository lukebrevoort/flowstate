...
def _is_content_blocks_content(content: list[dict | str] | str) -> TypeGuard[list[dict]]:
    return isinstance(content, list) and len(content) > 0 and isinstance(content[0], dict) and "type" in content[0]
...
def add_inline_agent_name(message: BaseMessage) -> BaseMessage:
    ...
    if _is_content_blocks_content(message.content):
        ...
        formatted_message.content = f"<name>{message.name}</name><content>{formatted_message.content}</content>"
...
def with_agent_name(
    ...
    if agent_name_mode == "inline":
        ...
        raise ValueError(f"Invalid agent name mode: {agent_name_mode}. Needs to be one of: {AgentNameMode.__args__}")
    ...
    chain = process_input_messages | model | RunnableLambda(process_output_message, name="process_output_message")
