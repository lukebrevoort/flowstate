...
def _make_call_agent(
    ...
    if output_mode not in get_args(OutputMode):
        raise ValueError(f"Invalid agent output mode: {output_mode}. Needs to be one of {get_args(OutputMode)}")
    ...
    def _process_output(output: dict) -> dict:
        ...
        if output_mode == "full_history":
            ...
            raise ValueError(f"Invalid agent output mode: {output_mode}. " f"Needs to be one of {OutputMode.__args__}")
...
def _get_handoff_destinations(tools: Sequence[BaseTool | Callable]) -> list[str]:
    ...
    return [
        ...
        for tool in tools
        if isinstance(tool, BaseTool) and tool.metadata is not None and METADATA_KEY_HANDOFF_DESTINATION in tool.metadata
    ]
    ...
    if handoff_destinations:
        ...
        handoff_tools = [
            create_handoff_tool(
                ...
                name=(None if handoff_tool_prefix is None else f"{handoff_tool_prefix}{_normalize_agent_name(agent_name)}"),
    ...
    *,
    ...
    if _should_bind_tools(model, all_tools):
        if _supports_disable_parallel_tool_calls(model):
            model = cast(BaseChatModel, model).bind_tools(all_tools, parallel_tool_calls=parallel_tool_calls)
        else:
