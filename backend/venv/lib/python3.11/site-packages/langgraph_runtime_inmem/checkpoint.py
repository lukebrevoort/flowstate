...
DISABLE_FILE_PERSISTENCE = os.getenv("LANGGRAPH_DISABLE_FILE_PERSISTENCE", "false").lower() == "true"
...
class InMemorySaver(MemorySaver):
    ...
    def put(
        ...
        metadata = {
            **{k: v for k, v in config["configurable"].items() if not k.startswith("__") and k not in _EXCLUDED_KEYS},
            **config.get("metadata", {}),
            **metadata,
        }
...
def Checkpointer(*args, unpack_hook=None, **kwargs):
    ...
    if unpack_hook is not None:
        ...
        saver = InMemorySaver(serde=Serializer(__unpack_ext_hook__=unpack_hook), **kwargs)
