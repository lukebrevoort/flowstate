...
class StreamManager:
    ...
    async def put(self, run_id: UUID | str, message: Message, resumable: bool = False) -> None:
        run_id = _ensure_uuid(run_id)
    ...
    async def add_queue(self, run_id: UUID | str) -> asyncio.Queue:
        ...
        for control_msg in self.control_queues[run_id]:
            try:
                ...
                logger.exception(f"Failed to put control message in queue: {control_msg}")
    ...
    def restore_messages(self, run_id: UUID | str, message_id: str | None) -> Iterator[Message]:
