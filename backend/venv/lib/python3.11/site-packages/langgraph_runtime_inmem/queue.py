    ...
    def cleanup(task: asyncio.Task):
        ...
        try:
            ...
            if exc and not isinstance(exc, asyncio.CancelledError):
                logger.exception(f"Background worker failed for task {task}", exc_info=exc)
                return
            result = task.result()
    ...
    try:
        ...
        while True:
            try:
                ...
                do_sweep = last_sweep_secs is None or loop.time() - last_sweep_secs > config.BG_JOB_HEARTBEAT * 2
                # check if we need to update stats
                if calc_stats := (last_stats_secs is None or loop.time() - last_stats_secs > config.STATS_INTERVAL_SECS):
