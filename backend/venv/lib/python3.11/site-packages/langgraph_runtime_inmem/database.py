...
class GlobalStore(PersistentDict):
    ...
    def clear(self):
        ...
        self["runs"] = []
        self["threads"] = []
        self["assistants"] = [a for a in assistants if a["metadata"].get("created_by") == "system"]
...
class InMemoryRetryCounter:
    def __init__(self):
        self._counters: dict[uuid.UUID, int] = PersistentDict(int, filename=RETRY_COUNTER_FILENAME)
