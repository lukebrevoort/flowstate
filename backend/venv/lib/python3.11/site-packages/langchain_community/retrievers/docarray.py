...
class DocArrayRetriever(BaseRetriever):
    ...
    def _get_relevant_documents(
        ...
        *,
        ...
        if self.search_type == SearchType.similarity:
            ...
            raise ValueError(f"Search type {self.search_type} does not exist. " f"Choose either 'similarity' or 'mmr'.")
    ...
    def _search(self, query_emb: np.ndarray, top_k: int) -> List[Union[Dict[str, Any], Any]]:
        ...
        if self.filters:
            ...
            docs = self.index.find(query=query_emb, search_field=search_field, limit=top_k).documents
        return docs
    ...
    def _mmr_search(self, query_emb: np.ndarray) -> List[Document]:
        ...
        mmr_selected = maximal_marginal_relevance(
            query_emb,
            [doc[self.search_field] if isinstance(doc, dict) else getattr(doc, self.search_field) for doc in docs],
            k=self.top_k,
        )
