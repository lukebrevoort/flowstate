...
class RemoteLangChainRetriever(BaseRetriever):
    ...
    def _get_relevant_documents(self, query: str, *, run_manager: CallbackManagerForRetrieverRun) -> List[Document]:
        ...
        return [
            Document(page_content=r[self.page_content_key], metadata=r[self.metadata_key]) for r in result[self.response_key]
        ]
    ...
    async def _aget_relevant_documents(
        ...
        async with aiohttp.ClientSession() as session:
            async with session.request("POST", self.url, headers=self.headers, json={self.input_key: query}) as response:
