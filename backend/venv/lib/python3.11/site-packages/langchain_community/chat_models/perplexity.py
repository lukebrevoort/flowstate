...
class ChatPerplexity(BaseChatModel):
    ...
    pplx_api_key: Optional[str] = Field(default_factory=from_env("PPLX_API_KEY", default=None), alias="api_key")
    ...
    def validate_environment(self) -> Self:
        ...
        try:
            self.client = openai.OpenAI(api_key=self.pplx_api_key, base_url="https://api.perplexity.ai")
    ...
    def _stream(
        ...
        **kwargs: Any,
        ...
        stream_resp = self.client.chat.completions.create(messages=message_dicts, stream=True, **params)
        first_chunk = True
            ...
            if total_usage := chunk.get("usage"):
                ...
                if prev_total_usage:
                    usage_metadata: Optional[UsageMetadata] = {
                        "input_tokens": lc_total_usage["input_tokens"] - prev_total_usage["input_tokens"],
                        "output_tokens": lc_total_usage["output_tokens"] - prev_total_usage["output_tokens"],
                        "total_tokens": lc_total_usage["total_tokens"] - prev_total_usage["total_tokens"],
                    }
                else:
    ...
    def with_structured_output(
        ...
        **kwargs: Any,
        ...
        if method == "json_schema":
            if schema is None:
                raise ValueError("schema must be specified when method is not 'json_schema'. " "Received None.")
            is_pydantic_schema = _is_pydantic_class(schema)
            if is_pydantic_schema and hasattr(schema, "model_json_schema"):  # accounting for pydantic v1 and v2
