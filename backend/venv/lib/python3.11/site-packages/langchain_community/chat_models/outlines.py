...
class ChatOutlines(BaseChatModel):
    ...
    def validate_environment(self) -> "ChatOutlines":
        ...
        if num_constraints > 1:
            raise ValueError(
                "Either none or exactly one of regex, type_constraints, " "json_schema, or grammar can be provided."
            )
    ...
    def build_client(self) -> "ChatOutlines":
        try:
            ...
            raise ImportError("Could not import the Outlines library. " "Please install it with `pip install outlines`.")
    ...
    def with_structured_output(
        ...
        **kwargs: Any,
        ...
        if isinstance(schema, type) and issubclass(schema, BaseModel):
            parser: Union[PydanticOutputParser, JsonOutputParser] = PydanticOutputParser(pydantic_object=schema)
        else:
            ...
            for chunk in self._stream(
                ...
                **kwargs,
                ...
                if isinstance(chunk.message.content, str):
                    ...
                    raise ValueError("Invalid content type, only str is supported, " f"got {type(chunk.message.content)}")
        else:
