...
class ChatKonko(ChatOpenAI):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        values["konko_api_key"] = convert_to_secret_str(get_from_dict_or_env(values, "konko_api_key", "KONKO_API_KEY"))
        try:
            import konko

        except ImportError:
            raise ImportError("Could not import konko python package. " "Please install it with `pip install konko`.")
        try:
            if is_openai_v1():
        ...
        if not hasattr(konko, "_is_legacy_openai"):
            warnings.warn(
                "You are using an older version of the 'konko' package. " "Please consider upgrading to access new features."
            )
    ...
    def get_available_models(
        ...
        if not konko_api_key:
            try:
                ...
                raise ValueError(
                    "Konko API key must be passed as keyword argument or " "set in environment variable KONKO_API_KEY."
                )
