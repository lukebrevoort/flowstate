...
def _convert_delta_to_message_chunk(dct: Dict[str, Any], default_class: Type[BaseMessageChunk]) -> BaseMessageChunk:
    role = dct.get("role")
    content = dct.get("content", "")
    additional_kwargs = {}
...
class MiniMaxChat(BaseChatModel):
    ...
    minimax_api_host: str = Field(default="https://api.minimaxi.chat/v1/text/chatcompletion_v2", alias="base_url")
    ...
    def validate_environment(cls, values: Dict) -> Any:
        ...
        default_values = {name: field.default for name, field in get_fields(cls).items() if field.default is not None}
    ...
    def _reformat_function_parameters(tools_arg: Dict[Any, Any]) -> None:
        ...
        for tool_arg in tools_arg:
            if tool_arg["type"] == "function" and not isinstance(tool_arg["function"]["parameters"], str):
    ...
    def _stream(
        ...
        **kwargs: Any,
        ...
        with httpx.Client(headers=headers, timeout=60) as client:
            with connect_httpx_sse(client, "POST", self.minimax_api_host, json=payload) as event_source:
    ...
    async def _astream(
        ...
        **kwargs: Any,
        ...
        async with httpx.AsyncClient(headers=headers, timeout=60) as client:
            async with aconnect_httpx_sse(client, "POST", self.minimax_api_host, json=payload) as event_source:
