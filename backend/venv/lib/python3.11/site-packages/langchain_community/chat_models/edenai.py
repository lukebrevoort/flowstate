...
class ChatEdenAI(BaseChatModel):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        values["edenai_api_key"] = convert_to_secret_str(get_from_dict_or_env(values, "edenai_api_key", "EDENAI_API_KEY"))
        return values
    ...
    def _stream(
        ...
        **kwargs: Any,
        ...
        if "available_tools" in kwargs:
            yield self._stream_with_tools_as_generate(messages, stop=stop, run_manager=run_manager, **kwargs)
            return
    ...
    async def _astream(
        ...
        **kwargs: Any,
        ...
        if "available_tools" in kwargs:
            yield await self._astream_with_tools_as_agenerate(messages, stop=stop, run_manager=run_manager, **kwargs)
            return
        url = f"{self.edenai_api_url}/text/chat/stream"
        headers = {
            "Authorization": f"Bearer {self._api_key}",
    ...
    def bind_tools(
        ...
        **kwargs: Any,
        ...
        return super().bind(available_tools=formatted_tools, tool_choice=formatted_tool_choice, **kwargs)
        ...
        **kwargs: Any,
        ...
        async with ClientSession() as session:
            async with session.post(url, json=payload, headers=headers) as response:
                ...
                return ChatResult(
                    generations=[ChatGeneration(message=AIMessage(content=provider_response["generated_text"]))],
                    llm_output=data,
                )
