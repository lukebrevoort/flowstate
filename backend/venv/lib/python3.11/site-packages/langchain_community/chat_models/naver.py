...
def _convert_chunk_to_message_chunk(sse: Any, default_class: Type[BaseMessageChunk]) -> BaseMessageChunk:
    sse_data = sse.json()
    if sse.event == "result":
...
def _convert_message_to_naver_chat_message(
    ...
    if isinstance(message, ChatMessage):
        ...
        logger.warning(
            "FunctionMessage, ToolMessage not yet supported " "(https://api.ncloud-docs.com/docs/clovastudio-chatcompletions)"
        )
...
def _raise_on_error(response: httpx.Response) -> None:
    ...
    if httpx.codes.is_error(response.status_code):
        ...
        raise httpx.HTTPStatusError(
            f"Error response {response.status_code} " f"while fetching {response.url}: {error_message}",
...
class ChatClovaX(BaseChatModel):
    ...
    def validate_model_after(self) -> Self:
        ...
        if not self.base_url:
            self.base_url = get_from_env("base_url", "NCP_CLOVASTUDIO_API_BASE_URL", _DEFAULT_BASE_URL)
    ...
    def _init_fields_on_old_api_key(self) -> None:
        if not self.ncp_apigw_api_key:
            self.ncp_apigw_api_key = convert_to_secret_str(get_from_env("ncp_apigw_api_key", "NCP_APIGW_API_KEY", ""))
    ...
    def default_headers(self) -> Dict[str, Any]:
        ...
        clovastudio_api_key = self.ncp_clovastudio_api_key.get_secret_value() if self.ncp_clovastudio_api_key else None
        ...
        if self._is_new_api_key():
            ...
            apigw_api_key = self.ncp_apigw_api_key.get_secret_value() if self.ncp_apigw_api_key else None
            if apigw_api_key:
    ...
    def _stream(
        ...
        **kwargs: Any,
        ...
        for sse in self._completion_with_retry(messages=message_dicts, run_manager=run_manager, **params):
    ...
    async def _astream(
        ...
        **kwargs: Any,
        ...
        default_chunk_class: Type[BaseMessageChunk] = AIMessageChunk
        async for chunk in await self._acompletion_with_retry(messages=message_dicts, run_manager=run_manager, **params):
