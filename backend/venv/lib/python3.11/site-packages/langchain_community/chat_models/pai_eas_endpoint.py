...
class PaiEasChatEndpoint(BaseChatModel):
    ...
    def validate_environment(cls, values: Dict) -> Any:
        ...
        values["eas_service_url"] = get_from_dict_or_env(values, "eas_service_url", "EAS_SERVICE_URL")
        values["eas_service_token"] = get_from_dict_or_env(values, "eas_service_token", "EAS_SERVICE_TOKEN")
    ...
    def format_request_payload(self, messages: List[BaseMessage], **model_kwargs: Any) -> dict:
        ...
        history = [history_item for _, history_item in enumerate(zip(user_content[:-1], assistant_content))]
    ...
    def _call_eas(self, query_body: dict) -> Any:
        ...
        if response.status_code != 200:
            raise Exception(f"Request failed with status code {response.status_code}" f" and message {response.text}")
        ...
        for chunk in response.iter_lines(chunk_size=8192, decode_unicode=False, delimiter=b"\0"):
            if chunk:
                ...
                if text:
                    ...
                    if run_manager:
                        await run_manager.on_llm_new_token(cast(str, content.content), chunk=cg_chunk)
                    yield cg_chunk
