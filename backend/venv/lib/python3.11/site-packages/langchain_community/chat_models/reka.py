        ...
        if isinstance(message, SystemMessage):
            ...
            if system_message:
                if (
                    ...
                    processed_content[0]["text"] = f"{system_message}\n{processed_content[0]['text']}"
                else:
...
class ChatReka(BaseChatModel):
    ...
    def validate_environment(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        reka_api_key = get_from_dict_or_env({"reka_api_key": reka_api_key}, "reka_api_key", "REKA_API_KEY")
        ...
        try:
            ...
            raise ImportError("Could not import Reka Python package. " "Please install it with `pip install reka-api`.")
        return values
    ...
    def get_num_tokens(self, input: Union[str, BaseMessage, List[BaseMessage]]) -> int:
        ...
        try:
            ...
            raise ImportError("Could not import tiktoken python package. " "Please install it with `pip install tiktoken`.")
    ...
    def bind_tools(
        ...
        **kwargs: Any,
        ...
        if tool_choice not in ("auto", "none", "tool"):
            raise ValueError(
                f"Invalid tool_choice '{tool_choice}' provided. " "Tool choice must be one of: 'auto', 'none', or 'tool'."
            )
