...
class ChatWriter(BaseChatModel):
    ...
    def validate_environment(cls, values: Dict) -> Any:
        ...
        try:
            ...
            raise ImportError(
                "Could not import writerai python package. " "Please install it with `pip install writerai`."
            ) from e

        if not values.get("client"):
            values.update({"client": Client(api_key=get_from_dict_or_env(values, "api_key", "WRITER_API_KEY"))})

        if not values.get("async_client"):
            values.update({"async_client": AsyncClient(api_key=get_from_dict_or_env(values, "api_key", "WRITER_API_KEY"))})

        if not (type(values.get("client")) is Client and type(values.get("async_client")) is AsyncClient):
            raise ValueError(
    ...
    def _convert_writer_to_langchain(response_message: Any) -> BaseMessage:
        ...
        if not isinstance(response_message, dict):
            response_message = json.loads(json.dumps(response_message, default=lambda o: o.__dict__))
    ...
    def bind_tools(
        ...
        **kwargs: Any,
        ...
        if tool_choice:
            kwargs["tool_choice"] = (
                (tool_choice) if tool_choice in ("auto", "none") else {"type": "function", "function": {"name": tool_choice}}
            )
