...
class CohereProvider(Provider):
    ...
    def chat_generation_info(self, response: Any) -> Dict[str, Any]:
        ...
        if response.data.chat_response.tool_calls:
            ...
            generation_info["tool_calls"] = _format_oci_tool_calls(response.data.chat_response.tool_calls)
    ...
    def messages_to_oci_params(self, messages: Sequence[ChatMessage], **kwargs: Any) -> Dict[str, Any]:
        ...
        for msg in messages[:-1]:
            if self.get_role(msg) == "USER" or self.get_role(msg) == "SYSTEM":
                ...
                if msg.tool_calls and is_force_single_step:
                    continue
                tool_calls = (
                    [self.oci_tool_call(name=tc["name"], parameters=tc["args"]) for tc in msg.tool_calls]
                    if msg.tool_calls
                    else None
                )
        ...
        for message in current_chat_turn_messages:
            if isinstance(message, ToolMessage):
                ...
                previous_ai_msgs = [
                    message for message in current_chat_turn_messages if isinstance(message, AIMessage) and message.tool_calls
                ]
                if previous_ai_msgs:
    ...
    def with_structured_output(
        ...
        **kwargs: Any,
        ...
        if method == "function_calling":
            ...
            raise ValueError(
                f"Unrecognized method argument. " f"Expected `function_calling` or `json_mode`." f"Received: `{method}`."
            )
        if include_raw:
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        if "tool_calls" in generation_info:
            tool_calls = [
                _convert_oci_tool_call_to_langchain(tool_call) for tool_call in response.data.chat_response.tool_calls
            ]
        else:
            tool_calls = []
