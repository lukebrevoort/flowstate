...
class ChatSambaNovaCloud(BaseChatModel):
    ...
    def _process_response(self, response: Response) -> AIMessage:
        ...
        try:
            ...
            raise RuntimeError(f"Sambanova /complete call failed couldn't get JSON response {e}" f"response: {response.text}")
        ...
        **kwargs: Any,
        ...
        # create request payload for generic v1 API
            ...
            if self.model_kwargs is not None:
                params = {**params, **self.model_kwargs}
            params = {
                key: {"type": type(value).__name__, "value": str(value)} for key, value in params.items() if value is not None
            }
            if streaming:
                data = {
