...
def _response_to_result(
    ...
    for choice in response.choices:
        ...
        if role == "assistant":
            ...
            generations.append(ChatGeneration(text=content, message=AIMessage(content=content, tool_calls=tools)))
        elif role == "user":
            generations.append(ChatGeneration(text=content, message=HumanMessage(content=content)))
        else:
            generations.append(ChatGeneration(text=content, message=ChatMessage(role=role, content=content)))
...
def _messages_to_prompt_dict(
    ...
    for input_msg in input_messages:
        if isinstance(input_msg, ToolMessage):
            ...
            tool_prompt += SINGLE_TOOL_PROMPT_TEMPLATE.format(tool_id=tool_id, tool_response=tool_result)
    if tool_prompt != "":
...
class ChatPremAI(BaseChatModel, BaseModel):
    ...
    def validate_environments(cls, values: Dict) -> Dict:
        ...
        try:
            ...
            raise ImportError(
                "Could not import Prem Python package." "Please install it with: `pip install premai`"
            ) from error

        try:
            premai_api_key: Union[str, SecretStr] = get_from_dict_or_env(values, "premai_api_key", "PREMAI_API_KEY")
            values["client"] = Prem(
                api_key=premai_api_key if isinstance(premai_api_key, str) else premai_api_key._secret_value
            )
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        if "template_id" in kwargs:
            system_prompt, messages_to_pass = _messages_to_prompt_dict(messages, template_id=kwargs["template_id"])
        else:
