...
class CassandraGraphVectorStore(GraphVectorStore):
    ...
    def _restore_links(self, doc: Document) -> Document:
        ...
        for incoming_link_key in [_metadata_link_key(link=link) for link in links if link.direction in ["in", "bidir"]]:
        ...
        **kwargs: Any,
            ...
            if outgoing_links:
                ...
                for outgoing_link in outgoing_links:
                    ...
                    metadata_search_tasks.append(
                        asyncio.create_task(self.vector_store.ametadata_search(filter=metadata_filter, n=1000))
                    )
        ...
        async def visit_targets(d: int, docs: Iterable[Document]) -> None:
            ...
            if new_ids_at_next_depth:
                ...
                visit_node_tasks.extend(visit_nodes(d=d, docs=[new_doc]) for new_doc in new_docs if new_doc is not None)
    ...
    async def _get_outgoing_links(self, source_ids: Iterable[str]) -> set[Link]:
        ...
        coroutines = [self.vector_store.aget_by_document_id(document_id=source_id) for source_id in source_ids]
