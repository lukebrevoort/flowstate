...
class MmrHelper:
    ...
    def _pop_candidate(self, candidate_id: str) -> tuple[float, NDArray[np.float32]]:
        ...
        if self.candidates[index].id != candidate_id:
            msg = "ID in self.candidate_id_to_index doesn't match the ID of the " "corresponding index in self.candidates"
    ...
    def pop_best(self) -> str | None:
        ...
        if self.candidate_embeddings.shape[0] > 0:
            similarity = cosine_similarity(self.candidate_embeddings, np.expand_dims(selected_embedding, axis=0))
