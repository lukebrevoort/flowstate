...
class HuggingFaceTextToSpeechModelInference(BaseTool):
    ...
    def __init__(
        ...
        *,
        ...
        if not huggingface_api_key:
            huggingface_api_key = SecretStr(os.getenv(_HUGGINGFACE_API_KEY_ENV_NAME, ""))
        ...
        if (
            ...
            raise ValueError(f"'{_HUGGINGFACE_API_KEY_ENV_NAME}' must be or set or passed")
        ...
        if file_naming_func == "uuid":
            ...
            raise ValueError(f"Invalid value for 'file_naming_func': {file_naming_func}")
    ...
    def _run(
        ...
        response = requests.post(
            self.api_url,
            headers={"Authorization": f"Bearer {self.huggingface_api_key.get_secret_value()}"},
            json={"inputs": query},
        )
