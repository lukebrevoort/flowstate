    ...
    async def _arun(
        ...
        try:
            if type is AppOperationType.SET_ADMIN:
                ...
                res = await self.interface.db.ref(f"/manage_app/{appName}/create/{getTimestamp()}").setValue(
                    transactionInput=ValueOnlyTransactionInput(value={"admin": {address: True for address in address}})
                )
