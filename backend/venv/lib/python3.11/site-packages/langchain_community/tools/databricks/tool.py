...
def _uc_type_to_pydantic_type(uc_type_json: Union[str, Dict[str, Any]]) -> Type:
    ...
    if isinstance(uc_type_json, str):
        ...
        if tpe == "array":
            ...
            assert key_type == "string", TypeError(f"Only support STRING key type for MAP but got {key_type}.")
            ...
            for field in uc_type_json["fields"]:
                ...
                comment = uc_type_json["metadata"].get("comment") if "metadata" in uc_type_json else None
...
def _get_default_workspace_client() -> Any:
    try:
        ...
        raise ImportError(
            "Could not import databricks-sdk python package. " "Please install it with `pip install databricks-sdk`."
        ) from e
...
class UCFunctionToolkit(BaseToolkit):
    warehouse_id: str = Field(description="The ID of a Databricks SQL Warehouse to execute functions.")
    ...
    def include(self, *function_names: str, **kwargs: Any) -> Self:
        ...
        for name in function_names:
            if name.endswith(".*"):
                ...
                functions = self.workspace_client.functions.list(catalog_name=catalog_name, schema_name=schema_name)
                for f in functions:
