...
def get_execute_function_sql_stmt(function: "FunctionInfo", json_params: Dict[str, Any]) -> ParameterizedStatement:
    from databricks.sdk.service.catalog import ColumnTypeName
        ...
        for p in function.input_params.parameters:
            if p.name not in json_params:
                ...
                if p.type_name in (
                    ...
                    output_params.append(StatementParameterListItem(name=p.name, value=json_value_str))
                    ...
                    arg_clause += f":{p.name}"
                    output_params.append(StatementParameterListItem(name=p.name, value=json_value, type=p.type_text))
...
def execute_function(
    ...
    try:
        ...
        raise ImportError("Could not import pandas python package. " "Please install it with `pip install pandas`.") from e
    ...
    allowed_execute_statement_args = inspect.signature(ws.statement_execution.execute_statement).parameters
    if not any(p.kind in (p.VAR_POSITIONAL, p.VAR_KEYWORD) for p in allowed_execute_statement_args.values()):
        invalid_params = set()
    ...
    if response.status and job_pending(response.status.state) and response.statement_id:
        ...
        while wait_time < client_execution_timeout:
            ...
            _logger.debug(f"Retrying {retry_cnt} time to get statement execution " f"status after {wait} seconds.")
            time.sleep(wait)
    ...
    if response.status.state != StatementState.SUCCEEDED:
        ...
        assert error is not None, f"Statement execution failed but no error message was provided: {response}"
    ...
    if is_scalar(function):
        ...
        assert schema is not None and schema.columns is not None, "Statement execution succeeded but no schema was provided."
        ...
        return FunctionExecutionResult(format="CSV", value=csv_buffer.getvalue(), truncated=truncated)
