...
class ZapierNLARunAction(BaseTool):
    ...
    def set_name_description(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not all(field in values["base_prompt"] for field in necessary_fields):
            raise ValueError(
                "Your custom base Zapier prompt must contain input fields for " "{zapier_description} and {params}."
            )
    ...
    def _run(self, instructions: str, run_manager: Optional[CallbackManagerForToolRun] = None) -> str:
        ...
        warn_deprecated(
            since="0.0.319",
            message=("This tool will be deprecated on 2023-11-17. See " "<https://nla.zapier.com/sunset/> for details"),
        )
...
ZapierNLAListActions.__doc__ = ZapierNLAWrapper.list.__doc__ + ZapierNLAListActions.__doc__  # type: ignore[operator]
