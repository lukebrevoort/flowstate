...
class AzureCosmosDBNoSqlVectorSearch(VectorStore):
    ...
    def __init__(
        ...
        *,
        ...
        if self._create_container:
            if self._indexing_policy["vectorIndexes"] is None or len(self._indexing_policy["vectorIndexes"]) == 0:
                raise ValueError("vectorIndexes cannot be null or empty in the indexing_policy.")
            if self._vector_embedding_policy is None or len(vector_embedding_policy["vectorEmbeddings"]) == 0:
            ...
            if self._full_text_search_enabled:
                if self._indexing_policy["fullTextIndexes"] is None or len(self._indexing_policy["fullTextIndexes"]) == 0:
                    raise ValueError(
                        "fullTextIndexes cannot be null or empty in the " "indexing_policy if full text search is enabled."
                    )
                if self._full_text_policy is None or len(self._full_text_policy["fullTextPaths"]) == 0:
                    raise ValueError(
                        "fullTextPaths cannot be null or empty in the " "full_text_policy if full text search is enabled."
                    )
    ...
    def from_connection_string_and_aad(
        ...
        **kwargs: Any,
        ...
        cosmos_client = CosmosClient(connection_string, defaultAzureCredential, user_agent=USER_AGENT)
    ...
    def similarity_search(
        ...
        **kwargs: Any,
        ...
        if query_type not in CosmosDBQueryType.__members__.values():
            raise ValueError(
                f"Invalid query_type: {query_type}. " f"Expected one of: {', '.join(t.value for t in CosmosDBQueryType)}."
            )
        else:
    ...
    def _generate_projection_fields(
        ...
        if query_type == CosmosDBQueryType.FULL_TEXT_RANK or query_type == CosmosDBQueryType.HYBRID:
            if projection_mapping:
                ...
                projection = f"c.id, c.{self._text_key} as text, " f"c.{self._metadata_key} as metadata"
            ...
            if query_type == CosmosDBQueryType.VECTOR or query_type == CosmosDBQueryType.HYBRID:
                projection += (
                    ", c[@embeddingKey] as embedding, " "VectorDistance(c[@embeddingKey], " "@embeddings) as SimilarityScore"
                )
        return projection
        ...
        for condition in pre_filter.conditions:
            ...
            if "full_text" in condition.operator:
                if not isinstance(condition.value, str):
                    raise ValueError(f"Expected a string for {condition.operator}, " f"got {type(condition.value)}")
