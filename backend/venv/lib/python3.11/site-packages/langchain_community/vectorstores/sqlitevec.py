...
class SQLiteVec(VectorStore):
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError(
                "Could not import sqlite-vec python package. " "Please install it with `pip install sqlite-vec`."
            )
    ...
    def add_texts(
        ...
        **kwargs: Any,
        ...
        """
        max_id = self._connection.execute(f"SELECT max(rowid) as rowid FROM {self._table}").fetchone()["rowid"]
        ...
        data_input = [
            (text, json.dumps(metadata), serialize_f32(embed)) for text, metadata, embed in zip(texts, metadatas, embeds)
        ]
        ...
        results = self._connection.execute(f"SELECT rowid FROM {self._table} WHERE rowid > {max_id}")
