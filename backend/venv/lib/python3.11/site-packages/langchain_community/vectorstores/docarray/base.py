...
def _check_docarray_import() -> None:
    try:
        ...
        raise ImportError("Could not import docarray python package. " "Please install it with `pip install docarray`.")
...
class DocArrayIndex(VectorStore, ABC):
    ...
    def similarity_search_with_score(self, query: str, k: int = 4, **kwargs: Any) -> List[Tuple[Document, float]]:
        ...
        result = [(Document(page_content=doc.text, metadata=doc.metadata), score) for doc, score in zip(docs, scores)]
        return result
    ...
    def max_marginal_relevance_search(
        ...
        **kwargs: Any,
        ...
        results = [Document(page_content=docs[idx].text, metadata=docs[idx].metadata) for idx in mmr_selected]
        return results
