...
class AtlasDB(VectorStore):
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError("Could not import nomic python package. " "Please install it with `pip install nomic`.")
        ...
        **kwargs: Any,
        ...
        if self._embedding_function is not None:
            ...
            if metadatas is None:
                data = [{AtlasDB._ATLAS_DEFAULT_ID_FIELD: ids[i], "text": texts[i]} for i, _ in enumerate(texts)]
            else:
            ...
            self.project._validate_map_data_inputs([], id_field=AtlasDB._ATLAS_DEFAULT_ID_FIELD, data=data)
            with self.project.wait_for_project_lock():
                self.project.add_embeddings(embeddings=embeddings, data=data)
        # Text upload case
        else:
            if metadatas is None:
                data = [{"text": text, AtlasDB._ATLAS_DEFAULT_ID_FIELD: ids[i]} for i, text in enumerate(texts)]
            else:
    ...
    def similarity_search(
        ...
        **kwargs: Any,
        ...
        docs = [Document(page_content=data[i]["text"], metadata=data[i]) for i, neighbor in enumerate(neighbors)]
        return docs
