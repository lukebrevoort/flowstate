...
class VespaStore(VectorStore):
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError("Could not import Vespa python package. " "Please install it with `pip install pyvespa`.")
        if not isinstance(app, Vespa):
            raise ValueError(f"app should be an instance of vespa.application.Vespa, got {type(app)}")
    ...
    def similarity_search_by_vector_with_score(
        ...
        try:
            ...
            raise RuntimeError(
                f"Could not retrieve data from Vespa: " f"{e.args[0][0]['summary']}. " f"Error: {e.args[0][0]['message']}"
            )
