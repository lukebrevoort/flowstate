...
IMPORT_OPENSEARCH_PY_ERROR = "Could not import OpenSearch. Please install it with `pip install opensearch-py`."
...
def _get_opensearch_client(opensearch_url: str, **kwargs: Any) -> OpenSearch:
    ...
    try:
        ...
        raise ImportError(f"OpenSearch client string provided is not in proper format. " f"Got error: {e} ")
    return client
...
def _get_async_opensearch_client(opensearch_url: str, **kwargs: Any) -> AsyncOpenSearch:
    ...
    try:
        ...
        raise ImportError(f"AsyncOpenSearch client string provided is not in proper format. " f"Got error: {e} ")
    return client
...
class OpenSearchVectorSearch(VectorStore):
    ...
    def search_pipeline_exists(self, pipeline_name: str) -> bool:
        ...
        existed_pipelines = self.client.transport.perform_request(method="GET", url="/_search/pipeline/")
    ...
    def get_search_pipeline_info(self, pipeline_name: str) -> Optional[Dict]:
        ...
        response = self.client.transport.perform_request(method="GET", url=f"/_search/pipeline/{pipeline_name}")
    ...
    def similarity_search_by_vector(
        ...
        **kwargs: Any,
        ...
        docs_with_scores = self.similarity_search_with_score_by_vector(embedding, k, score_threshold, **kwargs)
        return [doc[0] for doc in docs_with_scores]
        ...
        **kwargs: Any,
        ...
        if search_type == "approximate_search":
            ...
            if lucene_filter != {} and boolean_filter != {}:
                raise ValueError(
                    "Both `lucene_filter` and `boolean_filter` are provided which " "is invalid. `lucene_filter` is deprecated"
                )
            ...
            if post_filter != {}:
                ...
                payload = _hybrid_search_query_with_post_filter(query_text, embeded_query, k, post_filter)
            else:
