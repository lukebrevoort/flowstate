...
class Lantern(VectorStore):
    ...
    def _get_op_class(self) -> str:
        if self.distance_strategy == DistanceStrategy.COSINE:
            ...
            raise ValueError("No supported operator class" f" for distance_strategy of {self._distance_strategy}.")
    ...
    def _get_operator(self) -> str:
        if self.distance_strategy == DistanceStrategy.COSINE:
            ...
            raise ValueError("No supported operator" f" for distance_strategy of {self._distance_strategy}.")
    ...
    def drop_index(self) -> None:
        with Session(self._conn) as session:
            # Drop the HNSW index
            session.execute(sqlalchemy.text("DROP INDEX IF EXISTS {}".format(quoted_name(self._index_name, True))))
            session.commit()
    ...
    def delete(
        ...
        **kwargs: Any,
        ...
        with Session(self._conn) as session:
            if ids is not None:
                self.logger.debug("Trying to delete vectors by ids (represented by the model " "using the custom ids field)")
    ...
    def __query_collection(
        ...
        with Session(self._conn) as session:
            ...
            query = session.query(
                self.EmbeddingStore,
                getattr(func, self.distance_function)(self.EmbeddingStore.embedding, embedding).label("distance"),
