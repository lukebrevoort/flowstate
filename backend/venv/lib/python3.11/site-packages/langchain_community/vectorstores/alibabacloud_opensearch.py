...
class AlibabaCloudOpenSearch(VectorStore):
        ...
        **kwargs: Any,
        ...
        def _upsert(push_doc_list: List[Dict]) -> List[str]:
            ...
            try:
                ...
                logger.error(
                    f"add doc to endpoint:{self.config.endpoint} " f"instance_id:{self.config.instance_id} failed.",
                    e,
                )
                raise e
    ...
    def similarity_search(
        ...
        **kwargs: Any,
        ...
        return self.create_results(self.inner_embedding_query(embedding=embedding, search_filter=search_filter, k=k))
    ...
    def inner_embedding_query(
        ...
        def create_filter(md_key: str, md_value: Any) -> str:
            ...
            if len(expr) != 2:
                logger.error(f"filter {md_filter_expr} express is not correct, " f"must contain mapping field and operator.")
                return ""
        ...
        try:
            ...
            if "errorCode" in json_response and "errorMsg" in json_response and len(json_response["errorMsg"]) > 0:
                logger.error(f"query {self.config.endpoint} {self.config.instance_id} " f"failed:{json_response['errorMsg']}.")
            else:
                return json_response
        except Exception as e:
            logger.error(
                f"query instance endpoint:{self.config.endpoint} " f"instance_id:{self.config.instance_id} failed.",
                e,
            )
        return {}
    ...
    def delete_documents_with_document_id(self, id_list: List[str]) -> bool:
        ...
        try:
            ...
            logger.error(
                f"delete doc from :{self.config.endpoint} " f"instance_id:{self.config.instance_id} failed.",
                e,
            )
            raise e
