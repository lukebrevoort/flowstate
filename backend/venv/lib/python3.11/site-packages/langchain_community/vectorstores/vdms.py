...
def _len_check_if_sized(x: Any, y: Any, x_name: str, y_name: str) -> None:
    ...
    if isinstance(x, Sized) and isinstance(y, Sized) and len(x) != len(y):
        raise ValueError(
            f"{x_name} and {y_name} expected to be equal length but " f"len({x_name})={len(x)} and len({y_name})={len(y)}"
        )
    return
...
def _results_to_docs_and_scores(results: Any) -> List[Tuple[Document, float]]:
    ...
    try:
        ...
        if len(responses) > 0 and "FindDescriptor" in responses[0] and "entities" in responses[0]["FindDescriptor"]:
            ...
            for ent in result_entities:
                ...
                for p in INVALID_DOC_METADATA_KEYS:
                    if p in ent:
                        del ent[p]
                props = {mkey: mval for mkey, mval in ent.items() if mval not in INVALID_METADATA_VALUE}
...
def VDMS_Client(host: str = "localhost", port: int = 55555) -> vdms.vdms:
    ...
    try:
        ...
        raise ImportError("Could not import vdms python package. " "Please install it with `pip install vdms.")
...
class VDMS(VectorStore):
    ...
    def __get_add_query(
        ...
        if id is None:
            ...
            if id_exists:
                skipped_value = {
                    prop_key: prop_val[-1] for prop_key, prop_val in query["FindDescriptor"]["constraints"].items()
                }
    ...
    def _check_required_inputs(self, collection_name: str, embedding_dimensions: Union[int, None]) -> None:
        # Check connection to client
        if not self._client.is_connected():
            raise ValueError(
                "VDMS client must be connected to a VDMS server." + "Please use VDMS_Client to establish a connection"
            )
        ...
        if self.similarity_search_engine not in AVAILABLE_ENGINES:
            raise ValueError(
                "engine must be either 'TileDBDense', 'TileDBSparse', " + "'FaissFlat', 'FaissIVFFlat', or 'Flinng'"
            )
        ...
        elif self.embedding is not None and (hasattr(self.embedding, "embed_image") or hasattr(self.embedding, "embed_video")):
    ...
    def similarity_search(
        ...
        **kwargs: Any,
        ...
        """
        docs_and_scores = self.similarity_search_with_score(query, k=k, fetch_k=fetch_k, filter=filter, **kwargs)
...
def _add_entity_with_blob(collection_name: str, all_properties: List) -> Tuple[Dict[str, Any], bytes]:
...
def _check_valid_response(all_queries: List[dict], response: Any) -> bool:
    ...
    valid_res = isinstance(response, list) and any(
        cmd in response[0] and "returned" in response[0][cmd] and response[0][cmd]["returned"] > 0 for cmd in cmd_list
    )
    return valid_res
