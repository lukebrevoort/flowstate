...
class PGVector(VectorStore):
    ...
    **DEPRECATED**: This class is pending deprecation and will likely receive
        ...
        *,
        ...
        if not use_jsonb:
            ...
            warn_deprecated(
                ...
                alternative=("Instantiate with use_jsonb=True to use JSONB instead " "of JSON for metadata."),
            )
        self.__post_init__()
    ...
    def __post_init__(
        ...
        EmbeddingStore, CollectionStore = _get_embedding_collection_store(self._embedding_length, use_jsonb=self.use_jsonb)
    ...
    def _create_filter_clause_deprecated(self, key: str, value: dict[str, Any]) -> SQLColumnExpression:
            ...
            if len(filters) == 1:
                ...
                if key.startswith("$"):
                    ...
                    if key.lower() not in ["$and", "$or"]:
                        raise ValueError(f"Invalid filter condition. Expected $and or $or " f"but got: {key}")
                else:
                    # Then it's a field
