...
class DashVector(VectorStore):
    ...
    def __init__(
        ...
        if not isinstance(collection, dashvector.Collection):
            raise ValueError(f"collection should be an instance of dashvector.Collection, " f"bug got {type(collection)}")
    ...
    def add_texts(
        ...
        **kwargs: Any,
        ...
        for i in range(0, len(text_list), batch_size):
            ...
            if not ret:
                raise ValueError(f"Fail to upsert docs to dashvector vector database," f"Error: {ret.message}")
        return ids
    ...
    def similarity_search_with_relevance_scores(
        ...
        **kwargs: Any,
        ...
        return self._similarity_search_with_score_by_vector(embedding, k=k, filter=filter, partition=partition)
    ...
    def similarity_search_by_vector(
        ...
        **kwargs: Any,
        ...
        """
        docs_and_scores = self._similarity_search_with_score_by_vector(embedding, k, filter, partition)
        return [doc for doc, _ in docs_and_scores]
    ...
    def max_marginal_relevance_search(
        ...
        **kwargs: Any,
        ...
        """
        embedding = self._embedding.embed_query(query)
        return self.max_marginal_relevance_search_by_vector(embedding, k, fetch_k, lambda_mult, filter, partition)
    ...
    def max_marginal_relevance_search_by_vector(
        ...
        **kwargs: Any,
        ...
        mmr_selected = maximal_marginal_relevance(np.array(embedding), candidate_embeddings, lambda_mult, k)

        metadatas = [ret.output[i].fields for i in mmr_selected]
        return [Document(page_content=metadata.pop(self._text_field), metadata=metadata) for metadata in metadatas]
    ...
    def from_texts(
        ...
        **kwargs: Any,
        ...
        dashvector_api_key = dashvector_api_key or get_from_env("dashvector_api_key", "DASHVECTOR_API_KEY")
