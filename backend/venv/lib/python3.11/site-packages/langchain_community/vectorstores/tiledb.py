...
class TileDB(VectorStore):
        ...
        **kwargs: Any,
        ...
        with tiledb.scope_ctx(ctx_or_config=config):
            ...
            self.vector_index_uri = (
                vector_index_uri if vector_index_uri != "" else get_vector_index_uri_from_group(index_group)
            )
            self.docs_array_uri = docs_array_uri if docs_array_uri != "" else get_documents_array_uri_from_group(index_group)
            index_group.close()
    ...
    def similarity_search_with_score_by_vector(
        ...
        **kwargs: Any,
        ...
        return self.process_index_results(ids=i[0], scores=d[0], filter=filter, k=k, score_threshold=score_threshold)
    ...
    def similarity_search(
        ...
        **kwargs: Any,
        ...
        """
        docs_and_scores = self.similarity_search_with_score(query, k=k, filter=filter, fetch_k=fetch_k, **kwargs)
        return [doc for doc, _ in docs_and_scores]
    ...
    def __from(
        ...
        **kwargs: Any,
        ...
        if metric not in INDEX_METRICS:
            raise ValueError((f"Unsupported distance metric: {metric}. " f"Expected one of {list(INDEX_METRICS)}"))
        tiledb_vs, tiledb = (
            guard_import("tiledb.vector_search"),
            guard_import("tiledb"),
        )
    ...
    def delete(self, ids: Optional[List[str]] = None, timestamp: int = 0, **kwargs: Any) -> Optional[bool]:
