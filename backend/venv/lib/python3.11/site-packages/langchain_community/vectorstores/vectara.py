...
class VectaraQueryConfig:
    ...
    def __init__(
        ...
        if n_sentence_context:
            ...
            warnings.warn(
                "n_sentence_context is deprecated. " "Please use n_sentence_before and n_sentence_after instead",
                DeprecationWarning,
            )
        else:
...
class Vectara(VectorStore):
    ...
    def __init__(
        ...
        if self._vectara_customer_id is None or self._vectara_corpus_id is None or self._vectara_api_key is None:
            logger.warning("Can't find Vectara credentials, customer_id or corpus_id in " "environment.")
        else:
    ...
    def add_texts(
        ...
        **kwargs: Any,
        ...
        doc = {
            ...
            section_key: [{"text": text, "metadataJson": json.dumps(md)} for text, md in zip(texts, metadatas)],
        }
            ...
            "query": [
                {
                    ...
                    "numResults": (
                        ...
                        if (config.rerank_config.reranker in ["mmr", "udf", "rerank_multilingual_v1"])
                        else config.k
                    ),
                    "contextConfig": {
    ...
    def vectara_query(
        ...
        **kwargs: Any,
        ...
        if response.status_code != 200:
            logger.error(
                "Query failed %s",
                f"(code {response.status_code}, reason {response.reason}, details " f"{response.text})",
            )
            return []
            ...
            if line:  # filter out keep-alive new lines
                ...
                if response_set is None:
                    ...
                    if len(summary.get("status")) > 0:
                        logger.error(
                            f"Summary generation failed with status " f"{summary.get('status')[0].get('statusDetail')}"
                        )
                        continue
                    ...
                    if self.config.score_threshold:
                        responses = [r for r in response_set["response"] if r["score"] > self.config.score_threshold]
                    else:
