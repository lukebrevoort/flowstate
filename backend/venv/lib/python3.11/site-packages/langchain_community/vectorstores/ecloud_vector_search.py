...
class EcloudESVectorStore(VectorStore):
    ...
    def __init__(
        ...
        **kwargs: Optional[dict],
        ...
        if es_url is not None:
            self.client = EcloudESVectorStore.es_client(es_url=es_url, username=user, password=password, **kwargs)
        else:
    ...
    def _search(
        ...
        query_body = self._query_body(query_vector=query_vector, filter=filter, search_params=search_params)
    ...
    def add_texts(
        ...
        **kwargs: Any,
        ...
        if len(requests) > 0:
            try:
                success, failed = bulk(self.client, requests, stats_only=True, refresh=refresh_indices)
