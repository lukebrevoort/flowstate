...
class HanaDB(VectorStore):
    ...
    **DEPRECATED**: This class is deprecated and will no longer be maintained.
    ...
    def __init__(
        ...
        *,
        ...
        if importlib.util.find_spec("hdbcli") is None:
            raise ImportError("Could not import hdbcli python package. " "Please install it with `pip install hdbcli`.")
        ...
        self.specific_metadata_columns = HanaDB._sanitize_specific_metadata_columns(specific_metadata_columns or [])
    ...
    def similarity_search_with_score_by_vector(
        ...
        """
        whole_result = self.similarity_search_with_score_and_vector_by_vector(embedding=embedding, k=k, filter=filter)
            ...
            for i, key in enumerate(filter.keys()):
                ...
                if isinstance(filter_value, bool):
                    ...
                    if special_op in COMPARISONS_TO_SQL:
                        ...
                        elif isinstance(special_val, dict) and "type" in special_val and special_val["type"] == "date":
                            # Date type
                ...
                selector = (
                    f' "{key}"' if key in self.specific_metadata_columns else f"JSON_VALUE({self.metadata_column}, '$.{key}')"
                )
    ...
    def max_marginal_relevance_search_by_vector(  # type: ignore[override]
        ...
        whole_result = self.similarity_search_with_score_and_vector_by_vector(embedding=embedding, k=fetch_k, filter=filter)
        embeddings = [result_item[2] for result_item in whole_result]
        mmr_doc_indexes = maximal_marginal_relevance(np.array(embedding), embeddings, lambda_mult=lambda_mult, k=k)
