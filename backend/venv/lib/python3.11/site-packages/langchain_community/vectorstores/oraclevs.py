...
def _get_connection(client: Any) -> Connection | None:
    ...
    try:
        ...
        raise ImportError("Unable to import oracledb, please install with `pip install -U oracledb`.") from e
...
def _create_hnsw_index(
    ...
    base_sql = f"create vector index {idx_name} on {table_name}(embedding) " f"ORGANIZATION INMEMORY NEIGHBOR GRAPH"
...
def _create_ivf_index(
    ...
    # Base SQL statement
    idx_name = config["idx_name"]
    base_sql = f"CREATE VECTOR INDEX {idx_name} ON {table_name}(embedding) " f"ORGANIZATION NEIGHBOR PARTITIONS"
    ...
    if "idx_type" in config and "neighbor_part" in config:
        parameters_part = f" PARAMETERS (type {config['idx_type']}, neighbor" f" partitions {config['neighbor_part']})"
...
class OracleVS(VectorStore):
    ...
    def add_texts(
        ...
        **kwargs: Any,
        ...
        with connection.cursor() as cursor:
            cursor.executemany(
                f"INSERT INTO {self.table_name} (id, embedding, metadata, " f"text) VALUES (:1, :2, :3, :4)",
                docs,
            )
            connection.commit()
        ...
        with connection.cursor() as cursor:
            ...
            for result in results:
                ...
                if not filter or all(metadata.get(key) in value for key, value in filter.items()):
                    ...
                    # adjust if necessary
                    current_embedding = np.array(result[4], dtype=np.float32) if result[4] else np.empty(0, dtype=np.float32)
    ...
    def max_marginal_relevance_search_with_score_by_vector(
        ...
        *,
        ...
        docs_scores_embeddings = self.similarity_search_by_vector_returning_embeddings(embedding, fetch_k, filter=filter)
