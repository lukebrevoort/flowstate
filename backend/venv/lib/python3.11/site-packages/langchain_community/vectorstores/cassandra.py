...
class Cassandra(VectorStore):
    ...
    def _get_embedding_dimension(self) -> int:
        if self._embedding_dimension is None:
            self._embedding_dimension = len(self.embedding.embed_query("This is a sample sentence."))
        return self._embedding_dimension

    async def _aget_embedding_dimension(self) -> int:
        if self._embedding_dimension is None:
            self._embedding_dimension = len(await self.embedding.aembed_query("This is a sample sentence."))
        return self._embedding_dimension
    ...
    def __init__(
        ...
        *,
        ...
        if cassio_version is not None and cassio_version < MIN_CASSIO_VERSION:
            msg = "Cassio version not supported. Please upgrade cassio " f"to version {MIN_CASSIO_VERSION} or higher."
    ...
    async def aadd_texts(
        ...
        **kwargs: Any,
        ...
        async def send_concurrently(row_id: str, text: str, embedding_vector: List[float], metadata: dict) -> None:
            async with sem:
        ...
        for i in range(0, len(_texts)):
            tasks = [asyncio.create_task(send_concurrently(ids[i], _texts[i], embedding_vectors[i], _metadatas[i]))]
    ...
    def _mmr_search_to_documents(
        ...
        mmr_hits = [pf_hit for pf_index, pf_hit in enumerate(prefetch_hits) if pf_index in mmr_chosen_indices]
