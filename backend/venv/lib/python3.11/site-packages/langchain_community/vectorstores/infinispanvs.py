...
class InfinispanVS(VectorStore):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._to_content = self._configuration.get("lambda_content", lambda item: self._default_content(item))
        self._to_metadata = self._configuration.get("lambda_metadata", lambda item: self._default_metadata(item))
...
"""
    ...
    def configure(self, metadata: dict, dimension: int) -> None:
        ...
        if not self.cache_exists():
            ...
            assert output.status_code == self.ispn.Codes.OK, "Unable to create cache. Already exists? "
            "Consider using clear_old=True"
...
class Infinispan:
        ...
        **kwargs: Any,
        ...
        if self._user and self._password:
            if self._schema == "http":
                auth: Union[Tuple[str, str], httpx.DigestAuth] = httpx.DigestAuth(username=self._user, password=self._password)
            else:
    ...
    def get(self, key: str, cache_name: str) -> Response:
        ...
        """
        api_url = self._default_node + self._cache_url + "/" + cache_name + "/" + key
        response = self._h2c.get(api_url, headers={"Content-Type": "application/json"}, timeout=REST_TIMEOUT)
        return response
