...
class UpstashVectorStore(VectorStore):
    ...
    def __init__(
        ...
        *,
        ...
        try:
            ...
            raise ImportError(
                "Could not import upstash_vector python package. " "Please install it with `pip install upstash_vector`."
            )

        if index:
            if not isinstance(index, Index):
                raise ValueError("Passed index object should be an " "instance of upstash_vector.Index, " f"got {type(index)}")
            self._index = index
            logger.info("Using the index passed as parameter")
        if async_index:
            if not isinstance(async_index, AsyncIndex):
                raise ValueError(
                    "Passed index object should be an " "instance of upstash_vector.AsyncIndex, " f"got {type(async_index)}"
                )
    ...
    def _embed_documents(self, texts: Iterable[str]) -> Union[List[List[float]], List[str]]:
        ...
        if not self._embeddings:
            raise ValueError("No embeddings object provided. " "Pass an embeddings object to the constructor.")
    ...
    async def aadd_texts(
        ...
        **kwargs: Any,
        ...
        for i in range(0, len(texts), embedding_chunk_size):
            ...
            for batch in batch_iterate(batch_size, zip(chunk_ids, embeddings, chunk_metadatas)):
                await self._async_index.upsert(vectors=batch, namespace=cast(str, namespace), **kwargs)
    ...
    def similarity_search(
        ...
        **kwargs: Any,
        ...
        Returns:
            List of Documents most similar to the query and score for each
        """
        docs_and_scores = self.similarity_search_with_score(query, k=k, filter=filter, namespace=namespace, **kwargs)
    ...
    async def asimilarity_search(
        ...
        **kwargs: Any,
        ...
        """
        docs_and_scores = await self.asimilarity_search_with_score(query, k=k, filter=filter, namespace=namespace, **kwargs)
        return [doc for doc, _ in docs_and_scores]
    ...
    async def _asimilarity_search_with_relevance_scores(
        ...
        **kwargs: Any,
        ...
        Since Upstash always returns relevance scores, default implementation is used.
        """
        return await self.asimilarity_search_with_score(query, k=k, filter=filter, namespace=namespace, **kwargs)
    ...
    def max_marginal_relevance_search_by_vector(
        ...
        **kwargs: Any,
        ...
        return [Document(page_content=metadata.pop((self._text_key)), metadata=metadata) for metadata in selected]
