...
class AstraDB(VectorStore):
    ...
    def __init__(
        ...
        *,
        ...
        self.bulk_insert_batch_concurrency: int = bulk_insert_batch_concurrency or DEFAULT_BULK_INSERT_BATCH_CONCURRENCY
        self.bulk_insert_overwrite_concurrency: int = (
            bulk_insert_overwrite_concurrency or DEFAULT_BULK_INSERT_OVERWRITE_CONCURRENCY
        )
        self.bulk_delete_concurrency: int = bulk_delete_concurrency or DEFAULT_BULK_DELETE_CONCURRENCY
    ...
    async def adelete(
        ...
        **kwargs: Any,
        ...
        return all(await gather_with_concurrency(concurrency, *[self.adelete_by_document_id(doc_id) for doc_id in ids]))
    ...
    def add_texts(
        ...
        **kwargs: Any,
        ...
        documents_to_insert = self._get_documents_to_insert(texts, embedding_vectors, metadatas, ids)
    ...
    def _get_mmr_hits(embedding: List[float], k: int, lambda_mult: float, prefetch_hits: List[DocDict]) -> List[Document]:
        ...
        mmr_hits = [
            prefetch_hit for prefetch_index, prefetch_hit in enumerate(prefetch_hits) if prefetch_index in mmr_chosen_indices
        ]
        return [
            Document(
                page_content=hit["content"],
