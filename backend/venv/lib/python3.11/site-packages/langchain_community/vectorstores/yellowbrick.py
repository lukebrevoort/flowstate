...
class Yellowbrick(VectorStore):
    ...
    class DatabaseConnection:
        ...
        def __new__(cls, connection_string: str, logger: logging.Logger) -> "Yellowbrick.DatabaseConnection":
        ...
        def get_managed_connection(self) -> Generator["PgConnection", None, None]:
            ...
            try:
                ...
                conn.rollback()
                self._logger.error("Database error occurred, rolling back transaction.", exc_info=True)
    ...
    def add_texts(
        ...
        **kwargs: Any,
        ...
        with self.connection.get_cursor() as cursor:
            ...
            content_writer = csv.writer(content_io, delimiter="\t", quotechar='"', quoting=csv.QUOTE_MINIMAL)
            embeddings_writer = csv.writer(embeddings_io, delimiter="\t", quotechar='"', quoting=csv.QUOTE_MINIMAL)
    ...
    def _copy_to_db(self, cursor: "PgCursor", content_io: StringIO, embeddings_io: StringIO) -> None:
        content_io.seek(0)
        embeddings_io.seek(0)
    ...
    def delete(
        ...
        **kwargs: Any,
        ...
        with self.connection.get_cursor() as cursor:
            ...
            query = sql.SQL("DELETE FROM {table} {where_sql}").format(table=table_identifier, where_sql=where_sql)
            cursor.execute(query)
            ...
            if self._table_exists(cursor, self._table + self.LSH_INDEX_TABLE, *schema_prefix):
                table_identifier = sql.Identifier(*schema_prefix, self._table + self.LSH_INDEX_TABLE)
                query = sql.SQL("DELETE FROM {table} {where_sql}").format(table=table_identifier, where_sql=where_sql)
                cursor.execute(query)
    ...
    def similarity_search_with_score_by_vector(
        ...
        with self.connection.get_cursor() as cursor:
            ...
            data_input = [
                (str(tmp_doc_id), embedding_id, embedding_value) for embedding_id, embedding_value in enumerate(embedding)
            ]
            insert_query = sql.SQL("INSERT INTO {} (doc_id, embedding_id, embedding) VALUES %s").format(
                sql.Identifier(tmp_embeddings_table)
            )
    ...
    def _update_lsh_hashes(
        ...
        schema_prefix = (self._schema,) if self._schema else ()
        lsh_hyperplane_table = sql.Identifier(*schema_prefix, self._table + self.LSH_HYPERPLANE_TABLE)
        ...
        condition = sql.SQL("WHERE e.doc_id = {doc_id}").format(doc_id=sql.Literal(str(doc_id))) if doc_id else sql.SQL("")
    ...
    def _generate_tmp_lsh_hashes(self, cursor: "PgCursor", tmp_embedding_table: str, tmp_hash_table: str) -> None:
        """Generate temp LSH"""
        from psycopg2 import sql
        ...
        query_prefix = sql.SQL("CREATE TEMPORARY TABLE {} ON COMMIT DROP AS").format(tmp_hash_table_id)
