...
class MyScale(VectorStore):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        index_params = (
            ", " + ",".join([f"'{k}={v}'" for k, v in self.config.index_param.items()]) if self.config.index_param else ""
        )
        schema_ = f"""
        ...
        try:
            ...
            logger.debug(
                f"Clickhouse version={self.client.server_version} - " "There is no allow_experimental_json_type parameter."
            )
