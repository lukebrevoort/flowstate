...
class LanceDB(VectorStore):
        ...
        if isinstance(connection, lancedb.db.LanceDBConnection):
            ...
            if self.api_key is None:
                ...
                if isinstance(uri, str):
                    if uri.startswith("db://"):
                        self._connection = lancedb.connect(uri, api_key=self.api_key, region=self.region)
                    else:
                        self._connection = lancedb.connect(uri)
                        warnings.warn(
    ...
    def add_images(
        ...
        **kwargs: Any,
        ...
        if self._embedding is not None and hasattr(self._embedding, "embed_image"):
            ...
            raise ValueError("embedding object should be provided and must have embed_image method.")
    ...
    def _query(
        ...
        **kwargs: Any,
        ...
        if metrics := kwargs.get("metrics"):
            ...
            lance_query = (
                tbl.search(query=query, vector_column_name=self._vector_key).limit(k).where(filter, prefilter=prefilter)
            )
        ...
        **kwargs: Any,
        ...
        if query_type == "fts" or query_type == "hybrid":
            if self.api_key is None and self._fts_index is None:
                ...
                raise NotImplementedError("Full text/ Hybrid search is not supported in LanceDB Cloud yet.")
        else:
            embedding = self._embedding.embed_query(query)
    ...
    def similarity_search(
        ...
        **kwargs: Any,
        ...
        """
        res = self.similarity_search_with_score(query=query, k=k, name=name, filter=filter, fts=fts, score=False, **kwargs)
        return res
        ...
        **kwargs: Any,
        ...
        if filter:
            ...
            if self.api_key is not None:
                raise NotImplementedError("Column operations currently not supported in LanceDB Cloud.")
            else:
