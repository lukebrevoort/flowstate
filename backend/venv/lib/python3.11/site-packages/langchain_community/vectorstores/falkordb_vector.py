...
class FalkorDBVector(VectorStore):
    ...
    def __init__(
        ...
        *,
        ...
        try:
            ...
            raise ImportError("Could not import falkordb python package." "Please install it with `pip install falkordb`")

        try:
            import redis.exceptions
        except ImportError:
            raise ImportError("Could not import redis.exceptions." "Please install it with `pip install redis`")
        ...
        if graph:
            ...
            try:
                ...
                raise ValueError("Could not connect to FalkorDB database." "Please ensure that the host and port is correct")
            except redis.exceptions.AuthenticationError:
                raise ValueError(
                    "Could not connect to FalkorDB database. " "Please ensure that the username and password are correct"
                )
    ...
    def retrieve_existing_node_index(
        ...
        if index_information:
            ...
            for dict in processed_index_information:
                if dict.get("entity_label", False) == node_label and dict.get("entity_type", False) == "NODE":
    ...
    def retrieve_existing_relationship_index(
        ...
        if relation_type:
            ...
            raise ValueError("Couldn't find any specified `relation_type`." " Check if you spelled it correctly")
        ...
        if index_information:
            processed_index_information = process_index_data(index_information.result_set)
            for dict in processed_index_information:
                if dict.get("entity_label", False) == relation_type and dict.get("entity_type", False) == "RELATIONSHIP":
                    if dict["index_type"] == "VECTOR":
                        embedding_dimension = int(dict["index_dimension"])
                        entity_type = str(dict["entity_type"])
                        entity_label = str(dict["entity_label"])
    ...
    def create_new_node_index(
        ...
        try:
            ...
            if "already indexed" in str(e):
                raise ValueError(
                    f"A vector index on (:{node_label}" "{" f"{embedding_node_property}" "}) has already been created"
                )
            else:
    ...
    def create_new_index_on_relationship(
        ...
        try:
            ...
            if "already indexed" in str(e):
                raise ValueError(
                    f"A vector index on [:{relation_type}" "{" f"{embedding_node_property}" "}] has already been created"
                )
            else:
                raise ValueError(f"Error occurred: {e}")
        ...
        for i, doc in enumerate(documents):
            ...
            if ids:
                ...
                try:
                    ...
                    raise ValueError("Your document wasn't added to the store" " successfully. Check your spellings.")
    ...
    def __from(
        ...
        **kwargs: Any,
        ...
        embedding_dimension, index_type, entity_label, entity_property = store.retrieve_existing_node_index()
    ...
    def from_existing_graph(
        ...
        **kwargs: Any,
        ...
        while True:
            ...
            params = {"data": [{"id": el[0], "embedding": embedding} for el, embedding in zip(data, text_embeddings)]}
        ...
        **kwargs: Any,
        ...
        if filter:
            if self.search_type == SearchType.HYBRID:
                raise ValueError("Metadata filtering can't be use in combination with " "a hybrid search approach")

            base_index_query = f"MATCH (n:{self.node_label}) WHERE " f"n.{self.embedding_node_property} IS NOT NULL AND "

            base_cosine_query = (
                " WITH n as node, " f" vec.cosineDistance(n.{self.embedding_node_property}" ", vecf32($embedding)) as score "
            )
        ...
        if not results:
            if not self.retrieval_query:
                ...
                raise ValueError("Inspect the `retrieval_query` and ensure it doesn't " "return None for the `text` column")
        ...
        if kwargs.get("return_embeddings") and any(result[2]["embedding"] is None for result in results):
            if not self.retrieval_query:
                raise ValueError(
                ...
                raise ValueError(
                    "Inspect the `retrieval_query` and ensure it doesn't " "return None for the `embedding` metadata column"
                )
    ...
    def similarity_search_with_relevance_scores(
        ...
        **kwargs: Any,
        ...
        docs_with_scores = self.similarity_search_with_score(query=query, k=k, filter=filter, **kwargs)
    ...
    def max_marginal_relevance_search(
        ...
        **kwargs: Any,
        ...
        selected_indices = maximal_marginal_relevance(np.array(query_embedding), got_embeddings, lambda_mult=lambda_mult, k=k)
