...
class PGEmbedding(VectorStore):
    ...
    def create_collection(self) -> None:
        ...
        with Session(self._conn) as session:
            CollectionStore.get_or_create(session, self.collection_name, cmetadata=self.collection_metadata)
            ...
            if filter is not None:
                ...
                for key, value in filter.items():
                    ...
                    if isinstance(value, dict) and IN in map(str.lower, value):
                        ...
                        filter_by_metadata = EmbeddingStore.cmetadata[key].astext.in_(value_case_insensitive[IN])
                        filter_clauses.append(filter_by_metadata)
                    elif isinstance(value, dict) and "substring" in map(str.lower, value):
                        filter_by_metadata = EmbeddingStore.cmetadata[key].astext.ilike(f"%{value['substring']}%")
                        filter_clauses.append(filter_by_metadata)
                    else:
