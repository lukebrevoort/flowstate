...
class SingleStoreDB(VectorStore):
    ...
    def delete(self, ids: Optional[List[str]] = None, **kwargs: Any) -> bool | None:
        ...
        try:
            cur = conn.cursor()
            try:
                cur.execute("DELETE FROM {} WHERE {} IN ({})".format(self.table_name, self.id_field, ",".join(ids)))
                if self.use_vector_index or self.use_full_text_search:
                    cur.execute("OPTIMIZE TABLE {} FLUSH;".format(self.table_name))
            finally:
                cur.close()
    ...
    def similarity_search_with_score(
        ...
        **kwargs: Any,
        ...
        if (
            ...
            raise ValueError(
                "Search strategy {} is not supported with distance strategy {}".format(search_strategy, self.distance_strategy)
            )
        ...
        try:
            ...
            try:
                if (
                    ...
                    search_options = ""
                    if self.use_vector_index and search_strategy == SingleStoreDB.SearchStrategy.FILTER_BY_TEXT:
                        search_options = "SEARCH_OPTIONS '{\"k\":%d}'" % (k * vector_select_count_multiplier)
                    cur.execute(
