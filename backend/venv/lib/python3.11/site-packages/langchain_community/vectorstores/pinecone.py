...
def _import_pinecone() -> Any:
    try:
        ...
        raise ImportError(
            "Could not import pinecone python package. " "Please install it with `pip3 install pinecone`."
        ) from e
    return pinecone
...
class Pinecone(VectorStore):
    ...
    def __init__(
        ...
        if not isinstance(embedding, Embeddings):
            warnings.warn(
                "Passing in `embedding` as a Callable is deprecated. Please pass in an" " Embeddings object instead."
            )
    ...
    def similarity_search_with_score(
        ...
        """
        return self.similarity_search_by_vector_with_score(self._embed_query(query), k=k, filter=filter, namespace=namespace)
    ...
    def _select_relevance_score_fn(self) -> Callable[[float], float]:
        ...
        if self.distance_strategy == DistanceStrategy.COSINE:
            ...
            raise ValueError("Unknown distance strategy, must be cosine, max_inner_product " "(dot product), or euclidean")
    ...
    def max_marginal_relevance_search(
        ...
        **kwargs: Any,
        ...
        return self.max_marginal_relevance_search_by_vector(embedding, k, fetch_k, lambda_mult, filter, namespace)
