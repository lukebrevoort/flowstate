...
def _get_search_index_query(search_type: SearchType, index_type: IndexType = DEFAULT_INDEX_TYPE) -> str:
    if index_type == IndexType.NODE:
        type_to_query_map = {
            SearchType.VECTOR: ("CALL db.index.vector.queryNodes($index, $k, $embedding) " "YIELD node, score "),
            SearchType.HYBRID: (
                "CALL { "
...
class Neo4jVector(VectorStore):
    ...
    def __init__(
        ...
        *,
        ...
        try:
            ...
            raise ImportError("Could not import neo4j python package. " "Please install it with `pip install neo4j`.")
        ...
        if graph:
            ...
            database = get_from_dict_or_env({"database": database}, "database", "NEO4J_DATABASE", "neo4j")
        ...
        if pre_delete_collection:
            ...
            self.query(f"MATCH (n:`{self.node_label}`) " "CALL (n) { DETACH DELETE n } " "IN TRANSACTIONS OF 10000 ROWS;")
            # Delete index
            try:
    ...
    def query(
        ...
        *,
        ...
        params = params or {}
        try:
            data, _, _ = self._driver.execute_query(query, database_=self._database, parameters_=params)
    ...
    def __from(
        ...
        **kwargs: Any,
        ...
        if create_id_index:
            store.query("CREATE CONSTRAINT IF NOT EXISTS " f"FOR (n:`{store.node_label}`) REQUIRE n.id IS UNIQUE;")
        ...
        **kwargs: Any,
        ...
        if filter:
            ...
            if not self.support_metadata_filter:
                raise ValueError("Metadata filtering is only supported in " "Neo4j version 5.18 or greater")
            ...
            parallel_query = "CYPHER runtime = parallel parallelRuntimeSupport=all " if self._is_enterprise else ""
        ...
        if kwargs.get("return_embeddings") and any(result["metadata"]["_embedding_"] is None for result in results):
            if not self.retrieval_query:
                raise ValueError(
                ...
                raise ValueError(
                    "Inspect the `retrieval_query` and ensure it doesn't " "return None for the `_embedding_` metadata column"
                )

        docs = [
            (
                Document(
                    page_content=dict_to_yaml_str(result["text"]) if isinstance(result["text"], dict) else result["text"],
    ...
    def from_existing_index(
        ...
        **kwargs: Any,
        ...
        if search_type == SearchType.HYBRID and not keyword_index_name:
            raise ValueError("keyword_index name has to be specified when using hybrid search option")
