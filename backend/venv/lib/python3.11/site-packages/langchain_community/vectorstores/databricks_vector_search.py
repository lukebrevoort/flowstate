...
class DatabricksVectorSearch(VectorStore):
        ...
        *,
        ...
        if not self._is_databricks_managed_embeddings():
            ...
            if embedding is not None:
                logger.warning("embedding model is not used in delta-sync index with " "Databricks-managed embeddings.")
    ...
    def from_texts(
        ...
        **kwargs: Any,
        ...
        raise NotImplementedError("`from_texts` is not supported. " "Use `add_texts` to add to existing direct-access index.")
    ...
    def max_marginal_relevance_search_by_vector(
        ...
        **kwargs: Any,
        ...
        embeddings_result_index = search_resp.get("manifest").get("columns").index({"name": embedding_column})
        embeddings = [doc[embeddings_result_index] for doc in search_resp.get("result").get("data_array")]
    ...
    def similarity_search_by_vector_with_score(
        ...
        **kwargs: Any,
        ...
        """
        if self._is_databricks_managed_embeddings():
            raise ValueError("`similarity_search_by_vector` is not supported for index with " "Databricks-managed embeddings.")
        if query_type is not None and query_type.upper() == "HYBRID":
            if query is None:
            ...
            if query is not None:
                raise ValueError(("Cannot specify both `embedding` and " '`query` unless `query_type="HYBRID"'))
            query_text = None
        search_resp = self.index.similarity_search(
            columns=self.columns,
            query_vector=embedding,
