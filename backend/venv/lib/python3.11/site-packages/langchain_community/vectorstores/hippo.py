...
class Hippo(VectorStore):
    ...
    def _create_connection_alias(self, connection_args: dict) -> HippoClient:
        ...
        try:
            ...
            raise ImportError("Unable to import transwarp_hipp_api, please install with " "`pip install hippo-api`.") from e
    ...
    def _get_env(self, embeddings: Optional[list] = None, metadatas: Optional[List[dict]] = None) -> None:
        logger.info("init ...")
    ...
    def _create_collection(self, embeddings: list, metadatas: Optional[List[dict]] = None) -> None:
        ...
        self.col = self.hc.get_table(self.table_name, self.database_name)
        logger.info(f"[_create_collection] : " f"create table {self.table_name} in {self.database_name} successfully")
    ...
    def _get_index(self) -> Optional[Dict[str, Any]]:
        ...
        if isinstance(self.col, HippoTable):
            ...
            if embedding_indexes is None:
                ...
                for x in self.hc.get_table_info(self.table_name, self.database_name)[self.table_name]["embedding_indexes"]:
    ...
    def similarity_search_with_score_by_vector(
        ...
        **kwargs: Any,
        ...
        for items in zip(*[res[0][field] for field in output_fields]):
            ...
            logger.debug(f"[similarity_search_with_score_by_vector] " f"res[0][score_col]:{res[0][score_col]}")
