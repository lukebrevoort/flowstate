...
class TablestoreVectorStore(VectorStore):
    ...
    def __init__(
        ...
        *,
        ...
        try:
            ...
            raise ImportError(
                "Could not import tablestore python package. " "Please install it with `pip install tablestore`."
            )
        ...
        if metadata_mappings:
            for mapping in metadata_mappings:
                if not isinstance(mapping, tablestore.FieldSchema):
                    raise ValueError(
                        f"meta_data mapping should be an " f"instance of tablestore.FieldSchema, " f"bug got {type(mapping)}"
                    )
    ...
    def create_search_index_if_not_exist(self) -> None:
        ...
        self.__tablestore_client.create_search_index(self.__table_name, self.__index_name, index_meta)
    ...
    def __write_row(self, row_id: str, content: str, embedding_vector: List[float], meta_data: dict) -> None:
        try:
            import tablestore
        except ImportError:
            raise ImportError(
    ...
    def __get_row(self, row_id: str) -> Document:
        ...
        primary_key = [("id", row_id)]
        try:
            _, row, _ = self.__tablestore_client.get_row(self.__table_name, primary_key, None, None, 1)
