...
class TextEmbedEmbeddings(BaseModel, Embeddings):
    ...
    def validate_environment(self) -> Self:
        ...
        self.client = AsyncOpenAITextEmbedEmbeddingClient(host=self.api_url, api_key=self.api_key.get_secret_value())
        return self
...
class AsyncOpenAITextEmbedEmbeddingClient:
    ...
    def _sync_request_embed(self, model: str, batch_texts: List[str]) -> List[List[float]]:
        ...
        if response.status_code != 200:
            raise Exception(
                f"TextEmbed responded with an unexpected status message " f"{response.status_code}: {response.text}"
            )
    ...
    async def _async_request(self, session: aiohttp.ClientSession, **kwargs: Dict[str, Any]) -> List[List[float]]:
        ...
        async with session.post(**kwargs) as response:  # type: ignore[arg-type]
            if response.status != 200:
                raise Exception(
                    f"TextEmbed responded with an unexpected status message " f"{response.status}: {response.text}"
                )
