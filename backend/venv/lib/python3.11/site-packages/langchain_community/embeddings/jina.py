...
class JinaEmbeddings(BaseModel, Embeddings):
    ...
    def validate_environment(cls, values: Dict) -> Any:
        ...
        try:
            jina_api_key = convert_to_secret_str(get_from_dict_or_env(values, "jina_api_key", "JINA_API_KEY"))
        except ValueError as original_exc:
            try:
                jina_api_key = convert_to_secret_str(get_from_dict_or_env(values, "jina_auth_token", "JINA_AUTH_TOKEN"))
            except ValueError:
                raise original_exc
    ...
    def _embed(self, input: Any) -> List[List[float]]:
        ...
        resp = self.session.post(JINA_API_URL, json={"input": input, "model": self.model_name}).json()
        if "data" not in resp:
