...
class FastEmbedEmbeddings(BaseModel, Embeddings):
    ...
    * https://qdrant.github.io/fastembed/
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        pkg_to_install = "fastembed-gpu" if providers and "CUDAExecutionProvider" in providers else "fastembed"
        ...
        try:
            ...
            raise ImportError(
                "Could not import 'fastembed' Python package. " f"Please install it with `pip install {pkg_to_install}`."
            )

        if importlib.metadata.version(pkg_to_install) < MIN_VERSION:
            raise ImportError(f"FastEmbedEmbeddings requires " f'`pip install -U "{pkg_to_install}>={MIN_VERSION}"`.')
    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        ...
        if self.doc_embed_type == "passage":
            embeddings = self.model.passage_embed(texts, batch_size=self.batch_size, parallel=self.parallel)
        else:
    ...
    def embed_query(self, text: str) -> List[float]:
        ...
        """
        query_embeddings: np.ndarray = next(self.model.query_embed(text, batch_size=self.batch_size, parallel=self.parallel))
