...
class BaichuanTextEmbeddings(BaseModel, Embeddings):
    ...
    def _embed(self, texts: List[str]) -> Optional[List[List[float]]]:
        ...
        Returns:
            ...
            error occurs.
        """
        chunk_texts = [texts[i : i + self.chunk_size] for i in range(0, len(texts), self.chunk_size)]
        embed_results = []
        for chunk in chunk_texts:
            response = self.session.post(BAICHUAN_API_URL, json={"input": chunk, "model": self.model_name})
            ...
            if response.status_code == 200:
                ...
                raise RequestException(f"Error: Received status code {response.status_code} from " "`BaichuanEmbedding` API")
