...
class TinyAsyncOpenAIInfinityEmbeddingClient:  #: :meta private:
    ...
    def _sync_request_embed(self, model: str, batch_texts: List[str]) -> List[List[float]]:
        ...
        if response.status_code != 200:
            raise Exception(
                f"Infinity returned an unexpected response with status " f"{response.status_code}: {response.text}"
            )
    ...
    async def _async_request(self, session: aiohttp.ClientSession, kwargs: Dict[str, Any]) -> List[List[float]]:
        async with session.post(**kwargs) as response:
            if response.status != 200:
                raise Exception(f"Infinity returned an unexpected response with status " f"{response.status}: {response.text}")
    ...
    async def aembed(self, model: str, texts: List[str]) -> List[List[float]]:
        ...
        # Request
        async with aiohttp.ClientSession(trust_env=True, connector=aiohttp.TCPConnector(limit=32)) as session:
