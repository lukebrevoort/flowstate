...
class InfinityEmbeddingsLocal(BaseModel, Embeddings):
    ...
    async def aembed_documents(self, texts: List[str]) -> List[List[float]]:
        ...
        if not self.engine.running:
            logger.warning(
                "Starting Infinity engine on the fly. This is not recommended." "Please start the engine before using it."
            )
            async with self:
    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        ...
        """
        logger.warning("This method is async only. " "Please use the async version `await aembed_documents`.")
        return asyncio.run(self.aembed_documents(texts))

    def embed_query(self, text: str) -> List[float]:
        """ """
        logger.warning("This method is async only." " Please use the async version `await aembed_query`.")
