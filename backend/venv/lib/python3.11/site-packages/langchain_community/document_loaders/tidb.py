...
class TiDBLoader(BaseLoader):
    ...
    def lazy_load(self) -> Iterator[Document]:
        ...
        engine: Engine = create_engine(self.connection_string, **(self.engine_args or {}))
        ...
        with engine.connect() as conn:
            ...
            for row in result:
                ...
                row_data = {column_names[index]: value for index, value in enumerate(row)}
                page_content = "\n".join(
                    f"{k}: {v}"
                    for k, v in row_data.items()
                    if self.page_content_columns is None or k in self.page_content_columns
                )
