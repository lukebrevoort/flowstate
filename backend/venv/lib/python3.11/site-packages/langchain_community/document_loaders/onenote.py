...
class OneNoteLoader(BaseLoader, BaseModel):
    ...
    def lazy_load(self) -> Iterator[Document]:
        ...
        try:
            ...
            raise ImportError("beautifulsoup4 package not found, please install it with " "`pip install bs4`")
    ...
    def _auth(self) -> None:
        ...
        if self.auth_with_token:
            ...
            try:
                ...
                raise ImportError("MSAL package not found, please install it with `pip install msal`") from e
            ...
            try:
                ...
                raise Exception(f"Could not create the folder {self.token_path.parent} " + "to store the access token.") from e
    ...
    def _url(self) -> str:
        ...
        if self.notebook_name is not None:
            filter_list.append("parentNotebook/displayName%20eq%20" + f"'{self.notebook_name.replace(' ', '%20')}'")
            expand_list.append("parentNotebook")
        if self.section_name is not None:
            filter_list.append("parentSection/displayName%20eq%20" + f"'{self.section_name.replace(' ', '%20')}'")
