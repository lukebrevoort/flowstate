...
class BiliBiliLoader(BaseLoader):
    ...
    def _get_bilibili_subs_and_info(self, url: str) -> Tuple[str, dict]:
        ...
        if page_match:
            cid = video_info["pages"][int(page_match.group(1)) - 1]["cid"]  # Bilibili page index starts from 1
        else:
            cid = video_info["cid"]
        ...
        if sub_list:
            ...
            if response.status_code == 200:
                ...
                warnings.warn(f"Failed to fetch subtitles for {url}. " f"HTTP Status Code: {response.status_code}")
        else:
