...
class GoogleApiClient:
    ...
    *Security Note*: Note that parsing of the transcripts relies on the standard
    ...
    def validate_channel_or_videoIds_is_set(cls, values: Any) -> Any:
        ...
        if not values.kwargs.get("credentials_path") and not values.kwargs.get("service_account_path"):
...
class YoutubeLoader(BaseLoader):
    ...
    def _make_chunk_document(self, chunk_pieces: List[Dict], chunk_start_seconds: int) -> Document:
        ...
        h, m = divmod(m, 60)
        return Document(
            page_content=" ".join(map(lambda chunk_piece: chunk_piece["text"].strip(" "), chunk_pieces)),
            metadata={
                **self._metadata,
    ...
    def _get_transcript_chunks(self, transcript_pieces: List[Dict]) -> Generator[Document, None, None]:
            ...
            return list(
                map(
                    lambda transcript_piece: Document(
                        ...
                        metadata=dict(filter(lambda item: item[0] != "text", transcript_piece.items())),
                    ),
                    transcript_pieces,
                )
            )
