...
class QuipLoader(BaseLoader):
    ...
    def get_thread_ids_by_folder_id(self, folder_id: str, depth: int, thread_ids: List[str]) -> None:
        ...
        try:
            ...
            if e.code == 403:
                ...
                logging.warning(f"depth {depth}, Skipped over folder {folder_id} " f"due to unknown error {e.code}")
            return
        except HTTPError as e:
            logging.warning(f"depth {depth}, Skipped over folder {folder_id} " f"due to HTTP error {e.code}")
            return
    ...
    def process_thread(self, thread_id: str, include_images: bool, include_messages: bool) -> Optional[Document]:
        ...
        logger.info(f"processing thread {thread_id} title {sanitized_title} " f"link {link} update_ts {update_ts}")
