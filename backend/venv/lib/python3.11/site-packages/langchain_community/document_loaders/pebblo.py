...
class PebbloSafeLoader(BaseLoader):
    ...
    def __init__(
        ...
        *,
        ...
        self.loader_details = {
            ...
            **({"source_path_size": str(self.source_path_size)} if self.source_path_size > 0 else {}),
        }
        # generate app
    ...
    def lazy_load(self) -> Iterator[Document]:
        ...
        while True:
            ...
            self.docs = list((doc,))
            self.docs_with_id = self._index_docs()
            classified_doc = self.pb_client.classify_documents(self.docs_with_id, self.app, self.loader_details)
    ...
    def _index_docs(self) -> List[IndexedDocument]:
        ...
        """
        docs_with_id = [IndexedDocument(pb_id=str(i), **doc.dict()) for i, doc in enumerate(self.docs)]
        return docs_with_id
    ...
    def _add_semantic_to_docs(self, classified_docs: Dict) -> List[Document]:
        ...
        """
        indexed_docs = {doc.pb_id: Document(page_content=doc.page_content, metadata=doc.metadata) for doc in self.docs_with_id}
    ...
    def _unindex_docs(self) -> List[Document]:
        ...
        """
        docs = [Document(page_content=doc.page_content, metadata=doc.metadata) for i, doc in enumerate(self.docs_with_id)]
        return docs
    ...
    def _add_pebblo_specific_metadata(self, classified_docs: dict) -> None:
        ...
        for doc in self.docs_with_id:
            ...
            if self.loader.__class__.__name__ == "SharePointLoader":
                doc_metadata["full_path"] = get_full_path(doc_metadata.get("source", self.source_path))
            else:
                doc_metadata["full_path"] = get_full_path(
                    doc_metadata.get("full_path", doc_metadata.get("source", self.source_path))
                )
            doc_metadata["pb_checksum"] = classified_docs.get(doc.pb_id, {}).get("pb_checksum", None)
