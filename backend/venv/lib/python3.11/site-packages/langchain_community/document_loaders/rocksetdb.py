...
class RocksetLoader(BaseLoader):
    ...
    def __init__(
        ...
        if not isinstance(client, RocksetClient):
            raise ValueError(f"client should be an instance of rockset.RocksetClient, " f"got {type(client)}")

        if not isinstance(query, QueryRequestSql):
            raise ValueError(f"query should be an instance of rockset.model.QueryRequestSql, " f"got {type(query)}")
    ...
    def lazy_load(self) -> Iterator[Document]:
        ...
        for doc in query_results:  # for each doc in the response
            try:
                yield Document(
                    page_content=self.content_columns_joiner([(col, doc[col]) for col in self.content_keys]),
                    metadata={col: doc[col] for col in self.metadata_keys} if self.metadata_keys is not None else doc,
                )  # try to yield the Document
            except KeyError as e:  # either content_columns or metadata_columns is invalid
                raise ColumnNotFoundError(e.args[0], self.query)  # raise that the column isn't in the db schema
