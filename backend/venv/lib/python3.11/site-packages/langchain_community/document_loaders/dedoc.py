...
class DedocBaseLoader(BaseLoader, ABC):
    ...
    def __init__(
        ...
        *,
        ...
        self.parsing_parameters = {
            key: value for key, value in locals().items() if key not in {"self", "file_path", "split", "with_tables"}
        }
        self.valid_split_values = {"document", "page", "node", "line"}
        if split not in self.valid_split_values:
            raise ValueError(f"Got {split} for `split`, but should be one of " f"`{self.valid_split_values}`")
        self.split = split
    ...
    def lazy_load(self) -> Iterator[Document]:
        ...
        try:
            ...
            raise ImportError("`dedoc` package not found, please install it with `pip install dedoc`")
        ...
        yield from self._split_document(document_tree=document_tree.to_api_schema().dict(), split=self.split)
    ...
    def _parse_subparagraphs(self, document_tree: dict, document_metadata: dict) -> Iterator[Document]:
    ...
    def _get_table(self, table: dict) -> Tuple[str, str]:
        ...
        for row in table["cells"]:
            ...
            for cell in row:
                ...
                table_html += f' colspan="{cell["colspan"]}" rowspan=' f'"{cell["rowspan"]}">{cell_text}</td>\n'
            table_html += "</tr>\n"
        table_html += "</tbody>\n</table>"
...
class DedocAPIFileLoader(DedocBaseLoader):
    ...
    def _send_file(self, url: str, file_path: str, parameters: dict) -> Dict[str, Union[list, dict, str]]:
