...
class MathpixPDFLoader(BasePDFLoader):
    ...
    def clean_pdf(self, contents: str) -> str:
        ...
        contents = contents.replace(r"\$", "$").replace(r"\%", "%").replace(r"\(", "(").replace(r"\)", ")")
        return contents
...
class AmazonTextractPDFLoader(BasePDFLoader):
    ...
    def _get_number_of_pages(blob: Blob) -> int:
        try:
            ...
            raise ImportError(
                "Could not import pypdf or Pilloe python package. " "Please install it with `pip install pypdf Pillow`."
            )
...
class ZeroxPDFLoader(BasePDFLoader):
    ...
    def lazy_load(self) -> Iterator[Document]:
        ...
        zerox_output = asyncio.run(zerox(file_path=str(self.file_path), model=self.model, **self.zerox_kwargs))
