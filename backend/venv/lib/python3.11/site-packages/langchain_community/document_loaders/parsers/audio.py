    ...
    def lazy_parse(self, blob: Blob) -> Iterator[Document]:
        ...
        for split_number, i in enumerate(range(0, len(audio), chunk_duration_ms)):
            ...
            while attempts < 3:
                try:
                    if is_openai_v1():
                        transcript = client.audio.transcriptions.create(model=self.model, file=file_obj, **self._create_params)
                    else:
...
class YandexSTTParser(BaseBlobParser):
    ...
    def lazy_parse(self, blob: Blob) -> Iterator[Document]:
        ...
        try:
            ...
            raise ImportError("yandex-speechkit package not found, please install it with " "`pip install yandex-speechkit`")
        ...
        if self.api_key:
            ...
            configure_credentials(yandex_credentials=creds.YandexCredentials(iam_token=self.iam_token))
