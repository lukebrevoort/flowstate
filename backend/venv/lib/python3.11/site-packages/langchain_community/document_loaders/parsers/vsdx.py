...
class VsdxParser(BaseBlobParser, ABC):
    ...
    def get_pages_content(self, zfile: zipfile.ZipFile, source: str) -> List[Tuple[int, str, str]]:
        ...
        try:
            ...
            raise ImportError(
                "The xmltodict library is required to parse vsdx files. " "Please install it with `pip install xmltodict`."
            )
        ...
        if isinstance(pagesxml_content["Pages"]["Page"], list):
            ...
            disordered_names: List[str] = [pagesxml_content["Pages"]["Page"]["@Name"].strip()]  # type: ignore[no-redef]
        if isinstance(pagesxmlrels_content["Relationships"]["Relationship"], list):
            disordered_paths: List[str] = [
                "visio/pages/" + rel["@Target"] for rel in pagesxmlrels_content["Relationships"]["Relationship"]
            ]
        else:
            disordered_paths: List[str] = [  # type: ignore[no-redef]
                "visio/pages/" + pagesxmlrels_content["Relationships"]["Relationship"]["@Target"]
            ]
        ordered_names: List[str] = appxml_content["Properties"]["TitlesOfParts"]["vt:vector"]["vt:lpstr"][
        ...
        pagexml_rels = [
            {
                "path": page_path,
                "content": xmltodict.parse(zfile.read(f"visio/pages/_rels/{Path(page_path).stem}.xml.rels")),
            }
        ...
        for page_number, (path, page_name) in enumerate(zip(ordered_paths, ordered_names)):
            ...
            page_content = "\n".join(
                [page_["page_content"] for page_ in disordered_pages if page_["page"] in relationships]
