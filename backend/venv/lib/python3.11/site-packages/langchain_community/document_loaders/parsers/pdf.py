...
def _purge_metadata(metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    for k, v in metadata.items():
        ...
        if k in ["creationdate", "moddate"]:
            try:
                new_metadata[k] = datetime.strptime(v.replace("'", ""), "D:%Y%m%d%H%M%S%z").isoformat("T")
            except ValueError:
                new_metadata[k] = v
        elif k in map_key:
...
def _merge_text_and_extras(extras: list[str], text_from_page: str) -> str:
    ...
    def _recurs_merge_text_and_extras(extras: list[str], text_from_page: str, recurs: bool) -> Optional[str]:
        if extras:
            for delim in _PARAGRAPH_DELIMITER:
                ...
                if pos != -1:
                    ...
                    previous_text = None
                    if recurs:
                        previous_text = _recurs_merge_text_and_extras(extras, text_from_page[:pos], False)
                    if previous_text:
        ...
        for obj in xObject:
            ...
            if xObject[obj]["/Subtype"] == "/Image":
                ...
                if img_filter in _PDF_FILTER_WITHOUT_LOSS:
                    ...
                    np_image = np.frombuffer(xObject[obj].get_data(), dtype=np.uint8).reshape(height, width, -1)
        ...
        *,
        ...
        if concatenate_pages is not None:
            if not PDFMinerParser._warn_concatenate_pages:
                ...
                logger.warning("`concatenate_pages` parameter is deprecated. " "Use `mode='single' or 'page'` instead.")
                    ...
                    def render(item: LTItem) -> None:
                        ...
                        if isinstance(item, LTTextBox):
                            ...
                            if self.images_parser:
                                ...
                                text_io.write(_format_inner_image(blob, image_text, self.images_inner_format))
                        else:
                            pass
        ...
        metadata = _purge_metadata(
            {
                ...
                **{k: doc.metadata[k] for k in doc.metadata if isinstance(doc.metadata[k], (str, int))},
            }
        )
    ...
    def lazy_parse(self, blob: Blob) -> Iterator[Document]:
        ...
        with PyPDFium2Parser._lock:
            with blob.as_bytes_io() as file_path:
                pdf_reader = None
                try:
                    pdf_reader = pypdfium2.PdfDocument(file_path, password=self.password, autoclose=True)
                    full_content = []
...
class PDFPlumberParser(BaseBlobParser):
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError("pillow package not found, please install it with `pip install pillow`")
        self.text_kwargs = text_kwargs or {}
        self.dedupe = dedupe
        self.extract_images = extract_images
