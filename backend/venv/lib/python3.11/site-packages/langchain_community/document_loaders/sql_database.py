...
class SQLDatabaseLoader(BaseLoader):
    ...
    def lazy_load(self) -> Iterator[Document]:
        try:
            ...
            raise ImportError(
                "Could not import sqlalchemy python package. " "Please install it with `pip install sqlalchemy`."
            )
        ...
        if isinstance(self.query, sa.SelectBase):
            result = self.db._execute(self.query, fetch="cursor", parameters=self.parameters)  # type: ignore[assignment]
    ...
    @staticmethod
    def metadata_default_mapper(row: RowMapping, column_names: Optional[List[str]] = None) -> Dict[str, Any]:
        """
