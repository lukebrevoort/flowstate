...
class _O365Settings(BaseSettings):
    ...
    model_config = SettingsConfigDict(case_sensitive=False, env_file=".env", env_prefix="", extra="ignore")
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        if self.handlers:
            ...
            try:
                ...
                mime_handlers = {self._mime_types[extension]: handler for extension, handler in self.handlers.items()}
            except ValueError:
                try:
            ...
            if not isinstance(self._blob_parser, MimeTypeBasedParser):
                raise TypeError(
                    'get_parser("default) was supposed to return MimeTypeBasedParser.' f"It returned {type(self._blob_parser)}"
                )
    ...
    def _load_from_object_ids(self, drive: Drive, object_ids: List[str]) -> Iterable[Blob]:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            for object_id in object_ids:
                ...
                if not file:
                    logging.warning("There isn't a file with" f"object_id {object_id} in drive {drive}.")
                    continue
                if file.is_file:
                    if file.mime_type in list(file_mime_types.values()):
                        source = file.web_url
