...
class ObsidianLoader(BaseLoader):
    ...
    DATAVIEW_INLINE_BRACKET_REGEX: Pattern = re.compile(r"\[(\w+)::\s*(.*)\]", re.MULTILINE)
    DATAVIEW_INLINE_PAREN_REGEX: Pattern = re.compile(r"\((\w+)::\s*(.*)\)", re.MULTILINE)
    ...
    def _parse_dataview_fields(self, content: str) -> dict:
        ...
        return {
            **{match[0]: match[1] for match in self.DATAVIEW_LINE_REGEX.findall(content)},
            **{match[0]: match[1] for match in self.DATAVIEW_INLINE_PAREN_REGEX.findall(content)},
            **{match[0]: match[1] for match in self.DATAVIEW_INLINE_BRACKET_REGEX.findall(content)},
        }
