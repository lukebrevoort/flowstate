    ...
    def validate_since(cls, v: Optional[str]) -> Optional[str]:
        if v:
            try:
                ...
                raise ValueError(
                    "Invalid value for 'since'. Expected a date string in " f"YYYY-MM-DDTHH:MM:SSZ format. Received: {v}"
                )
        return v
...
class GithubFileLoader(BaseGitHubLoader, ABC):
    ...
    def get_file_paths(self) -> List[Dict]:
        base_url = f"{self.github_api_url}/repos/{self.repo}/git/trees/" f"{self.branch}?recursive=1"
