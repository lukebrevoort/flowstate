...
def _get_open_meteo_api(llm: BaseLanguageModel) -> BaseTool:
    try:
        ...
        raise ImportError(
            "API tools require the library `langchain` to be installed." " Please install it with `pip install langchain`."
        )
    ...
    for name in tool_names:
        ...
        if name == "requests_all":
            ...
            if not allow_dangerous_tools:
                raise_dangerous_tools_exception(name)
            requests_method_tools = [_tool for _tool in DANGEROUS_TOOLS if _tool.startswith("requests_")]
...
def get_all_tool_names() -> List[str]:
    ...
    return list(_BASE_TOOLS) + list(_EXTRA_OPTIONAL_TOOLS) + list(_EXTRA_LLM_TOOLS) + list(_LLM_TOOLS) + list(DANGEROUS_TOOLS)
