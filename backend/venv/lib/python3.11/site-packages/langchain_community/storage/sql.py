...
class SQLStore(BaseStore[str, bytes]):
    ...
    async def amset(self, key_value_pairs: Sequence[Tuple[str, bytes]]) -> None:
        async with self._make_async_session() as session:
            ...
            session.add_all([LangchainKeyValueStores(namespace=self.namespace, key=k, value=v) for k, v in key_value_pairs])
    ...
    def mset(self, key_value_pairs: Sequence[Tuple[str, bytes]]) -> None:
        ...
        with self._make_sync_session() as session:
            ...
            session.add_all([LangchainKeyValueStores(namespace=self.namespace, key=k, value=v) for k, v in values.items()])
            session.commit()
    ...
    def yield_keys(self, *, prefix: Optional[str] = None) -> Iterator[str]:
        with self._make_sync_session() as session:
            for v in session.query(LangchainKeyValueStores).filter(LangchainKeyValueStores.namespace == self.namespace):
    ...
    async def ayield_keys(self, *, prefix: Optional[str] = None) -> AsyncIterator[str]:
        async with self._make_async_session() as session:
            stmt = select(LangchainKeyValueStores).filter(LangchainKeyValueStores.namespace == self.namespace)
            for v in await session.scalars(stmt):
                if str(v.key).startswith(prefix or ""):
                    yield str(v.key)
            await session.close()
