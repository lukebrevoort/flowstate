...
"""
SELECT_TABLE_CQL_TEMPLATE = """SELECT row_id, body_blob FROM  {keyspace}.{table} WHERE row_id IN ?;"""
...
class CassandraByteStore(ByteStore):
        ...
        *,
        ...
        if not session or not keyspace:
            try:
                ...
                raise ImportError(
                    "Could not import a recent cassio package." "Please install it with `pip install --upgrade cassio`."
                )
        else:
    ...
    async def amget(self, keys: Sequence[str]) -> List[Optional[bytes]]:
        ...
        docs_dict = {}
        for row in await aexecute_cql(self.session, self.get_select_statement(), parameters=[ValueSequence(keys)]):
            docs_dict[row.row_id] = row.body_blob
        return [docs_dict.get(key) for key in keys]
    ...
    async def amdelete(self, keys: Sequence[str]) -> None:
        ...
        await self.aensure_db_setup()
        await aexecute_cql(self.session, self.get_delete_statement(), parameters=[ValueSequence(keys)])

    def yield_keys(self, *, prefix: Optional[str] = None) -> Iterator[str]:
        self.ensure_db_setup()
        for row in self.session.execute(SELECT_ALL_TABLE_CQL_TEMPLATE.format(keyspace=self.keyspace, table=self.table)):
            key = row.row_id
