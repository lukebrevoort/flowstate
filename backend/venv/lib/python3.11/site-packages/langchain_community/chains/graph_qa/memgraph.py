    ...
    def from_llm(
        ...
        **kwargs: Any,
        ...
        if qa_prompt and qa_llm_kwargs:
            raise ValueError(
                "Specifying qa_prompt and qa_llm_kwargs together is" " not allowed. Please pass prompt via qa_llm_kwargs."
            )
        ...
        if "prompt" not in use_cypher_llm_kwargs:
            use_cypher_llm_kwargs["prompt"] = cypher_prompt if cypher_prompt is not None else MEMGRAPH_GENERATION_PROMPT
        ...
        if prompt is not None and llm_to_use is not None:
            ...
            raise ValueError("Missing required components for the cypher generation chain: " "'prompt' or 'llm'")
