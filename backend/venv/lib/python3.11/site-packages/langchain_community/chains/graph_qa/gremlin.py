...
class GremlinQAChain(Chain):
    ...
    *Security note*: Make sure that the database connection uses credentials
    ...
    def from_llm(
        ...
        *,
        gremlin_fix_prompt: BasePromptTemplate = PromptTemplate(
            ...
            template=GRAPHDB_SPARQL_FIX_TEMPLATE.replace("SPARQL", "Gremlin").replace("in Turtle format", ""),
        ),
    ...
    def _call(
        ...
        generated_gremlin = extract_gremlin(chain_response[self.gremlin_generation_chain.output_key])
        ...
        if generated_gremlin:
            context = self.execute_with_retry(_run_manager, callbacks, generated_gremlin)[: self.top_k]
        else:
            context = []
