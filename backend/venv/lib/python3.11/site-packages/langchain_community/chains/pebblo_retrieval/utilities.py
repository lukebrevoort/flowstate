...
class PebbloRetrievalAPIWrapper(BaseModel):
    ...
    async def acheck_prompt_validity(self, question: str) -> Tuple[bool, Dict[str, Any]]:
        ...
        if self.classifier_location == "local":
            headers = self._make_headers()
            prompt_gov_api_url = f"{self.classifier_url}{Routes.prompt_governance.value}"
            pebblo_resp = await self.amake_request("POST", prompt_gov_api_url, headers, prompt_payload)
            if pebblo_resp:
            ...
            async with aiohttp.ClientSession() as asession:
                async with asession.request(
                    ...
                    if response.status >= HTTPStatus.INTERNAL_SERVER_ERROR:
                        ...
                        logger.warning(f"Pebblo returned an unexpected response code: " f"{response.status}")
    ...
    def build_prompt_qa_payload(
        ...
        qa = Qa(
            ...
            user_identities=auth_context.user_auth if auth_context and hasattr(auth_context, "user_auth") else [],
