...
def _apply_qdrant_semantic_filter(search_kwargs: dict, semantic_context: Optional[SemanticContext]) -> None:
    """
    ...
    try:
        ...
        raise ValueError(
            "Could not import `qdrant-client.http` python package. " "Please install it with `pip install qdrant-client`."
        ) from e
...
def _apply_qdrant_authorization_filter(search_kwargs: dict, auth_context: Optional[AuthContext]) -> None:
    """
        ...
        if isinstance(existing_filter, rest.Filter):
            ...
            if existing_filter.must:
                ...
                for condition in existing_filter.must:
                    if hasattr(condition, "key") and condition.key == "metadata.authorized_identities":
                        continue
...
def _apply_pinecone_semantic_filter(search_kwargs: dict, semantic_context: Optional[SemanticContext]) -> None:
    """
...
def _apply_pinecone_authorization_filter(search_kwargs: dict, auth_context: Optional[AuthContext]) -> None:
    """
    Set identity enforcement filter in search_kwargs for Pinecone vectorstore.
    """
    if auth_context is not None:
        search_kwargs.setdefault("filter", {})["authorized_identities"] = {"$in": auth_context.user_auth}


def _apply_pgvector_filter(search_kwargs: dict, filters: Optional[Any], pebblo_filter: dict) -> None:
    """
    ...
    if isinstance(filters, dict):
        ...
        raise ValueError(f"Invalid filter. Expected a dictionary/None but got type: {type(filters)}")


def _pgvector_clear_pebblo_filters(search_kwargs: dict, filters: dict, pebblo_filter_key: str) -> None:
    """
...
def _apply_pgvector_semantic_filter(search_kwargs: dict, semantic_context: Optional[SemanticContext]) -> None:
    """
    ...
    if semantic_context is not None:
        ...
        if semantic_context.pebblo_semantic_topics is not None:
            # Add pebblo_semantic_topics filter to search_kwargs
            topic_filter: dict = {"pebblo_semantic_topics": {"$eq": semantic_context.pebblo_semantic_topics.deny}}
            _semantic_filters.append(topic_filter)

        if semantic_context.pebblo_semantic_entities is not None:
            # Add pebblo_semantic_entities filter to search_kwargs
            entity_filter: dict = {"pebblo_semantic_entities": {"$eq": semantic_context.pebblo_semantic_entities.deny}}
...
def _apply_pgvector_authorization_filter(search_kwargs: dict, auth_context: Optional[AuthContext]) -> None:
    """
...
def _set_identity_enforcement_filter(retriever: VectorStoreRetriever, auth_context: Optional[AuthContext]) -> None:
    """
...
def _set_semantic_enforcement_filter(retriever: VectorStoreRetriever, semantic_context: Optional[SemanticContext]) -> None:
    """
