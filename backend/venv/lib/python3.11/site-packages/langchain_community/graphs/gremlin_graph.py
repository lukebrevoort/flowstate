...
class GremlinGraph(GraphStore):
    ...
    *Implementation details*:
    ...
    def refresh_schema(self) -> None:
        ...
        vertex_properties = (
            self.client.submit("g.V().group().by(label).by(properties().label().dedup().fold())").all().result()[0]
        )
    ...
    def add_graph_documents(self, graph_documents: List[GraphDocument], include_source: bool = False) -> None:
        ...
        for document in graph_documents:
            if include_source:
                ...
                doc_node = self.add_node(Node(id=doc_id, type="Document", properties=doc_props), node_cache)
