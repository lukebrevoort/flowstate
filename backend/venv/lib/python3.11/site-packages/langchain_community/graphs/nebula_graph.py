    ...
    *Security note*: Make sure that the database connection uses credentials
    ...
    def _get_session_pool(self) -> Any:
        ...
        try:
            ...
            raise ValueError(
                "Could not connect to NebulaGraph database. " "Please ensure that the address and port are correct"
            )
        ...
        try:
            ...
            raise ValueError(
                "Could not connect to NebulaGraph database. " "Please ensure that the username and password are correct"
            )
    ...
    def execute(self, query: str, params: Optional[dict] = None, retry: int = 0) -> Any:
        ...
        try:
            ...
            if not result.is_succeeded():
                logger.warning(f"Error executing query to NebulaGraph. " f"Error: {result.error_msg()}\n" f"Query: {query} \n")
            return result
            ...
            if retry < RETRY_TIMES:
                retry += 1
                logger.warning(
                    f"Connection issue with NebulaGraph. " f"Retrying ({retry}/{RETRY_TIMES})...\n to recreate session pool"
                )
                self.session_pool = self._get_session_pool()
                return self.execute(query, params, retry)
