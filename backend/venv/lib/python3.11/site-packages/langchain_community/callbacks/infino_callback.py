...
class InfinoCallbackHandler(BaseCallbackHandler):
    ...
    def on_llm_end(self, response: LLMResult, **kwargs: Any) -> None:
        ...
        if self.is_chat_openai_model:
            messages = " ".join(cast(str, cast(ChatGeneration, generation).message.content) for generation in generations)
            completion_tokens = get_num_tokens(messages, openai_model_name=self.chat_openai_model_name)
            self._send_to_infino("completion_tokens", completion_tokens)
