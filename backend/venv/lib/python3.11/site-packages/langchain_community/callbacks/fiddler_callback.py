...
class FiddlerCallbackHandler(BaseCallbackHandler):
    def __init__(
        ...
        if self.project not in self.fiddler_client.get_project_names():
            ...
            try:
                ...
                print(f"Error adding project {self.project}:" "{e}. Fiddler integration will not work.")  # noqa: T201
                raise e
        ...
        if self.model not in self.fiddler_client.get_model_names(self.project):
            if self.model not in self.fiddler_client.get_dataset_names(self.project):
                print(f"adding dataset {self.model} to project {self.project}." "This only has to be done once.")  # noqa: T201
                try:
                    ...
                    print(f"Error adding dataset {self.model}: {e}." "Fiddler integration will not work.")  # noqa: T201
                    raise e
            ...
            print(f"adding model {self.model} to project {self.project}." "This only has to be done once.")  # noqa: T201
            try:
                ...
                print(f"Error adding model {self.model}: {e}." "Fiddler integration will not work.")  # noqa: T201
                raise e
    ...
    def on_llm_end(self, response: LLMResult, **kwargs: Any) -> None:
        ...
        if isinstance(response.llm_output, dict):
            token_usage_dict = {
                k: v for k, v in response.llm_output.items() if k in [TOTAL_TOKENS, PROMPT_TOKENS, COMPLETION_TOKENS]
            }
