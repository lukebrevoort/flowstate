...
class ArthurCallbackHandler(BaseCallbackHandler):
    ...
    def __init__(
        ...
        if len([x for x in self.arthur_model.get_attributes() if x.value_type == ValueType.TokenLikelihoods]) > 0:
            self.token_likelihood_attr = [
                x for x in self.arthur_model.get_attributes() if x.value_type == ValueType.TokenLikelihoods
            ][0].name
    ...
    def on_llm_end(self, response: LLMResult, **kwargs: Any) -> None:
        ...
        try:
            ...
            raise ImportError("Could not import pytz. Please install it with 'pip install pytz'.") from e
