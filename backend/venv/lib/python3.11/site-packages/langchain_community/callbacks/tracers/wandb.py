...
def _serialize_io(run_io: Optional[dict]) -> dict:
    ...
    for key, value in run_io.items():
        if isinstance(value, Message):
            ...
            serialized_inputs[key] = value.model_dump_json() if hasattr(value, "model_dump_json") else value.json()
...
def truncate_run_iterative(runs: List[Dict[str, Any]], keep_keys: Tuple[str, ...] = ()) -> List[Dict[str, Any]]:
...
def build_tree(runs: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for child_id, parent_id in child_to_parent.items():
        ...
        parent_dict[next(iter(parent_dict))][next(iter(id_to_data[child_id]))] = id_to_data[child_id][
...
class WandbTracer(BaseTracer):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            raise ImportError("Could not import wandb python package." "Please install it with `pip install -U wandb`.") from e
        self._wandb = wandb
    ...
    def process_model_dict(self, run: Run) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            processed = modify_serialized_iterative(processed, exact_keys=exact_keys, partial_keys=partial_keys)
        ...
        def create_trace(run: "Run", parent: Optional["Trace"] = None) -> Optional["Trace"]:
            ...
            try:
                ...
                trace_tree = self._trace_tree.Trace(
                    ...
                    start_time_ms=int(run.start_time.timestamp() * 1000) if run.start_time is not None else None,
                    end_time_ms=int(run.end_time.timestamp() * 1000) if run.end_time is not None else None,
                    metadata=metadata,
