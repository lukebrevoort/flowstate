...
def analyze_text(
    ...
    if textstat is not None:
        text_complexity_metrics = {key: getattr(textstat, key)(text) for key in get_text_complexity_metrics()}
...
class MlflowCallbackHandler(BaseMetadataCallbackHandler, BaseCallbackHandler):
        ...
        for generations in response.generations:
            for idx, generation in enumerate(generations):
                ...
                if "text_complexity_metrics" in generation_resp:
                    complexity_metrics: Dict[str, float] = generation_resp.pop("text_complexity_metrics")
                    self.mlflg.metrics(
                        complexity_metrics,
        ...
        retriever_documents = [
            {
                ...
                "metadata": {
                    k: (str(v) if not isinstance(v, list) else ",".join(str(x) for x in v)) for k, v in doc.metadata.items()
                },
            }
    ...
    def _create_session_analysis_df(self) -> Any:
        ...
        if "name" in on_llm_start_records_df.columns:
            ...
            on_llm_start_records_df["name"] = on_llm_start_records_df["id"].apply(lambda id_: id_[-1])
            llm_input_columns.append("name")
        llm_input_prompts_df = (
            on_llm_start_records_df[llm_input_columns].dropna(axis=1).rename({"step": "prompt_step"}, axis=1)
        )
        complexity_metrics_columns = get_text_complexity_metrics() if self.textstat is not None else []
