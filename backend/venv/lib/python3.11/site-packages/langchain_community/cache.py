...
class SQLAlchemyCache(BaseCache):
    ...
    def update(self, prompt: str, llm_string: str, return_val: RETURN_VAL_TYPE) -> None:
        ...
        items = [
            self.cache_schema(prompt=prompt, llm=llm_string, response=dumps(gen), idx=i) for i, gen in enumerate(return_val)
        ]
    ...
    def update(self, prompt: str, llm_string: str, return_val: RETURN_VAL_TYPE) -> None:
        ...
        for gen in return_val:
            if not isinstance(gen, Generation):
                raise ValueError("UpstashRedisCache supports caching of normal LLM generations, " f"got {type(gen)}")
            if isinstance(gen, ChatGeneration):
                warnings.warn(
                    "NOTE: Generation has not been cached. UpstashRedisCache does not" " support caching ChatModel outputs."
                )
                return
        # Write to a HASH
...
class _RedisCacheBase(BaseCache, ABC):
    ...
    def _ensure_generation_type(return_val: RETURN_VAL_TYPE) -> None:
        for gen in return_val:
            if not isinstance(gen, Generation):
                raise ValueError("RedisCache only supports caching of normal LLM generations, " f"got {type(gen)}")
    ...
    @staticmethod
    def _configure_pipeline_for_update(key: str, pipe: Any, return_val: RETURN_VAL_TYPE, ttl: Optional[int] = None) -> None:
        pipe.hset(
            key,
...
class AsyncRedisCache(_RedisCacheBase):
    ...
    def __init__(self, redis_: Any, *, ttl: Optional[int] = None):
        ...
        try:
            ...
            raise ImportError(
                "Could not import `redis.asyncio` python package. " "Please install it with `pip install redis`."
            )
        if not isinstance(redis_, Redis):
    ...
    def lookup(self, prompt: str, llm_string: str) -> Optional[RETURN_VAL_TYPE]:
        ...
        raise NotImplementedError(
            "This async Redis cache does not implement `lookup()` method. " "Consider using the async `alookup()` version."
        )
    ...
    def update(self, prompt: str, llm_string: str, return_val: RETURN_VAL_TYPE) -> None:
        """Update cache based on prompt and llm_string."""
        raise NotImplementedError(
            "This async Redis cache does not implement `update()` method. " "Consider using the async `aupdate()` version."
        )
    ...
    def clear(self, **kwargs: Any) -> None:
        """Clear cache. If `asynchronous` is True, flush asynchronously."""
        raise NotImplementedError(
            "This async Redis cache does not implement `clear()` method. " "Consider using the async `aclear()` version."
        )
...
class RedisSemanticCache(BaseCache):
    ...
    def update(self, prompt: str, llm_string: str, return_val: RETURN_VAL_TYPE) -> None:
        ...
        for gen in return_val:
            if not isinstance(gen, Generation):
                raise ValueError("RedisSemanticCache only supports caching of " f"normal LLM generations, got {type(gen)}")
        llm_cache = self._get_llm_cache(llm_string)
...
class GPTCache(BaseCache):
    ...
    def update(self, prompt: str, llm_string: str, return_val: RETURN_VAL_TYPE) -> None:
        ...
        for gen in return_val:
            if not isinstance(gen, Generation):
                raise ValueError("GPTCache only supports caching of normal LLM generations, " f"got {type(gen)}")
...
class MomentoCache(BaseCache):
    ...
    def update(self, prompt: str, llm_string: str, return_val: RETURN_VAL_TYPE) -> None:
        ...
        for gen in return_val:
            if not isinstance(gen, Generation):
                raise ValueError("Momento only supports caching of normal LLM generations, " f"got {type(gen)}")
...
class CassandraCache(BaseCache):
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError("Could not import cassio python package. " "Please install it with `pip install -U cassio`.")
...
class CassandraSemanticCache(BaseCache):
    ...
    async def alookup_with_id(self, prompt: str, llm_string: str) -> Optional[Tuple[str, RETURN_VAL_TYPE]]:
        """
        Look up based on prompt and llm_string.
        If there are hits, return (document_id, cached_entry)
        """
...
class AzureCosmosDBSemanticCache(BaseCache):
    ...
    def update(self, prompt: str, llm_string: str, return_val: RETURN_VAL_TYPE) -> None:
        ...
        for gen in return_val:
            if not isinstance(gen, Generation):
                raise ValueError(
                    "CosmosDBMongoVCoreSemanticCache only supports caching of " f"normal LLM generations, got {type(gen)}"
                )
...
class AzureCosmosDBNoSqlSemanticCache(BaseCache):
    ...
    def update(self, prompt: str, llm_string: str, return_val: RETURN_VAL_TYPE) -> None:
        ...
        for gen in return_val:
            if not isinstance(gen, Generation):
                raise ValueError(
                    "CosmosDBNoSqlSemanticCache only supports caching of " f"normal LLM generations, got {type(gen)}"
                )
        llm_cache = self._get_llm_cache(llm_string)
        metadata = {
            "llm_string": llm_string,
...
class OpenSearchSemanticCache(BaseCache):
    ...
    def update(self, prompt: str, llm_string: str, return_val: RETURN_VAL_TYPE) -> None:
        ...
        for gen in return_val:
            if not isinstance(gen, Generation):
                raise ValueError(
                    "OpenSearchSemanticCache only supports caching of " f"normal LLM generations, got {type(gen)}"
                )
        llm_cache = self._get_llm_cache(llm_string)
        metadata = {
            "llm_string": llm_string,
...
class SingleStoreDBSemanticCache(BaseCache):
    ...
    def lookup(self, prompt: str, llm_string: str) -> Optional[RETURN_VAL_TYPE]:
        ...
        if results:
            for document_score in results:
                if (
                    document_score[1] > self.search_threshold and llm_cache.distance_strategy == DistanceStrategy.DOT_PRODUCT
                ) or (
    ...
    def update(self, prompt: str, llm_string: str, return_val: RETURN_VAL_TYPE) -> None:
        ...
        for gen in return_val:
            if not isinstance(gen, Generation):
                raise ValueError(
                    "SingleStoreDBSemanticCache only supports caching of " f"normal LLM generations, got {type(gen)}"
                )
        llm_cache = self._get_llm_cache(llm_string)
        metadata = {
            "llm_string": llm_string,
...
class MemcachedCache(BaseCache):
    ...
    def __init__(self, client_: Any):
        ...
        try:
            ...
            raise ImportError(
                "Could not import pymemcache python package. " "Please install it with `pip install -U pymemcache`."
            )
    ...
    def update(self, prompt: str, llm_string: str, return_val: RETURN_VAL_TYPE) -> None:
        ...
        for gen in return_val:
            if not isinstance(gen, Generation):
                raise ValueError("Memcached only supports caching of normal LLM generations, " + f"got {type(gen)}")
