...
class GitHubAPIWrapper(BaseModel):
    ...
    def validate_environment(cls, values: Dict) -> Any:
        ...
        try:
            ...
            raise ValueError(f"Please make sure to give correct github parameters Error message: {e}")
        ...
        for i in range(1000):
            try:
                ...
                if e.status == 422 and "Reference already exists" in e.data["message"]:
                    ...
                    raise Exception("Unable to create branch name from proposed_branch_name: " f"{proposed_branch_name}")
        return (
            ...
            for file in files_page:
                try:
                    ...
                    if file_content_response.status_code == 200:
                        ...
                        print(  # noqa: T201
                            "Failed downloading file content " f"(Error {file_content_response.status_code}). Skipping"
                        )
                        continue

                    file_tokens = len(
                        tiktoken.get_encoding("cl100k_base").encode(file_content + file.filename + "file_name file_contents")
                    )
    ...
    def read_file(self, file_path: str) -> str:
        ...
        try:
            ...
            return f"File not found `{file_path}` on branch" f"`{self.active_branch}`. Error: {str(e)}"
    ...
    def search_code(self, query: str) -> str:
        ...
        for code in search_result:
            ...
            file_content = self.github_repo_instance.get_contents(code.path, ref=self.active_branch).decoded_content.decode()
