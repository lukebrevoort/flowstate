...
class CassandraDatabase:
    ...
    def format_keyspace_to_markdown(self, keyspace: str, tables: Optional[List[Table]] = None) -> str:
        """
    ...
    def _resolve_schema(self, keyspaces: Optional[List[str]] = None) -> Dict[str, List[Table]]:
        ...
        for table_data in tables_data:
            ...
            table_columns = [
                (c.column_name, c.type) for c in columns_data if c.keyspace_name == keyspace and c.table_name == table_name
            ]

            partition_keys = [
                c.column_name
                for c in columns_data
                if c.kind == "partition_key" and c.keyspace_name == keyspace and c.table_name == table_name
            ]

            clustering_keys = [
                (c.column_name, c.clustering_order)
                for c in columns_data
                if c.kind == "clustering" and c.keyspace_name == keyspace and c.table_name == table_name
            ]
...
class Table(BaseModel):
    ...
    def _resolve_columns(
        ...
        cluster = [
            (column_name, clustering_order) for column_name, clustering_order, _ in sorted(cluster_info, key=lambda x: x[2])
        ]

        return columns, partition, cluster

    @staticmethod
    def _resolve_indexes(keyspace: str, table_name: str, db: CassandraDatabase) -> List[Tuple[str, str, str]]:
        indexes = []
        results = db.run(
