...
class PowerBIDataset(BaseModel):
    ...
    def headers(self) -> Dict[str, str]:
        ...
        if self.credential:
            try:
                token = self.credential.get_token("https://analysis.windows.net/powerbi/api/.default").token
                return {
                    "Content-Type": "application/json",
                    "Authorization": "Bearer " + token,
                }
            except Exception as exc:  # pylint: disable=broad-exception-caught
                raise ClientAuthenticationError("Could not get a token from the supplied credentials.") from exc
