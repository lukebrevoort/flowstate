    ...
    try:
        ...
        # For in-memory types:
            ...
            if loader_dict.get("folder_id"):
                ...
                location = ", ".join([f"https://drive.google.com/file/d/{file_id}/view" for file_id in file_ids])
            elif loader_dict.get("document_ids"):
                document_ids = loader_dict.get("document_ids", [])
                location = ", ".join([f"https://docs.google.com/document/d/{doc_id}/edit" for doc_id in document_ids])
...
def generate_size_based_batches(docs: List[Document], max_batch_size: int = 100 * 1024) -> List[List[Document]]:
    """
...
class PebbloLoaderAPIWrapper(BaseModel):
    ...
    def __init__(self, **kwargs: Any):
        ...
        kwargs["cloud_url"] = get_from_dict_or_env(kwargs, "cloud_url", "PEBBLO_CLOUD_URL", _DEFAULT_PEBBLO_CLOUD_URL)
    ...
    def classify_documents(
        ...
        docs, source_aggregate_size = self.prepare_docs_for_classification(docs_with_id, source_path, loader_details)
    ...
    def make_request(
        ...
        """
        try:
            response = request(method=method, url=url, headers=headers, json=payload, timeout=timeout)
            logger.debug(
            ...
            if response.status_code >= HTTPStatus.INTERNAL_SERVER_ERROR:
                ...
                logger.warning(f"Pebblo returned an unexpected response code: " f"{response.status_code}")
    ...
    def prepare_docs_for_classification(
        ...
        for doc in doc_content:
            ...
            if loader_details["loader"] == "SharePointLoader":
                doc_source_path = get_full_path(doc_metadata.get("source", loader_details["source_path"]))
            else:
            ...
            docs.append(
                {
                    ...
                    **({"authorized_identities": doc_authorized_identities} if doc_authorized_identities else {}),
