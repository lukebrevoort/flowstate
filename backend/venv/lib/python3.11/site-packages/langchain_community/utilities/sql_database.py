...
def _format_index(index: sqlalchemy.engine.interfaces.ReflectedIndex) -> str:
    return f"Name: {index['name']}, Unique: {index['unique']}," f" Columns: {str(index['column_names'])}"
...
class SQLDatabase:
    ...
    def from_databricks(
        ...
        **kwargs: Any,
        ...
        uri = f"databricks://token:{api_token}@{host}?" f"http_path={http_path}&catalog={catalog}&schema={schema}"
    ...
    def from_cnosdb(
        ...
        try:
            ...
            raise ImportError("cnos-connector package not found, please install with" " `pip install cnos-connector`")
    ...
    def get_table_info(self, table_names: Optional[List[str]] = None, get_col_comments: bool = False) -> str:
    ...
    def run(
        ...
        *,
        ...
        res = [{column: truncate_word(value, length=self._max_string_length) for column, value in r.items()} for r in result]
