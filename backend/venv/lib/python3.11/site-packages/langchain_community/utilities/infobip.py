...
class InfobipAPIWrapper(BaseModel):
    ...
    def validate_environment(cls, values: Dict) -> Any:
        ...
        values["infobip_api_key"] = get_from_dict_or_env(values, "infobip_api_key", "INFOBIP_API_KEY")
        values["infobip_base_url"] = get_from_dict_or_env(values, "infobip_base_url", "INFOBIP_BASE_URL")
        return values
    ...
    def _send_sms(self, sender: str, destination_phone_numbers: List[str], text: str) -> str:
        ...
        json: Dict = {
            "messages": [
                {
                    "destinations": [{"to": destination} for destination in destination_phone_numbers],
                    "from": sender,
                    "text": text,
                }
            ]
    ...
    def _send_email(self, from_email: str, to_email: str, subject: str, body: str) -> str:
        ...
        try:
            ...
            raise ImportError(
                "Unable to import requests_toolbelt, please install it with " "`pip install -U requests-toolbelt`."
            ) from e
        form_data: Dict = {
            "from": from_email,
            "to": to_email,
