...
def _import_riva_client() -> "riva.client":
    ...
    try:
        ...
        raise ImportError(
            "Could not import the NVIDIA Riva client library. " "Please install it with `pip install nvidia-riva-client`."
        ) from err
    return riva.client
...
class AudioStream:
    ...
    async def __aiter__(self) -> AsyncIterator[StreamInputType]:
        ...
        while True:
            # get next item
            try:
                next_val = await asyncio.get_event_loop().run_in_executor(None, self._queue.get, True, _QUEUE_GET_TIMEOUT)
            except queue.Empty:
                continue
    ...
    def complete(self) -> bool:
        ...
        output_done = self._worker is not None and not self._worker.is_alive() and self.output.empty()
    ...
    def put(self, item: StreamInputType, timeout: Optional[int] = None) -> None:
        ...
        with self._put_lock:
            if self.hungup:
                raise RuntimeError("The audio stream has already been hungup. Cannot put more data.")
            if item is HANGUP:
                self.hangup.set()
