...
class ZapierNLAWrapper(BaseModel):
    ...
    def list(self) -> List[Dict]:
        ...
        try:
            ...
            if response.status_code == 401:
                ...
                raise requests.HTTPError(
                    f"An unauthorized response occurred. Check that your api " f"key is correct. Err: {http_err}",
                    response=response,
                )
            raise http_err
    ...
    async def apreview(self, action_id: str, instructions: str, params: Optional[Dict] = None) -> Dict:
        """Same as run, but instead of actually executing the action, will
        instead return a preview of params that have been guessed by the AI in
        case you need to explicitly review before executing."""
        response = await self._arequest(
