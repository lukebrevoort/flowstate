...
class PolygonAPIWrapper(BaseModel):
    ...
    def get_financials(self, ticker: str) -> Optional[dict]:
        ...
        """
        url = f"{POLYGON_BASE_URL}vX/reference/financials?" f"ticker={ticker}&" f"apiKey={self.polygon_api_key}"
    ...
    def get_ticker_news(self, ticker: str) -> Optional[dict]:
        ...
        /v2/reference/news
        """
        url = f"{POLYGON_BASE_URL}v2/reference/news?" f"ticker={ticker}&" f"apiKey={self.polygon_api_key}"
        response = requests.get(url)
        data = response.json()
