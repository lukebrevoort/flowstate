...
class YouSearchAPIWrapper(BaseModel):
    ...
    def warn_if_deprecated_endpoints_are_used(self) -> Self:
        if self.endpoint_type == "snippets":
            warnings.warn(
                (f'`endpoint_type="{self.endpoint_type}"` is deprecated. ' 'Use `endpoint_type="search"` instead.'),
                DeprecationWarning,
            )
        return self
    ...
    def _parse_results(self, raw_search_results: Dict) -> List[Document]:
        ...
        if self.endpoint_type == "news":
            ...
            return [Document(page_content=result["description"], metadata=result) for result in news_results]
