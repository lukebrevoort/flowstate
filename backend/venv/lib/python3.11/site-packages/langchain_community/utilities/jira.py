...
class JiraAPIWrapper(BaseModel):
    ...
    def validate_environment(cls, values: Dict) -> Any:
        ...
        jira_api_token = get_from_dict_or_env(values, "jira_api_token", "JIRA_API_TOKEN", default="")
        ...
        if jira_oauth2 and isinstance(jira_oauth2, str):
            try:
                ...
                raise ValueError(f"The format of the JIRA_OAUTH2 string is " f"not a valid dictionary: {e}")
        ...
        if jira_api_token and jira_oauth2:
            raise ValueError("You have to provide either a jira_api_token or a jira_oauth2. " "Not both.")

        try:
            from atlassian import Confluence, Jira
        except ImportError:
            raise ImportError(
                "atlassian-python-api is not installed. " "Please install it with `pip install atlassian-python-api`"
            )
