...
class OpenVINOReranker(BaseDocumentCompressor):
    ...
    def __init__(self, **kwargs: Any):
        ...
        if require_model_export(self.model_name_or_path):
            ...
            # use local model
            self.ov_model = OVModelForSequenceClassification.from_pretrained(self.model_name_or_path, **self.model_kwargs)

        try:
            from transformers import AutoTokenizer
        except ImportError as e:
            raise ImportError("Unable to import transformers, please install with " "`pip install -U transformers`.") from e
    ...
    def rerank(self, request: Any) -> Any:
        ...
        if length.is_dynamic:
            input_tensors = self.tokenizer(query_passage_pairs, padding=True, truncation=True, return_tensors="pt")
        else:
