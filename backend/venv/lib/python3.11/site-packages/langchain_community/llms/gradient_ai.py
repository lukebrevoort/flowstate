...
class GradientLLM(BaseLLM):
    ...
    def validate_environment(cls, values: Dict) -> Any:
        ...
        values["gradient_access_token"] = get_from_dict_or_env(values, "gradient_access_token", "GRADIENT_ACCESS_TOKEN")
        values["gradient_workspace_id"] = get_from_dict_or_env(values, "gradient_workspace_id", "GRADIENT_WORKSPACE_ID")

        values["gradient_api_url"] = get_from_dict_or_env(values, "gradient_api_url", "GRADIENT_API_URL")
        return values
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if response.status_code != 200:
                raise Exception(
                    f"Gradient returned an unexpected response with status " f"{response.status_code}: {response.text}"
                )
        ...
        **kwargs: Any,
        ...
        if not self.aiosession:
            async with aiohttp.ClientSession() as session:
                async with session.post(**self._kwargs_post_request(prompt=prompt, kwargs=kwargs)) as response:
                    if response.status != 200:
                        raise Exception(
                            f"Gradient returned an unexpected response with status " f"{response.status}: {response.text}"
                        )
                    text = (await response.json())["generatedOutput"]
        else:
            async with self.aiosession.post(**self._kwargs_post_request(prompt=prompt, kwargs=kwargs)) as response:
                if response.status != 200:
                    raise Exception(
                        f"Gradient returned an unexpected response with status " f"{response.status}: {response.text}"
                    )
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        def _inner_generate(prompt: str) -> List[Generation]:
            return [Generation(text=self._call(prompt=prompt, stop=stop, run_manager=run_manager, **kwargs))]
    ...
    async def _agenerate(
        ...
        **kwargs: Any,
        ...
        for generation in await asyncio.gather(
            *[self._acall(prompt, stop=stop, run_manager=run_manager, **kwargs) for prompt in prompts]
        ):
    ...
    async def atrain_unsupervised(
        ...
        **kwargs: Any,
        ...
        if not self.aiosession:
            async with aiohttp.ClientSession() as session:
                async with session.post(**self._kwargs_post_fine_tune_request(inputs, kwargs)) as response:
                    if response.status != 200:
                        raise Exception(
                            f"Gradient returned an unexpected response with status " f"{response.status}: {response.text}"
                        )
            ...
            async with self.aiosession.post(**self._kwargs_post_fine_tune_request(inputs, kwargs)) as response:
                if response.status != 200:
                    raise Exception(
                        f"Gradient returned an unexpected response with status " f"{response.status}: {response.text}"
                    )
