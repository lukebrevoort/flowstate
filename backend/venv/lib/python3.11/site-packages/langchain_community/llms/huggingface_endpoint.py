...
class HuggingFaceEndpoint(LLM):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        if huggingfacehub_api_token is not None:
            try:
                ...
                raise ValueError("Could not authenticate with huggingface_hub. " "Please check your API token.") from e
    ...
    async def _astream(
        ...
        **kwargs: Any,
        ...
        invocation_params = self._invocation_params(stop, **kwargs)
        async for response in await self.async_client.text_generation(prompt, **invocation_params, stream=True):
            # identify stop sequence in generated text, if any
            stop_seq_found: Optional[str] = None
            for stop_seq in invocation_params["stop_sequences"]:
                if stop_seq in response:
