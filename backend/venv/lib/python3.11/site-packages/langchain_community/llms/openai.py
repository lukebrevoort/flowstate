        ...
        **kwargs: Any,
        ...
        for _prompts in sub_prompts:
            if self.streaming:
                ...
                response = completion_with_retry(self, prompt=_prompts, run_manager=run_manager, **params)
        ...
        **kwargs: Any,
        ...
        for _prompts in sub_prompts:
            if self.streaming:
                ...
                response = await acompletion_with_retry(self, prompt=_prompts, run_manager=run_manager, **params)
                if not isinstance(response, dict):
                    response = response.dict()
                choices.extend(response["choices"])
                update_token_usage(_keys, response, token_usage)
...
class AzureOpenAI(BaseOpenAI):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        values["openai_api_key"] = values["openai_api_key"] or os.getenv("AZURE_OPENAI_API_KEY") or os.getenv("OPENAI_API_KEY")
