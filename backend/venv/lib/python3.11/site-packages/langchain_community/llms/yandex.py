...
class _BaseYandexGPT(Serializable):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        if values["iam_token"]:
            values["grpc_metadata"] = [("authorization", f"Bearer {values['iam_token'].get_secret_value()}")]
            if values["folder_id"]:
...
async def _amake_request(self: YandexGPT, prompt: str) -> str:
    ...
    async with grpc.aio.secure_channel(self.url, channel_credentials) as channel:
        ...
        async with grpc.aio.secure_channel(operation_api_url, channel_credentials) as operation_channel:
