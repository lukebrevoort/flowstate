...
class LLMInputOutputAdapter:
        ...
        for event in stream:
            ...
            if messages_api and chunk_obj.get("type") in (
                ...
                yield GenerationChunk(
                    text=(chunk_obj[output_key] if provider != "mistral" else chunk_obj[output_key][0]["text"]),
                    generation_info={
                        GUARDRAILS_BODY_KEY: (
                            chunk_obj.get(GUARDRAILS_BODY_KEY) if GUARDRAILS_BODY_KEY in chunk_obj else None
                        ),
                    },
                )
    ...
    async def aprepare_output_stream(
        ...
        for event in stream:
            ...
            if provider == "mistral" and chunk_obj.get(output_key, [{}])[0].get("stop_reason", "") == "stop":
                return

            yield GenerationChunk(text=(chunk_obj[output_key] if provider != "mistral" else chunk_obj[output_key][0]["text"]))
...
class BedrockBase(BaseModel, ABC):
    ...
    async def _aprepare_input_and_invoke_stream(
        ...
        **kwargs: Any,
        ...
        params = {**_model_kwargs, **kwargs}
        input_body = LLMInputOutputAdapter.prepare_input(provider=provider, prompt=prompt, model_kwargs=params)
        body = json.dumps(input_body)
        ...
        async for chunk in LLMInputOutputAdapter.aprepare_output_stream(provider, response, stop):
            if run_manager is not None and asyncio.iscoroutinefunction(run_manager.on_llm_new_token):
