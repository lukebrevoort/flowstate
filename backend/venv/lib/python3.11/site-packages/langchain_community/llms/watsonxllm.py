...
class WatsonxLLM(BaseLLM):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        if "cloud.ibm.com" in values.get("url", "").get_secret_value():
            values["apikey"] = convert_to_secret_str(get_from_dict_or_env(values, "apikey", "WATSONX_APIKEY"))
        else:
            if (
                not values["token"]
                ...
                values["token"] = convert_to_secret_str(get_from_dict_or_env(values, "token", "WATSONX_TOKEN"))
            elif values["password"] or "WATSONX_PASSWORD" in os.environ:
                values["password"] = convert_to_secret_str(get_from_dict_or_env(values, "password", "WATSONX_PASSWORD"))
                values["username"] = convert_to_secret_str(get_from_dict_or_env(values, "username", "WATSONX_USERNAME"))
        ...
        try:
            ...
            credentials = {
                ...
                "instance_id": (values["instance_id"].get_secret_value() if values["instance_id"] else None),
                "version": (values["version"].get_secret_value() if values["version"] else None),
            }
            credentials_without_none_value = {key: value for key, value in credentials.items() if value is not None}
    ...
    def _stream(
        ...
        **kwargs: Any,
        ...
        """
        params = self._get_chat_params(stop=stop)
        for stream_resp in self.watsonx_model.generate_text_stream(prompt=prompt, raw_response=True, params=params):
