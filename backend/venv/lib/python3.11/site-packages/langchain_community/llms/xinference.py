...
class Xinference(LLM):
    ...
    def _check_cluster_authenticated(self) -> None:
        ...
        if response.status_code == 404:
            ...
            if response.status_code != 200:
                raise RuntimeError(f"Failed to get cluster information, " f"detail: {response.json()['detail']}")
    ...
    def _create_generate_stream(self, prompt: str, generate_config: Optional[Dict[str, List[str]]] = None) -> Iterator[str]:
        if self.client is None:
            raise ValueError("Client is not initialized!")
        model = self.client.get_model(self.model_uid)
            ...
            async with session.post(
                ...
                if response.status != 200:
                    if response.status == 404:
                        ...
                        raise ValueError(
                            f"astream call failed with status code {response.status}." f" Details: {optional_detail}"
                        )
