...
class Tongyi(BaseLLM):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        values["dashscope_api_key"] = get_from_dict_or_env(values, ["dashscope_api_key", "api_key"], "DASHSCOPE_API_KEY")
        try:
            import dashscope
        except ImportError:
        ...
        if self.streaming:
            ...
            for prompt in prompts:
                completion = await asyncio.get_running_loop().run_in_executor(
                    None,
                    functools.partial(generate_with_retry, **{"llm": self, "prompt": prompt, **params}),
                )
                generations.append([Generation(**self._generation_from_qwen_resp(completion))])
        return LLMResult(
            generations=generations,
