...
class ManifestWrapper(LLM):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        try:
            ...
            raise ImportError("Could not import manifest python package. " "Please install it with `pip install manifest-ml`.")
        return values
    ...
    def _call(
        ...
        **kwargs: Any,
        ...
        if stop is not None and len(stop) != 1:
            raise NotImplementedError(f"Manifest currently only supports a single stop token, got {stop}")
