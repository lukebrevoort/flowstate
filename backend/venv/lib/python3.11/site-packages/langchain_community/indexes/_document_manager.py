...
IMPORT_MOTOR_ASYNCIO_ERROR = "Could not import AsyncIOMotorClient. Please install it with `pip install motor`."
...
def _get_motor_client(mongodb_url: str, **kwargs: Any) -> Any:
    ...
    try:
        ...
        raise ImportError(f"AsyncIOMotorClient string provided is not in proper format. " f"Got error: {e} ")
    return client
...
class MongoDocumentManager(RecordManager):
    ...
    def exists(self, keys: Sequence[str]) -> List[bool]:
        ...
        existing_keys = {
            doc["key"] for doc in self.sync_collection.find({"namespace": self.namespace, "key": {"$in": keys}}, {"key": 1})
        }
    ...
    async def aexists(self, keys: Sequence[str]) -> List[bool]:
        ...
        cursor = self.async_collection.find({"namespace": self.namespace, "key": {"$in": keys}}, {"key": 1})
