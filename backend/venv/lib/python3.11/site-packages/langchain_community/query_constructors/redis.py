...
class RedisTranslator(Visitor):
    ...
    def _attribute_to_filter_field(self, attribute: str) -> RedisFilterField:
        if attribute in [tf.name for tf in self._schema.text]:
            ...
            raise ValueError(
                f"Invalid attribute {attribute} not in vector store schema. Schema is:" f"\n{self._schema.as_dict()}"
            )
    ...
    def visit_operation(self, operation: Operation) -> Any:
        ...
        if len(operation.arguments) > 2:
            right = self.visit_operation(Operation(operator=operation.operator, arguments=operation.arguments[1:]))
        else:
            right = operation.arguments[1].accept(self)
        redis_operator = RedisFilterOperator.OR if operation.operator == Operator.OR else RedisFilterOperator.AND
