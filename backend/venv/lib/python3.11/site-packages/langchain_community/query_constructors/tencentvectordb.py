...
class TencentVectorDBTranslator(Visitor):
    ...
    def visit_operation(self, operation: Operation) -> str:
        ...
        if operation.operator in (Operator.AND, Operator.OR):
            ret = f" {operation.operator.value} ".join([arg.accept(self) for arg in operation.arguments])
    ...
    def visit_comparison(self, comparison: Comparison) -> str:
        ...
        if comparison.comparator in self.COMPARATOR_MAP:
            if comparison.comparator in [Comparator.IN, Comparator.NIN]:
                ...
                return f"{comparison.attribute}" f" {self.COMPARATOR_MAP[comparison.comparator]} " f"({', '.join(value)})"
            if isinstance(comparison.value, str):
                return f"{comparison.attribute} " f"{self.COMPARATOR_MAP[comparison.comparator]}" f' "{comparison.value}"'
            return f"{comparison.attribute}" f" {self.COMPARATOR_MAP[comparison.comparator]} " f"{comparison.value}"
        else:
