...
def merge_chat_runs_in_session(chat_session: ChatSession, delimiter: str = "\n\n") -> ChatSession:
    ...
    for message in chat_session["messages"]:
        ...
        if not isinstance(message.content, str):
            raise ValueError("Chat Loaders only support messages with content type string, " f"got {message.content}")
        if not messages:
            ...
            if not isinstance(messages[-1].content, str):
                raise ValueError("Chat Loaders only support messages with content type string, " f"got {messages[-1].content}")
