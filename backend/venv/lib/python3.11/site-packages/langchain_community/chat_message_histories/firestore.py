...
def _get_firestore_client() -> Client:
    try:
        ...
        raise ImportError(
            "Could not import firebase-admin python package. " "Please install it with `pip install firebase-admin`."
        )
...
class FirestoreChatMessageHistory(BaseChatMessageHistory):
    ...
    def prepare_firestore(self) -> None:
        ...
        """
        self._document = self.firestore_client.collection(self.collection_name).document(self.session_id)
        self.load_messages()
