        ...
        for idx, singleTag in enumerate(tagSet.superTags):
            ...
            if LOG:
                LOG(
                    ...
                    % (isConstructed and "constructed " or "", singleTag, debug.hexdump(bytes(header)))
                )

            header += self.encodeLength(len(substrate), defModeOverride)

            if LOG:
                LOG("encoded %s octets (tag + payload) into %s" % (len(substrate), debug.hexdump(bytes(header))))
...
class SequenceEncoder(AbstractItemEncoder):
    ...
    def encodeValue(self, value, asn1Spec, encodeFun, **options):
        ...
        if asn1Spec is None:
            ...
            if inconsistency:
                raise error.PyAsn1Error(f"ASN.1 object {value.__class__.__name__} is inconsistent")
...
class Encoder(object):
    ...
    def __init__(self, tagMap=_MISSING, typeMap=_MISSING, **options):
        self._singleItemEncoder = self.SINGLE_ITEM_ENCODER(tagMap=tagMap, typeMap=typeMap, **options)
