    ...
    if type_args:
        ...
        if is_union_origin(origin_type):
            ...
            resolved_type_args = tuple(
                arg for arg in resolved_type_args if not (typing_objects.is_noreturn(arg) or typing_objects.is_never(arg))
            )
...
def get_cached_generic_type_late(
    ...
    generic_types_cache = _GENERIC_TYPES_CACHE.get()
    if generic_types_cache is None:  # pragma: no cover (early cache is guaranteed to run first and initialize the cache)
        generic_types_cache = GenericTypesCache()
        _GENERIC_TYPES_CACHE.set(generic_types_cache)
...
def set_cached_generic_type(
    ...
    """
    generic_types_cache = _GENERIC_TYPES_CACHE.get()
    if generic_types_cache is None:  # pragma: no cover (cache lookup is guaranteed to run first and initialize the cache)
        generic_types_cache = GenericTypesCache()
        _GENERIC_TYPES_CACHE.set(generic_types_cache)
