    ...
    *,
    ...
    def wrap(cls: Type[Any]) -> "DataclassClassOrWrapper":
        should_use_proxy = (
            ...
            else (is_builtin_dataclass(cls) and (cls.__bases__[0] is object or set(dir(cls)) == set(dir(cls.__bases__[0]))))
        )
        if should_use_proxy:
            dc_cls_doc = ""
...
def _dataclass_validate_values(self: "Dataclass") -> None:
    ...
    if getattr(self, "__pydantic_has_field_info_default__", False):
        ...
        input_data = {
            k: v for k, v in self.__dict__.items() if not (isinstance(v, FieldInfo) or _is_field_cached_property(self, k))
        }
    else:
