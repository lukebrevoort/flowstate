...
def _get_language_locale_dict():
    ...
    available_language_names = [shortname for shortname in available_locale_names if not re.search(r"-[A-Z0-9]+$", shortname)]
...
def _get_language_order(language_locale_dict):
        ...
        if response.ok:
            try:
                ...
                new_most_common_locales = [
                    i.replace("https://w3techs.com/technologies/details/cl", "").strip("-") for i in bars
                ]
    ...
    language_order_with_duplicates = most_common_locales + sorted(
        language_population_dict.keys(), key=lambda x: (language_population_dict[x], x), reverse=True
    )
    language_order = sorted(set(language_order_with_duplicates), key=lambda x: language_order_with_duplicates.index(x))
...
def main():
    ...
    language_order_string = "language_order = " + json.dumps(language_order, separators=(",", ": "), indent=4)
    ...
    language_map_data_string = "language_map = " + json.dumps(language_map_data, separators=(",", ": "), indent=4)
