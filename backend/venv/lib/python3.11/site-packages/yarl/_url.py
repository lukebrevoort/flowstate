...
def from_parts_uncached(scheme: str, netloc: str, path: str, query: str, fragment: str) -> "URL":
...
class URL:
    ...
    def __hash__(self) -> int:
        if (ret := self._cache.get("hash")) is None:
            ...
            ret = self._cache["hash"] = hash((self._scheme, self._netloc, path, self._query, self._fragment))
        return ret
    ...
    def with_query(self, *args: Any, **kwargs: Any) -> "URL":
        ...
        return from_parts_uncached(self._scheme, self._netloc, self._path, query, self._fragment)
    ...
    def without_query_params(self, *query_params: str) -> "URL":
        ...
        if not params_to_remove:
            return self
        return self.with_query(tuple((name, value) for name, value in self.query.items() if name not in params_to_remove))
    ...
    def with_fragment(self, fragment: Union[str, None]) -> "URL":
        ...
        if self._fragment == raw_fragment:
            return self
        return from_parts(self._scheme, self._netloc, self._path, self._query, raw_fragment)
    ...
    def human_repr(self) -> str:
        ...
        query_string = "&".join(
            "{}={}".format(human_quote(k, "#&+;="), human_quote(v, "#&+;=")) for k, v in self.query.items()
        )
...
def _encode_host(host: str, validate_host: bool) -> str:
    ...
    if host.isascii():
        ...
        if validate_host and (invalid := NOT_REG_NAME.search(host)):
            ...
            raise ValueError(f"Host {host!r} cannot contain {value!r} (at position {pos}){extra}") from None
        return host
