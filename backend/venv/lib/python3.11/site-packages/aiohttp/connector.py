    ...
    def _close(self, *, abort_ssl: bool = False) -> List[Awaitable[object]]:
        ...
        try:
            ...
            for data in self._conns.values():
                for proto, _ in data:
                    if abort_ssl and proto.transport and proto.transport.get_extra_info("sslcontext") is not None:
                        proto.abort()
                    else:
                        proto.close()
    ...
    def __init__(
        ...
        *,
        ...
        if ssl_shutdown_timeout is sentinel:
            ...
            if sys.version_info < (3, 11) and ssl_shutdown_timeout is not None and ssl_shutdown_timeout != 0:
                warnings.warn(
...
class NamedPipeConnector(BaseConnector):
    ...
    async def _create_connection(self, req: ClientRequest, traces: List["Trace"], timeout: "ClientTimeout") -> ResponseHandler:
        try:
            async with ceil_timeout(timeout.sock_connect, ceil_threshold=timeout.ceil_threshold):
                _, proto = await self._loop.create_pipe_connection(self._factory, self._path)  # type: ignore[attr-defined]
