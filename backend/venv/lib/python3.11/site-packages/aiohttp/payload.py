...
class Payload(ABC):
    ...
    async def write_with_length(self, writer: AbstractStreamWriter, content_length: Optional[int]) -> None:
        """
...
class IOBasePayload(Payload):
    ...
    def _read_and_available_len(self, remaining_content_len: Optional[int]) -> Tuple[Optional[int], bytes]:
        """
        ...
        return size, self._value.read(min(READ_SIZE, size or READ_SIZE, remaining_content_len or READ_SIZE))
    ...
    async def write_with_length(self, writer: AbstractStreamWriter, content_length: Optional[int]) -> None:
        ...
        available_len, chunk = await loop.run_in_executor(None, self._read_and_available_len, remaining_content_len)
        ...
        while chunk:
            ...
            chunk = await loop.run_in_executor(
                None,
                self._read,
                (min(READ_SIZE, remaining_content_len) if remaining_content_len is not None else READ_SIZE),
            )
...
class TextIOPayload(IOBasePayload):
    ...
    def _read_and_available_len(self, remaining_content_len: Optional[int]) -> Tuple[Optional[int], bytes]:
        ...
        self._set_or_restore_start_position()
        size = self.size
        chunk = self._value.read(min(READ_SIZE, size or READ_SIZE, remaining_content_len or READ_SIZE))
...
class AsyncIterablePayload(Payload):
    ...
    def __init__(self, value: _AsyncIterable, *args: Any, **kwargs: Any) -> None:
        if not isinstance(value, AsyncIterable):
            raise TypeError(
                "value argument must support " "collections.abc.AsyncIterable interface, " "got {!r}".format(type(value))
            )
