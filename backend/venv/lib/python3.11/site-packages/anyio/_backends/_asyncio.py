...
def _maybe_set_event_loop_policy(policy: asyncio.AbstractEventLoopPolicy | None, use_uvloop: bool) -> None:
    ...
    if policy is None and use_uvloop and sys.implementation.name == "cpython":
        try:
            ...
            if not hasattr(asyncio.AbstractEventLoop, "shutdown_default_executor") or hasattr(
...
class TaskGroup(abc.TaskGroup):
    ...
    async def _run_wrapped_task(self, coro: Coroutine, task_status_future: asyncio.Future | None) -> None:
...
def run_async_from_thread(func: Callable[..., Awaitable[T_Retval]], *args: object) -> T_Retval:
    f: concurrent.futures.Future[T_Retval] = asyncio.run_coroutine_threadsafe(func(*args), threadlocals.loop)
    return f.result()
...
def setup_process_pool_exit_at_shutdown(workers: set[Process]) -> None:
    kwargs: dict[str, Any] = {"name": "AnyIO process pool shutdown task"} if _native_task_names else {}
...
class UNIXSocketStream(abc.SocketStream):
    ...
    async def receive_fds(self, msglen: int, maxfds: int) -> tuple[bytes, list[int]]:
        ...
        with self._receive_guard:
            while True:
                try:
                    message, ancdata, flags, addr = self.__raw_socket.recvmsg(msglen, socket.CMSG_LEN(maxfds * fds.itemsize))
                except BlockingIOError:
                    await self._wait_until_readable(loop)
                except OSError as exc:
                    if self._closing:
        ...
        with self._send_guard:
            while True:
                try:
                    ...
                    self.__raw_socket.sendmsg([message], [(socket.SOL_SOCKET, socket.SCM_RIGHTS, fdarray)])
                    break
                except BlockingIOError:
                    await self._wait_until_writable(loop)
                except OSError as exc:
...
async def connect_tcp(host: str, port: int, local_addr: tuple[str, int] | None = None) -> SocketStream:
    transport, protocol = cast(
        ...
        await get_running_loop().create_connection(StreamProtocol, host, port, local_addr=local_addr),
    )
...
async def getaddrinfo(
    ...
    *,
    ...
    result = await get_running_loop().getaddrinfo(host, port, family=family, type=type, proto=proto, flags=flags)
