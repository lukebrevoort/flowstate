...
class TLSStream(ByteStream):
    ...
    async def _call_sslobject_method(self, func: Callable[..., T_Retval], *args: object) -> T_Retval:
        while True:
            try:
                ...
                self._read_bio.write_eof()
                self._write_bio.write_eof()
                if isinstance(exc, ssl.SSLEOFError) or "UNEXPECTED_EOF_WHILE_READING" in exc.strerror:
    ...
    async def send_eof(self) -> None:
        ...
        if match:
            ...
            if (major, minor) < (1, 3):
                raise NotImplementedError(f"send_eof() requires at least TLSv1.3; current " f"session uses {tls_version}")
    ...
    def extra_attributes(self) -> Mapping[Any, Callable[[], Any]]:
        return {
            **self.transport_stream.extra_attributes,
            ...
            TLSAttribute.shared_ciphers: lambda: self._ssl_object.shared_ciphers() if self._ssl_object.server_side else None,
