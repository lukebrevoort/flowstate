...
async def test_supervisor_multi_agent_workflow():
    ...
    assert "schedule_task" in all_tool_calls or "manage_assignments" in all_tool_calls, \
    ...
    assert "format_response" in all_tool_calls or any(
        isinstance(msg, AIMessage) and msg.content and "<" in str(msg.content)
        for msg in messages
...
async def test_supervisor_formats_final_response():
    ...
    assert has_format_call or has_jsx_content, \
        "Expected format_response call or JSX content in response"
