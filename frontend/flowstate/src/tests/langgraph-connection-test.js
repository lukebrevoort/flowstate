...
const testLangGraphConnection = async () => {
  ...
  const baseUrl = process.env.NEXT_PUBLIC_LANGGRAPH_API_URL || 'http://localhost:9876';
  ...
  try {
    ...
    if (!assistantResponse.ok) {
      throw new Error(`Failed to list assistants: ${assistantResponse.status} ${assistantResponse.statusText}`);
    }
    ...
    if (assistants.length > 0 && threadId) {
      ...
      console.log(`3. Testing message sending with assistant ${assistantId} and thread ${threadId}...`);
      ...
      const historyResponse = await fetch(`${baseUrl}/threads/${threadId}/messages`);
      if (!historyResponse.ok) {
        throw new Error(`Failed to get thread history: ${historyResponse.status} ${historyResponse.statusText}`);
      }
      const history = await historyResponse.json();
      console.log(`✅ Successfully retrieved thread history with ${history.length} messages`);
    }

    console.log('All tests passed! Your LangGraph connection is working correctly.');
    return true;
  } catch (error) {
    console.error('Test failed:', error);
    console.error('This could indicate that your LangGraph Docker API is not running or is not accessible at the URL:', baseUrl);
    console.log('Make sure your Docker containers are running with: docker-compose up -d');
    console.log('Check if your LangGraph API is using a different endpoint structure.');
    return false;
  }
};
