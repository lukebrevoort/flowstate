...
const convertLangGraphMessages = (
  messages: Array<{ type: string; content: string | object }>
) => {
      ...
      try {
        ...
        const isCompleteJSX = (jsx: string) => {
          ...
          const hasUnClosedQuotes =
            (jsx.match(/className="[^"]*$/g) || []).length > 0;
        ...
        if (!isCompleteJSX(content)) {
          ...
          return (
            <div className='p-4 bg-yellow-50 border border-yellow-200 rounded-md'>
              <Typography
                variant='h3'
                className='text-yellow-700 mb-2 font-semibold'
              >
                ⚠️ Response Processing Issue
              </Typography>
              <Typography variant='p' className='text-yellow-600 mb-3'>
                The response was interrupted during transmission. Here&apos;s
                what I was able to process:
              </Typography>
        ...
        const processedContent = content
          .replace(
            /onClick={\(\) => handleCheckAssignments\(\)}/g,
            `onClick={handleCheckAssignments}`
          )
          .replace(
            /onClick={\(\) => handleCheckCourses\(\)}/g,
            `onClick={handleCheckCourses}`
          )
          .replace(
            /onClick={\(\) => handleCheckSchedule\(\)}/g,
            `onClick={handleCheckSchedule}`
          )
          .replace(
            /onClick={\(\) => handleCreateAssignment\(\)}/g,
        ...
        try {
          ...
          console.warn(
            'JSX parsing failed, attempting manual button parsing:',
            jsxError
          );
          ...
          const parseAndCreateButtons = (htmlContent: string) => {
            ...
            let contentWithoutButtons = htmlContent.replace(
              buttonRegex,
              '|||BUTTON|||'
            );
            ...
            contentWithoutButtons = contentWithoutButtons
              .replace(
                /<Typography variant="([^"]*)" className="([^"]*)">/g,
                '<div className="$2">'
              )
